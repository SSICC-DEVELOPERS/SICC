...
public class FacturaPeru2 extends DocumentoFAC {
...
private byte[] documento;
  ...
  public FacturaPeru2(){
      ...
      UtilidadesLog.info("FacturaPeru2.componerFactura(DTODocLegal datosDocLegal): Entrada"); 
...
//PARTE 3
      ...
      for(int x=0;x<misDetalles.size();x++){
          ...
          if(!miDetalle.getNoImprimible()){ 
              if("C".equals(miDetalle.getIndicadorDentroFueraCajaBolsa())) {
                ...
                //  Modificado por Incidencia Sicc20070208
                if(miDetalle.getPrecioSinImpuUnitLocal()!=null){
                    //  Valor venta catalogo unitario
                    datos.add(3,new BigDecimal( miDetalle.getPrecioSinImpuUnitLocal().floatValue()).setScale(2,BigDecimal.ROUND_HALF_EVEN).toString());
                    //  Valor venta catalogo unitario con descuento
                    datos.add(4,new BigDecimal( miDetalle.getPrecioSinImpuUnitLocal().floatValue()).setScale(2,BigDecimal.ROUND_HALF_EVEN).toString());
                } else {
                    datos.add(3,"  ");
                    datos.add(4,"  ");
                }
                //  Valor venta catalogo con descuento
                if(miDetalle.getPrecioSinImpuTotalLocal()!=null){
                    datos.add(5,new BigDecimal( miDetalle.getPrecioSinImpuTotalLocal().floatValue()).setScale(2,BigDecimal.ROUND_HALF_EVEN).toString());
                } else {
                    datos.add(5,"  ");
                }
    ...
    //  Agregado por Incidencia Sicc2007020208
    BigDecimal totalVentaBG = null;
    if ( miCabecera.getTotalAPagarLocal()!=null )   {
        float totalVenta = (miCabecera.getTotalAPagarLocal().floatValue())+
                     (miCabecera.getImpDesc1TotLocal()!=null?miCabecera.getImpDesc1TotLocal().floatValue():0)+
                     (miCabecera.getImpDesc3TotalLocal()!=null?miCabecera.getImpDesc3TotalLocal().floatValue():0);
        totalVentaBG = new BigDecimal(totalVenta).setScale(2,BigDecimal.ROUND_HALF_EVEN);
    }   
    
	String res="CERO";
    if(miCabecera.getTotalAPagarLocal()!=null){
        Numtotext numero = new Numtotext(1);
        res = numero.procesar(new Float(totalVentaBG.floatValue()));
    }
    String monedaAux=" ";
    ...
    if (miCabecera.getImporteImpuestosTotalLocal()!=null && miCabecera.getImporteImpuestosTotalLocal().floatValue()<0){
        ...
        float impTotLoca = (miCabecera.getImporteImpuestosTotalLocal().floatValue())+
                     (miCabecera.getImpDesc1TotLocal()!=null?miCabecera.getImpDesc1TotLocal().floatValue():0)-
                     (miCabecera.getImpDesc1SinImpTotLocal()!=null?miCabecera.getImpDesc1SinImpTotLocal().floatValue():0);
        datos.add(2,new BigDecimal(impTotLoca).setScale(2,BigDecimal.ROUND_HALF_EVEN).toString());
    ...
    //  Modificado por Incidencia Sicc20070208
	if(totalVentaBG!=null)  {
        datos.add(3, totalVentaBG.toString());
    }else{
        datos.add(3," ");
    }
    ...
    datos.add(8, totalVentaBG.toString());
    ...
    try		{
      sf1.componerDocumento();
			sf1.saveDocument("c:\\", "facturaPeru2.txt");
			this.documento = sf1.bufferOut;
		}	catch (Exception e) {
			UtilidadesLog.error("ERROR",e);
		}
      if(log.isDebugEnabled()) //sapaza -- cambio Optimizacion Logs -- 05/03/2010
	UtilidadesLog.debug("FacturaPeru2: "+sf1.toString());
        
    UtilidadesLog.info("FacturaPeru2.componerFactura(DTODocLegal datosDocLegal): Salida"); 
  }
    ...
    public String calcularCodigoCompletoVenta(String codPeriodoEntrada, String codVenta, Float precioCatalogoUnitLocalEntrada){
     UtilidadesLog.info("FacturaPeru2.calcularCodigoCompletoVenta(String codPeriodoEntrada, String codVenta, Float precioCatalogoUnitLocalEntrada): Entrada"); 
    ...
    if(precioCatalogoUnitLocalEntrada!=null && (precioCatalogoUnitLocalEntrada.floatValue()!=0)) {
     ...
     UtilidadesLog.info("FacturaPeru2.calcularCodigoCompletoVenta(String codPeriodoEntrada, String codVenta, Float precioCatalogoUnitLocalEntrada): Salida"); 
     return salida;
    }
    ...
    public static void main(String[] arg) {
    	FacturaPeru2 fac = new FacturaPeru2();
    }
