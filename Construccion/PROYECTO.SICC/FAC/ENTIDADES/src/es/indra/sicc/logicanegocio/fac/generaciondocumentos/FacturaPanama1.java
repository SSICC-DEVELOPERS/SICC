...
public class FacturaPanama1 extends DocumentoFAC {
    private byte[] documento;
    private UtilidadesLog log = new UtilidadesLog();
    
    public FacturaPanama1() {
    }

    public void componerFactura(DTODocLegal datosDocLegal) {
      UtilidadesLog.info("FacturaPanama1.componerFactura(DTODocLegal datosDocLegal): Entrada"); 
      
      ArrayList listaPaginas = datosDocLegal.getlistaPaginas();
      SerializadorTextFacturaPanama1 sf1 = new SerializadorTextFacturaPanama1();
      ...
      try {
		...
		sf1.saveDocument("c:\\", "FacturaPanama1.txt");
		this.documento = sf1.bufferOut;
	  }	catch (Exception e) {
	    UtilidadesLog.error("ERROR",e);
	  }
	  if(log.isDebugEnabled()) //sapaza -- cambio Optimizacion Logs -- 05/03/2010
            UtilidadesLog.debug("FacturaPanama1: " + sf1.toString());
            
      UtilidadesLog.info("FacturaPanama1.componerFactura(DTODocLegal datosDocLegal): Salida"); 
  }
    ...
    public String calcularCodigoCompletoVenta(String codPeriodoEntrada, String codVenta, Float precioCatalogoUnitLocalEntrada){
     UtilidadesLog.info("FacturaPanama1.calcularCodigoCompletoVenta(String codPeriodoEntrada, String codVenta, Float precioCatalogoUnitLocalEntrada): Entrada"); 
    ...
    if(precioCatalogoUnitLocalEntrada!=null && (precioCatalogoUnitLocalEntrada.floatValue()!=0)) {
     ...
     UtilidadesLog.info("FacturaPanama1.calcularCodigoCompletoVenta(String codPeriodoEntrada, String codVenta, Float precioCatalogoUnitLocalEntrada): Salida"); 
     return salida;
    }
    ...
    private void generarSeccion2(SerializadorTextFacturaPanama1 sf1, DTOFACDocumentoContableCabecera miCabecera) {       
      this.generarTotalesColumnas(sf1, miCabecera);  
      this.generarTotales(sf1, miCabecera);
    }


    private void generarTotalesColumnas(SerializadorTextFacturaPanama1 sf1, DTOFACDocumentoContableCabecera miCabecera) {  
    ...
    private void generarTotales(SerializadorTextFacturaPanama1 sf1, DTOFACDocumentoContableCabecera miCabecera) {  
      ArrayList datos = new ArrayList();
      datos.add(0, validarSignosImportes(this.calcularBaseImponibleIVA(miCabecera).toString()));
      datos.add(1, miCabecera.getTasaImpuesto().intValue() + "%");   
      datos.add(2, validarSignosImportes(miCabecera.getImporteImpuestosTotalLocal().toString())); 
      datos.add(3, miCabecera.getTasaImpuestoFlete().intValue() + "%");
      datos.add(4, validarSignosImportes(miCabecera.getImporteFleteTotalLocal().toString())); 
      datos.add(5, validarSignosImportes(miCabecera.getTotalAPagarLocal().toString())); 
      
      String[] arraySon = (String[])this.generarTotalLetras(miCabecera);
    
      datos.add(6, arraySon[0]);
      datos.add(7, arraySon[1]);
      datos.add(8, arraySon[2]);
    ...
    private void generarSeccion1(SerializadorTextFacturaPanama1 sf1, DTOFACDocumentoContableCabecera miCabecera, DTOFACDocumentoContable docuContable) {
      this.generarDetalles(sf1, miCabecera, docuContable);      
    }
    
    private void generarDetalles(SerializadorTextFacturaPanama1 sf1, DTOFACDocumentoContableCabecera miCabecera, DTOFACDocumentoContable docuContable) {
   ...
   private void generarSeccion0(SerializadorTextFacturaPanama1 sf1, DTOFACDocumentoContableCabecera miCabecera, int totalPaginas, int numPagina, Boolean indOnLine) { 
     ArrayList datos = new ArrayList();
/* Agregado por RI 20080690 --- AL --- 04/08/2008 */
...
/* Eliminado por RI 20080690 --- AL --- 04/08/2008
    datos.add(0, validarToString(miCabecera.getNumConsolidado()));
     if (indOnLine != null && indOnLine.booleanValue()) {
       datos.add(1, validarToString(miCabecera.getNumDocContableInterno()));
     } else {
