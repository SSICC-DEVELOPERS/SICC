SELECT distinct
	   PAIS.COD_PAIS, GEN_PAIS.VAL_I18N, 
	   MARC.DES_MARC, GEN_CAN.VAL_I18N,
	   SUBG.COD_SUBG_VENT, SUBG.DES_SUBG_VENT,  
	   REG.COD_REGI, REG.DES_REGI,
	   ZON.COD_ZONA, ZON.DES_ZONA,
	   SEC.COD_SECC, SEC.DES_SECCI,
	   GEO.DES_GEOG,
	   GEO.ORDE_1, GEO.ORDE_2, GEO.ORDE_3, GEO.ORDE_4, GEO.ORDE_5, GEO.ORDE_6, GEO.ORDE_7, GEO.ORDE_8, GEO.ORDE_9,
...
WHERE 
	  ...
	  AND SEC.OID_SECC = TERSEC.ZSCC_OID_SECC(+)
	  AND TERSEC.TERR_OID_TERR = TER.OID_TERR(+)
	  ...
	  and subg.cana_oid_cana = gen_can.val_oid(+)
	  and subg.PAIS_OID_PAIS = gen_pais.val_oid(+)
	  AND SUBG.MARC_OID_MARC = 1 
	  AND SUBG.CANA_OID_CANA = 1
	  AND SUBG.PAIS_OID_PAIS = 1 
	  AND SUBG.IND_BORR = 0
ORDER BY
	  SUBG.COD_SUBG_VENT, 
	  REG.COD_REGI, 
