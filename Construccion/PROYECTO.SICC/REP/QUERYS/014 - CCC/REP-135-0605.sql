...
SELECT GEN_PAIS.VAL_I18N PAIS,
	   ...
	   DETALLE.ORDEN_GRUPO,
	   DETALLE.GRUPO,
	   DETALLE.ORDEN_SUBGRUPO,
	   DETALLE.SUBGRUPO,
	   DETALLE.MARCA,
	   NEGO.COD_NEGO||' '||GEN_NEGO.VAL_I18N NEGOCIO,
	   DETALLE.IMPORTE,
	   TOTALES.VENTA_LINEA,
...
(		
	SELECT VIRTUAL.PAIS,
		   ...
		   VIRTUAL.ORDEN_GRUPO,
		   VIRTUAL.GRUPO,
		   VIRTUAL.ORDEN_SUBGRUPO,
		   VIRTUAL.SUBGRUPO,
		   VIRTUAL.OID_MARC_PROD,
		   VIRTUAL.MARCA,
		   VIRTUAL.OID_NEGO,
		   DATOS.IMPORTE
	FROM		
		(	
			(
				SELECT NEGO.PAIS_OID_PAIS PAIS,
					   ...
					   NEGO.OID_NEGO,
					   SUM(NVL(DAT.IMP_DATO_SAP, 0)) IMPORTE
				...
				WHERE DAT.IND_FACT_CCC = 'F'
					  ...
					  AND TO_DATE(DAT.FEC_CONT) = TO_DATE('26/05/2005','DD/MM/YYYY')   -- PARAMETRO DE FECHA CONTABLE -- 
					  AND DAT.VAL_PERI_CONT = 1	  									   -- PARAMETRO DE PERIODO CONTABLE--
					  AND DAT.VAL_EJER_CONT = 2005									   -- PARAMETRO DE EJERCICIO CONTABLE --
					  AND PER_COR.OID_PERI = 1  									   -- PARAMETRO DE EJERCICIO COMERCIAL --
					  AND PER_COR.TIPE_OID_TIPO_PERI = 1							   -- PARAMETRO DE TIPO DE PERIODO COMERCIAL --
					  AND NEGO.PAIS_OID_PAIS = 1	   								   -- PARAMETRO DE PAIS --
		...
		(		
			 ...
			 (
			  	( 
					SELECT DISTINCT NEGO.PAIS_OID_PAIS PAIS,
						   ...
						   'A' ORDEN_SUBGRUPO,	
						   MAR_PRO.OID_MARC_PROD,			
						   MAR_PRO.COD_MARC_PROD||' '||MAR_PRO.DES_MARC_PROD MARCA,
						   NEGO.OID_NEGO						   
					FROM INT_DATOS_SAPFI DAT,
						 SEG_PERIO_CORPO PER_COR,
					 	 SEG_MARCA_PRODU MAR_PRO,
				 ...
				 UNION ALL
			  	 (
						SELECT NEGO.PAIS_OID_PAIS PAIS,
							   ...
							   'B' ORDEN_SUBGRUPO,
							   SUBA.OID_SBAC,
							   TO_CHAR(GEN_SUBA.VAL_I18N) BELCENTER,
							   NEGO.OID_NEGO						   
						FROM SEG_ACCES ACCE,
							 SEG_SUBAC SUBA,
							 MAE_NEGOC NEGO,
						...
						WHERE SUBA.ACCE_OID_ACCE = ACCE.OID_ACCE
							  AND ACCE.IND_ACCE = 'B'
							  AND NEGO.PAIS_OID_PAIS = 1	   								   -- PARAMETRO DE PAIS --
				...
				(
						SELECT NEGO.PAIS_OID_PAIS PAIS,
							   ...
							   'C' ORDEN_SUBGRUPO,
							   CANAL.OID_CANA,
							   TO_CHAR(GEN_CANAL.VAL_I18N) CANALES,
							   NEGO.OID_NEGO						   
			...
			(					
				(
					SELECT 'A' ORDEN_GRUPO,
						   'VENTA BRUTA' GRUPO						   
					FROM DUAL
				)
				UNION ALL
				(
					SELECT 'B' ORDEN_GRUPO,
						   'DEVOLUCIONES'						   
					FROM DUAL
				)
				UNION ALL
				(
					SELECT 'C' ORDEN_GRUPO,
						   'VENTA NETA'
					FROM DUAL
				)
			),
	...
	WHERE DATOS.PAIS(+) = VIRTUAL.PAIS 
		  ...
		  AND DATOS.GRUPO(+) = VIRTUAL.GRUPO
		  AND DATOS.SUBGRUPO(+) = VIRTUAL.SUBGRUPO
		  AND DATOS.MARCA(+) = VIRTUAL.OID_MARC_PROD
...
WHERE DETALLE.PAIS = TOTALES.PAIS(+)			
	  AND DETALLE.SOCI = TOTALES.SOCI(+)
	  AND DETALLE.GRUPO = TOTALES.GRUPO(+)	
	  AND DETALLE.SUBGRUPO = TOTALES.SUBGRUPO(+)
	  AND DETALLE.OID_MARC_PROD = TOTALES.MARCA(+)
	  AND DETALLE.PAIS = GEN_PAIS.VAL_OID(+)
	  AND DETALLE.SOCI = GEN_SOCI.VAL_OID(+)
	  AND DETALLE.OID_NEGO = NEGO.OID_NEGO
