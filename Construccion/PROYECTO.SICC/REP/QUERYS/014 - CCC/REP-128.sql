 /* PARA REALIZAR EL LAYOUT HAY QUE PEDIR QUE SE PASEN POR PARAMETRO LAS DESCRIPCIONES DE :
...
(
	SELECT PAIS,
		   ...
		   GERENTE,
		   DES_ZONA,
		   SUM(IMP_PENDIENTE) IMP_PENDIENTE,
		   SUM(PAG_21) PAG_21,
		   SUM(PAG_31) PAG_31,
		   SUM(PAG_42) PAG_42,
		   SUM(PAG_63) PAG_63,
		   ...
		   SUM(PAG_126) PAG_126,
		   SUM(PAG_189) PAG_189,
		   SUM(PAG_189_MAS) PAG_189_MAS,
		   SUM(TC_FACTURACION) TC_FACTURACION,
		   SUM(TC_CARGOS_D) TC_CARGOS_D,
		   SUM(TC_PAGO_FRAC) TC_PAGO_FRAC,
		   SUM(TA_DIRECTOS) TA_DIRECTOS, 
		   SUM(TA_NOTA) TA_NOTA,
		   SUM(TA_PAGO_FRAC) TA_PAGO_FRAC,
		   SUM(TA_INCOBRA) TA_INCOBRA		   	   
	FROM
		(
		SELECT GEN_PAIS.VAL_I18N PAIS,
			   ...
			   VAL_NOMB_PERI,
			   COD_ZONA,
			   CASE WHEN (CLIE_OID_CLIE IS NULL) THEN
			   		'GERENTE NO ASIGNADO' 
			   ELSE
			   ...
			   NVL((
			   CASE WHEN (TIPO = 'A' AND PROC_ORIGEN = 'CCC001' AND IMPORTE > 0 AND PROC_ULTIMO <> 'CCC006') THEN
			   		SUM(IMPORTE)
			   END), 0) TC_FACTURACION,
			   NVL((
			   CASE WHEN (TIPO = 'A' AND PROC_ORIGEN = 'CCC003' AND IMPORTE > 0 AND PROC_ULTIMO <> 'CCC006') THEN
			   		SUM(IMPORTE)
			   END), 0) TC_CARGOS_D,
			   NVL((
			   CASE WHEN (TIPO = 'A' AND PROC_ORIGEN <> 'CCC001' AND PROC_ORIGEN <> 'CCC003' AND IMPORTE > 0 AND PROC_ULTIMO <> 'CCC006') THEN
			   		SUM(IMPORTE)
			   END), 0) TC_PAGO_FRAC,
			   NVL((
			   CASE WHEN (TIPO = 'B' AND PROC_ORIGEN = 'CCC003' AND IMPORTE < 0 AND PROC_ULTIMO <> 'CCC006') THEN
			   		SUM(IMPORTE)
			   END), 0) TA_DIRECTOS,
			   NVL((
			   CASE WHEN (TIPO = 'B' AND PROC_ORIGEN = 'CCC001' AND IMPORTE < 0 AND PROC_ULTIMO <> 'CCC006') THEN
			   		SUM(IMPORTE)
			   END), 0) TA_NOTA,
			   NVL((
			   CASE WHEN (TIPO = 'B' AND PROC_ORIGEN <> 'CCC001' AND PROC_ORIGEN <> 'CCC003' AND IMPORTE < 0 AND PROC_ULTIMO <> 'CCC006') THEN
			   		SUM(IMPORTE)
			   END), 0) TA_PAGO_FRAC,
			   NVL((
			   CASE WHEN (TIPO = 'B' AND PROC_ULTIMO = 'CCC006') THEN
			   		SUM(IMPORTE)
			   END), 0) TA_INCOBRA
		FROM
			(
	...
	------------------------- MOVIMIENTOS DE CUENTA CORRIENTE --------------------------------------			
				SELECT SUBG.PAIS_OID_PAIS,
					   ...
					   FROM CCC_SUBPR SUBP,
					   		CCC_PROCE PRO
					   WHERE SUBP.OID_SUBP = MOV_CCC.SUBP_OID_SUBP_CREA
					   ...
					   FROM CCC_SUBPR SUBP,
					   		CCC_PROCE PRO
					   WHERE SUBP.OID_SUBP = MOV_CCC.SUBP_OID_SUBP_ULTI
					   		 AND SUBP.CCPR_OID_proc= PRO.OID_PROC
					   ) PROC_ULTIMO,
					   'A' AS TIPO,
				...
				FROM CCC_MOVIM_CUENT_CORRI MOV_CCC,
					 ...
					 FROM	  
					 	(	  
						SELECT PERI.FEC_INIC 
						FROM CRA_PERIO PERI
						WHERE PERI.OID_PERI = 60  	   									 -- PARAMETRO DE PERIODO DESDE --  	  
						),	  
						(
						SELECT PERI.FEC_FINA
						FROM CRA_PERIO PERI
						WHERE PERI.OID_PERI = 1 		   									 -- PARAMETRO DE PERIODO HASTA --  	  
						)	  	  
					) RANGO
				WHERE MOV_CCC.ZORG_OID_REGI = REG.OID_REGI
					  AND MOV_CCC.ZSCC_OID_SECC = SEC.OID_SECC 
				...
				--	  AND PERI.FEC_INIC >= RANGO.FEC_INIC
				--	  AND PERI.FEC_FINA <= RANGO.FEC_FINA
				)
				UNION
				(
	-------------------- MOVIMIENTOS EN HISTORICO DE CUENTAS CORRIENTES ------------------------------------------
			...
			WHERE DATOS.PAIS_OID_PAIS = GEN_PAIS.VAL_OID(+)
				  ...
				  AND CLI.OID_CLIE(+) = DATOS.CLIE_OID_CLIE
