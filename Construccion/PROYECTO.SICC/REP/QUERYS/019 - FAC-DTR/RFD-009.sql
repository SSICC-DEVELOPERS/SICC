		...
		(
			  ...
			  -------------------------------- CALCULO DE FECHA DEL PERIODO NUEMRO 18 -------------------------------			   
				   CASE WHEN ((OID_CAMP_INGRESO IS NOT NULL) AND (PERIODO_ULTIMO_PEDIDO IS NOT NULL)) THEN
					   CASE WHEN (OWN_PERU.FN_112_CALC_PER_ANT(1, 1, 1, OID_CAMP_INGRESO, PERIODO_ULTIMO_PEDIDO) > 18) THEN  -- PARAMETRO DE PAIS, MARCA, CANAL --
							...
							SELECT FEC_INIC 
							FROM CRA_PERIO
							WHERE OID_PERI = OWN_PERU.FN_008_CALC_OID_PER_ANT(1,1,1, PERIODO_ULTIMO_PEDIDO,18) -- PARAMETRO DE PAIS, MARCA, CANAL --
							)    				   		
					   ELSE
					   	   (	
						   SELECT FEC_INIC
			...
			--					 AND SOL.FEC_FACT IS NOT NULL
					   ...
					   WHERE CLI_OBS.CLIE_OID_CLIE = SEG_CLI.CLIE_OID_CLIE
					   		 AND CLI_OBS.MARC_OID_MARC = 1  			  			-- PARAMETRO DE MARCA --
					   ) OBSERVACIONES
				FROM DTR_SEGME_CLIEN SEG_CLI,
					 ...
					 ZON_REGIO REG,
					 (
					 SELECT ROUND(MIN(MM.VAL_MONT_MINI_NOMI)*1.2, 2) VAR
					 FROM PED_TIPO_SOLIC_PAIS TS,
					 	  PED_MONTO_MINIM MM
					 WHERE TS.PAIS_OID_PAIS = 1	 					-- PARAMETRO DE PAIS --
					...
					WHERE FI.FEC_INIC <= PERI.FEC_INIC
						  ...
						  AND PQ_APL_REP.FN_CU_CALC_SALDO_CTA_CTE(PERI.PAIS_OID_PAIS, SEG_CLI.CLIE_OID_CLIE) <= 0
					--	  AND SEG_CLI.NVSG_OID_NIVE_SEGM = 8
