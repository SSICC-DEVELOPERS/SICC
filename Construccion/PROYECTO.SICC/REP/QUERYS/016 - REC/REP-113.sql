	...
	(
	 	...
	 	FROM
			...
			FROM PED_SOLIC_CABEC SOL_CAB,
				 ...
				 SELECT FEC_INIC
				 FROM CRA_PERIO
				 WHERE OID_PERI = 57 	 	-- PARAMETRO DE PERIODO INICIAL --
				 ) FI,
				 (
				 SELECT FEC_FINA
				 FROM CRA_PERIO
				 WHERE OID_PERI = DECODE(-1, -1, 57, 59) -- PARAMETRO DE PERIDO FINAL --
 -- PUEDE SER NULO --
				 ) FF	 
	...
	(
		...
		FROM PED_SOLIC_CABEC SOL_CAB,
			 ...
			 SELECT FEC_FINA
			 FROM CRA_PERIO
			 WHERE OID_PERI = DECODE(-1, -1, 57, 59) -- PARAMETRO DE PERIDO FINAL -- -- PUEDE SER NULO --
			 ) FF	 
		WHERE PERI.FEC_INIC >= FI.FEC_INIC
			  AND SOL_CAB.ZZON_OID_ZONA = ZONA.OID_ZONA
			  AND SOL_CAB.PERD_OID_PERI = PERI.OID_PERI
