SELECT 
	   ...
	   (COUNT(DISTINCT(ENVIO_ATRIBUTO.CLIE_OID_CLIE))*DETAL_MAV.NUM_UNID_CLIE) AS CANTIDAD_ESTIMADA,
	   ...
	   SCRIT.DES_SUBC,
	   (
	   	CASE WHEN (DETAL_MAV.SCAS_OID_SUBC = 3) THEN 
				 TO_CHAR(DETAL_MAV.NUM_ANIV)
		 	WHEN (DETAL_MAV.SCAS_OID_SUBC = 4) THEN 
				 TO_CHAR(DETAL_MAV.VAL_EDAD_DESD || ' - ' || DETAL_MAV.VAL_EDAD_HAST)  
		 	WHEN (DETAL_MAV.SCAS_OID_SUBC = 5) THEN 
				 TO_CHAR(DETAL_MAV.VAL_MONT)
		 	WHEN (DETAL_MAV.SCAS_OID_SUBC = 6) THEN 
				 TO_CHAR(DETAL_MAV.VAL_MONT)
		 	WHEN (DETAL_MAV.SCAS_OID_SUBC = 7) THEN 
				 TO_CHAR(DETAL_MAV.VAL_MONT)
			ELSE ''
	   END
	   ) AS VALOR_CRITERIO, 
...
----------------------------------------------------------------------------
	...
	(
/******************************************************************************************************
Me traigo de envio_mav la informacion corresp a criterio de asignacion "Por atributo" (1) con la que
despues, segun el subcriterio, se realizara el filtro correspondiente.
******************************************************************************************************/
	 SELECT 
		   ENV.CLIE_OID_CLIE, ENV.DENV_OID_DETA_MAV, ENV.ACTI_OID_ACTI, ENV.NUM_UNID,
		   FECH_DESDE_HASTA.FEC_INIC, 
		   TO_DATE(  TO_CHAR(  CLIEN_ADICI.FEC_NACI,'DD/MM' ) || 
		   			 		   			CASE WHEN ( 
											 	  TO_CHAR(FECH_DESDE_HASTA.FEC_INIC,'YYYY') <
											 	  TO_CHAR(FECH_DESDE_HASTA.FEC_FINA,'YYYY')
												  )
											 THEN TO_CHAR(FECH_DESDE_HASTA.FEC_INIC,'YYYY')
											 ELSE TO_CHAR(FECH_DESDE_HASTA.FEC_FINA,'YYYY') 
										END) AS CUMPLE,
		   FECH_DESDE_HASTA.FEC_FINA,
	 	   TRUNC((FECH_DESDE_HASTA.FEC_FINA-CLIEN.FEC_INGR)/365) AS ANIVERSARIO,
		   CLIEN_ADICI.VAL_EDAD,
		   NVL((-- Calculo del monto de todos los pedidos del cliente ...................... 
		    SELECT SUM(PED.VAL_PREC_CATA_TOTA_LOC_UNI_DEM) AS MONTO
			FROM PED_SOLIC_CABEC PED
			WHERE PERD_OID_PERI = PERI_ANTERIOR.OID_PERI
				  AND PED.CLIE_OID_CLIE = ENV.CLIE_OID_CLIE 
			GROUP BY PERD_OID_PERI, PED.CLIE_OID_CLIE
		   ),0) AS MONTO,
		   NVL((-- Calculo del monto de todos los pedidos del cliente para los productos de la marca determinada...
		    SELECT SUM(PED_POSIC.VAL_PREC_CATA_TOTA_LOCA_UNID) AS MONTO_MARCA
			FROM PED_SOLIC_CABEC PED,
				 PED_SOLIC_POSIC PED_POSIC,
				 MAE_PRODU PRODU
			WHERE PED.OID_SOLI_CABE = PED_POSIC.SOCA_OID_SOLI_CABE
				  AND PED_POSIC.PROD_OID_PROD = PRODU.OID_PROD
				  AND PRODU.MAPR_OID_MARC_PROD = PERI_ANTERIOR.MARC_OID_MARC
				  AND PERD_OID_PERI = PERI_ANTERIOR.OID_PERI
				  AND PED.CLIE_OID_CLIE = ENV.CLIE_OID_CLIE 
		   ),0) AS MONTO_MARCA,
		   NVL((-- Calculo del monto de todos los pedidos del cliente para los productos de la un. negocio determ. ...
		    SELECT SUM(PED_POSIC.VAL_PREC_CATA_TOTA_LOCA_UNID) AS MONTO_MARCA
			FROM PED_SOLIC_CABEC PED,
				 PED_SOLIC_POSIC PED_POSIC,
				 MAE_PRODU PRODU
			WHERE PED.OID_SOLI_CABE = PED_POSIC.SOCA_OID_SOLI_CABE
				  AND PED_POSIC.PROD_OID_PROD = PRODU.OID_PROD
				  AND PRODU.NEGO_OID_NEGO = DET.NEGO_OID_NEGO
				  AND PERD_OID_PERI = PERI_ANTERIOR.OID_PERI
				  AND PED.CLIE_OID_CLIE = ENV.CLIE_OID_CLIE 
		   ),0) AS MONTO_UN_NEGO
	 FROM
		   ...
		   (
		 	SELECT OID_PERI, FEC_INIC, MARC_OID_MARC  
			FROM (
				  SELECT PER3.OID_PERI, PER3.FEC_INIC, PER3.MARC_OID_MARC
			 	  FROM (
				  	   	SELECT PER2.OID_PERI, PER2.FEC_INIC, PER2.MARC_OID_MARC
					 	FROM CRA_PERIO PER2
		   ...
		   (
		    SELECT PERI_DH.OID_PERI, PERI_DH.FEC_INIC, PERI_DH.FEC_FINA
			FROM CRA_PERIO PERI_DH
		   ) FECH_DESDE_HASTA
	 WHERE
		   ...
		   AND SCRI.CRAS_OID_CRIT_ASIG = CRI.OID_CRIT_ASIG
	 	   ...
	 	   AND CRI.OID_CRIT_ASIG = 1
		   AND FECH_DESDE_HASTA.OID_PERI = DET.PERD_OID_PERI
-----------------------------------------------------------------------------
	 	   AND ENV.CLIE_OID_CLIE = UNADMIN.CLIE_OID_CLIE
...
-----------------------------------------------------------------------------
		   AND ENV.CLIE_OID_CLIE = CLIEN_ADICI.CLIE_OID_CLIE
		   AND ENV.CLIE_OID_CLIE = CLIEN.OID_CLIE		   
/******************************************************************************************************/
	) ENVIO_ATRIBUTO,
	(
...
-----------------------------------------------------------------------------------
	...
	AND ENVIO_ATRIBUTO.ACTI_OID_ACTI(+) = DETAL_MAV.ACTI_OID_ACTI
	AND ENVIO_ATRIBUTO.DENV_OID_DETA_MAV(+) = DETAL_MAV.OID_DETA_MAV
----------------------------------------------------------------------------------------------------------------------
	AND (
--------------------		 FILTRO CUMPLEAÑOS			 ------------------------------------
		 ( DETAL_MAV.SCAS_OID_SUBC = 2 AND ENVIO_ATRIBUTO.CUMPLE BETWEEN ENVIO_ATRIBUTO.FEC_INIC AND ENVIO_ATRIBUTO.FEC_FINA  )
		  OR 
--------------------		 FILTRO ANIVERSARIO		 ------------------------------------	
		 ( DETAL_MAV.SCAS_OID_SUBC = 3 AND ENVIO_ATRIBUTO.ANIVERSARIO = DETAL_MAV.NUM_ANIV  )
		  OR 
--------------------		 FILTRO EDAD			 ------------------------------------	
		 ( DETAL_MAV.SCAS_OID_SUBC = 4 AND ENVIO_ATRIBUTO.VAL_EDAD BETWEEN DETAL_MAV.VAL_EDAD_DESD AND DETAL_MAV.VAL_EDAD_HAST ) 
		  OR 
--------------------		 FILTRO MONTO			 ------------------------------------	
		 ( DETAL_MAV.SCAS_OID_SUBC = 5 AND ENVIO_ATRIBUTO.MONTO > DETAL_MAV.VAL_MONT ) 
		  OR 
--------------------		 FILTRO MONTO MARCA	 	 ------------------------------------	
		 ( DETAL_MAV.SCAS_OID_SUBC = 6 AND ENVIO_ATRIBUTO.MONTO_MARCA > DETAL_MAV.VAL_MONT ) 
		  OR 
--------------------		 FILTRO MONTO UNIDAD DE NEGOCIO	 	 ------------------------------------	
		 ( DETAL_MAV.SCAS_OID_SUBC = 7 AND ENVIO_ATRIBUTO.MONTO_UN_NEGO > DETAL_MAV.VAL_MONT ) 
		)--FIN DEL AND 
----------------------------------------------------------------------------------------------------------------------
...
GROUP BY    
	I18PAIS.VAL_I18N,
	   ...
	   T_DESPA.DES_TIPO_DESP, 
	   DETAL_MAV.VAL_CRIT,	    
	   DETAL_MAV.NUM_UNID_CLIE,
	   SCRIT.DES_SUBC,
	   NVL((DETAL_MAV.VAL_BASE_ESTI_DEST * DETAL_MAV.VAL_FACT_CORR * DETAL_MAV.NUM_UNID_ESTI), 0),
	   DETAL_MAV.SCAS_OID_SUBC, DETAL_MAV.NUM_ANIV, DETAL_MAV.VAL_EDAD_DESD, DETAL_MAV.VAL_EDAD_HAST, DETAL_MAV.VAL_MONT
ORDER BY
	I18ACTIV.VAL_I18N,
	OFDET.VAL_CODI_VENT, 
	PRODU.COD_SAP
