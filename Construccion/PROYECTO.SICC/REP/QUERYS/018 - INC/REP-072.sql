...
(
	SELECT DES_SUBG_VENT,
		   ...
		   SUM(CONSTANCIA) CON_MET_CONS,
		   DECODE(COUNT(*),0,0,(SUM(CONSTANCIA)/COUNT(*))*100) PORC_MET_CONS
	FROM
	(
			SELECT PC.NUM_CONC,
				   PC.VAL_NOMB,
				   ...
				   CASE WHEN (((SELECT 1
				   			    ...
				   			    WHERE CG.CLIE_OID_CLIE = MET.CLIE_OID_CLIE
							  	      AND CG.COPA_OID_PARA_GRAL = PC.OID_PARA_GRAL
									  AND CG.IND_META_SUPE = 1
							  		  AND ROWNUM = 1) = 1) AND (SELECT COUNT(*)
									        					FROM INC_CANDI_GANAD GAN
  																WHERE GAN.CLIE_OID_CLIE = MET.CLIE_OID_CLIE
  																	  AND GAN.COPA_OID_PARA_GRAL = PC.OID_PARA_GRAL
															    ) = 
								(
								 ...
								 WHERE CAN_GAN.CLIE_OID_CLIE = MET.CLIE_OID_CLIE
								 	   AND CAN_GAN.COPA_OID_PARA_GRAL =PC.OID_PARA_GRAL )      
									) THEN
									1
								ELSE
									0
