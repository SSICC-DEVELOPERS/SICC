...
(
		   ...
		   ELSE
				 ...
				 FROM INC_METAS_VENTA_PROYE_ZONA MET2,
				 	  (
				 	   SELECT P.OID_PERI
				 	   FROM CRA_PERIO P
					   WHERE P.PAIS_OID_PAIS = 1 			 -- PARAMETRO DE PAIS --
	...
	WHERE SUBG.OID_SUBG_VENT = REG.ZSGV_OID_SUBG_VENT 
		  ...
		  AND MET.COPA_OID_PARA_GRAL = 1									-- PARAMETRO DE CONCURSO --
		  ...
		  AND ZON.OID_ZONA IN (1,2,3,4,5,6,7,8,9)							-- PARAMETRO DE ZONA --
	GROUP BY COD_ZONA,
		...
		FROM ZON_SUB_GEREN_VENTA SUBG,
			 ...
			 (
				SELECT CASE WHEN ((PERI.FEC_INIC <= SYSDATE) AND (PERI.FEC_FINA >= SYSDATE)) THEN
					   	   FN_112_CALC_PER_ANT(PERI.PAIS_OID_PAIS, PERI.MARC_OID_MARC,PERI.CANA_OID_CANA, CPG.PERD_OID_PERI_DESD, PERI.OID_PERI)
					   ELSE
					   	   FN_112_CALC_PER_ANT(PERI.PAIS_OID_PAIS, PERI.MARC_OID_MARC,PERI.CANA_OID_CANA, CPG.PERD_OID_PERI_DESD, (				 	  
								 	   SELECT P.OID_PERI
								 	   FROM CRA_PERIO P
									   WHERE P.PAIS_OID_PAIS = 1 			 -- PARAMETRO DE PAIS --
									   		 AND P.MARC_OID_MARC = 1		 -- PARAMETRO DE MARCA --
				...
				FROM CRA_PERIO PERI,
					 INC_CONCU_PARAM_GENER CPG	
				WHERE CPG.OID_PARA_GRAL = 1						-- PARAMETRO DE CONCURSO --
	...
	(
		SELECT ZON.COD_ZONA,
			   ...
			   SUM(MET.NUM_PEDI) PEDI_PLAN,
	   		   (SUM(RES.IMP_VENT_NETA_ESTA)/SUM(RES.NUM_PEDI))/PERIODO.CANT_PERI PVP_REAL,
			   (SUM(MET.IMP_VENT_NETA_ESTA)/SUM(RES.NUM_PEDI))/PERIODO.CANT_PERI PVP_PLAN
		FROM ZON_SUB_GEREN_VENTA SUBG,
			 ZON_REGIO REG,
			 ZON_ZONA ZON,
			 INC_METAS_VENTA_PROYE_ZONA MET,
...
(
	...
	(
		...
		(
			SELECT DATOS.COD_ZONA,
				   ...
				   NVL(DATOS.VENTA_REAL*(SELECT VVRC.VAL_PESO/VVRC.VAL_DIVI
									 ...
									 WHERE VVRC.COPA_OID_PARA_GRAL = 1	   -- PARAMETRO DE CONCURSO --
		...
		ORDER BY PUNT_REAL,
			  	 ((PUNT_REAL-PUNT_PLAN)*100)/PUNT_PLAN
	)
)
