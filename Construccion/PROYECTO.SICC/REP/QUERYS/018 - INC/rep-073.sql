...
( 
	...
	(
		select DES_SUBG_VENT,
			   ...
			   --------------------------------------------------------------------------------------------------------
			   case when (monto_venta * 100) / meta >= 100 then ((monto_venta * 100) / meta ) - 100 end as por_variacion,
			   own_peru.FN_072_RANGO(meta) grupo,
			   --------------------------------------------------------------------------------------------------------
			   decode(
		...
		from (
			select subg.DES_SUBG_VENT,
 				...
 				case when 
 					(
 					 select sum(can_gan.IND_META_SUPE)
 					...
 					(
 					 ...
 					 where gan.CLIE_OID_CLIE = un_adm.clie_oid_clie
 						  and gan.COPA_OID_PARA_GRAL = con.oid_para_gral
 					 ) = 
 					(
 					 select SUM(case when can_gan.VAL_REQU_PREM_SUPE = 1 then 1
 					 		else 							(
 														   	select 1 
 															from ped_solic_cabec sol
 															where sol.clie_oid_clie = can_gan.clie_oid_clie
 																  and sol.PERD_OID_PERI = can_gan.PERD_OID_PERI 
 															) end) 
 					 from inc_candi_ganad can_gan
 					 where can_gan.clie_oid_clie = un_adm.clie_oid_clie
 					 	   and can_gan.copa_oid_para_gral = con.oid_para_gral
 					) then 1 else 0 end as ind_meta_const
 				-----------------------------------------------------------------------------------
...
where grupo in (0,1000)  --PARAMETRO MONTO DE VENTA DESDE--
group by DES_SUBG_VENT,
	   DES_REGI,
	   des_zona,
	   num_conc,
