SELECT SUBG,
	   ...
	   GRUPO_HAS,
	   TOTAL_CON_RANGO,
	   (TOTAL_CON_RANGO/TOTAL_CON_CONCU)*100 PORC_CON_RANGO,
	   SUM((CASE WHEN (XX = 1) THEN
	   			 ((TOTAL_CON_RANGO/TOTAL_CON_CONCU)*100)
		    ELSE
				0
			END)) OVER (PARTITION BY SUBG, REGION, ZONA, NUM_CONC ORDER BY GRUPO_DES) PORC_ACUM,
	   GRUPO_PEDI,
	   COUNT(*) CANT_PERI,
	   (COUNT(*)/TOTAL_CON_RANGO)*100 PORC_PERI	   
FROM
(	
	SELECT SUBG,
		   REGION,
		   ...
		   ELSE
		   	    GRUPO + 1000
		   END GRUPO_HAS,	
   		   SUM(1) OVER (PARTITION BY SUBG, REGION, ZONA, NUM_CONC, GRUPO ORDER BY GRUPO_PEDI) XX,  
		   COUNT(CLIEN) OVER (PARTITION BY SUBG, REGION, ZONA, OID_PARA_GRAL, NUM_CONC, GRUPO) TOTAL_CON_RANGO,
		   COUNT(CLIEN) OVER (PARTITION BY SUBG, REGION, ZONA, OID_PARA_GRAL, NUM_CONC) TOTAL_CON_CONCU,
		   GRUPO_PEDI		   
	FROM
		(
			SELECT SUBG,
				   ...
				   NUM_CONC,
				   CLIEN,
				   FN_072_RANGO(META) GRUPO,
				   CASE WHEN (COUNT(*) - 1 = SUM(PASO_PED)) THEN
				   		SUM(PASO_PED)||'-'||COUNT(*)
				   END GRUPO_PEDI
			FROM
				(
				 SELECT DAT.SUBG,
						...
						WHERE SC.CLIE_OID_CLIE = DAT.CLIEN
							  AND SC.PERD_OID_PERI = PERIO.OID_PERI
					    ) PASO_PED
				 FROM
				 	(		
					SELECT SUBG.DES_SUBG_VENT SUBG,
						   ...
						   CASE WHEN (CLI.FEC_INGR <= P_INI.FEC_INIC) THEN
						   	   P_INI.FEC_INIC						   
						   ELSE
						   	   (
							   SELECT FEC_INIC
							   FROM CRA_PERIO P
							   WHERE P.OID_PERI = PERI2.OID_PERI
							   )
						   END FEC_PERIDESDE,
						   (
							   SELECT P.FEC_FINA
							   FROM CRA_PERIO P
							   WHERE P.OID_PERI = CPG.PERD_OID_PERI_HAST
						   ) FEC_PERIHASTA
					...
					WHERE CPG.OID_PARA_GRAL = 1	  					-- PARAMETRO DE NUMERO DE CONCURSO --
						...
						--  AND CD.COD_CAUS IN ('FC', 'FA')
						  ...
						  AND CLI.OID_CLIE = MET.CLIE_OID_CLIE
						  ...
						  AND PERI2.FEC_INIC <= CLI.FEC_INGR
						  AND PERI2.FEC_FINA >= CLI.FEC_INGR
						  AND PERI2.PAIS_OID_PAIS = SUBG.PAIS_OID_PAIS
						  AND PERI2.MARC_OID_MARC = SUBG.MARC_OID_MARC
						  AND PERI2.CANA_OID_CANA = SUBG.CANA_OID_CANA
...
--						  AND CLI_SUB.SBTI_OID_SUBT_CLIE = 1			 -- PARAMETRO DE SUBTIPO DE CLIENTE --
				...
				SELECT *
				FROM CRA_PERIO PE
				WHERE PE.PAIS_OID_PAIS = 1				   -- PARAMETRO DE PAIS --
					  AND PE.MARC_OID_MARC = 1			   -- PARAMETRO DE MARCA --
					  AND PE.CANA_OID_CANA = 1			   -- PARAMETRO DE CANAL --
				) PERIO
				WHERE PERIO.FEC_INIC >= DAT.FEC_PERIDESDE
					  AND PERIO.FEC_FINA <= DAT.FEC_PERIHASTA 
	--				  AND PUN_OBTE >= PUN_META
			  )
			GROUP BY SUBG,
				   REGION,
				   ZONA,
	...
	--WHERE GRUPO_PEDI IS NOT NULL
)
WHERE GRUPO IN (1500, 6000, 49000)
GROUP BY SUBG,
	   REGION,
	   ZONA,
	   NUM_CONC,
