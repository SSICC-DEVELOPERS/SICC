SELECT SUBG,
	   ...
	   VENTA_IND_RECTE,
	   BENEFICIO,
	   DECODE((SUM(BENEFICIO) OVER (PARTITION BY SUBG, REGION, ZONA, COD_CLI_RECTE)), 0, 0, BENEFICIO/(SUM(BENEFICIO) OVER (PARTITION BY SUBG, REGION, ZONA, COD_CLI_RECTE))) PROM_BENE,
	   COD_CLIE_RECDO,
	   NOMBRE_RECDO,
	   VENTA_IND_RECDA
FROM
(
	SELECT SUBG,
		   ...
		   VENTA_IND_RECTE,
		   CASE WHEN (L_CALI = 1) THEN
		   		SUM(VENTA_IND_RECDA) OVER (PARTITION BY SUBG, REGION, ZONA,PERIODO, COD_CLI_RECTE)
		   ELSE
		   	   0
		   END BENEFICIO,	
		   COD_CLIE_RECDO,
	...
	(
		...
		FROM INC_CONCU_PARAM_GENER CGP,
			 (
				 SELECT FEC_FINA 
				 FROM CRA_PERIO 
				 WHERE OID_PERI = 1	   	   			  				   -- PARAMETRO DE PERIODO -- 	   		
			 ) P_FF,
			 (
			 SELECT *
			 FROM CRA_PERIO PE
			 WHERE PE.OID_PERI = (
				 ...
				 FROM CRA_PERIO P_FI,
					  ...
					  (
						  ...
						  FROM CRA_PERIO P
						  WHERE P.OID_PERI =  (
											  SELECT FN_008_CALC_OID_PER_ANT(1,1,1,1,5)			   -- PARAMETRO DE PAIS, MARCA, CANAL , PERIODO --
											  FROM DUAL
											  )
					  ) P_5
				 WHERE P_FI.OID_PERI = CPG.PERD_OID_PERI_DESD
				 	   AND CPG.OID_PARA_GRAL = 1			 			   -- PARAMETRO DE CONCURSO --
				)
			 ) P_FI,
			 CRA_PERIO PERI,
			 INC_PLANT_CONCU PLA,
		...
		WHERE CGP.OID_PARA_GRAL = CPC.COPA_OID_PARA_GRAL
			  ...
			  AND CRM.CLIE_OID_CLIE = CRD.CLR2_CLIE_OID_CLIE
    		  ...
    		  AND PLA.TVEN_OID_TIPO_VENT = 1	   			 	-- PARAMETRO DE TIPO DE VENTA --
			  ...
			  AND CGP.OID_PARA_GRAL = 1	   						-- PARAMETRO DE CONCURSO --
	)		 
)
