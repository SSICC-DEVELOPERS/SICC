select 
	   ...
	   (	  
		...
		--los campos siguientes no estan en el layout---
	    mat_cur.IND_ALCA_GEOG,
	  est_cli.COD_ESTA_CLIE,
	   sub_clien.COD_SUBT_CLIE,
   	   tipo_cla.COD_TIPO_CLAS,
	   clas.COD_CLAS,
	   mat_cur.NUM_ORDE,
	   per.VAL_NOMB_PERI per_inic_const,
   	   per2.VAL_NOMB_PERI per_fina_const,
   	   per3.VAL_NOMB_PERI per_inic_comp,
   	   per4.VAL_NOMB_PERI per_fina_comp,
	   mat_cur.VAL_MONT_VENT,
	   per5.VAL_NOMB_PERI per_ingreso,
	   mat_cur.FEC_INGR,
	   mat_cur.NUM_CAMPA,
	   mat_cur.FEC_ULTI_CURS,
	   mat_cur.IND_COND_PEDI,
	   mat_cur.IND_CTRL_MORO,
	   mat_cur.IND_CTRL_func*/
	   --
from edu_matri_curso mat_cur,
	 ...
	 mae_subti_clien sub_clien,
	 mae_tipo_clasi_clien tipo_cla,
	 mae_clien_clasi cla_cli,
	 mae_clasi clas,*/
	 cra_perio per,
 	 cra_perio per2,
 	 cra_perio per3,
...
--	  and mat_cur.CPLC_OID_CABE_PLAN_CURS = 1 --PARAMETRO--
	  ...
	  and mat_cur.ESTA_OID_ESTA_CLIE = est_cli.OID_ESTA_CLIE(+)
	  and mat_cur.TCCL_OID_TIPO_CLAS = tipo_cla.OID_TIPO_CLAS(+)*/
	  and mat_cur.TICL_OID_TIPO_CLIE = tipo_clien.OID_TIPO_CLIE(+)/*
	  and mat_cur.SBTI_OID_SUBT_CLIE = sub_clien.OID_SUBT_CLIE(+)
	  and mat_cur.CLAS_OID_CLAS = cla_cli.OID_CLIE_CLAS(+)
	  and cla_cli.CLAS_OID_CLAS = clas.OID_CLAS(+)*/
...
--	  and mat_cur.PERD_OID_PERI_INGR = per5.OID_PERI(+)
