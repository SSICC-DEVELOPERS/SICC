...
--		 AND SOLPOSIC.TPOS_OID_TIPO_POSI = 2 -- (RE - Recuperacion - PED_TIPO_POSIC) 
	...
	(
	 ...
	 WHERE
		 ...
		 AND ZON.ZORG_OID_REGI = REG.OID_REGI
--		 AND LOPEREC.TIMO_OID_TIPO_MOVI = 1000 -- (EL UNICO EN CODIGO EN -->> REC_TIMO_MOVIM ) 
		 AND REG.OID_REGI = 1  	 			 						 		 -- PARAMETRO -- 
...
WHERE
	...
	AND MRECU.MAFA_OID_COD_PPAL = ORIG.OID_MATR_FACT  
	AND MRECU.MAFA_OID_COD_RECU = RECU.OID_MATR_FACT
	AND UN_RECUP.VAL_CODI_VENT(+) = RECU.VAL_CODI_VENT
	AND UN_DEVU.VAL_CODI_VENT(+) = RECU.VAL_CODI_VENT
/*
	AND RECU.VAL_CODI_VENT >= 1	   			--//  Estos son, en teoria los codigos por los que hay que filtrar pero 
	AND RECU.VAL_CODI_VENT <= 9999999		--// Matias no los tiene y levanto una incidencia 
*/
--	AND ( ORIG.OID_PERI = RECU.OID_PERI )			  	  			-- Condicion para Tipo Recuperacion Inmediata --
--	AND ( ORIG.OID_PERI <> RECU.OID_PERI AND IND_RECU_AUTO = 1 )	-- Condicion para Tipo Recuperacion Obligatoria --
--	AND ( ORIG.OID_PERI <> RECU.OID_PERI AND IND_RECU_AUTO = 0 )	-- Condicion para Tipo Recuperacion Opcional --
ORDER BY
	RECU.PAIS, RECU.DES_MARC, RECU.CANAL,
	TIPORECUPERACION,
	VAL_CODI_VENT,
