...
<!--
   ...
   $Id: drd30_text.xsl,v 1.1 2009/12/03 19:03:44 pecbazalar Exp $
-->
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
   ...
   <!--   estilo  -->
   <xsl:template match="TEXT" mode="estilo">
      <xsl:if test="@x and @y">
   ...
   <xsl:template match="TEXT" mode="dinamico">
      ...
      <xsl:variable name="vValidDefecto" select="$vgConfig/DATOSPROY/VALIDACIONES/CTEXTO"/>
   ...
   <xsl:template match="TEXT" mode="script">
      ...
      <xsl:text>.</xsl:text>
      <xsl:value-of select="@nombre"/>
      <xsl:text>RO='</xsl:text><xsl:value-of select="@readonly"/><xsl:text>';</xsl:text>
      ...
      <xsl:text>window.colReadOnly='</xsl:text><xsl:value-of select="$vgColReadOnly"/><xsl:text>';
</xsl:text>
      ...
      <!-- EVENTO ONKEYUP asociado a onmax -->
      ...
      <!-- EVENTO ONKEYDOWN asociado a onreturn y ontab-->
   ...
   <xsl:template match="TEXT" mode="bodyNormal">
...
<!--            <xsl:attribute name="onFocus">
         ...
         <xsl:choose>
            ...
            <xsl:when test="not(@onkeyup) and $vgConfig/DATOSPROY/EVENTO_ONKEYUP/CTEXTO">
               <xsl:attribute name="onKeyUp">
                  <xsl:value-of select="$vgConfig/DATOSPROY/EVENTO_ONKEYUP/CTEXTO"/>
               </xsl:attribute>
            </xsl:when>
