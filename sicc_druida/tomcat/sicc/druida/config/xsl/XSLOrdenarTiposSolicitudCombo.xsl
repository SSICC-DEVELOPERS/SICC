...
<xsl:template match="//ROWSET2">
  <ROWSET2>
   ...
   <xsl:apply-templates select="./ROWSET[@ID='DTOSalida.resultadoTipoSolicitudPorPais_ROWSET']"></xsl:apply-templates>
  </ROWSET2>
 </xsl:template>
 ...
 <xsl:template match="ROWSET" >
   <ROWSET>
    <xsl:apply-templates select="@*"></xsl:apply-templates>
     ...
     <xsl:sort select="CAMPO[@NOMBRE='DES_TIPO_SOLI']"></xsl:sort>
    </xsl:apply-templates>
   </ROWSET>
 </xsl:template>
