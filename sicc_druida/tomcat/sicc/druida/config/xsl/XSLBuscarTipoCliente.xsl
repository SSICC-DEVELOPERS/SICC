...
<xsl:output version="1.0" encoding="ISO-8859-1" omit-xml-declaration="no" method="xml" indent="yes"/> 
 ...
 <xsl:template match="ROWSET" >
   <ROWSET>
    <xsl:apply-templates select="@*"></xsl:apply-templates>
     ...
     <xsl:sort select="CAMPO[@NOMBRE='DES_TIPO_CLIE']"></xsl:sort>
    </xsl:apply-templates>
   </ROWSET>
 </xsl:template>
 <xsl:template match="ROW" >
  <ROW>
   ...
   <xsl:choose>
	<xsl:when test="./CAMPO[position()=3]=0"><CAMPO NOMBRE="IND_EVAL_STAT" TIPO="STRING">No</CAMPO></xsl:when>
	<xsl:otherwise><CAMPO NOMBRE="IND_EVAL_STAT" TIPO="STRING">Si</CAMPO></xsl:otherwise>
  </xsl:choose>
  </ROW>
 </xsl:template>
