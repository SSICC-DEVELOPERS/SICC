...
<!--
   INDRA/CAR/PROY
   $Id: XslMantenimientoPerfilesInsertar.xsl,v 1.1 2009/12/03 19:02:31 pecbazalar Exp $
   DESC
-->
			...
			<td  class="datosCamposCenterPar" align="middle">
				...
				<input type="checkbox">
					...
					<xsl:attribute name="onclick">
						<xsl:text disable-output-escaping="yes">mostrar('_</xsl:text><xsl:value-of 
select="CAMPO[@NOMBRE='idfuncion']"/><xsl:text disable-output-escaping="yes">')</xsl:text>
			...
			<td class="datosCamposCenterImpar" align="middle">
				...
				<input type="checkbox">
					...
					<xsl:attribute name="onclick">
						<xsl:text disable-output-escaping="yes">inicio('_</xsl:text><xsl:value-of 
select="CAMPO[@NOMBRE='idfuncion']"/><xsl:text disable-output-escaping="yes">')</xsl:text>
					</xsl:attribute>
				</input>
			</td>
