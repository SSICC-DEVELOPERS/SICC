...
<!--
	$Id: XSLFuncionesMenu_Modificar.xsl,v 1.1 2009/12/03 19:02:34 pecbazalar Exp $
-->
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">

   <xsl:param name="seleccion" select="0"/>
   <xsl:param name="padre" select="0"/>
   <xsl:param name="nivel" select="5"/>
                     ...
                     <xsl:choose>
                        <xsl:when test="number(CAMPO[@NOMBRE='nodofinal'])=0">
                           <input type="radio" name="inputSeleccionado" value="{CAMPO[@NOMBRE='idfuncion']}">
                              <xsl:if test="$padre = CAMPO[@NOMBRE='idfuncion']">
                                 <xsl:attribute name="checked">
                                    <xsl:text>checked</xsl:text>
                                 </xsl:attribute>
                              </xsl:if>
                              <xsl:if test="$seleccion = CAMPO[@NOMBRE='idfuncion']">
