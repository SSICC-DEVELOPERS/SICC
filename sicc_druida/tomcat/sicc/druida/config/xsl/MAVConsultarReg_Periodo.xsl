...
<xsl:output version="1.0" encoding="ISO-8859-1" omit-xml-declaration="no" method="xml" indent="yes"/>
         <xsl:template match="//ROWSET2">
                  <ROWSET2>
                           ...
                           <xsl:apply-templates select="./ROWSET[@ID='DTOPeriodoSubgerencia.periodo_ROWSET']"></xsl:apply-templates>
                  </ROWSET2>
         </xsl:template>
         ...
         <xsl:template match="ROW">
                  <xsl:copy>
				    ...
				    <xsl:value-of select="CAMPO[position()=1]"></xsl:value-of>-<xsl:value-of select="CAMPO[position()=2]"></xsl:value-of>
                           ...
                           <xsl:if test="CAMPO[position()=3]">
	                           <xsl:apply-templates select="CAMPO[position()=3]"></xsl:apply-templates>
                           </xsl:if>
                  </xsl:copy>
         </xsl:template>
