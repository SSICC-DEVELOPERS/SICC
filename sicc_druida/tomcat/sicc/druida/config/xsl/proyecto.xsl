...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0"
	...
	exclude-result-prefixes="xmlns:drdcambia">
   <xsl:output method="html" indent="no" standalone="yes" encoding="ISO-8859-1"/>
   <xsl:variable name="vgConfig" select="document('../config/globales.xml')/DATOSPROY"/>
   <xsl:variable name="cssPath" select="$vgConfig/CSSPATH"/>
   <xsl:variable name="jsPath" select="$vgConfig/JSPATH"/>
   <xsl:variable name="imgPath" select="$vgConfig/IMGPATH"/>
   <xsl:variable name="drdPath" select="$vgConfig/DRDPATH"/>
   <xsl:variable name="ProxyServlet" select="$vgConfig/PROXYSERVLET"/>
   <xsl:variable name="imgTransparente" select="$vgConfig/IMAGENES/ESPACIADO/ICONO"/>
   <xsl:variable name="imgCampoRequerido" select="$vgConfig/IMAGENES/CAMPOREQ/ICONO"/>
   <xsl:variable name="txtCampoRequerido" select="$vgConfig/IMAGENES/CAMPOREQ/TEXTO"/>
   <xsl:variable name="vgTooltip" select="$vgConfig/TIPOTOOLTIP"/>
   ...
   <xsl:template name="EXT.init">
	<!--Menu secundario-->
	&#xa;try{document.onkeydown = fnCheckKeys;document.onhelp=fnShowHelp;}catch(e){;}&#xa;APP_CONTEXT_ROOT='<xsl:value-of select="$vgConfig/APP-CONTEXT-ROOT"/>';
   ...
   <xsl:template name="EXT.estilo">
      ...
      <xsl:choose>
         <xsl:when test="$vgConfig/COMPATIBLE_NT4X">
            <xsl:value-of select="$vgConfig/COMPATIBLE_NT4X"/>
         </xsl:when>
      ...
      <xsl:choose>
         <xsl:when test="$vgConfig/RUTAENTORNOV">
            <xsl:value-of select="$vgConfig/RUTAENTORNOV"/>
         </xsl:when>
         <xsl:otherwise>
   ...
   </xsl:variable><xsl:variable name="vgScrollResize" select="$vgConfig/SCROLLRESIZE"/><xsl:variable name="vgColReadOnly" select="$vgConfig/COLORREADONLY"/><xsl:variable name="vgColReq" select="$vgConfig/COLORCREQ"/><xsl:variable name="vgColNoReq" select="$vgConfig/COLORCNOREQ"/><xsl:variable name="vgColDeshabilitados" select="$vgConfig/COLORCDES"/><xsl:variable name="vgDisableFontSize" select="$vgConfig/DISABLEFONTSIZE"/><xsl:variable name="vgEnableFontSize" select="$vgConfig/ENABLEFONTSIZE"/><xsl:variable name="vgFormElementID" select="$vgConfig/FORMELEMENTID"/><xsl:variable name="vgRadioCheckColorOnText" select="$vgConfig/RADIOCHECKCOLORONTEXT"/><xsl:variable name="vglabelRC" select="$vgConfig/LABELRC"/><xsl:variable name="vLang" select="/PAGINA/@xml:lang"/><xsl:variable name="vLangExist" select="boolean(/PAGINA/@xml:lang)"/><xsl:variable name="vgTxtCampoRequerido">
      <xsl:choose>
         <xsl:when test="$vLangExist">
            <xsl:value-of select="$vgConfig/IMAGENES[lang($vLang)]/CAMPOREQ/TEXTO"/>
         </xsl:when>
         <xsl:otherwise>
            <xsl:value-of select="$vgConfig/IMAGENES/CAMPOREQ/TEXTO"/>
         </xsl:otherwise>
      </xsl:choose> 
   </xsl:variable><xsl:variable name="vgEstiloTooltip">
      <xsl:choose>
         <xsl:when test="$vgConfig/TOOLTIP">
            <xsl:value-of select="$vgConfig/TOOLTIP"/>
         </xsl:when>
      ...
      <xsl:choose>
         <xsl:when test="$vgConfig/TIPOTOOLTIP">
            <xsl:value-of select="$vgConfig/TIPOTOOLTIP"/>
         </xsl:when>
         <xsl:otherwise/>
      </xsl:choose> 
      ...
      <xsl:choose>
        <xsl:when test="$vgConfig/TIPOASIGNARJS"><xsl:value-of select="$vgConfig/TIPOASIGNARJS/text()"/></xsl:when>
        <xsl:otherwise/>
      </xsl:choose> 
   </xsl:variable><xsl:variable name="frameCache">
      <xsl:choose>
         <xsl:when test="$vgConfig/RUTA_FRAME_CACHE"><xsl:value-of select="$vgConfig/RUTA_FRAME_CACHE"/></xsl:when>
         <xsl:otherwise/>
      </xsl:choose> 
   </xsl:variable><xsl:variable name="noHacerNormalizeSpace">
      <xsl:choose>
         <xsl:when test="$vgConfig/NORMALIZAR_ESPACIOS and $vgConfig/NORMALIZAR_ESPACIOS='N'">S</xsl:when>
         <xsl:otherwise>N</xsl:otherwise>
      </xsl:choose> 
   </xsl:variable><xsl:variable name="vgQuieroDruidaBack">
      <xsl:choose>
         <xsl:when test="$vgConfig/DRUIDABACK and $vgConfig/DRUIDABACK='S'">S</xsl:when>
         <xsl:otherwise>N</xsl:otherwise>
      </xsl:choose> 
...
DrMainEstatico.moveTo('</xsl:text>
      ...
      <xsl:if test="$vNavegador">
          <xsl:choose>
            <xsl:when test="$vgConfig/TIPOSCROLL = 'NATIVO'  and  $vNavegador='IE'">S</xsl:when>
            <xsl:otherwise>N</xsl:otherwise>
          </xsl:choose>
      </xsl:if>
  ...
  <xsl:choose>
	...
	<xsl:if test="@estilosCeldas and string-length(@estilosCeldas) &gt; 0">
	  ...
	  <xsl:if test="@ontab and string-length(@ontab)&gt;0">
         ...
         if (ns) return true;var codigoTecla = (window.Event) ? e.which : event.keyCode;var shift_pul=(ie)?e.shiftKey:false;if (shift_pul || codigoTecla!=9) {return true;}</xsl:text>
         <xsl:value-of select="@ontab"/>
         <xsl:text>;event.returnValue=false;}</xsl:text>
      </xsl:if>
      <xsl:if test="@onshtab and string-length(@onshtab)&gt;0">
...
DrMainEstatico.</xsl:text>
   ...
   <xsl:apply-templates mode="dinamico"/>
      ...
      <TABLE border="0" cellspaceng="0" cellpadding="0">
         <TR><TD>
         <FORM name="{@nombre}_form" METHOD="post" action="{$vgConfig/PROXYSERVLET}?ON=ObjetoPaginado">
...
<TABLE width="100%" border="0" cellspacing="1" cellpadding="0" bgcolor="#FFEBD8">      
	...
	<TR ALIGN="CENTER" style="font-family: Geneva, Arial, Helvetica, san-serif;font-size: 11px;color: #000000;background-color: #FFF1E3;padding-right: 3px;padding-left: 7px;">
		...
		<xsl:choose>
			...
			<xsl:otherwise>VACIO</xsl:otherwise> 
		</xsl:choose>
		</TD>  
		<TD bgcolor="FFF7F0"> 
		<xsl:choose>
...
onLoadUsua='</xsl:text>
      ...
      <HTML>
         ...
         <SCRIPT>
               <xsl:if test="$vgConfig/ALERTMODAL">
                  <xsl:text>alertModal=new drdAlertModal("</xsl:text>
                  <xsl:value-of select="$vgConfig/ALERTMODAL/ON"/>
                  <xsl:text>","</xsl:text>
                  <xsl:value-of select="$vgConfig/PROXYSERVLET"/>
                  <xsl:text>",</xsl:text>
                  <xsl:choose>
                     <xsl:when test="$vgConfig/ALERTMODAL/CARACTERISTICAS">
                        <xsl:text>"</xsl:text>
                        <xsl:value-of select="$vgConfig/ALERTMODAL/CARACTERISTICAS"/>
                        <xsl:text>"</xsl:text>
                     </xsl:when>
                     <xsl:otherwise>null</xsl:otherwise>
                  </xsl:choose>
                  <xsl:if test="$vgConfig/ALERTMODAL/FJS">
                     <xsl:text>,"</xsl:text>
                     <xsl:value-of select="$vgConfig/ALERTMODAL/FJS"/>
                     <xsl:text>"</xsl:text>
                  </xsl:if>
                  <xsl:text>);</xsl:text>
               </xsl:if>
...
<metaInformation>
<scenarios ><scenario default="yes" name="Scenario1" userelativepaths="yes" externalpreview="no" url="..\xmlTest\contenido_actividades_insertar_mav.xml" htmlbaseurl="" outputurl="" processortype="xalan" useresolver="yes" profilemode="7" profiledepth="" profilelength="" urlprofilexml="" commandline="" additionalpath="" additionalclasspath="" postprocessortype="none" postprocesscommandline="" postprocessadditionalpath="" postprocessgeneratedext="" validateoutput="no" validator="internal" customvalidator=""/></scenarios><MapperMetaTag><MapperInfo srcSchemaPathIsRelative="yes" srcSchemaInterpretAsXML="no" destSchemaPath="" destSchemaRoot="" destSchemaPathIsRelative="yes" destSchemaInterpretAsXML="no"/><MapperBlockPosition></MapperBlockPosition><TemplateContext></TemplateContext><MapperFilter side="source"></MapperFilter></MapperMetaTag>
</metaInformation>
-->
