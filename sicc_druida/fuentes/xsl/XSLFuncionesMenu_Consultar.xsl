...
<!--
	$Id: XSLFuncionesMenu_Consultar.xsl,v 1.1 2009/12/03 19:02:41 pecbazalar Exp $
-->
            ...
            <xsl:for-each select="//ROW[CAMPO[@NOMBRE='idfuncion']!=2 and CAMPO[@NOMBRE='jerarquia']!=2]">
               ...
               <tr>
                  ...
                  <td colspan="{$jerarquia}">
                     <xsl:if test="number(CAMPO[@NOMBRE='idfuncion'])!=1">
                       <input type="radio" name="inputSeleccionado" value="{CAMPO[@NOMBRE='idfuncion']}"/>
                       <input type="hidden" name="{CAMPO[@NOMBRE='idfuncion']}" value="{CAMPO[@NOMBRE='nivel']}"></input>
                     </xsl:if>
                  </td>
