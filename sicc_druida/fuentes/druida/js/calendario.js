...
function CalendarioInit() {
	...
	html+='<TD WIDTH=16 ID='+this.idmesanno+'><A HREF=\'javascript:'+this.nombre+'.decrementaAnno();void(null);\' onMouseOver=\''+this.nombre+'.over("RA",true);return true;\' onMouseOut=\''+this.nombre+'.over("RA")\'><IMG NAME=\''+this.nombre+'RA\' SRC='+this.ruta+'cr1.gif BORDER=0 ALT="Retroceder al año anterior"></A></TD>';
	html+='<TD WIDTH='+ancho3+' ID='+this.idmesanno+' ALIGN=center>&nbsp;</TD>';
	html+='<TD WIDTH=16 ID='+this.idmesanno+'><A HREF=\'javascript:'+this.nombre+'.incrementaAnno();void(null);\' onMouseOver=\''+this.nombre+'.over("AA",true);return true;\' onMouseOut=\''+this.nombre+'.over("AA")\'><IMG NAME=\''+this.nombre+'AA\' SRC='+this.ruta+'ca1.gif BORDER=0 ALT="Avanzar al año siguiente"></A></TD>';
	html+='</TR></TABLE>';
	if (ns) {
...
function CalendarioOver(opcion,over) {
	...
	switch(opcion) {
		...
		case 'RA': estado='Retroceder al año anterior'; imagen=0; break;
		case 'AA': estado='Avanzar al año siguiente'; imagen=2; break;
	}
