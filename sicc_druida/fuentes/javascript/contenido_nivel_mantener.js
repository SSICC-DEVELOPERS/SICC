var datosNiveles = null;
  ...
  function onLoadPag(){
    ...
    if(!fMostrarMensajeError()){
          ...
          // Plantilla
          cargarPantallaPlantilla();
          
          // Carga los datos de niveles
          var nivelesString = get('formulario.datosNiveles');
          //alert(nivelesString);
          datosNiveles = nivelesString.split(patron2);
          //alert(patron2);
          //alert(datosNiveles[0]);
...
//    alert("listado4: "+listado4);
  ...
  function deshabilitarListadoNiveles() {  
	  ...
	  listado.tiposCol[10][8]=true;
      
      listado.actualizaDat();
      listado.repinta();
  }
  
  function cargarPantallaPlantilla() {     
  
    // Deshabilita electivos      
    if(get('formulario.hIndPremiosElectivos')==get('formulario.IND_INACTIVO')) {
          
          listado.tiposCol[3][6]=true;
    }
    
    // Deshabilita puntaje servicio   
          //alert("get('formulario.hIndPuntajeServicio'): "+get('formulario.hIndPuntajeServicio'));
          //alert("get('formulario.IND_INACTIVO'): "+get('formulario.IND_INACTIVO'));
          //alert("listado.tiposCol[1][8]: "+listado.tiposCol[1][8]);
    if(get('formulario.hIndPuntajeServicio')==get('formulario.IND_INACTIVO')) {
                    //alert("*** Entro!!!");
          listado.tiposCol[1][8]=true;
          ...
          // Deshabilita Nro Aspirante - dmaneiro 15/11/2005 -> Inc. 21039
          if(get('formulario.hIndRanking')== get('formulario.IND_INACTIVO')) {
                     
                      listado.tiposCol[8][8]=true;          
                      
          } 

          // Deshabilita Bolsa Premios  dmaneiro 20/12/2005 -> Inc. 21040 
          if(get('formulario.hCbTipoSeleccion')== get('formulario.OID_TIPO_PREM_NIVELES')) {
              listado.tiposCol[4][8]=true;        
          } else {
                            // INC 61 - vbongiov 24/02/2006
                            listado.tiposCol[5][8]=true;	
                            listado.tiposCol[6][8]=true;	
                   }

          // Deshabilita productos exigidos  dmaneiro 12/12/2005 -> Inc. 21041
           if(get('formulario.hIndProductosExigidos')==get('formulario.IND_INACTIVO')) {
  ...
  function accionDefinirPremio() {   
    ...
    if(codigo!="") {  
          ...
          if(get('formulario.hCbTipoSeleccion')!= get('formulario.OID_TIPO_PREM_NIVELES')) { 
              // Bolsa de Premios
              if(listado.extraeDato(codigo, 4)=="") {
                    GestionarMensaje('1578');
                    eval("listado.preparaCamposDR()");
                    focaliza('frm'+'listado'+'.'+'txtCantidadFijoPuntos'+'_'+codigo,'');
                    return;
              }
          } else {//Agregado segun inc. 21381 - dmaneiro
                              
                              // Valida Cantidad Desde
                              if(listado.extraeDato(codigo, 5)=="") {
                                         GestionarMensaje('1579');
                                         eval("listado.preparaCamposDR()");
          ...
          if(listado.extraeDato(codigo, 5)=="") {
              ...
              focaliza('frm'+'listado'+'.'+'txtCantidadDesde'+'_'+codigo,'');
              return;
          }Segun Inc. 21381 ya no es obligatorio*/
          ...
          if(!sicc_valida_numero_lista(listado.extraeDato(codigo, 7), GestionarMensaje('1550'),0, 9999999)) {
              eval("listado.preparaCamposDR()");
          ...
          var nivelPremio = nivel.split(patron1);
  ...
  function premioArticulos(nivelPremio) {  
      ...
      */
      ...
      accion('formulario.cbCenServEntrega', '.disabled=true');  
           ...
           // Agregado por HRCS - Fecha 20/02/2007 - Incidencia DBLG700000167
           var codSelec = listado.codSeleccionados();
           if ( listado.extraeDato(codSelec[0], 3)== get('formulario.IND_ACTIVO') ) {
         // Habilita botones
         deshabilitarHabilitarBoton('botonContenido','btnAnadirLotes','A');
         deshabilitarHabilitarBoton('botonContenido','btnBuscar','A');
         deshabilitarHabilitarBoton('botonContenido','btnAnadirProductos','A');
  ...
  function llenaListaLotes(lotes){
      ...
      if(lotes!="BLANCO"){      
            ...
            lote = lotes.split(patron4);
            
            for(var i=0; i<lote.length; i++) {
                  //alert(lote[i]);
                  loteDatos = lote[i].split(patron5);
  ...
  function onSelectLotesPremio(fila){  
      ...
      if(codigo !="") {
            ...
            //alert(listaProdLotes);
            //alert(patron6);
            
            if(listaProdLotes!="BLANCO") {
                  prodLotes = listaProdLotes.split(patron6);
                  //alert("prodLotes: " + prodLotes);
                  
                  for(var i=0; i<prodLotes.length; i++){
                  
                      prodLote = prodLotes[i].split(patron3);
                  
                      oid = listado2.generaCodigo(secArtProd);
                      secArtProd = oid;
                      
                      codProducto = prodLote[0];
					  descProducto = prodLote[1]; 
                      numUnidades = prodLote[2];
                      precioPublico = prodLote[3];
                      despachoArt = prodLote[4]=="S"?GestionarMensaje("84"):GestionarMensaje("86");
                      codVentaFicticio = prodLote[5]=="BLANCO"?"":prodLote[5];            
                      indCentSerGar = prodLote[6]=="S"?GestionarMensaje("84"):GestionarMensaje("86");
                      
                      descCentSerGar = prodLote[7]=="BLANCO"?"":prodLote[7];   
                      numMeses = prodLote[8]=="BLANCO"?"":prodLote[8];   
                      obs = prodLote[9]=="BLANCO"?"":prodLote[9];   
                      tipoEntrega = prodLote[10]; 
                      descCentSerEntrega = prodLote[11]=="BLANCO"?"":prodLote[11];
                      oidCentSerGar = prodLote[12]=="BLANCO"?"":prodLote[12];
                      oidCentSerEntrega = prodLote[13]=="BLANCO"?"":prodLote[13];
                      
                      oidProducto = prodLote[13]=="BLANCO"?"":prodLote[14];           
                      oidLoteProd = prodLote[14]=="BLANCO"?"":prodLote[15];            
                     
                      listado2.insertar(new Array(oid, codProducto, descProducto, numUnidades, precioPublico, despachoArt, codVentaFicticio, 
            ...
            if(get('formulario.codLotelista')!=""){
                ...
                listado4.insertaDato(codigoModif,'2',obtenerListaProdLotes());
                  ...
                  if ( get('formulario.hIndPremiosElectivosPremios')== get('formulario.IND_INACTIVO') ||
                                          listado.extraeDato(codSelec[0], 3)== get('formulario.IND_INACTIVO') ) {
                           // Deshabilita botones
                           deshabilitarHabilitarBoton('botonContenido','btnAnadirLotes','D');
                           deshabilitarHabilitarBoton('botonContenido','btnBuscar','D');
                           deshabilitarHabilitarBoton('botonContenido','btnAnadirProductos','D');
                  ...
                  if( ( get('formulario.hIndPremiosElectivosPremios')== get('formulario.IND_INACTIVO') ||
                    ...
                    listado2.limpia();
                      ...
                      listProdLote = obtenerListaProdLotes();
                                           //alert("listProdLote: "+listProdLote);
                      oidLote = ''; 
  ...
  function obtenerListaProdLotes(){   
      ...
      var codigos = listado2.codigos();
                      ...
                      for (var i=0; i<codigos.length ; i++){
						   ...
						   descProducto = listado2.extraeDato(codigos[i], 1);
                           cadena = cadena + descProducto + "&&&";

                           numUnidades = listado2.extraeDato(codigos[i], 2);
                           cadena = cadena + numUnidades + "&&&";

                           precioPublico = listado2.extraeDato(codigos[i], 3);
                           cadena = cadena + precioPublico + "&&&";

                           despachoArt = listado2.extraeDato(codigos[i], 4)==GestionarMensaje("84")?"S":"N";
                           cadena = cadena + despachoArt + "&&&";

                           codVentaFicticio = listado2.extraeDato(codigos[i], 5)==""?"BLANCO":listado2.extraeDato(codigos[i], 5);
                           cadena = cadena + codVentaFicticio + "&&&";

                           indCentSerGar = listado2.extraeDato(codigos[i], 6)==GestionarMensaje("84")?"S":"N";
                           cadena = cadena + indCentSerGar + "&&&";
                            
                           descCentSerGar = listado2.extraeDato(codigos[i], 7)==""?"BLANCO":listado2.extraeDato(codigos[i], 7);
                           cadena = cadena + descCentSerGar + "&&&";
                            
                           numMeses = listado2.extraeDato(codigos[i], 8)==""?"BLANCO":listado2.extraeDato(codigos[i], 8);
                           cadena = cadena + numMeses + "&&&";
                            
                           obs = listado2.extraeDato(codigos[i], 9)==""?"BLANCO":listado2.extraeDato(codigos[i], 9);
                           cadena = cadena + obs + "&&&";
                            
                           tipoEntrega = listado2.extraeDato(codigos[i], 10);
                           cadena = cadena + tipoEntrega + "&&&";
                            
                           descCentSerEntrega = listado2.extraeDato(codigos[i], 11)==""?"BLANCO":listado2.extraeDato(codigos[i], 11);
                           cadena = cadena + descCentSerEntrega + "&&&";
                            
                           oidCentSerGar = listado2.extraeDato(codigos[i], 12)==""?"BLANCO":listado2.extraeDato(codigos[i], 12);
                           cadena = cadena + oidCentSerGar + "&&&";
                            
                           oidCentSerEntrega = listado2.extraeDato(codigos[i], 13)==""?"BLANCO":listado2.extraeDato(codigos[i], 13);
                           cadena = cadena + oidCentSerEntrega + "&&&";
                  
                           oidProducto = listado2.extraeDato(codigos[i], 14)==""?"BLANCO":listado2.extraeDato(codigos[i], 14);
                           cadena = cadena + oidProducto + "&&&";
                           
                           oidLoteProd = listado2.extraeDato(codigos[i], 15)==""?"BLANCO":listado2.extraeDato(codigos[i], 15);
      ...
      if(sicc_validaciones_generales("gArticuloProducto")) {
         ...
         if((get('formulario.ChkIndCenServGar')=="N" || sicc_validaciones_generales("gCenServGar"))
           && (get('formulario.rdTipoEntrega')=="B" || sicc_validaciones_generales("gCenServEntrega"))) {
            ...
            // vbongiov -- Cambio 20090930 -- 16/07/2009	
			set("formulario.codProducto", get('formulario.txtCodProducto'));
			set("formulario.accion", "obtenerDescProducto");
			set("formulario.conectorAction", "LPMantenerParametrosGeneralesPremiacion");
 ...
 // vbongiov -- Cambio 20090930 -- 16/07/2009	
 function ejecutarErrorAniadirProductoLote() {
  ...
  // vbongiov -- Cambio 20090930 -- 16/07/2009
  function aniadirProductoLote(descProducto){      
  ...
  function llenaListaPagos(pagos){
      ...
      if(pagos!="BLANCO"){      
            ...
            pago = pagos.split(patron6);
            numPago = 0;
            
            for(var i=0; i<pago.length; i++) {
                  //alert(pago[i]);
                  pagoDatos = pago[i].split(patron3);
                  numPago = numPago + 1;
  ...
  function llenaListaArticulos(articulos){
      ...
      if(articulos!="BLANCO"){      
            ...
            articulo = articulos.split(patron6);
            
            for(var i=0; i<articulo.length; i++) {
                  //alert(articulo[i]);
                  articuloDatos = articulo[i].split(patron3);
  ...
  function accionAniadirProducto(){ 
      if(sicc_validaciones_generales("gDescuentoProducto")) {
            ...
            negDesc = get('formulario.cbNegocio','T');
                     ...
                     //Ahora se toma la descripcion de la caja de texto
                     superGenerDesc = get('formulario.cbSupergenerico');
            //superGenerDesc = get('formulario.cbSupergenerico','T');            
                     
                     //Ahora se toma la descripcion de la caja de texto
                     generDesc = get('formulario.cbGenerico');
            ...
            oidNeg = get('formulario.cbNegocio');
                     ...
                     //Ahora se toma el oid de una variable oculta
                     oidSuperGener = get('formulario.varCbSuperGenerico');
            //oidSuperGener = get('formulario.cbSupergenerico');            
                     
                     //Ahora se toma el oid de una variable oculta
                     oidGener = get('formulario.varCbGenerico');
      ...
      if(get('formulario.opcionMenu')!="Consultar Concurso"){  
            ...
            if(tipoPremio == get('formulario.OID_TIPO_PREMIO_PUNTOS')) {
                  ...
                  if(sicc_validaciones_generales("gMonetario")) {
                      ...
                      //INICIO - MODIFICADO POR CRISTIAN VALENZUELA 9/8/2005
                      //SE VALIDAN LOS CAMPOS CANTIDAD Y PORCENTAJE
                      //DEPENDIENDO DEL VALOR DEL COMBO "TIPO PREMIOS MONETARIOS"
                      // Valida cantidad y porcentaje
                      ...
                      //FIN - MODIFICADO POR CRISTIAN VALENZUELA 9/8/2005
                  ...
                  if(sicc_validaciones_generales("gLotes")) {
  ...
  function accionAceptarNiveles(){ 
    if(validacionNiveles()){
          set('formulario.cantidadPremiosArticulos', cantidadPremiosArticulos());
  ...
  function cantidadPremiosArticulos(){
  
      var codigos = listado.codigos();  
      var cantidadPremiosElectivos="";
      
                      for (var i=0; i<codigos.length ; i++){
          
          nivel = datosNiveles[i];
          nivelPremio = nivel.split(patron1);

          tipoPremioAlmacenado= nivelPremio[1];
          
          if(tipoPremioAlmacenado==get('formulario.OID_TIPO_PREMIO_ARTICULO')){
          
              lotes = nivelPremio[4];
              lote = lotes.split(patron4);
              
              if(cantidadPremiosElectivos=="") {
                    cantidadPremiosElectivos = lote.length;
              } else {
                    cantidadPremiosElectivos = cantidadPremiosElectivos + "---" + lote.length;
              }
          }
      }
      
      return cantidadPremiosElectivos;
  }
  ...
  function fLimpiar(){
      if(get('formulario.opcionMenu')!="Crear concurso"){
            // Reinicia la modal
            set('formulario.conectorAction', 'LPMantenerParametrosGeneralesPremiacion');
  ...
  function cerrarModal(mensaje){
     ...
     if(mensaje=='S'){
          GestionarMensaje('1556');
     }
     //alert("cerrar modal");
     var retorno = new Array();
                     retorno[0] = get('formulario.cantidadPremiosArticulos');
  ...
  function armarListaDatosNiveles(){    
      var codigos = listado.codigos();
                      ...
                      for (var i=0; i<codigos.length ; i++){
								  ...
								  // vbongiov -- Cambio 20090930 -- 16/07/2009
								  descripcionNivel = listado.extraeDato(codigos[i], 10)==""?"BLANCO":listado.extraeDato(codigos[i], 10);
                                  cadena = cadena + descripcionNivel;
                     }
  ...
  function focalizaDatosNiveles() {  
      ...
      if(listado.datos.length!=0) {
                      ...
                      if(get('formulario.hIndPuntajeServicio')==get('formulario.IND_INACTIVO')) {
                                 focaliza('frm'+'listado'+'.'+'cbTipoPremio'+'_0','');
                      } else {
