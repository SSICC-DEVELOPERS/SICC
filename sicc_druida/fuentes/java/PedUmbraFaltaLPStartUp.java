/*
    INDRA/CAR/mmg
    $Id: PedUmbraFaltaLPStartUp.java,v 1.1 2009/12/03 18:36:38 pecbazalar Exp $
    DESC
*/
import java.util.*;
...
public class PedUmbraFaltaLPStartUp extends MMGDruidaBase{
	...
	public void ejecucion() throws Exception {
		try{
			...
			asignarAtributo("PAGINA", "PedUmbraFaltaPage", "cod", "PedUmbraFalta." + accion + ".label");
			...
			limpiarBotoneraLista("pedUmbraFalta", accion);
	...
	protected void cmdLov() throws Exception{
		...
		//asignarAtributo("CAPA", "pedUmbraFaltaLovLayer", "visibilidad", "visible");
		asignarAtributo("PAGINA", "PedUmbraFaltaPage", "onblur", "window.focus();");
	...
	*/
	protected void cmdView() throws Exception{
		...
		PedUmbraFaltaData pedUmbraFaltaQuery= new PedUmbraFaltaData();
		pedUmbraFaltaQuery.setId(new Long(conectorParametro("idSelection")));
		
		//Asiganmos el valor a la variable de acción anterior para poder volver hacia atras y hacer requery
		asignar("VAR", VAR_ACCION_ANTERIOR, conectorParametro(VAR_ACCION_ANTERIOR));
		
		//Sacamos de la base de datos la entidad a visualizar
		PedUmbraFaltaData datosResult = (PedUmbraFaltaData)getSingleObject(BUSINESSID_QUERY, pedUmbraFaltaQuery, 
			pedUmbraFaltaQuery.clone(), "pedUmbraFalta");
		...
		PedUmbraFaltaData pedUmbraFalta = datosResult;
		es.indra.belcorp.mso.MaeNegocData negoOidNego = pedUmbraFalta.getNegoOidNego();
		if(negoOidNego != null && negoOidNego.getId() != null) checkCombo("negoOidNego" , negoOidNego.getId().toString());
		asignar("TEXT", "valUmbrFalt", FormatUtils.formatObject(pedUmbraFalta.getValUmbrFalt(), 
		...
		//Deshabilitamos todos los campos para que no se editen
					...
					asignarAtributo("COMBO", "negoOidNego", "disabled", "S");asignarAtributo("TEXT", "valUmbrFalt", "disabled", "S");
	...
	protected void cmdCreate() throws Exception{
		...
		asignarAtributo("TEXT", "valUmbrFalt", "req", "S");
		...
		asignarAtributo("TEXT", "valUmbrFalt", "validacion", "mensaje = ValidaNumeroMMG(cdos_trim(valor), " + (9 - MMGDruidaHelper.getUserDecimalPrecision(this)) + ", " + MMGDruidaHelper.getUserDecimalPrecision(this)+ ", 'PedUmbraFalta.valUmbrFalt', 0, " + MMGDruidaHelper.getMaxValueForDruidaValidaFloat((9 - MMGDruidaHelper.getUserDecimalPrecision(this)), this) + ");");
	...
	protected void cmdUpdateForm() throws Exception{
		...
		//asignarAtributo("FORMULARIO", "pedUmbraFaltaFrm", "oculto", "N");
		...
		//Construimos el mso con los elementos de la búsqueda
		PedUmbraFaltaData pedUmbraFaltaQuery= new PedUmbraFaltaData();
		pedUmbraFaltaQuery.setId(new Long(id));
		...
		//asignar("VAR","timestamp",Long.toString(pedUmbraFalta.jdoGetTimeStamp()));
	...
	*/
	protected void cambiarAcciones() throws Exception{
				asignarAtributo("AREATEXTO", "id", "ontab","ejecutarAccionFocoModificacion(\"pedUmbraFaltaFrm\",1, true)");
				asignarAtributo("AREATEXTO", "id", "onshtab","ejecutarAccionFocoModificacion(\"pedUmbraFaltaFrm\",1, false)");
					asignarAtributo("COMBO","paisOidPais", "ontab", "ejecutarAccionFocoModificacion(\"pedUmbraFaltaFrm\",2, true)");
					asignarAtributo("COMBO","paisOidPais", "onshtab", "ejecutarAccionFocoModificacion(\"pedUmbraFaltaFrm\",2, false)");
					asignarAtributo("COMBO","negoOidNego", "ontab", "ejecutarAccionFocoModificacion(\"pedUmbraFaltaFrm\",3, true)");
					asignarAtributo("COMBO","negoOidNego", "onshtab", "ejecutarAccionFocoModificacion(\"pedUmbraFaltaFrm\",3, false)");
				asignarAtributo("AREATEXTO", "valUmbrFalt", "ontab","ejecutarAccionFocoModificacion(\"pedUmbraFaltaFrm\",4, true)");
				asignarAtributo("AREATEXTO", "valUmbrFalt", "onshtab","ejecutarAccionFocoModificacion(\"pedUmbraFaltaFrm\",4, false)");
	}
	...
	protected void removeLovElements() throws Exception{
		borrarNodo("TD","nombre","pedUmbraFaltaTdLovNullSelectionButton");
	}
	
	protected void removeQueryElements() throws Exception{
		//Borramos los botones para realizar la búsuqeda
		borrarNodo("TR","nombre","pedUmbraFaltaTrButtons");
	...
	*/
	protected void desactivateNonModifyFields() throws Exception{
			traza("MMG: en desactivateNonModifyFields ");		
				...
				traza("MMG: el atributo es valUmbrFalt...");		
	...
	*/
	protected void establecerSizeCapas(String accion, String origen) throws Exception{
		...
		asignarAtributo("CAPA","pedUmbraFaltaFormLayer", "alto", String.valueOf(sizeForm));		
		
		//Estabelcemos la posicion y de la capa correspondiente a la lista y de su botonera
		asignarAtributo("CAPA","pedUmbraFaltaListLayer", "y", String.valueOf(sizeForm + 26));		
		asignarAtributo("CAPA","pedUmbraFaltaListButtonsLayer", "y", String.valueOf(sizeForm + 278));		
	}

	/**
	* Nos permite cargar las combos finales de la jerarquías de dependencia
	*/
	protected void loadLeafDependenceCombos(PedUmbraFaltaData pedUmbraFalta) throws Exception{
		...
		if(pedUmbraFalta != null && pedUmbraFalta.getPaisOidPais() != null){
				checkCombo("paisOidPais" , pedUmbraFalta.getPaisOidPais().getId().toString());
			}
		...
		if(pedUmbraFalta != null && pedUmbraFalta.getNegoOidNego() != null){
				checkCombo("negoOidNego" , pedUmbraFalta.getNegoOidNego().getId().toString());
			}
	...
	* @param pedUmbraFalta La el objetopar la cual se va a realizar la carga
	*/
	protected void loadNonLeafDependenceCombos(PedUmbraFaltaData pedUmbraFalta) 
		throws Exception{
	...
	*/
	protected void loadNonLeafDependenceCombosForRequery() throws Exception{
		...
		PedUmbraFaltaData pedUmbraFalta = 
			(PedUmbraFaltaData) conectorParametroSesion(SESSION_ATTRIBUTE_LAST_QUERY_OBJ_FROM);
		
		//Cargamos todos los objetos completos correspodnientes a los valores de los
		//Campos que sean raiz de una jerarquía de combos dependientes
		if(pedUmbraFalta.getPaisOidPais() != null){
			pedUmbraFalta.setPaisOidPais((es.indra.belcorp.mso.SegPaisViewData)
			getSingleObject(BUSINESSID_PAISOIDPAIS_QUERY, 
			pedUmbraFalta.getPaisOidPais(),
			pedUmbraFalta.getPaisOidPais().clone(),
			"segPaisView"));
		}
		if(pedUmbraFalta.getNegoOidNego() != null){
			pedUmbraFalta.setNegoOidNego((es.indra.belcorp.mso.MaeNegocData)
			getSingleObject(BUSINESSID_NEGOOIDNEGO_QUERY, 
			pedUmbraFalta.getNegoOidNego(),
			pedUmbraFalta.getNegoOidNego().clone(),
			"maeNegoc"));
		}
		
		//Una vez que tenemos completo los datos de los atributos raices de jerarquías de dependencias
		//cargamos las combos y seleccionamos elementos
		loadNonLeafDependenceCombos(pedUmbraFalta);
		loadLeafDependenceCombos(pedUmbraFalta);
	}
