/*
    INDRA/CAR/mmg
    $Id: ZonSubesGeopoLPStartUp.java,v 1.1 2009/12/03 18:39:25 pecbazalar Exp $
    DESC
*/
import java.util.*;
...
public class ZonSubesGeopoLPStartUp extends MMGDruidaBase{
	...
	public static final String BUSINESSID_QUERY = "MMGZonSubesGeopoQueryFromToUserPage";
	...
	public ZonSubesGeopoLPStartUp(){
		super();
	}

	public void inicio() throws Exception {
		pagina("ZonSubesGeopoPage");
	}
	...
	public void ejecucion() throws Exception {
		try{
			...
			asignarAtributo("PAGINA", "ZonSubesGeopoPage", "cod", "ZonSubesGeopo." + accion + ".label");
			...
			limpiarBotoneraLista("zonSubesGeopo", accion);
	...
	protected void cmdLov() throws Exception{
		...
		asignarAtributo("TEXT", "codSube", "validacion", "mensaje = ValidaInt(cdos_trim(valor), 2, 0, 99);");
		...
		//asignarAtributo("CAPA", "zonSubesGeopoLovLayer", "visibilidad", "visible");
		asignarAtributo("PAGINA", "ZonSubesGeopoPage", "onblur", "window.focus();");
	...
	protected void cmdPreView() throws Exception{
		...
		conectorAction("ZonSubesGeopoLPStartUp");
	...
	*/
	protected void cmdView() throws Exception{
		...
		ZonSubesGeopoData zonSubesGeopoQuery= new ZonSubesGeopoData();
		zonSubesGeopoQuery.setId(new Long(conectorParametro("idSelection")));
		
		//Asiganmos el valor a la variable de acción anterior para poder volver hacia atras y hacer requery
		asignar("VAR", VAR_ACCION_ANTERIOR, conectorParametro(VAR_ACCION_ANTERIOR));
		
		//Sacamos de la base de datos la entidad a visualizar
		ZonSubesGeopoData datosResult = (ZonSubesGeopoData)getSingleObject(BUSINESSID_QUERY, zonSubesGeopoQuery, 
			zonSubesGeopoQuery.clone(), "zonSubesGeopo");
		...
		preservaHiddensI18n(new Integer(1), "ZON_SUBES_GEOPO", "zonSubesGeopoFrm", "Descripcion", true);
		...
		ZonSubesGeopoData zonSubesGeopo = datosResult;
		es.indra.belcorp.mso.ZonEstruGeopoData egeoOidEstrGeop = zonSubesGeopo.getEgeoOidEstrGeop();
		if(egeoOidEstrGeop != null && egeoOidEstrGeop.getId() != null) checkCombo("egeoOidEstrGeop" , egeoOidEstrGeop.getId().toString());
		asignar("TEXT", "codSube", FormatUtils.formatObject(zonSubesGeopo.getCodSube(), 
		...
		loadLocalizationVars(zonSubesGeopo);
		
			
		//Deshabilitamos todos los campos para que no se editen
		
					asignarAtributo("COMBO", "egeoOidEstrGeop", "disabled", "S");asignarAtributo("TEXT", "codSube", "disabled", "S");
	...
	protected void cmdCreate() throws Exception{
		//Creamos los campos ocultos para la internacionalización
		preservaHiddensI18n(new Integer(1), "ZON_SUBES_GEOPO", "zonSubesGeopoFrm", "Descripcion", false);
		...
		asignarAtributo("TEXT", "codSube", "req", "S");
		...
		asignarAtributo("TEXT", "Descripcion", "onblur", "setDefaultLanguageValue('Descripcion', get('zonSubesGeopoFrm.Descripcion'))");
	...
	protected void cmdUpdateForm() throws Exception{
		...
		//asignarAtributo("FORMULARIO", "zonSubesGeopoFrm", "oculto", "N");
		...
		traza("MMG::Creando ZonSubesGeopo");
		String id = conectorParametro("idSelection");
		traza("MMG:: id del atributo a modificar: " + id);
	
		//Construimos el mso con los elementos de la búsqueda
		ZonSubesGeopoData zonSubesGeopoQuery= new ZonSubesGeopoData();
		zonSubesGeopoQuery.setId(new Long(id));
		...
		//asignar("VAR","timestamp",Long.toString(zonSubesGeopo.jdoGetTimeStamp()));
	...
	*/
	protected void cambiarAcciones() throws Exception{
				asignarAtributo("AREATEXTO", "id", "ontab","ejecutarAccionFocoModificacion(\"zonSubesGeopoFrm\",1, true)");
				asignarAtributo("AREATEXTO", "id", "onshtab","ejecutarAccionFocoModificacion(\"zonSubesGeopoFrm\",1, false)");
					asignarAtributo("COMBO","egeoOidEstrGeop", "ontab", "ejecutarAccionFocoModificacion(\"zonSubesGeopoFrm\",2, true)");
					asignarAtributo("COMBO","egeoOidEstrGeop", "onshtab", "ejecutarAccionFocoModificacion(\"zonSubesGeopoFrm\",2, false)");
				asignarAtributo("AREATEXTO", "codSube", "ontab","ejecutarAccionFocoModificacion(\"zonSubesGeopoFrm\",3, true)");
				asignarAtributo("AREATEXTO", "codSube", "onshtab","ejecutarAccionFocoModificacion(\"zonSubesGeopoFrm\",3, false)");
				asignarAtributo("AREATEXTO", "Descripcion", "ontab","ejecutarAccionFocoModificacion(\"zonSubesGeopoFrm\",4, true)");
				asignarAtributo("AREATEXTO", "Descripcion", "onshtab","ejecutarAccionFocoModificacion(\"zonSubesGeopoFrm\",4, false)");
	}
	...
	protected void removeLovElements() throws Exception{
		borrarNodo("TD","nombre","zonSubesGeopoTdLovNullSelectionButton");
	}
	
	protected void removeQueryElements() throws Exception{
		//Borramos los botones para realizar la búsuqeda
		borrarNodo("TR","nombre","zonSubesGeopoTrButtons");
	...
	*/
	protected void establecerSizeCapas(String accion, String origen) throws Exception{
		...
		asignarAtributo("CAPA","zonSubesGeopoFormLayer", "alto", String.valueOf(sizeForm));		
		
		//Estabelcemos la posicion y de la capa correspondiente a la lista y de su botonera
		asignarAtributo("CAPA","zonSubesGeopoListLayer", "y", String.valueOf(sizeForm + 26));		
		asignarAtributo("CAPA","zonSubesGeopoListButtonsLayer", "y", String.valueOf(sizeForm + 278));		
	}

	/**
	* Nos permite cargar las combos finales de la jerarquías de dependencia
	*/
	protected void loadLeafDependenceCombos(ZonSubesGeopoData zonSubesGeopo) throws Exception{
		HashMap checkedCombos = new HashMap();
		ZonEstruGeopoData egeoOidEstrGeop = new ZonEstruGeopoData();
		egeoOidEstrGeop_ApplyFilter(egeoOidEstrGeop);
		...
		if(zonSubesGeopo != null && zonSubesGeopo.getEgeoOidEstrGeop() != null){
				checkCombo("egeoOidEstrGeop" , zonSubesGeopo.getEgeoOidEstrGeop().getId().toString());
			}
	...
	* @param zonSubesGeopo La el objetopar la cual se va a realizar la carga
	*/
	protected void loadNonLeafDependenceCombos(ZonSubesGeopoData zonSubesGeopo) 
		throws Exception{
	...
	*/
	protected void loadNonLeafDependenceCombosForRequery() throws Exception{
		...
		ZonSubesGeopoData zonSubesGeopo = 
			(ZonSubesGeopoData) conectorParametroSesion(SESSION_ATTRIBUTE_LAST_QUERY_OBJ_FROM);
		
		//Cargamos todos los objetos completos correspodnientes a los valores de los
		//Campos que sean raiz de una jerarquía de combos dependientes
		if(zonSubesGeopo.getEgeoOidEstrGeop() != null){
			zonSubesGeopo.setEgeoOidEstrGeop((es.indra.belcorp.mso.ZonEstruGeopoData)
			getSingleObject(BUSINESSID_EGEOOIDESTRGEOP_QUERY, 
			zonSubesGeopo.getEgeoOidEstrGeop(),
			zonSubesGeopo.getEgeoOidEstrGeop().clone(),
			"zonEstruGeopo"));
		}
		
		//Una vez que tenemos completo los datos de los atributos raices de jerarquías de dependencias
		//cargamos las combos y seleccionamos elementos
		loadNonLeafDependenceCombos(zonSubesGeopo);
		loadLeafDependenceCombos(zonSubesGeopo);
	}
	...
	*/
	...
	protected void loadLocalizationVars(ZonSubesGeopoData zonSubesGeopo) throws Exception{
		GenDetaSiccData mmgLocalizationQuery = null;
		Vector mmgLocalizationDatosResult = null;
		mmgLocalizationQuery = new GenDetaSiccData();
		mmgLocalizationQuery.setEnti("ZON_SUBES_GEOPO");
		mmgLocalizationQuery.setEntiPk(zonSubesGeopo.getId());
	...
	protected void egeoOidEstrGeop_ApplyFilter(ZonEstruGeopoData zonEstruGeopo) throws Exception {
		
		SegPaisViewData segP = new SegPaisViewData();
		segP.setId(new Long(MMGDruidaHelper.getUserDefaultCountry(this)));
		zonEstruGeopo.setPaisOidPais(segP);
		zonEstruGeopo.setIndBorr(new Long(0));
