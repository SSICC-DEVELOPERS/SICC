...
/*
    INDRA/CAR/mmg
    $Id: PedAtribEspecListFormatter.java,v 1.1 2009/12/03 18:33:54 pecbazalar Exp $
    DESC
*/
 ...
 * @author Indra
 */
public class PedAtribEspecListFormatter extends MMGDruidaFormatoObjeto {
	
	public PedAtribEspecListFormatter() {
	}
		...
		for (int i = 0; i < pedAtribEspecList.size(); i++) {
			...
			row.add((pedAtribEspecData.getValForm() != null ? 
				FormatUtils.formatObject(pedAtribEspecData.getValForm(), 
				 MMGDruidaHelper.getUserDecimalFormatPattern(this), 
				symbols) : ""));
			row.add((pedAtribEspecData.getModuOidModu() != null ? 
				FormatUtils.formatObject(pedAtribEspecData.getModuOidModu(), 
				 MMGDruidaHelper.getUserDecimalFormatPattern(this), 
				symbols) : ""));
