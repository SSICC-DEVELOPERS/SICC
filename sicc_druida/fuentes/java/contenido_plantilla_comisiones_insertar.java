...
public class contenido_plantilla_comisiones_insertar  implements es.indra.druida.base.ObjetoXML {
...
/* Primer nodo */
   ...
   private void getXML0(Document doc) {
      ...
      ((Element)v.get(0)).setAttribute("nombre","contenido_plantilla_comisiones_insertar" );
      ...
      v.add(doc.createTextNode("\r   \r\r  function onLoadPag()   \r  {		configurarMenuSecundario(\"formulario\")\r\r		ucs = _get('casoDeUso')\r		\r		if( ucs=='copiar'){\r			btnProxy(1,0);\r			btnProxy(5,0);\r		}else{\r			btnProxy(1,1);\r			btnProxy(2,0);\r			btnProxy(5,1);\r		}\r\r\r		\r		fMostrarMensajeError();\r		\r		if(ucs == \"cerrar\")\r		{ \r		   returnValue = false;\r		   window.close();\r		   return;\r		}\r		        \r		        combo_add('formulario.cbComparativo', 'COM_SI', 'Sí');\r		combo_add('formulario.cbComparativo', 'COM_NO', 'No');\r\r		        combo_add('formulario.cbTratamientoDiferencialAntiguas', 'COM_SI', 'Sí');\r		combo_add('formulario.cbTratamientoDiferencialAntiguas', 'COM_NO', 'No');\r\r		if(ucs=='modificar' || ucs=='consultar' || ucs=='copiar' || ucs=='eliminar')\r		{\r						            modif = true\r			document.all[\"Cplistado1\"].style.display='none';\r			document.all[\"CpLin1listado1\"].style.display='none';\r			document.all[\"CpLin2listado1\"].style.display='none';\r			document.all[\"CpLin3listado1\"].style.display='none';\r			document.all[\"CpLin4listado1\"].style.display='none';\r												document.all[\"separaDiv\"].style.display='none'\r\r\r\r			arrCan = new Array();\r			arrCan[0]  = _get('oidCanal')\r            set(\"formulario.cbCanal\",arrCan);\r\r			onChangeCanal();\r\r						var arrSeteo = new Array();\r			var arrTemp = new Array();\r						\r			arrSeteo = new Array();\r			arrTemp = new Array();\r			arrTemp = get(\"formulario.arrMarca\").split('|');\r\r			for( i=0; i<arrTemp.length ; i++ )\r				arrSeteo[i] = arrTemp[i];\r			\r			set(\"formulario.cbMarca\", arrSeteo);\r\r			arrSeteo = new Array();\r			arrTemp = new Array();\r			arrTemp = get(\"formulario.arrPais\").split('|');\r\r			for( i=0; i<arrTemp.length ; i++ )\r				arrSeteo[i] = arrTemp[i];\r			\r			set(\"formulario.cbPais\", arrSeteo);\r\r			arrComp = new Array();\r			arrComp[0]  = _get('comparativo')\r\r			set(\"formulario.cbComparativo\",arrComp);\r\r\r			arrDif = new Array();\r			arrDif[0]  = _get('tratDifAntiguas')\r\r			set(\"formulario.cbTratamientoDiferencialAntiguas\",arrDif);\r\r			arrCom = new Array();\r			arrCom[0]  = _get('oidTipoComision')\r			set(\"formulario.cbTipoComision\", arrCom);\r\r			hide('btnDetalleDiv')\r			showLista(false)\r\r            btnProxy(2,1);\r			btnProxy(3,0);\r						if(ucs=='consultar' || ucs=='eliminar' || ucs=='copiar')\r			{  accion(_form+'txtDescripcionPlantilla', '.disabled=true');\r			   accion(_form+'cbCanal', '.disabled=true');\r			   accion(_form+'cbTipoComision', '.disabled=true');\r			   accion(_form+'cbComparativo', '.disabled=true');\r			   accion(_form+'cbTratamientoDiferencialAntiguas', '.disabled=true');\r\r				accion(_form+'cbPais', '.disabled=true');\r				accion(_form+'cbMarca', '.disabled=true');\r				accion(_form+'cbAcceso', '.disabled=true');\r			   btnProxy(1,0);\r			   btnProxy(5,0);\r			}			\r		}\r		else\r		{	\r\r			var arr = new Array();      \r			arr[arr.length] = new Array(\"oidIdioma\", get(\"formulario.oidIdioma\"));\r			arr[arr.length] = new Array(\"oidPais\", get(\"formulario.oidPais\"));  \r			\r			setTimeout('a();', 1000);\r\r			configurarPaginado(mipgndo,'COMObtenerPlantillaCom',\r									   'ConectorObtenerPlantillaCom',\r									   'es.indra.sicc.cmn.negocio.auditoria.DTOSiccPaginacion', \r										arr); \r			show('btnDetalleDiv')\r					}	\r		focaliza(_form+'txtDescripcionPlantilla')\r	}\r\r	function a()\r	{\r		return;\r	}\r\r    var modif = false;\r	function setValueAcceso(d)\r	{\r	   if(modif)\r	   {          \r		  set_combo(\"formulario.cbAcceso\",arguments[0]);\r		  var arrTemp  = new Array(); \r		  var arrSeteo  = new Array();\r		  arrTemp = get(\"formulario.arrAcceso\").split('|');\r		  for(i=0;i<arrTemp.length;i++)\r			  arrSeteo[i] = arrTemp[i];\r		    set(\"formulario.cbAcceso\", arrSeteo);\r	   }\r	}\r\r	function onChangeCanal()\r	{\r		if(get(\"formulario.cbCanal\")!=\"\")\r		{	var arr = new Array();      \r			arr[arr.length] = new Array(\"oid\", get(\"formulario.cbCanal\"));\r			arr[arr.length] = new Array(\"oidPais\", get(\"formulario.oidPais\")); \r			arr[arr.length] = new Array(\"oidIdioma\", get(\"formulario.oidIdioma\")); \r            if(modif)\r			   recargaCombo(\"formulario.cbAcceso\", 'SEGObtieneAccesosPorCanal','es.indra.sicc.util.DTOOID',arr,'setValueAcceso(datos)');\r			else\r			   recargaCombo(\"formulario.cbAcceso\", 'SEGObtieneAccesosPorCanal','es.indra.sicc.util.DTOOID',arr);\r		}\r	}\r\r\r	function onClickDetalle()\r	{	pos=chkPos()\r\r		if(pos != null)	\r		{	var obj = new Object();\r						obj.oidPlantilla = listado1.codSeleccionados();\r									obj.accion = 'detalle'\r			obj.casoDeuso = 'insertar'\r			obj.conectorAction='LPPlantillaComisiones'\r			mostrarModalSICC('LPPlantillaComisiones','detalle',obj,800,350);		\r		} \r		else \r		{  GestionarMensaje('0',null,null,null);\r		   return false;\r		}\r	}\r\r\r	function fGuardar()\r	{	\r	\r		if(sicc_validaciones_generales()) \r		{						    set(\"formulario.descripcion\", get(\"formulario.txtDescripcionPlantilla\"));            			\r						\r			set(\"formulario.arrPais\", get(\"formulario.cbPais\"));\r			set(\"formulario.arrMarca\", get(\"formulario.cbMarca\"));\r			set(\"formulario.oidCanal\", get(\"formulario.cbCanal\"));\r\r			set(\"formulario.arrAcceso\", get(\"formulario.cbAcceso\"));\r\r			set(\"formulario.oidTipoComision\", get(\"formulario.cbTipoComision\"));\r			set(\"formulario.comparativo\", get(\"formulario.cbComparativo\"));\r            \r			set(\"formulario.tratDiferencial\", get(\"formulario.cbTratamientoDiferencialAntiguas\"));\r									set(\"formulario.conectorAction\", \"LPPlantillaComisiones\");\r			set(\"formulario.accion\", \"grabar\");		\r			set(\"formulario.casoDeUso\",get(\"formulario.casoDeUso\") );				\r			enviaSICC(\"formulario\",null,null,'N');  \r														}\r	}\r\r	function fLimpiar()\r	{  if(ucs=='insertar')\r	   {   var arr = new Array();  arr[0] = \"\";\r		   set(_form+'cbMarca', arr)\r		   set(_form+'cbPais', arr)\r		   set(_form+'cbTipoComision', arr)\r		   set(_form+'cbCanal')\r           set(_form+'cbAcceso',arr)\r		   set_combo(_form+'cbAcceso','')\r		   set(_form+'cbComparativo', arr)\r		   set(_form+'cbTratamientoDiferencialAntiguas', arr)\r		   set(_form+'txtDescripcionPlantilla',\"\")\r           limpiaI18N('formulario','1'); \r		   focaliza(_form+'txtDescripcionPlantilla')\r	   }\r	   else if(ucs=='modificar' || ucs=='copiar')\r	               onLoadPag()\r	}\r    \r	function fVolver()\r	{ if(ucs=='modificar' || ucs=='consultar' || ucs=='copiar')\r	     window.close();  \r	}\r\r	function toDetalle()\r	{  if(document.all[\"btnDetalleDiv\"].style.visibility == 'visible')\r		 focalizaBotonHTML('botonContenido','btnDetalle');\r	   else\r	   { if(arguments[0])\r		   focaliza('formulario.cbTratamientoDiferencialAntiguas');\r		 else\r		   focaliza('formulario.txtDescripcionPlantilla');\r	   }\r	}\r	_form='formulario.';ucs=null\r	function _get(){return get(_form+arguments[0])} \r	function hide(){document.all[arguments[0]].style.visibility='hidden'}\r	function show(){document.all[arguments[0]].style.visibility='visible'}\r        function Args(){this.i=0;this.vArray=new Array()}\r	Args.prototype.put=function(){this.vArray[this.i]=new Array(arguments[0],arguments[1]);this.i++}\r	Args.prototype.getArgs=function(){return this.vArray}\r    	\r	function muestraLista2( ultima, rowset, error)\r	{\r	  return (rowset.length > 0)?showLista(true):showLista(false)\r	}\r\r	function muestraLista( ultima, rowset, error)\r	{\r			var tamano = rowset.length;\r			if (tamano > 0){\r			  muestraLista2();\r			  eval (ON_RSZ);      \r			  return true;\r			}\r			else {\r			  ocultaLista2();\r			  eval (ON_RSZ);                      \r			  return false;\r			}\r    }\r    \r\r	function muestraLista2()\r    {	     \r		DrdEnsanchaConMargenDcho('listado1',12);	    \r		document.all[\"Cplistado1\"].style.visibility='';\r		document.all[\"CpLin1listado1\"].style.visibility='';\r		document.all[\"CpLin2listado1\"].style.visibility='';\r		document.all[\"CpLin3listado1\"].style.visibility='';\r		document.all[\"CpLin4listado1\"].style.visibility='';\r		document.all[\"primera1Div\"].style.visibility='';\r		document.all[\"ret1Div\"].style.visibility='';\r		document.all[\"ava1Div\"].style.visibility='';\r		document.all[\"separaDiv\"].style.visibility='';\r    }\r\r	function ocultaLista2()\r    {	     \r		DrdEnsanchaConMargenDcho('listado1',12);	    \r		document.all[\"Cplistado1\"].style.visibility='hidden';\r		document.all[\"CpLin1listado1\"].style.visibility='hidden';\r		document.all[\"CpLin2listado1\"].style.visibility='hidden';\r		document.all[\"CpLin3listado1\"].style.visibility='hidden';\r		document.all[\"CpLin4listado1\"].style.visibility='hidden';\r		document.all[\"primera1Div\"].style.visibility='hidden';\r		document.all[\"ret1Div\"].style.visibility='hidden';\r		document.all[\"ava1Div\"].style.visibility='hidden';\r		document.all[\"separaDiv\"].style.visibility='hidden';\r    }\r\r    function showLista()\r    {	v=(arguments[0])?'':'hidden';      \r		DrdEnsanchaConMargenDcho('listado1',12);	    \r		document.all[\"Cplistado1\"].style.visibility=v;\r		document.all[\"CpLin1listado1\"].style.visibility=v;\r		document.all[\"CpLin2listado1\"].style.visibility=v;\r		document.all[\"CpLin3listado1\"].style.visibility=v;\r		document.all[\"CpLin4listado1\"].style.visibility=v;\r		document.all[\"primera1Div\"].style.visibility=v;\r		document.all[\"ret1Div\"].style.visibility=v;\r		document.all[\"ava1Div\"].style.visibility=v;\r		document.all[\"separaDiv\"].style.visibility=v;\r		eval (ON_RSZ);\r		return arguments[0];\r    }\r	function chkPos() 	{	if(listado1.numSelecc() == 1)\r			return obtenerPosicionListaEditable(listado1.codSeleccionados(),listado1);			\r		else\r		{  if(listado1.numSelecc() > 1)\r			cdos_mostrarAlert(GestionarMensaje(\"240\"));		  \r		   else\r		   {  if(listado1.numSelecc() < 1)\r				 GestionarMensaje(\"4\");\r		   }\r		}\r		return null;\r	}\r		function eliminarByList()\r	{	if(listado1.numSelecc() < 1)\r			GestionarMensaje(\"4\");    		else\r		{	eliminarFilas(listado1.codSeleccionados(),arguments[0],mipgndo);\r			onClickBuscar();\r		}\r	}\r\r\r\rfunction reInicio(){\r	window.close();\r}\r\r\r\r \r"));
      ...
      ((Element)v.get(8)).setAttribute("name","cbPais" );
      ((Element)v.get(8)).setAttribute("required","true" );
      ((Element)v.get(8)).setAttribute("cod","5" );
      ...
      v.add(doc.createElement("ELEMENTO"));
      ((Element)v.get(12)).setAttribute("name","txtDescripcionPlantilla" );
      ((Element)v.get(12)).setAttribute("format","t" );
      ((Element)v.get(12)).setAttribute("required","true" );
      ((Element)v.get(12)).setAttribute("cod","1697" );
      ...
      v.add(doc.createElement("VAR"));
      ((Element)v.get(20)).setAttribute("nombre","oidPlantilla" );
      ...
      v.add(doc.createElement("VAR"));
      ((Element)v.get(21)).setAttribute("nombre","codPlantilla" );
   ...
   private void getXML90(Document doc) {
      v.add(doc.createElement("VAR"));
      ((Element)v.get(22)).setAttribute("nombre","descripcionPlantilla" );
      ...
      v.add(doc.createElement("VAR"));
      ((Element)v.get(26)).setAttribute("nombre","arrPais" );
      ...
      v.add(doc.createElement("VAR"));
      ((Element)v.get(27)).setAttribute("nombre","arrAcceso" );
      ...
      v.add(doc.createElement("VAR"));
      ((Element)v.get(28)).setAttribute("nombre","arrMarca" );
      ...
      v.add(doc.createElement("VAR"));
      ((Element)v.get(29)).setAttribute("nombre","oidTipoComision" );
      ...
      v.add(doc.createElement("VAR"));
      ((Element)v.get(30)).setAttribute("nombre","comparativo" );
      ...
      v.add(doc.createElement("VAR"));
      ((Element)v.get(31)).setAttribute("nombre","tratDifAntiguas" );
      ...
      v.add(doc.createElement("VAR"));
      ((Element)v.get(32)).setAttribute("nombre","tratDiferencial" );
      ...
      v.add(doc.createElement("VAR"));
      ((Element)v.get(33)).setAttribute("nombre","modificar" );
      ...
      v.add(doc.createElement("VAR"));
      ((Element)v.get(34)).setAttribute("nombre","ejecutar" );
   ...
   private void getXML180(Document doc) {
      ...
      v.add(doc.createElement("table"));
      ((Element)v.get(54)).setAttribute("width","499" );
      ...
      v.add(doc.createElement("LABELC"));
      ((Element)v.get(62)).setAttribute("nombre","lblDescripcionPlantilla" );
      ((Element)v.get(62)).setAttribute("alto","13" );
      ((Element)v.get(62)).setAttribute("filas","1" );
      ((Element)v.get(62)).setAttribute("valor","" );
      ((Element)v.get(62)).setAttribute("cod","1697" );
      ...
      ((Element)v.get(69)).setAttribute("nombre","txtDescripcionPlantilla" );
      ((Element)v.get(69)).setAttribute("id","datosCampos" );
   }

   private void getXML270(Document doc) {
      ((Element)v.get(69)).setAttribute("max","30" );
      ...
      ((Element)v.get(69)).setAttribute("onshtab","toDetalle(true)" );
      ...
      ((Element)v.get(73)).setAttribute("onclick","levantarI18N('formulario','1');" );
      ...
      v.add(doc.createElement("table"));
      ((Element)v.get(81)).setAttribute("width","499" );
      ...
      v.add(doc.createElement("LABELC"));
      ((Element)v.get(86)).setAttribute("nombre","lblPais" );
      ...
      ((Element)v.get(86)).setAttribute("cod","5" );
   ...
   private void getXML360(Document doc) {
      ...
      ((Element)v.get(97)).setAttribute("nombre","cbPais" );
      ...
      v.add(doc.createElement("table"));
      ((Element)v.get(111)).setAttribute("width","499" );
   ...
   private void getXML450(Document doc) {
      ...
      v.add(doc.createElement("LABELC"));
      ((Element)v.get(116)).setAttribute("nombre","lblCanal" );
      ...
      ((Element)v.get(127)).setAttribute("onchange","onChangeCanal();" );
      ...
      v.add(doc.createElement("COMBO"));
      ((Element)v.get(132)).setAttribute("nombre","cbAcceso" );
   ...
   private void getXML540(Document doc) {
      ...
      v.add(doc.createElement("table"));
      ((Element)v.get(141)).setAttribute("width","499" );
   ...
   private void getXML630(Document doc) {
      ...
      v.add(doc.createElement("COMBO"));
      ((Element)v.get(161)).setAttribute("nombre","cbTipoComision" );
      ...
      v.add(doc.createElement("COMBO"));
      ((Element)v.get(166)).setAttribute("nombre","cbComparativo" );
      ...
      v.add(doc.createElement("COMBO"));
      ((Element)v.get(171)).setAttribute("nombre","cbTratamientoDiferencialAntiguas" );
      ...
      ((Element)v.get(171)).setAttribute("ontab","toDetalle()" );
      ...
      /* Empieza nodo:178 / Elemento padre: 44   */
      v.add(doc.createElement("td"));
      ((Element)v.get(44)).appendChild((Element)v.get(178));
      ...
      /* Empieza nodo:180 / Elemento padre: 36   */
      v.add(doc.createElement("tr"));
      ((Element)v.get(36)).appendChild((Element)v.get(180));
   ...
   private void getXML720(Document doc) {
      ...
      ((Element)v.get(187)).setAttribute("nombre","listado1" );
      ((Element)v.get(187)).setAttribute("ancho","503" );
      ((Element)v.get(187)).setAttribute("alto","301" );
      ((Element)v.get(187)).setAttribute("x","12" );
      ((Element)v.get(187)).setAttribute("y","310" );
      ...
      ((Element)v.get(191)).setAttribute("ordenar","ascendente_on.gif" );
      ((Element)v.get(191)).setAttribute("ordenarInv","descendente_on.gif" );
      ...
      ((Element)v.get(193)).setAttribute("borde","1" );
      ((Element)v.get(193)).setAttribute("horizDatos","1" );
      ((Element)v.get(193)).setAttribute("horizCabecera","1" );
      ((Element)v.get(193)).setAttribute("vertical","0" );
      ((Element)v.get(193)).setAttribute("horizTitulo","1" );
      ((Element)v.get(193)).setAttribute("horizBase","1" );
      ((Element)v.get(192)).appendChild((Element)v.get(193));
      /* Termina nodo:193   */

      /* Empieza nodo:194 / Elemento padre: 192   */
      v.add(doc.createElement("COLOR"));
      ((Element)v.get(194)).setAttribute("borde","#999999" );
      ((Element)v.get(194)).setAttribute("vertCabecera","#E0E0E0" );
      ((Element)v.get(194)).setAttribute("vertDatos","#FFFFFF" );
      ((Element)v.get(194)).setAttribute("horizDatos","#FFFFFF" );
      ((Element)v.get(194)).setAttribute("horizCabecera","#999999" );
      ((Element)v.get(194)).setAttribute("horizTitulo","#999999" );
      ((Element)v.get(194)).setAttribute("horizBase","#999999" );
      ...
      ((Element)v.get(195)).setAttribute("cod","00310" );
      ...
      ((Element)v.get(197)).setAttribute("ajustarMinimo","S" );
      ((Element)v.get(197)).setAttribute("permiteOrdenar","S" );
      ((Element)v.get(197)).setAttribute("blancosAInsertar","1" );
      ((Element)v.get(197)).setAttribute("sinSaltoLinea","S" );
      ((Element)v.get(197)).setAttribute("AnchoMinimizadas","20" );
      ((Element)v.get(197)).setAttribute("botonOrdenar","S" );
      ((Element)v.get(187)).appendChild((Element)v.get(197));

      /* Empieza nodo:198 / Elemento padre: 197   */
      v.add(doc.createElement("COL"));
      ((Element)v.get(198)).setAttribute("ancho","20" );
      ...
      ((Element)v.get(199)).setAttribute("alto","20" );
      ((Element)v.get(199)).setAttribute("IDScroll","EstCab" );
      ((Element)v.get(199)).setAttribute("imgFondo","" );
      ((Element)v.get(199)).setAttribute("colFondo","#CCCCCC" );
   }
   ...
   private void getXML810(Document doc) {
      ...
      ((Element)v.get(200)).setAttribute("cod","00311" );
      ...
      ((Element)v.get(201)).setAttribute("tipoEnvio","edicion" );
      ...
      ((Element)v.get(205)).setAttribute("y","592" );
      ...
      ((Element)v.get(206)).setAttribute("y","592" );
      ...
      ((Element)v.get(13)).appendChild((Element)v.get(207));
      /* Termina nodo:207   */

      /* Empieza nodo:208 / Elemento padre: 13   */
      v.add(doc.createElement("BOTON"));
      ((Element)v.get(208)).setAttribute("nombre","separa" );
      ((Element)v.get(208)).setAttribute("x","59" );
      ((Element)v.get(208)).setAttribute("y","588" );
      ...
      ((Element)v.get(13)).appendChild((Element)v.get(208));
      /* Termina nodo:208   */

      /* Empieza nodo:209 / Elemento padre: 13   */
      v.add(doc.createElement("BOTON"));
      ((Element)v.get(209)).setAttribute("nombre","btnDetalle" );
      ((Element)v.get(209)).setAttribute("x","80" );
      ((Element)v.get(209)).setAttribute("y","589" );
   ...
   private void getXML900(Document doc) {
      ...
      ((Element)v.get(209)).setAttribute("cod","3" );
      ((Element)v.get(209)).setAttribute("accion","onClickDetalle()" );
      ((Element)v.get(209)).setAttribute("onshtab","focaliza('formulario.cbTratamientoDiferencialAntiguas')" );
      ((Element)v.get(209)).setAttribute("ontab","focaliza('formulario.txtDescripcionPlantilla')" );
      ((Element)v.get(13)).appendChild((Element)v.get(209));
      /* Termina nodo:209   */

      /* Empieza nodo:210 / Elemento padre: 13   */
      v.add(doc.createElement("CAPA"));
      ...
      ((Element)v.get(210)).setAttribute("y","611" );
      ((Element)v.get(210)).setAttribute("zindex","" );
      ((Element)v.get(13)).appendChild((Element)v.get(210));
      /* Termina nodo:210   */
      /* Termina nodo:13   */
