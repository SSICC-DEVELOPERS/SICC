/*
    INDRA/CAR/mmg
    $Id: MaeEncueNseLPStartUp.java,v 1.1 2009/12/03 18:40:30 pecbazalar Exp $
    DESC
*/
import java.util.*;
...
public class MaeEncueNseLPStartUp extends MMGDruidaBase{
	...
	public void ejecucion() throws Exception {
		try{
			...
			asignarAtributo("PAGINA", "MaeEncueNsePage", "cod", "MaeEncueNse." + accion + ".label");
			...
			limpiarBotoneraLista("maeEncueNse", accion);
	...
	protected void cmdLov() throws Exception{
		...
		//asignarAtributo("CAPA", "maeEncueNseLovLayer", "visibilidad", "visible");
		asignarAtributo("PAGINA", "MaeEncueNsePage", "onblur", "window.focus();");
	...
	*/
	protected void cmdView() throws Exception{
		...
		MaeEncueNseData maeEncueNseQuery= new MaeEncueNseData();
		maeEncueNseQuery.setId(new Long(conectorParametro("idSelection")));
		
		//Asiganmos el valor a la variable de acción anterior para poder volver hacia atras y hacer requery
		asignar("VAR", VAR_ACCION_ANTERIOR, conectorParametro(VAR_ACCION_ANTERIOR));
		
		//Sacamos de la base de datos la entidad a visualizar
		MaeEncueNseData datosResult = (MaeEncueNseData)getSingleObject(BUSINESSID_QUERY, maeEncueNseQuery, 
			maeEncueNseQuery.clone(), "maeEncueNse");
		...
		preservaHiddensI18n(new Integer(1), "MAE_ENCUE_NSE", "maeEncueNseFrm", "Descripcion", true);
		...
		MaeEncueNseData maeEncueNse = datosResult;
		asignar("TEXT", "codTipoEncu", FormatUtils.formatObject(maeEncueNse.getCodTipoEncu(), 
			 MMGDruidaHelper.getUserDecimalFormatPattern(this), decimalFormatSymbols));
		asignar("TEXT", "Descripcion", FormatUtils.formatObject(maeEncueNse.getDescripcion(), 
			 MMGDruidaHelper.getUserDecimalFormatPattern(this), decimalFormatSymbols));
		es.indra.belcorp.mso.SegMarcaData marcOidMarc = maeEncueNse.getMarcOidMarc();
		...
		loadLocalizationVars(maeEncueNse);
	...
	protected void cmdCreate() throws Exception{
		//Creamos los campos ocultos para la internacionalización
		preservaHiddensI18n(new Integer(1), "MAE_ENCUE_NSE", "maeEncueNseFrm", "Descripcion", false);
		...
		asignarAtributo("TEXT", "Descripcion", "onblur", "setDefaultLanguageValue('Descripcion', get('maeEncueNseFrm.Descripcion'))");
	...
	protected void cmdUpdateForm() throws Exception{
		...
		//asignarAtributo("FORMULARIO", "maeEncueNseFrm", "oculto", "N");
		...
		//Construimos el mso con los elementos de la búsqueda
		MaeEncueNseData maeEncueNseQuery= new MaeEncueNseData();
		maeEncueNseQuery.setId(new Long(id));
		...
		//asignar("VAR","timestamp",Long.toString(maeEncueNse.jdoGetTimeStamp()));
	...
	*/
	protected void cambiarAcciones() throws Exception{
				asignarAtributo("AREATEXTO", "id", "ontab","ejecutarAccionFocoModificacion(\"maeEncueNseFrm\",1, true)");
				asignarAtributo("AREATEXTO", "id", "onshtab","ejecutarAccionFocoModificacion(\"maeEncueNseFrm\",1, false)");
					asignarAtributo("COMBO","paisOidPais", "ontab", "ejecutarAccionFocoModificacion(\"maeEncueNseFrm\",2, true)");
					asignarAtributo("COMBO","paisOidPais", "onshtab", "ejecutarAccionFocoModificacion(\"maeEncueNseFrm\",2, false)");
				asignarAtributo("AREATEXTO", "codTipoEncu", "ontab","ejecutarAccionFocoModificacion(\"maeEncueNseFrm\",3, true)");
				asignarAtributo("AREATEXTO", "codTipoEncu", "onshtab","ejecutarAccionFocoModificacion(\"maeEncueNseFrm\",3, false)");
				asignarAtributo("AREATEXTO", "Descripcion", "ontab","ejecutarAccionFocoModificacion(\"maeEncueNseFrm\",4, true)");
				asignarAtributo("AREATEXTO", "Descripcion", "onshtab","ejecutarAccionFocoModificacion(\"maeEncueNseFrm\",4, false)");
					asignarAtributo("COMBO","marcOidMarc", "ontab", "ejecutarAccionFocoModificacion(\"maeEncueNseFrm\",7, true)");
					asignarAtributo("COMBO","marcOidMarc", "onshtab", "ejecutarAccionFocoModificacion(\"maeEncueNseFrm\",7, false)");
	}
	...
	protected void removeLovElements() throws Exception{
		borrarNodo("TD","nombre","maeEncueNseTdLovNullSelectionButton");
	}
	
	protected void removeQueryElements() throws Exception{
		//Borramos los botones para realizar la búsuqeda
		borrarNodo("TR","nombre","maeEncueNseTrButtons");
	...
	*/
	protected void establecerSizeCapas(String accion, String origen) throws Exception{
		...
		asignarAtributo("CAPA","maeEncueNseFormLayer", "alto", String.valueOf(sizeForm));		
		
		//Estabelcemos la posicion y de la capa correspondiente a la lista y de su botonera
		asignarAtributo("CAPA","maeEncueNseListLayer", "y", String.valueOf(sizeForm + 26));		
		asignarAtributo("CAPA","maeEncueNseListButtonsLayer", "y", String.valueOf(sizeForm + 278));		
	}

	/**
	* Nos permite cargar las combos finales de la jerarquías de dependencia
	*/
	protected void loadLeafDependenceCombos(MaeEncueNseData maeEncueNse) throws Exception{
		...
		if(maeEncueNse != null && maeEncueNse.getPaisOidPais() != null){
				checkCombo("paisOidPais" , maeEncueNse.getPaisOidPais().getId().toString());
			}
		...
		if(maeEncueNse != null && maeEncueNse.getMarcOidMarc() != null){
				checkCombo("marcOidMarc" , maeEncueNse.getMarcOidMarc().getId().toString());
			}
	...
	* @param maeEncueNse La el objetopar la cual se va a realizar la carga
	*/
	protected void loadNonLeafDependenceCombos(MaeEncueNseData maeEncueNse) 
		throws Exception{
	...
	*/
	protected void loadNonLeafDependenceCombosForRequery() throws Exception{
		...
		MaeEncueNseData maeEncueNse = 
			(MaeEncueNseData) conectorParametroSesion(SESSION_ATTRIBUTE_LAST_QUERY_OBJ_FROM);
		
		//Cargamos todos los objetos completos correspodnientes a los valores de los
		//Campos que sean raiz de una jerarquía de combos dependientes
		if(maeEncueNse.getPaisOidPais() != null){
			maeEncueNse.setPaisOidPais((es.indra.belcorp.mso.SegPaisViewData)
			getSingleObject(BUSINESSID_PAISOIDPAIS_QUERY, 
			maeEncueNse.getPaisOidPais(),
			maeEncueNse.getPaisOidPais().clone(),
			"segPaisView"));
		}
		if(maeEncueNse.getMarcOidMarc() != null){
			maeEncueNse.setMarcOidMarc((es.indra.belcorp.mso.SegMarcaData)
			getSingleObject(BUSINESSID_MARCOIDMARC_QUERY, 
			maeEncueNse.getMarcOidMarc(),
			maeEncueNse.getMarcOidMarc().clone(),
			"segMarca"));
		}
		
		//Una vez que tenemos completo los datos de los atributos raices de jerarquías de dependencias
		//cargamos las combos y seleccionamos elementos
		loadNonLeafDependenceCombos(maeEncueNse);
		loadLeafDependenceCombos(maeEncueNse);
	}
	...
	*/
	...
	protected void loadLocalizationVars(MaeEncueNseData maeEncueNse) throws Exception{
		...
		mmgLocalizationQuery.setEntiPk(maeEncueNse.getId());
