...
public class contenido_informe_resumen_CCC_generar  implements es.indra.druida.base.ObjetoXML {
...
/* Primer nodo */
   ...
   private void getXML0(Document doc) {
      ...
      ((Element)v.get(0)).setAttribute("nombre","contenido_informe_resumen_CCC_generar" );
      ((Element)v.get(0)).setAttribute("cod","0842" );
      ...
      v.add(doc.createTextNode("\r   \r   \r   function onLoad(){\r      configurarMenuSecundario('Formulario');\r      fMostrarMensajeError();\r      setearFechas();\r      focaliza('Formulario.txtFechaCorteDesde');\r      \r   }\r   function generar(){\r      if(sicc_validaciones_generales()){\r        if(validarFechas()){\r            /*recolectarParametros();\r            set('Formulario.conectorAction', 'CCCResumenCC');           \r            set('Formulario.accion', 'generar');\r            enviaSICC('Formulario');*/\r            var a = new Object();\r            a.accion = 'generar';\r            a.fechaDesde = get('Formulario.txtFechaCorteDesde');  \r            a.fechaHasta = get('Formulario.txtFechaCorteHasta');           \r            mostrarModalSICC('CCCResumenCC','generar', a);  \r        }\r      }\r   }\r\r   function recolectarParametros(){\r           \r      set('Formulario.fechaDesde', get('Formulario.txtFechaCorteDesde'));  \r      set('Formulario.fechaHasta', get('Formulario.txtFechaCorteHasta'));\r    \r   }\r   \r\r   function fLimpiar(){\r     focaliza('Formulario.txtFechaCorteDesde');\r   }\r\r   function validarFechas(){\r      \r      var fechaDesde = get('Formulario.txtFechaCorteDesde');\r      var fechaHasta = get('Formulario.txtFechaCorteHasta');\r      var formatoFecha = get('Formulario.hFormatoFechaPais');\r            \r     \r      if (! EsFechaValida_SICC(fechaDesde, \"Formulario\")){\r          GestionarMensaje(\"1006\", formatoFecha, null, null);\r          focaliza(\"Formulario.txtFechaCorteDesde\");\r          return false;\r      }\r     \r      if(! EsFechaValida_SICC(fechaHasta, \"Formulario\")){\r          GestionarMensaje(\"1006\", formatoFecha, null, null);\r          focaliza(\"Formulario.txtFechaCorteHasta\");\r          return false;\r      }\r                 \r      var errorLevel = EsFechaValida( fechaDesde, fechaHasta, \"Formulario\", \"S\");\r      if ( errorLevel == 3){\r          GestionarMensaje(\"1191\");\r          focaliza('Formulario.txtFechaCorteDesde');\r          return false;\r      }\r     \r      return true;   \r   \r   }\r\r\r   function setearFechas(){\r      var fechaActual = get('Formulario.fechaActual');\r      \r      var fechaDesde = fechaActual.split(\"/\");\r  \r      fechaDesde = '01/' + fechaDesde[1] + '/' + fechaDesde[2];\r      set('Formulario.txtFechaCorteDesde', fechaDesde);\r      set('Formulario.txtFechaCorteHasta', '31/05/2005');\r                      \r      \r      \r      \r      \r      \r     \r     \r\r        \r      \r   \r   }\r\r   function cerosIzq(sVal, nPos){ \r      var sRes = sVal; \r      for (var i = sVal.length; i < nPos; i++){ \r         sRes = \"0\" + sRes; \r      }\r      return sRes; \r   } \r   function armaFecha(nDia, nMes, nAno){ \r      var sRes = cerosIzq(String(nDia), 2); \r      sRes = sRes + \"/\" + cerosIzq(String(nMes), 2); \r      sRes = sRes + \"/\" + cerosIzq(String(nAno), 4); \r      return sRes; \r   } \r   function sumaMes(nDia, nMes, nAno, nSum){ \r      if (nSum >= 0){ \r         for (var i = 0; i < Math.abs(nSum); i++){ \r            if (nMes == 12){ \r               nMes = 1; \r               nAno += 1; \r            }else{\r               nMes += 1; \r            }\r         } \r      }\r    return armaFecha(nDia, nMes, nAno); \r   } \r   \r   function calcula(){ \r      var sFc0 = get('Formulario.txtFechaCorteDesde')\r      alert('sFc0 ' + sFc0);\r      var nSum = parseInt(1); \r      var sFc1 = sFc0; \r      if (!isNaN(nSum)){ \r         var nDia = parseInt(sFc0.substr(0, 2)); \r         var nMes = parseInt(sFc0.substr(3, 2)); \r         var nAno = parseInt(sFc0.substr(6, 4)); \r         sFc1 = sumaMes(nDia, nMes, nAno, nSum); \r      } \r      return sFc1;\r   } \r\r\r   \r  \r  "));
      ...
      ((Element)v.get(5)).setAttribute("name","txtFechaCorteDesde" );
      ((Element)v.get(5)).setAttribute("required","true" );
      ((Element)v.get(5)).setAttribute("cod","2664" );
      ((Element)v.get(4)).appendChild((Element)v.get(5));
      /* Termina nodo:5   */

      /* Empieza nodo:6 / Elemento padre: 4   */
      v.add(doc.createElement("ELEMENTO"));
      ((Element)v.get(6)).setAttribute("name","txtFechaCorteHasta" );
      ((Element)v.get(6)).setAttribute("required","true" );
      ((Element)v.get(6)).setAttribute("cod","2665" );
      ...
      v.add(doc.createElement("VAR"));
      ((Element)v.get(14)).setAttribute("nombre","fechaDesde" );
      ...
      v.add(doc.createElement("VAR"));
      ((Element)v.get(15)).setAttribute("nombre","fechaHasta" );
      ...
      v.add(doc.createElement("VAR"));
      ((Element)v.get(16)).setAttribute("nombre","fechaActual" );
   ...
   private void getXML180(Document doc) {
      ...
      v.add(doc.createElement("CTEXTO"));
      ((Element)v.get(59)).setAttribute("nombre","txtFechaCorteHasta" );
   ...
   private void getXML270(Document doc) {
      ...
      ((Element)v.get(75)).setAttribute("nombre","btnGenerarInforme" );
      ((Element)v.get(75)).setAttribute("ID","botonContenido" );
      ((Element)v.get(75)).setAttribute("tipo","html" );
      ((Element)v.get(75)).setAttribute("accion","generar();" );
      ((Element)v.get(75)).setAttribute("estado","false" );
      ((Element)v.get(75)).setAttribute("cod","2495" );
      ((Element)v.get(75)).setAttribute("ontab","focaliza('Formulario.txtFechaCorteDesde')" );
