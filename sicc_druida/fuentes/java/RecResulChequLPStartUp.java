/*
    INDRA/CAR/mmg
    $Id: RecResulChequLPStartUp.java,v 1.1 2009/12/03 18:42:53 pecbazalar Exp $
    DESC
*/
import java.util.*;
...
public class RecResulChequLPStartUp extends MMGDruidaBase{
	...
	protected static Hashtable indResuValues = new Hashtable();
	static{
		indResuValues.put(new java.lang.Long(1), "Si");
		indResuValues.put(new java.lang.Long(0), "No");
	}
	
	
	// Definicion del constructor
	public RecResulChequLPStartUp(){
		super();
	}

	public void inicio() throws Exception {
		pagina("RecResulChequPage");
	}
	...
	public void ejecucion() throws Exception {
		try{
			...
			asignarAtributo("PAGINA", "RecResulChequPage", "cod", "RecResulChequ." + accion + ".label");
			...
			limpiarBotoneraLista("recResulChequ", accion);
	...
	protected void cmdLov() throws Exception{
		...
		asignarAtributo("TEXT", "codResuCheq", "validacion", "mensaje = ValidaLongitud(cdos_trim(valor), 1, 2);");
		...
		//asignarAtributo("CAPA", "recResulChequLovLayer", "visibilidad", "visible");
		asignarAtributo("PAGINA", "RecResulChequPage", "onblur", "window.focus();");
	...
	protected void cmdPreView() throws Exception{
		...
		conectorAction("RecResulChequLPStartUp");
	...
	*/
	protected void cmdView() throws Exception{
		...
		RecResulChequData recResulChequQuery= new RecResulChequData();
		recResulChequQuery.setId(new Long(conectorParametro("idSelection")));
		
		//Asiganmos el valor a la variable de acción anterior para poder volver hacia atras y hacer requery
		asignar("VAR", VAR_ACCION_ANTERIOR, conectorParametro(VAR_ACCION_ANTERIOR));
		
		//Sacamos de la base de datos la entidad a visualizar
		RecResulChequData datosResult = (RecResulChequData)getSingleObject(BUSINESSID_QUERY, recResulChequQuery, 
			recResulChequQuery.clone(), "recResulChequ");
		...
		preservaHiddensI18n(new Integer(1), "REC_RESUL_CHEQU", "recResulChequFrm", "Descripcion", true);
		...
		RecResulChequData recResulChequ = datosResult;
		asignar("TEXT", "codResuCheq", FormatUtils.formatObject(recResulChequ.getCodResuCheq(), 
			 MMGDruidaHelper.getUserDecimalFormatPattern(this), decimalFormatSymbols));
		asignar("TEXT", "Descripcion", FormatUtils.formatObject(recResulChequ.getDescripcion(), 
			 MMGDruidaHelper.getUserDecimalFormatPattern(this), decimalFormatSymbols));
		if(recResulChequ.getIndResu() != null){	
			checkCombo("indResu", FormatUtils.formatObject(recResulChequ.getIndResu(), 
			 MMGDruidaHelper.getUserDecimalFormatPattern(this), decimalFormatSymbols));
		}
		...
		loadLocalizationVars(recResulChequ);
		
			
		//Deshabilitamos todos los campos para que no se editen
		asignarAtributo("TEXT", "codResuCheq", "disabled", "S");
		asignarAtributo("TEXT", "Descripcion", "disabled", "S");
		
					asignarAtributo("COMBO", "indResu", "disabled", "S");
	...
	protected void cmdCreate() throws Exception{
		//Creamos los campos ocultos para la internacionalización
		preservaHiddensI18n(new Integer(1), "REC_RESUL_CHEQU", "recResulChequFrm", "Descripcion", false);
		...
		asignarAtributo("TEXT", "codResuCheq", "req", "S");
		asignarAtributo("TEXT", "Descripcion", "req", "S");
		asignarAtributo("COMBO", "indResu", "req", "S");
		
		
		//Asiganamos a los campos internacionalizables la función de onblur para actualizar el 
		//valor del idioma por defecto
		asignarAtributo("TEXT", "Descripcion", "onblur", "setDefaultLanguageValue('Descripcion', get('recResulChequFrm.Descripcion'))");
	...
	protected void cmdUpdateForm() throws Exception{
		...
		//asignarAtributo("FORMULARIO", "recResulChequFrm", "oculto", "N");
		...
		traza("MMG::Creando RecResulChequ");
		String id = conectorParametro("idSelection");
		traza("MMG:: id del atributo a modificar: " + id);
	
		//Construimos el mso con los elementos de la búsqueda
		RecResulChequData recResulChequQuery= new RecResulChequData();
		recResulChequQuery.setId(new Long(id));
		...
		//asignar("VAR","timestamp",Long.toString(recResulChequ.jdoGetTimeStamp()));
	...
	*/
	protected void cambiarAcciones() throws Exception{
				asignarAtributo("AREATEXTO", "id", "ontab","ejecutarAccionFocoModificacion(\"recResulChequFrm\",1, true)");
				asignarAtributo("AREATEXTO", "id", "onshtab","ejecutarAccionFocoModificacion(\"recResulChequFrm\",1, false)");
					asignarAtributo("COMBO","paisOidPais", "ontab", "ejecutarAccionFocoModificacion(\"recResulChequFrm\",2, true)");
					asignarAtributo("COMBO","paisOidPais", "onshtab", "ejecutarAccionFocoModificacion(\"recResulChequFrm\",2, false)");
				asignarAtributo("AREATEXTO", "codResuCheq", "ontab","ejecutarAccionFocoModificacion(\"recResulChequFrm\",3, true)");
				asignarAtributo("AREATEXTO", "codResuCheq", "onshtab","ejecutarAccionFocoModificacion(\"recResulChequFrm\",3, false)");
				asignarAtributo("AREATEXTO", "Descripcion", "ontab","ejecutarAccionFocoModificacion(\"recResulChequFrm\",4, true)");
				asignarAtributo("AREATEXTO", "Descripcion", "onshtab","ejecutarAccionFocoModificacion(\"recResulChequFrm\",4, false)");
				asignarAtributo("TEXT", "indResu", "ontab","ejecutarAccionFocoModificacion(\"recResulChequFrm\",5, true)");
				asignarAtributo("TEXT", "indResu", "onshtab","ejecutarAccionFocoModificacion(\"recResulChequFrm\",5, false)");
	}
	...
	protected void removeLovElements() throws Exception{
		borrarNodo("TD","nombre","recResulChequTdLovNullSelectionButton");
	}
	
	protected void removeQueryElements() throws Exception{
		//Borramos los botones para realizar la búsuqeda
		borrarNodo("TR","nombre","recResulChequTrButtons");
	...
	*/
	protected void establecerSizeCapas(String accion, String origen) throws Exception{
		...
		asignarAtributo("CAPA","recResulChequFormLayer", "alto", String.valueOf(sizeForm));		
		
		//Estabelcemos la posicion y de la capa correspondiente a la lista y de su botonera
		asignarAtributo("CAPA","recResulChequListLayer", "y", String.valueOf(sizeForm + 26));		
		asignarAtributo("CAPA","recResulChequListButtonsLayer", "y", String.valueOf(sizeForm + 278));		
	}

	/**
	* Nos permite cargar las combos finales de la jerarquías de dependencia
	*/
	protected void loadLeafDependenceCombos(RecResulChequData recResulChequ) throws Exception{
		...
		if(recResulChequ != null && recResulChequ.getPaisOidPais() != null){
				checkCombo("paisOidPais" , recResulChequ.getPaisOidPais().getId().toString());
			}
	...
	* @param recResulChequ La el objetopar la cual se va a realizar la carga
	*/
	protected void loadNonLeafDependenceCombos(RecResulChequData recResulChequ) 
		throws Exception{
	...
	*/
	protected void loadNonLeafDependenceCombosForRequery() throws Exception{
		...
		RecResulChequData recResulChequ = 
			(RecResulChequData) conectorParametroSesion(SESSION_ATTRIBUTE_LAST_QUERY_OBJ_FROM);
		
		//Cargamos todos los objetos completos correspodnientes a los valores de los
		//Campos que sean raiz de una jerarquía de combos dependientes
		if(recResulChequ.getPaisOidPais() != null){
			recResulChequ.setPaisOidPais((es.indra.belcorp.mso.SegPaisViewData)
			getSingleObject(BUSINESSID_PAISOIDPAIS_QUERY, 
			recResulChequ.getPaisOidPais(),
			recResulChequ.getPaisOidPais().clone(),
			"segPaisView"));
		}
		
		//Una vez que tenemos completo los datos de los atributos raices de jerarquías de dependencias
		//cargamos las combos y seleccionamos elementos
		loadNonLeafDependenceCombos(recResulChequ);
		loadLeafDependenceCombos(recResulChequ);
	}
	...
	*/
	...
	protected void loadLocalizationVars(RecResulChequData recResulChequ) throws Exception{
		GenDetaSiccData mmgLocalizationQuery = null;
		Vector mmgLocalizationDatosResult = null;
		mmgLocalizationQuery = new GenDetaSiccData();
		mmgLocalizationQuery.setEnti("REC_RESUL_CHEQU");
		mmgLocalizationQuery.setEntiPk(recResulChequ.getId());
