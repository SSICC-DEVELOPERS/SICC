/*
    INDRA/CAR/mmg
    $Id: CobArgumLPStartUp.java,v 1.1 2009/12/03 18:36:44 pecbazalar Exp $
    DESC
*/
import java.util.*;
...
public class CobArgumLPStartUp extends MMGDruidaBase{
	...
	public void ejecucion() throws Exception {
		try{
			...
			asignarAtributo("PAGINA", "CobArgumPage", "cod", "CobArgum." + accion + ".label");
			...
			limpiarBotoneraLista("cobArgum", accion);
	...
	protected void cmdLov() throws Exception{
		...
		asignarAtributo("TEXT", "codArgu", "validacion", "mensaje = ValidaLongitud(cdos_trim(valor), 1, 4);");
		...
		//asignarAtributo("CAPA", "cobArgumLovLayer", "visibilidad", "visible");
		asignarAtributo("PAGINA", "CobArgumPage", "onblur", "window.focus();");
	...
	protected void cmdPreView() throws Exception{
		...
		conectorAction("CobArgumLPStartUp");
	...
	*/
	protected void cmdView() throws Exception{
		...
		CobArgumData cobArgumQuery= new CobArgumData();
		cobArgumQuery.setId(new Long(conectorParametro("idSelection")));
		
		//Asiganmos el valor a la variable de acción anterior para poder volver hacia atras y hacer requery
		asignar("VAR", VAR_ACCION_ANTERIOR, conectorParametro(VAR_ACCION_ANTERIOR));
		
		//Sacamos de la base de datos la entidad a visualizar
		CobArgumData datosResult = (CobArgumData)getSingleObject(BUSINESSID_QUERY, cobArgumQuery, 
			cobArgumQuery.clone(), "cobArgum");
		...
		CobArgumData cobArgum = datosResult;
		asignar("TEXT", "codArgu", FormatUtils.formatObject(cobArgum.getCodArgu(), 
			 MMGDruidaHelper.getUserDecimalFormatPattern(this), decimalFormatSymbols));
		asignar("TEXT", "valDesc", FormatUtils.formatObject(cobArgum.getValDesc(), 
			 MMGDruidaHelper.getUserDecimalFormatPattern(this), decimalFormatSymbols));
		asignar("TEXT", "valObse", FormatUtils.formatObject(cobArgum.getValObse(), 
			 MMGDruidaHelper.getUserDecimalFormatPattern(this), decimalFormatSymbols));
		...
		asignarAtributo("TEXT", "codArgu", "disabled", "S");
	...
	protected void cmdCreate() throws Exception{
		...
		asignarAtributo("TEXT", "codArgu", "req", "S");
	...
	protected void cmdUpdateForm() throws Exception{
		...
		//asignarAtributo("FORMULARIO", "cobArgumFrm", "oculto", "N");
		...
		traza("MMG::Creando CobArgum");
		String id = conectorParametro("idSelection");
		traza("MMG:: id del atributo a modificar: " + id);
	
		//Construimos el mso con los elementos de la búsqueda
		CobArgumData cobArgumQuery= new CobArgumData();
		cobArgumQuery.setId(new Long(id));
		...
		//asignar("VAR","timestamp",Long.toString(cobArgum.jdoGetTimeStamp()));
	...
	*/
	protected void cambiarAcciones() throws Exception{
				asignarAtributo("AREATEXTO", "id", "ontab","ejecutarAccionFocoModificacion(\"cobArgumFrm\",1, true)");
				asignarAtributo("AREATEXTO", "id", "onshtab","ejecutarAccionFocoModificacion(\"cobArgumFrm\",1, false)");
				asignarAtributo("AREATEXTO", "codArgu", "ontab","ejecutarAccionFocoModificacion(\"cobArgumFrm\",2, true)");
				asignarAtributo("AREATEXTO", "codArgu", "onshtab","ejecutarAccionFocoModificacion(\"cobArgumFrm\",2, false)");
				asignarAtributo("AREATEXTO", "valDesc", "ontab","ejecutarAccionFocoModificacion(\"cobArgumFrm\",3, true)");
				asignarAtributo("AREATEXTO", "valDesc", "onshtab","ejecutarAccionFocoModificacion(\"cobArgumFrm\",3, false)");
				asignarAtributo("AREATEXTO", "valObse", "ontab","ejecutarAccionFocoModificacion(\"cobArgumFrm\",4, true)");
				asignarAtributo("AREATEXTO", "valObse", "onshtab","ejecutarAccionFocoModificacion(\"cobArgumFrm\",4, false)");
					asignarAtributo("COMBO","paisOidPais", "ontab", "ejecutarAccionFocoModificacion(\"cobArgumFrm\",5, true)");
					asignarAtributo("COMBO","paisOidPais", "onshtab", "ejecutarAccionFocoModificacion(\"cobArgumFrm\",5, false)");
	}
	...
	protected void removeLovElements() throws Exception{
		borrarNodo("TD","nombre","cobArgumTdLovNullSelectionButton");
	}
	
	protected void removeQueryElements() throws Exception{
		//Borramos los botones para realizar la búsuqeda
		borrarNodo("TR","nombre","cobArgumTrButtons");
	...
	*/
	protected void desactivateNonModifyFields() throws Exception{
			traza("MMG: en desactivateNonModifyFields ");		
				...
				traza("MMG: el atributo es codArgu...");		
	...
	*/
	protected void establecerSizeCapas(String accion, String origen) throws Exception{
		...
		asignarAtributo("CAPA","cobArgumFormLayer", "alto", String.valueOf(sizeForm));		
		
		//Estabelcemos la posicion y de la capa correspondiente a la lista y de su botonera
		asignarAtributo("CAPA","cobArgumListLayer", "y", String.valueOf(sizeForm + 26));		
		asignarAtributo("CAPA","cobArgumListButtonsLayer", "y", String.valueOf(sizeForm + 278));		
	}

	/**
	* Nos permite cargar las combos finales de la jerarquías de dependencia
	*/
	protected void loadLeafDependenceCombos(CobArgumData cobArgum) throws Exception{
		...
		if(cobArgum != null && cobArgum.getPaisOidPais() != null){
				checkCombo("paisOidPais" , cobArgum.getPaisOidPais().getId().toString());
			}
	...
	* @param cobArgum La el objetopar la cual se va a realizar la carga
	*/
	protected void loadNonLeafDependenceCombos(CobArgumData cobArgum) 
		throws Exception{
	...
	*/
	protected void loadNonLeafDependenceCombosForRequery() throws Exception{
		...
		CobArgumData cobArgum = 
			(CobArgumData) conectorParametroSesion(SESSION_ATTRIBUTE_LAST_QUERY_OBJ_FROM);
		
		//Cargamos todos los objetos completos correspodnientes a los valores de los
		//Campos que sean raiz de una jerarquía de combos dependientes
		if(cobArgum.getPaisOidPais() != null){
			cobArgum.setPaisOidPais((es.indra.belcorp.mso.SegPaisViewData)
			getSingleObject(BUSINESSID_PAISOIDPAIS_QUERY, 
			cobArgum.getPaisOidPais(),
			cobArgum.getPaisOidPais().clone(),
			"segPaisView"));
		}
		
		//Una vez que tenemos completo los datos de los atributos raices de jerarquías de dependencias
		//cargamos las combos y seleccionamos elementos
		loadNonLeafDependenceCombos(cobArgum);
		loadLeafDependenceCombos(cobArgum);
	}
