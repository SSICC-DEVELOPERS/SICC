/*
    INDRA/CAR/mmg
    $Id: MaeNivelEstudLPStartUp.java,v 1.1 2009/12/03 18:38:39 pecbazalar Exp $
    DESC
*/
import java.util.*;
...
public class MaeNivelEstudLPStartUp extends MMGDruidaBase{
	...
	public MaeNivelEstudLPStartUp(){
		super();
	}

	public void inicio() throws Exception {
		pagina("MaeNivelEstudPage");
	}
	...
	public void ejecucion() throws Exception {
		try{
			...
			asignarAtributo("PAGINA", "MaeNivelEstudPage", "cod", "MaeNivelEstud." + accion + ".label");
			...
			limpiarBotoneraLista("maeNivelEstud", accion);
	...
	protected void cmdLov() throws Exception{
		...
		asignarAtributo("TEXT", "codNiveEstu", "validacion", "mensaje = ValidaLongitud(cdos_trim(valor), 1, 2);");
		...
		//asignarAtributo("CAPA", "maeNivelEstudLovLayer", "visibilidad", "visible");
		asignarAtributo("PAGINA", "MaeNivelEstudPage", "onblur", "window.focus();");
	...
	protected void cmdPreView() throws Exception{
		...
		conectorAction("MaeNivelEstudLPStartUp");
	...
	*/
	protected void cmdView() throws Exception{
		...
		MaeNivelEstudData maeNivelEstudQuery= new MaeNivelEstudData();
		maeNivelEstudQuery.setId(new Long(conectorParametro("idSelection")));
		
		//Asiganmos el valor a la variable de acción anterior para poder volver hacia atras y hacer requery
		asignar("VAR", VAR_ACCION_ANTERIOR, conectorParametro(VAR_ACCION_ANTERIOR));
		
		//Sacamos de la base de datos la entidad a visualizar
		MaeNivelEstudData datosResult = (MaeNivelEstudData)getSingleObject(BUSINESSID_QUERY, maeNivelEstudQuery, 
			maeNivelEstudQuery.clone(), "maeNivelEstud");
		...
		preservaHiddensI18n(new Integer(1), "MAE_NIVEL_ESTUD", "maeNivelEstudFrm", "Descripcion", true);
		...
		MaeNivelEstudData maeNivelEstud = datosResult;
		asignar("TEXT", "codNiveEstu", FormatUtils.formatObject(maeNivelEstud.getCodNiveEstu(), 
		...
		loadLocalizationVars(maeNivelEstud);
		
			
		//Deshabilitamos todos los campos para que no se editen
		asignarAtributo("TEXT", "codNiveEstu", "disabled", "S");
	...
	protected void cmdCreate() throws Exception{
		//Creamos los campos ocultos para la internacionalización
		preservaHiddensI18n(new Integer(1), "MAE_NIVEL_ESTUD", "maeNivelEstudFrm", "Descripcion", false);
		...
		asignarAtributo("TEXT", "codNiveEstu", "req", "S");
		...
		asignarAtributo("TEXT", "Descripcion", "onblur", "setDefaultLanguageValue('Descripcion', get('maeNivelEstudFrm.Descripcion'))");
	...
	protected void cmdUpdateForm() throws Exception{
		...
		//asignarAtributo("FORMULARIO", "maeNivelEstudFrm", "oculto", "N");
		...
		traza("MMG::Creando MaeNivelEstud");
		String id = conectorParametro("idSelection");
		traza("MMG:: id del atributo a modificar: " + id);
	
		//Construimos el mso con los elementos de la búsqueda
		MaeNivelEstudData maeNivelEstudQuery= new MaeNivelEstudData();
		maeNivelEstudQuery.setId(new Long(id));
		...
		//asignar("VAR","timestamp",Long.toString(maeNivelEstud.jdoGetTimeStamp()));
	...
	*/
	protected void cambiarAcciones() throws Exception{
				asignarAtributo("AREATEXTO", "id", "ontab","ejecutarAccionFocoModificacion(\"maeNivelEstudFrm\",1, true)");
				asignarAtributo("AREATEXTO", "id", "onshtab","ejecutarAccionFocoModificacion(\"maeNivelEstudFrm\",1, false)");
					asignarAtributo("COMBO","paisOidPais", "ontab", "ejecutarAccionFocoModificacion(\"maeNivelEstudFrm\",2, true)");
					asignarAtributo("COMBO","paisOidPais", "onshtab", "ejecutarAccionFocoModificacion(\"maeNivelEstudFrm\",2, false)");
				asignarAtributo("AREATEXTO", "codNiveEstu", "ontab","ejecutarAccionFocoModificacion(\"maeNivelEstudFrm\",3, true)");
				asignarAtributo("AREATEXTO", "codNiveEstu", "onshtab","ejecutarAccionFocoModificacion(\"maeNivelEstudFrm\",3, false)");
				asignarAtributo("AREATEXTO", "Descripcion", "ontab","ejecutarAccionFocoModificacion(\"maeNivelEstudFrm\",4, true)");
				asignarAtributo("AREATEXTO", "Descripcion", "onshtab","ejecutarAccionFocoModificacion(\"maeNivelEstudFrm\",4, false)");
	}
	...
	protected void removeLovElements() throws Exception{
		borrarNodo("TD","nombre","maeNivelEstudTdLovNullSelectionButton");
	}
	
	protected void removeQueryElements() throws Exception{
		//Borramos los botones para realizar la búsuqeda
		borrarNodo("TR","nombre","maeNivelEstudTrButtons");
	...
	*/
	protected void desactivateNonModifyFields() throws Exception{
			traza("MMG: en desactivateNonModifyFields ");		
				...
				traza("MMG: el atributo es codNiveEstu...");		
				
					traza("MMG: en desactivateNonModifyFields encontrado atributo no editable...");
				
						// borrarCampo("codNiveEstu");
						asignarAtributo ("TEXT", "codNiveEstu", "disabled", "S");
	...
	*/
	protected void establecerSizeCapas(String accion, String origen) throws Exception{
		...
		asignarAtributo("CAPA","maeNivelEstudFormLayer", "alto", String.valueOf(sizeForm));		
		
		//Estabelcemos la posicion y de la capa correspondiente a la lista y de su botonera
		asignarAtributo("CAPA","maeNivelEstudListLayer", "y", String.valueOf(sizeForm + 26));		
		asignarAtributo("CAPA","maeNivelEstudListButtonsLayer", "y", String.valueOf(sizeForm + 278));		
	}

	/**
	* Nos permite cargar las combos finales de la jerarquías de dependencia
	*/
	protected void loadLeafDependenceCombos(MaeNivelEstudData maeNivelEstud) throws Exception{
		...
		if(maeNivelEstud != null && maeNivelEstud.getPaisOidPais() != null){
				checkCombo("paisOidPais" , maeNivelEstud.getPaisOidPais().getId().toString());
			}
	...
	* @param maeNivelEstud La el objetopar la cual se va a realizar la carga
	*/
	protected void loadNonLeafDependenceCombos(MaeNivelEstudData maeNivelEstud) 
		throws Exception{
	...
	*/
	protected void loadNonLeafDependenceCombosForRequery() throws Exception{
		...
		MaeNivelEstudData maeNivelEstud = 
			(MaeNivelEstudData) conectorParametroSesion(SESSION_ATTRIBUTE_LAST_QUERY_OBJ_FROM);
		
		//Cargamos todos los objetos completos correspodnientes a los valores de los
		//Campos que sean raiz de una jerarquía de combos dependientes
		if(maeNivelEstud.getPaisOidPais() != null){
			maeNivelEstud.setPaisOidPais((es.indra.belcorp.mso.SegPaisViewData)
			getSingleObject(BUSINESSID_PAISOIDPAIS_QUERY, 
			maeNivelEstud.getPaisOidPais(),
			maeNivelEstud.getPaisOidPais().clone(),
			"segPaisView"));
		}
		
		//Una vez que tenemos completo los datos de los atributos raices de jerarquías de dependencias
		//cargamos las combos y seleccionamos elementos
		loadNonLeafDependenceCombos(maeNivelEstud);
		loadLeafDependenceCombos(maeNivelEstud);
	}
	...
	*/
	...
	protected void loadLocalizationVars(MaeNivelEstudData maeNivelEstud) throws Exception{
		GenDetaSiccData mmgLocalizationQuery = null;
		Vector mmgLocalizationDatosResult = null;
		mmgLocalizationQuery = new GenDetaSiccData();
		mmgLocalizationQuery.setEnti("MAE_NIVEL_ESTUD");
		mmgLocalizationQuery.setEntiPk(maeNivelEstud.getId());
