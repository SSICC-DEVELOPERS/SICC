/*
    INDRA/CAR/mmg
    $Id: GenUsersLPStartUp.java,v 1.1 2009/12/03 18:33:12 pecbazalar Exp $
    DESC
*/
import java.util.*;
...
public class GenUsersLPStartUp extends MMGDruidaBase{
	//Constantes que definen la altura de los formularios según el modo
	public static final short QUERYFORM_SIZE = 273;
	public static final short CREATEFORM_SIZE = 251;
	public static final short UPDATEFORM_SIZE = 251;
	public static final short VIEWFORM_SIZE = 251;
	...
	public static final String BUSINESSID_QUERY = "MMGGenUsersQueryFromToUserPage";
				...
				public static final String BUSINESSID_MYID_QUERY ="MMGGenPrincipalsQueryFromToUser";
				public static final String MYID_COMBO_CONNECTOR = "GenPrincipalsConectorCombo";
	...
	public void ejecucion() throws Exception {
		try{
			...
			asignarAtributo("PAGINA", "GenUsersPage", "cod", "GenUsers." + accion + ".label");
			...
			limpiarBotoneraLista("genUsers", accion);
	...
	protected void cmdLov() throws Exception{
		...
		asignarAtributo("TEXT", "credential", "validacion", "mensaje = ValidaLongitud(cdos_trim(valor), 0, 127);");
		asignarAtributo("TEXT", "setdate", "validacion", "mensaje = ValidaFechaMMG(cdos_trim(valor), '" + datePattern + "');");
		asignarAtributo("TEXT", "retries", "validacion", "mensaje = ValidaInt(cdos_trim(valor), 2, 0, 99);");
		asignarAtributo("TEXT", "blocked", "validacion", "mensaje = ValidaInt(cdos_trim(valor), 1, 0, 9);");
		
		//Primero determianmos el nombre del campo en la entidad de oriegen donde se van a cargar los datos
		asignar("VAR", "idAttributeLovOpener", conectorParametro(PARAMETRO_LOV_ATTRIBUTO));
		asignar("VAR", "idEntityLovOpener", conectorParametro(PARAMETRO_LOV_ENTIDAD));
		//asignarAtributo("CAPA", "genUsersLovLayer", "visibilidad", "visible");
		asignarAtributo("PAGINA", "GenUsersPage", "onblur", "window.focus();");
	...
	protected void cmdPreView() throws Exception{
		...
		conectorAction("GenUsersLPStartUp");
	...
	*/
	protected void cmdView() throws Exception{
		...
		GenUsersData genUsersQuery= new GenUsersData();
		genUsersQuery.setId(new Long(conectorParametro("idSelection")));
		
		//Asiganmos el valor a la variable de acción anterior para poder volver hacia atras y hacer requery
		asignar("VAR", VAR_ACCION_ANTERIOR, conectorParametro(VAR_ACCION_ANTERIOR));
		
		//Sacamos de la base de datos la entidad a visualizar
		GenUsersData datosResult = (GenUsersData)getSingleObject(BUSINESSID_QUERY, genUsersQuery, 
			genUsersQuery.clone(), "genUsers");
		...
		GenUsersData genUsers = datosResult;
		es.indra.belcorp.mso.GenPrincipalsData myid = genUsers.getMyid();
		if(myid != null && myid.getId() != null) checkCombo("myid" , myid.getId().toString());
		asignar("TEXT", "credential", FormatUtils.formatObject(genUsers.getCredential(), 
			 MMGDruidaHelper.getUserDecimalFormatPattern(this), decimalFormatSymbols));
		asignar("TEXT", "setdate", FormatUtils.formatObject(genUsers.getSetdate(), 
			 UtilidadesSession.getFormatoFecha(this), decimalFormatSymbols));
		asignar("TEXT", "retries", FormatUtils.formatObject(genUsers.getRetries(), 
			 MMGDruidaHelper.getUserDecimalFormatPattern(this), decimalFormatSymbols));
		asignar("TEXT", "blocked", FormatUtils.formatObject(genUsers.getBlocked(), 
			 MMGDruidaHelper.getUserDecimalFormatPattern(this), decimalFormatSymbols));
		...
		//Deshabilitamos todos los campos para que no se editen
					...
					asignarAtributo("COMBO", "myid", "disabled", "S");asignarAtributo("TEXT", "credential", "disabled", "S");
		asignarAtributo("TEXT", "setdate", "disabled", "S");
		asignarAtributo("TEXT", "retries", "disabled", "S");
		asignarAtributo("TEXT", "blocked", "disabled", "S");
	...
	protected void cmdCreate() throws Exception{
		...
		asignarAtributo("COMBO", "myid", "req", "S");
	...
	protected void cmdUpdateForm() throws Exception{
		...
		//asignarAtributo("FORMULARIO", "genUsersFrm", "oculto", "N");
		...
		traza("MMG::Creando GenUsers");
		String id = conectorParametro("idSelection");
		traza("MMG:: id del atributo a modificar: " + id);
	
		//Construimos el mso con los elementos de la búsqueda
		GenUsersData genUsersQuery= new GenUsersData();
		genUsersQuery.setId(new Long(id));
		...
		//asignar("VAR","timestamp",Long.toString(genUsers.jdoGetTimeStamp()));
	...
	*/
	protected void cambiarAcciones() throws Exception{
				asignarAtributo("AREATEXTO", "id", "ontab","ejecutarAccionFocoModificacion(\"genUsersFrm\",0, true)");
				asignarAtributo("AREATEXTO", "id", "onshtab","ejecutarAccionFocoModificacion(\"genUsersFrm\",0, false)");
					asignarAtributo("COMBO","myid", "ontab", "ejecutarAccionFocoModificacion(\"genUsersFrm\",0, true)");
					asignarAtributo("COMBO","myid", "onshtab", "ejecutarAccionFocoModificacion(\"genUsersFrm\",0, false)");
				asignarAtributo("AREATEXTO", "credential", "ontab","ejecutarAccionFocoModificacion(\"genUsersFrm\",0, true)");
				asignarAtributo("AREATEXTO", "credential", "onshtab","ejecutarAccionFocoModificacion(\"genUsersFrm\",0, false)");
				asignarAtributo("AREATEXTO", "setdate", "ontab","ejecutarAccionFocoModificacion(\"genUsersFrm\",0, true)");
				asignarAtributo("AREATEXTO", "setdate", "onshtab","ejecutarAccionFocoModificacion(\"genUsersFrm\",0, false)");
				asignarAtributo("AREATEXTO", "retries", "ontab","ejecutarAccionFocoModificacion(\"genUsersFrm\",0, true)");
				asignarAtributo("AREATEXTO", "retries", "onshtab","ejecutarAccionFocoModificacion(\"genUsersFrm\",0, false)");
				asignarAtributo("AREATEXTO", "blocked", "ontab","ejecutarAccionFocoModificacion(\"genUsersFrm\",0, true)");
				asignarAtributo("AREATEXTO", "blocked", "onshtab","ejecutarAccionFocoModificacion(\"genUsersFrm\",0, false)");
	}
	...
	protected void removeLovElements() throws Exception{
		borrarNodo("TD","nombre","genUsersTdLovNullSelectionButton");
	}
	
	protected void removeQueryElements() throws Exception{
		//Borramos los botones para realizar la búsuqeda
		borrarNodo("TR","nombre","genUsersTrButtons");
	...
	*/
	protected void desactivateNonModifyFields() throws Exception{
			traza("MMG: en desactivateNonModifyFields ");		
				...
				traza("MMG: el atributo es myid...");		
				
				traza("MMG: el atributo es credential...");		
				
				traza("MMG: el atributo es setdate...");		
				
				traza("MMG: el atributo es retries...");		
				
				traza("MMG: el atributo es blocked...");		
	...
	*/
	protected void establecerSizeCapas(String accion, String origen) throws Exception{
		...
		asignarAtributo("CAPA","genUsersFormLayer", "alto", String.valueOf(sizeForm));		
		
		//Estabelcemos la posicion y de la capa correspondiente a la lista y de su botonera
		asignarAtributo("CAPA","genUsersListLayer", "y", String.valueOf(sizeForm + 26));		
		asignarAtributo("CAPA","genUsersListButtonsLayer", "y", String.valueOf(sizeForm + 278));		
	}

	/**
	* Nos permite cargar las combos finales de la jerarquías de dependencia
	*/
	protected void loadLeafDependenceCombos(GenUsersData genUsers) throws Exception{
		HashMap checkedCombos = new HashMap();
		GenPrincipalsData myid = new GenPrincipalsData();
		
						asignarComboResultados(BUSINESSID_MYID_QUERY,MYID_COMBO_CONNECTOR, 
			myid, myid.clone(), "genPrincipals", "myid", "result_ROWSET" );
		if(genUsers != null && genUsers.getMyid() != null){
				checkCombo("myid" , genUsers.getMyid().getId().toString());
			}
	...
	* @param genUsers La el objetopar la cual se va a realizar la carga
	*/
	protected void loadNonLeafDependenceCombos(GenUsersData genUsers) 
		throws Exception{
	...
	*/
	protected void loadNonLeafDependenceCombosForRequery() throws Exception{
		...
		GenUsersData genUsers = 
			(GenUsersData) conectorParametroSesion(SESSION_ATTRIBUTE_LAST_QUERY_OBJ_FROM);
		
		//Cargamos todos los objetos completos correspodnientes a los valores de los
		//Campos que sean raiz de una jerarquía de combos dependientes
		if(genUsers.getMyid() != null){
			genUsers.setMyid((es.indra.belcorp.mso.GenPrincipalsData)
			getSingleObject(BUSINESSID_MYID_QUERY, 
			genUsers.getMyid(),
			genUsers.getMyid().clone(),
			"genPrincipals"));
		}
		
		//Una vez que tenemos completo los datos de los atributos raices de jerarquías de dependencias
		//cargamos las combos y seleccionamos elementos
		loadNonLeafDependenceCombos(genUsers);
		loadLeafDependenceCombos(genUsers);
	}
