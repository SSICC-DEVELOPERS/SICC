/*
    INDRA/CAR/mmg
    $Id: MaeCriteBusquLPStartUp.java,v 1.1 2009/12/03 18:42:38 pecbazalar Exp $
    DESC
*/
import java.util.*;
...
public class MaeCriteBusquLPStartUp extends MMGDruidaBase{
	...
	public static final String BUSINESSID_QUERY = "MMGMaeCriteBusquQueryFromToUserPage";
				...
				public static final String BUSINESSID_ATREOIDATRIESP1_QUERY ="MMGPedAtribEspecQueryFromToUser";
				public static final String ATREOIDATRIESP1_COMBO_CONNECTOR = "PedAtribEspecConectorCombo";
	
				public static final String BUSINESSID_ATREOIDATRIESP2_QUERY ="MMGPedAtribEspecQueryFromToUser";
				public static final String ATREOIDATRIESP2_COMBO_CONNECTOR = "PedAtribEspecConectorCombo";
	...
	public void ejecucion() throws Exception {
		try{
			...
			asignarAtributo("PAGINA", "MaeCriteBusquPage", "cod", "MaeCriteBusqu." + accion + ".label");
			...
			limpiarBotoneraLista("maeCriteBusqu", accion);
	...
	protected void cmdLov() throws Exception{
		...
		//asignarAtributo("CAPA", "maeCriteBusquLovLayer", "visibilidad", "visible");
		asignarAtributo("PAGINA", "MaeCriteBusquPage", "onblur", "window.focus();");
	...
	*/
	protected void cmdView() throws Exception{
		...
		MaeCriteBusquData maeCriteBusquQuery= new MaeCriteBusquData();
		maeCriteBusquQuery.setId(new Long(conectorParametro("idSelection")));
		
		//Asiganmos el valor a la variable de acción anterior para poder volver hacia atras y hacer requery
		asignar("VAR", VAR_ACCION_ANTERIOR, conectorParametro(VAR_ACCION_ANTERIOR));
		
		//Sacamos de la base de datos la entidad a visualizar
		MaeCriteBusquData datosResult = (MaeCriteBusquData)getSingleObject(BUSINESSID_QUERY, maeCriteBusquQuery, 
			maeCriteBusquQuery.clone(), "maeCriteBusqu");
		...
		MaeCriteBusquData maeCriteBusqu = datosResult;
		es.indra.belcorp.mso.PedAtribEspecData atreOidAtriEsp1 = maeCriteBusqu.getAtreOidAtriEsp1();
		if(atreOidAtriEsp1 != null && atreOidAtriEsp1.getId() != null) checkCombo("atreOidAtriEsp1" , atreOidAtriEsp1.getId().toString());
		es.indra.belcorp.mso.PedAtribEspecData atreOidAtriEsp2 = maeCriteBusqu.getAtreOidAtriEsp2();
		if(atreOidAtriEsp2 != null && atreOidAtriEsp2.getId() != null) checkCombo("atreOidAtriEsp2" , atreOidAtriEsp2.getId().toString());
		...
		//Deshabilitamos todos los campos para que no se editen
					...
					asignarAtributo("COMBO", "atreOidAtriEsp1", "disabled", "S");
					asignarAtributo("COMBO", "atreOidAtriEsp2", "disabled", "S");
	...
	protected void cmdCreate() throws Exception{
		...
		asignarAtributo("COMBO", "atreOidAtriEsp1", "req", "S");
		asignarAtributo("COMBO", "atreOidAtriEsp2", "req", "S");
	...
	protected void cmdUpdateForm() throws Exception{
		...
		//asignarAtributo("FORMULARIO", "maeCriteBusquFrm", "oculto", "N");
		...
		//Construimos el mso con los elementos de la búsqueda
		MaeCriteBusquData maeCriteBusquQuery= new MaeCriteBusquData();
		maeCriteBusquQuery.setId(new Long(id));
		...
		//asignar("VAR","timestamp",Long.toString(maeCriteBusqu.jdoGetTimeStamp()));
	...
	*/
	protected void cambiarAcciones() throws Exception{
				asignarAtributo("AREATEXTO", "id", "ontab","ejecutarAccionFocoModificacion(\"maeCriteBusquFrm\",1, true)");
				asignarAtributo("AREATEXTO", "id", "onshtab","ejecutarAccionFocoModificacion(\"maeCriteBusquFrm\",1, false)");
					asignarAtributo("COMBO","paisOidPais", "ontab", "ejecutarAccionFocoModificacion(\"maeCriteBusquFrm\",2, true)");
					asignarAtributo("COMBO","paisOidPais", "onshtab", "ejecutarAccionFocoModificacion(\"maeCriteBusquFrm\",2, false)");
					asignarAtributo("COMBO","atreOidAtriEsp1", "ontab", "ejecutarAccionFocoModificacion(\"maeCriteBusquFrm\",3, true)");
					asignarAtributo("COMBO","atreOidAtriEsp1", "onshtab", "ejecutarAccionFocoModificacion(\"maeCriteBusquFrm\",3, false)");
					asignarAtributo("COMBO","atreOidAtriEsp2", "ontab", "ejecutarAccionFocoModificacion(\"maeCriteBusquFrm\",4, true)");
					asignarAtributo("COMBO","atreOidAtriEsp2", "onshtab", "ejecutarAccionFocoModificacion(\"maeCriteBusquFrm\",4, false)");
	}
	...
	protected void removeLovElements() throws Exception{
		borrarNodo("TD","nombre","maeCriteBusquTdLovNullSelectionButton");
	}
	
	protected void removeQueryElements() throws Exception{
		//Borramos los botones para realizar la búsuqeda
		borrarNodo("TR","nombre","maeCriteBusquTrButtons");
	...
	*/
	protected void desactivateNonModifyFields() throws Exception{
			traza("MMG: en desactivateNonModifyFields ");		
				...
				traza("MMG: el atributo es atreOidAtriEsp1...");		
				
				traza("MMG: el atributo es atreOidAtriEsp2...");		
	...
	*/
	protected void establecerSizeCapas(String accion, String origen) throws Exception{
		...
		asignarAtributo("CAPA","maeCriteBusquFormLayer", "alto", String.valueOf(sizeForm));		
		
		//Estabelcemos la posicion y de la capa correspondiente a la lista y de su botonera
		asignarAtributo("CAPA","maeCriteBusquListLayer", "y", String.valueOf(sizeForm + 26));		
		asignarAtributo("CAPA","maeCriteBusquListButtonsLayer", "y", String.valueOf(sizeForm + 278));		
	}

	/**
	* Nos permite cargar las combos finales de la jerarquías de dependencia
	*/
	protected void loadLeafDependenceCombos(MaeCriteBusquData maeCriteBusqu) throws Exception{
		...
		if(maeCriteBusqu != null && maeCriteBusqu.getPaisOidPais() != null){
				checkCombo("paisOidPais" , maeCriteBusqu.getPaisOidPais().getId().toString());
			}
		...
		PedAtribEspecData atreOidAtriEsp1 = new PedAtribEspecData();
		
						asignarComboResultados(BUSINESSID_ATREOIDATRIESP1_QUERY,ATREOIDATRIESP1_COMBO_CONNECTOR, 
			atreOidAtriEsp1, atreOidAtriEsp1.clone(), "pedAtribEspec", "atreOidAtriEsp1", "result_ROWSET" );
		if(maeCriteBusqu != null && maeCriteBusqu.getAtreOidAtriEsp1() != null){
				checkCombo("atreOidAtriEsp1" , maeCriteBusqu.getAtreOidAtriEsp1().getId().toString());
			}
		PedAtribEspecData atreOidAtriEsp2 = new PedAtribEspecData();
		
						asignarComboResultados(BUSINESSID_ATREOIDATRIESP2_QUERY,ATREOIDATRIESP2_COMBO_CONNECTOR, 
			atreOidAtriEsp2, atreOidAtriEsp2.clone(), "pedAtribEspec", "atreOidAtriEsp2", "result_ROWSET" );
		if(maeCriteBusqu != null && maeCriteBusqu.getAtreOidAtriEsp2() != null){
				checkCombo("atreOidAtriEsp2" , maeCriteBusqu.getAtreOidAtriEsp2().getId().toString());
			}
	...
	* @param maeCriteBusqu La el objetopar la cual se va a realizar la carga
	*/
	protected void loadNonLeafDependenceCombos(MaeCriteBusquData maeCriteBusqu) 
		throws Exception{
	...
	*/
	protected void loadNonLeafDependenceCombosForRequery() throws Exception{
		...
		MaeCriteBusquData maeCriteBusqu = 
			(MaeCriteBusquData) conectorParametroSesion(SESSION_ATTRIBUTE_LAST_QUERY_OBJ_FROM);
		
		//Cargamos todos los objetos completos correspodnientes a los valores de los
		//Campos que sean raiz de una jerarquía de combos dependientes
		if(maeCriteBusqu.getPaisOidPais() != null){
			maeCriteBusqu.setPaisOidPais((es.indra.belcorp.mso.SegPaisViewData)
			getSingleObject(BUSINESSID_PAISOIDPAIS_QUERY, 
			maeCriteBusqu.getPaisOidPais(),
			maeCriteBusqu.getPaisOidPais().clone(),
			"segPaisView"));
		}
		if(maeCriteBusqu.getAtreOidAtriEsp1() != null){
			maeCriteBusqu.setAtreOidAtriEsp1((es.indra.belcorp.mso.PedAtribEspecData)
			getSingleObject(BUSINESSID_ATREOIDATRIESP1_QUERY, 
			maeCriteBusqu.getAtreOidAtriEsp1(),
			maeCriteBusqu.getAtreOidAtriEsp1().clone(),
			"pedAtribEspec"));
		}
		if(maeCriteBusqu.getAtreOidAtriEsp2() != null){
			maeCriteBusqu.setAtreOidAtriEsp2((es.indra.belcorp.mso.PedAtribEspecData)
			getSingleObject(BUSINESSID_ATREOIDATRIESP2_QUERY, 
			maeCriteBusqu.getAtreOidAtriEsp2(),
			maeCriteBusqu.getAtreOidAtriEsp2().clone(),
			"pedAtribEspec"));
		}
		
		//Una vez que tenemos completo los datos de los atributos raices de jerarquías de dependencias
		//cargamos las combos y seleccionamos elementos
		loadNonLeafDependenceCombos(maeCriteBusqu);
		loadLeafDependenceCombos(maeCriteBusqu);
	}
