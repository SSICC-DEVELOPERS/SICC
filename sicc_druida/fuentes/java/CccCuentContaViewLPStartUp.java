/*
    INDRA/CAR/mmg
    $Id: CccCuentContaViewLPStartUp.java,v 1.1 2009/12/03 18:34:25 pecbazalar Exp $
    DESC
*/
import java.util.*;
...
public class CccCuentContaViewLPStartUp extends MMGDruidaBase{
	...
	public static final String BUSINESSID_QUERY = "MMGCccCuentContaViewQueryFromToUserPage";
	...
	public CccCuentContaViewLPStartUp(){
		super();
	}

	public void inicio() throws Exception {
		pagina("CccCuentContaViewPage");
	}
	...
	public void ejecucion() throws Exception {
		try{
			...
			buildJsPageSize("CccCuentContaView");
			...
			asignarAtributo("PAGINA", "CccCuentContaViewPage", "cod", "CccCuentContaView." + accion + ".label");
			...
			limpiarBotoneraLista("cccCuentContaView", accion);
	...
	protected void cmdLov() throws Exception{
		...
		asignarAtributo("TEXT", "codCuenCont", "validacion", "mensaje = ValidaLongitud(cdos_trim(valor), 1, 4);");
		asignarAtributo("TEXT", "Descripcion", "validacion", "mensaje = ValidaLongitud(cdos_trim(valor), 1, 30);");
		asignarAtributo("TEXT", "codCuenSap", "validacion", "mensaje = ValidaLongitud(cdos_trim(valor), 1, 10);");
		
		//Primero determianmos el nombre del campo en la entidad de oriegen donde se van a cargar los datos
		asignar("VAR", "idAttributeLovOpener", conectorParametro(PARAMETRO_LOV_ATTRIBUTO));
		asignar("VAR", "idEntityLovOpener", conectorParametro(PARAMETRO_LOV_ENTIDAD));
		//asignarAtributo("CAPA", "cccCuentContaViewLovLayer", "visibilidad", "visible");
		asignarAtributo("PAGINA", "CccCuentContaViewPage", "onblur", "window.focus();");
	...
	protected void cmdPreView() throws Exception{
		...
		conectorAction("CccCuentContaViewLPStartUp");
	...
	*/
	protected void cmdView() throws Exception{
		...
		CccCuentContaViewData cccCuentContaViewQuery= new CccCuentContaViewData();
		cccCuentContaViewQuery.setId(new Long(conectorParametro("idSelection")));
		
		//Asiganmos el valor a la variable de acción anterior para poder volver hacia atras y hacer requery
		asignar("VAR", VAR_ACCION_ANTERIOR, conectorParametro(VAR_ACCION_ANTERIOR));
		
		//Sacamos de la base de datos la entidad a visualizar
		CccCuentContaViewData datosResult = (CccCuentContaViewData)getSingleObject(BUSINESSID_QUERY, cccCuentContaViewQuery, 
			cccCuentContaViewQuery.clone(), "cccCuentContaView");
		...
		preservaHiddensI18n(new Integer(1), "CCC_CUENT_CONTA", "cccCuentContaViewFrm", "Descripcion", true);
		...
		CccCuentContaViewData cccCuentContaView = datosResult;
		asignar("TEXT", "codCuenCont", FormatUtils.formatObject(cccCuentContaView.getCodCuenCont(), 
			 MMGDruidaHelper.getUserDecimalFormatPattern(this), decimalFormatSymbols));
		asignar("TEXT", "Descripcion", FormatUtils.formatObject(cccCuentContaView.getDescripcion(), 
			 MMGDruidaHelper.getUserDecimalFormatPattern(this), decimalFormatSymbols));
		asignar("TEXT", "codCuenSap", FormatUtils.formatObject(cccCuentContaView.getCodCuenSap(), 
			 MMGDruidaHelper.getUserDecimalFormatPattern(this), decimalFormatSymbols));
		asignar("AREATEXTO", "valObse", FormatUtils.formatObject(cccCuentContaView.getValObse(), 
			 MMGDruidaHelper.getUserDecimalFormatPattern(this), decimalFormatSymbols));
		...
		loadLocalizationVars(cccCuentContaView);
		
			
		//Deshabilitamos todos los campos para que no se editen
		asignarAtributo("TEXT", "codCuenCont", "disabled", "S");
		asignarAtributo("TEXT", "Descripcion", "disabled", "S");
		asignarAtributo("TEXT", "codCuenSap", "disabled", "S");
	...
	protected void cmdCreate() throws Exception{
		//Creamos los campos ocultos para la internacionalización
		preservaHiddensI18n(new Integer(1), "CCC_CUENT_CONTA", "cccCuentContaViewFrm", "Descripcion", false);
		...
		asignarAtributo("TEXT", "codCuenCont", "req", "S");
		asignarAtributo("TEXT", "Descripcion", "req", "S");
		asignarAtributo("TEXT", "codCuenSap", "req", "S");
		
		
		//Asiganamos a los campos internacionalizables la función de onblur para actualizar el 
		//valor del idioma por defecto
		asignarAtributo("TEXT", "Descripcion", "onblur", "setDefaultLanguageValue('Descripcion', get('cccCuentContaViewFrm.Descripcion'))");
	...
	protected void cmdUpdateForm() throws Exception{
		...
		//asignarAtributo("FORMULARIO", "cccCuentContaViewFrm", "oculto", "N");
		...
		traza("MMG::Creando CccCuentContaView");
		String id = conectorParametro("idSelection");
		traza("MMG:: id del atributo a modificar: " + id);
	
		//Construimos el mso con los elementos de la búsqueda
		CccCuentContaViewData cccCuentContaViewQuery= new CccCuentContaViewData();
		cccCuentContaViewQuery.setId(new Long(id));
		...
		//asignar("VAR","timestamp",Long.toString(cccCuentContaView.jdoGetTimeStamp()));
	...
	*/
	protected void cambiarAcciones() throws Exception{
				asignarAtributo("AREATEXTO", "id", "ontab","ejecutarAccionFocoModificacion(\"cccCuentContaViewFrm\",1, true)");
				asignarAtributo("AREATEXTO", "id", "onshtab","ejecutarAccionFocoModificacion(\"cccCuentContaViewFrm\",1, false)");
					asignarAtributo("COMBO","paisOidPais", "ontab", "ejecutarAccionFocoModificacion(\"cccCuentContaViewFrm\",2, true)");
					asignarAtributo("COMBO","paisOidPais", "onshtab", "ejecutarAccionFocoModificacion(\"cccCuentContaViewFrm\",2, false)");
				asignarAtributo("AREATEXTO", "codCuenCont", "ontab","ejecutarAccionFocoModificacion(\"cccCuentContaViewFrm\",3, true)");
				asignarAtributo("AREATEXTO", "codCuenCont", "onshtab","ejecutarAccionFocoModificacion(\"cccCuentContaViewFrm\",3, false)");
				asignarAtributo("AREATEXTO", "Descripcion", "ontab","ejecutarAccionFocoModificacion(\"cccCuentContaViewFrm\",4, true)");
				asignarAtributo("AREATEXTO", "Descripcion", "onshtab","ejecutarAccionFocoModificacion(\"cccCuentContaViewFrm\",4, false)");
				asignarAtributo("AREATEXTO", "codCuenSap", "ontab","ejecutarAccionFocoModificacion(\"cccCuentContaViewFrm\",5, true)");
				asignarAtributo("AREATEXTO", "codCuenSap", "onshtab","ejecutarAccionFocoModificacion(\"cccCuentContaViewFrm\",5, false)");
				asignarAtributo("AREATEXTO", "valObse", "ontab","ejecutarAccionFocoModificacion(\"cccCuentContaViewFrm\",6, true)");
				asignarAtributo("AREATEXTO", "valObse", "onshtab","ejecutarAccionFocoModificacion(\"cccCuentContaViewFrm\",6, false)");
	}
	...
	protected void removeLovElements() throws Exception{
		borrarNodo("TD","nombre","cccCuentContaViewTdLovNullSelectionButton");
	}
	
	protected void removeQueryElements() throws Exception{
		//Borramos los botones para realizar la búsuqeda
		borrarNodo("TR","nombre","cccCuentContaViewTrButtons");
	...
	*/
	protected void establecerSizeCapas(String accion, String origen) throws Exception{
		...
		asignarAtributo("CAPA","cccCuentContaViewFormLayer", "alto", String.valueOf(sizeForm));		
		
		//Estabelcemos la posicion y de la capa correspondiente a la lista y de su botonera
		asignarAtributo("CAPA","cccCuentContaViewListLayer", "y", String.valueOf(sizeForm + 26));		
		asignarAtributo("CAPA","cccCuentContaViewListButtonsLayer", "y", String.valueOf(sizeForm + 278));		
	}

	/**
	* Nos permite cargar las combos finales de la jerarquías de dependencia
	*/
	protected void loadLeafDependenceCombos(CccCuentContaViewData cccCuentContaView) throws Exception{
		...
		if(cccCuentContaView != null && cccCuentContaView.getPaisOidPais() != null){
				checkCombo("paisOidPais" , cccCuentContaView.getPaisOidPais().getId().toString());
			}
	...
	* @param cccCuentContaView La el objetopar la cual se va a realizar la carga
	*/
	protected void loadNonLeafDependenceCombos(CccCuentContaViewData cccCuentContaView) 
		throws Exception{
	...
	*/
	protected void loadNonLeafDependenceCombosForRequery() throws Exception{
		...
		CccCuentContaViewData cccCuentContaView = 
			(CccCuentContaViewData) conectorParametroSesion(SESSION_ATTRIBUTE_LAST_QUERY_OBJ_FROM);
		
		//Cargamos todos los objetos completos correspodnientes a los valores de los
		//Campos que sean raiz de una jerarquía de combos dependientes
		if(cccCuentContaView.getPaisOidPais() != null){
			cccCuentContaView.setPaisOidPais((es.indra.belcorp.mso.SegPaisViewData)
			getSingleObject(BUSINESSID_PAISOIDPAIS_QUERY, 
			cccCuentContaView.getPaisOidPais(),
			cccCuentContaView.getPaisOidPais().clone(),
			"segPaisView"));
		}
		
		//Una vez que tenemos completo los datos de los atributos raices de jerarquías de dependencias
		//cargamos las combos y seleccionamos elementos
		loadNonLeafDependenceCombos(cccCuentContaView);
		loadLeafDependenceCombos(cccCuentContaView);
	}
	...
	*/
	...
	protected void loadLocalizationVars(CccCuentContaViewData cccCuentContaView) throws Exception{
		GenDetaSiccData mmgLocalizationQuery = null;
		Vector mmgLocalizationDatosResult = null;
		mmgLocalizationQuery = new GenDetaSiccData();
		mmgLocalizationQuery.setEnti("CCC_CUENT_CONTA");
		mmgLocalizationQuery.setEntiPk(cccCuentContaView.getId());
