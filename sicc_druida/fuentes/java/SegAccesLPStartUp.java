/*
    INDRA/CAR/mmg
    $Id: SegAccesLPStartUp.java,v 1.1 2009/12/03 18:36:15 pecbazalar Exp $
    DESC
*/
import java.util.*;
...
public class SegAccesLPStartUp extends MMGDruidaBase{
	...
	public static final String BUSINESSID_QUERY = "MMGSegAccesQueryFromToUserPageStructuralSecurity";
				...
				public static final String BUSINESSID_CANAOIDCANA_QUERY ="MMGSegCanalViewQueryFromToUserStructuralSecurity";
	...
	protected static Hashtable indAcceValues = new Hashtable();
	static{
		indAcceValues.put(new java.lang.String("C"), "Call Center");
		indAcceValues.put(new java.lang.String("B"), "Belcenter");
		indAcceValues.put(new java.lang.String("G"), "Gerente");
		indAcceValues.put(new java.lang.String("O"), "Otros");
	}
	
	
	// Definicion del constructor
	public SegAccesLPStartUp(){
		super();
	}
	...
	public void ejecucion() throws Exception {
		try{
			...
			asignarAtributo("PAGINA", "SegAccesPage", "cod", "SegAcces." + accion + ".label");
			...
			limpiarBotoneraLista("segAcces", accion);
	...
	protected void cmdLov() throws Exception{
		...
		asignarAtributo("TEXT", "codAcce", "validacion", "mensaje = ValidaLongitud(cdos_trim(valor), 1, 3);");
		...
		//asignarAtributo("CAPA", "segAccesLovLayer", "visibilidad", "visible");
		asignarAtributo("PAGINA", "SegAccesPage", "onblur", "window.focus();");
	...
	protected void cmdPreView() throws Exception{
		...
		conectorAction("SegAccesLPStartUp");
	...
	*/
	protected void cmdView() throws Exception{
		...
		SegAccesData segAccesQuery= new SegAccesData();
		segAccesQuery.setId(new Long(conectorParametro("idSelection")));
		
		//Asiganmos el valor a la variable de acción anterior para poder volver hacia atras y hacer requery
		asignar("VAR", VAR_ACCION_ANTERIOR, conectorParametro(VAR_ACCION_ANTERIOR));
		
		//Sacamos de la base de datos la entidad a visualizar
		SegAccesData datosResult = (SegAccesData)getSingleObject(BUSINESSID_QUERY, segAccesQuery, 
			segAccesQuery.clone(), "segAcces", false);
		...
		preservaHiddensI18n(new Integer(1), "SEG_ACCES", "segAccesFrm", "Descripcion", true);
		...
		SegAccesData segAcces = datosResult;
		asignar("TEXT", "codAcce", FormatUtils.formatObject(segAcces.getCodAcce(), 
			 MMGDruidaHelper.getUserDecimalFormatPattern(this), decimalFormatSymbols));
		asignar("TEXT", "Descripcion", FormatUtils.formatObject(segAcces.getDescripcion(), 
			 MMGDruidaHelper.getUserDecimalFormatPattern(this), decimalFormatSymbols));
		es.indra.belcorp.mso.SegCanalViewData canaOidCana = segAcces.getCanaOidCana();
		if(canaOidCana != null && canaOidCana.getId() != null) checkCombo("canaOidCana" , canaOidCana.getId().toString());
		if(segAcces.getIndAcce() != null){	
			checkCombo("indAcce", FormatUtils.formatObject(segAcces.getIndAcce(), 
			 MMGDruidaHelper.getUserDecimalFormatPattern(this), decimalFormatSymbols));
		}
		...
		loadLocalizationVars(segAcces);
		
			
		//Deshabilitamos todos los campos para que no se editen
		asignarAtributo("TEXT", "codAcce", "disabled", "S");
		asignarAtributo("TEXT", "Descripcion", "disabled", "S");
		
					asignarAtributo("COMBO", "canaOidCana", "disabled", "S");
					asignarAtributo("COMBO", "indAcce", "disabled", "S");
	...
	protected void cmdCreate() throws Exception{
		//Creamos los campos ocultos para la internacionalización
		preservaHiddensI18n(new Integer(1), "SEG_ACCES", "segAccesFrm", "Descripcion", false);
		...
		asignarAtributo("TEXT", "codAcce", "req", "S");
		asignarAtributo("TEXT", "Descripcion", "req", "S");
		asignarAtributo("COMBO", "canaOidCana", "req", "S");
		asignarAtributo("COMBO", "indAcce", "req", "S");
		
		
		//Asiganamos a los campos internacionalizables la función de onblur para actualizar el 
		//valor del idioma por defecto
		asignarAtributo("TEXT", "Descripcion", "onblur", "setDefaultLanguageValue('Descripcion', get('segAccesFrm.Descripcion'))");
	...
	protected void cmdUpdateForm() throws Exception{
		...
		//asignarAtributo("FORMULARIO", "segAccesFrm", "oculto", "N");
		...
		traza("MMG::Creando SegAcces");
		String id = conectorParametro("idSelection");
		traza("MMG:: id del atributo a modificar: " + id);
	
		//Construimos el mso con los elementos de la búsqueda
		SegAccesData segAccesQuery= new SegAccesData();
		segAccesQuery.setId(new Long(id));
		...
		//asignar("VAR","timestamp",Long.toString(segAcces.jdoGetTimeStamp()));
	...
	*/
	protected void cambiarAcciones() throws Exception{
				asignarAtributo("AREATEXTO", "id", "ontab","ejecutarAccionFocoModificacion(\"segAccesFrm\",1, true)");
				asignarAtributo("AREATEXTO", "id", "onshtab","ejecutarAccionFocoModificacion(\"segAccesFrm\",1, false)");
				asignarAtributo("AREATEXTO", "codAcce", "ontab","ejecutarAccionFocoModificacion(\"segAccesFrm\",2, true)");
				asignarAtributo("AREATEXTO", "codAcce", "onshtab","ejecutarAccionFocoModificacion(\"segAccesFrm\",2, false)");
				asignarAtributo("AREATEXTO", "Descripcion", "ontab","ejecutarAccionFocoModificacion(\"segAccesFrm\",3, true)");
				asignarAtributo("AREATEXTO", "Descripcion", "onshtab","ejecutarAccionFocoModificacion(\"segAccesFrm\",3, false)");
					asignarAtributo("COMBO","canaOidCana", "ontab", "ejecutarAccionFocoModificacion(\"segAccesFrm\",4, true)");
					asignarAtributo("COMBO","canaOidCana", "onshtab", "ejecutarAccionFocoModificacion(\"segAccesFrm\",4, false)");
				asignarAtributo("TEXT", "indAcce", "ontab","ejecutarAccionFocoModificacion(\"segAccesFrm\",5, true)");
				asignarAtributo("TEXT", "indAcce", "onshtab","ejecutarAccionFocoModificacion(\"segAccesFrm\",5, false)");
	}
	...
	protected void removeLovElements() throws Exception{
		borrarNodo("TD","nombre","segAccesTdLovNullSelectionButton");
	}
	
	protected void removeQueryElements() throws Exception{
		//Borramos los botones para realizar la búsuqeda
		borrarNodo("TR","nombre","segAccesTrButtons");
	...
	*/
	protected void desactivateNonModifyFields() throws Exception{
			traza("MMG: en desactivateNonModifyFields ");		
				...
				traza("MMG: el atributo es codAcce...");		
				...
				traza("MMG: el atributo es canaOidCana...");		
					...
					traza("MMG: en desactivateNonModifyFields encontrado atributo no editable...");
						...
						asignarAtributo("COMBO", "canaOidCana", "disabled", "S");
				traza("MMG: el atributo es indAcce...");		
	...
	*/
	protected void establecerSizeCapas(String accion, String origen) throws Exception{
		...
		asignarAtributo("CAPA","segAccesFormLayer", "alto", String.valueOf(sizeForm));		
		
		//Estabelcemos la posicion y de la capa correspondiente a la lista y de su botonera
		asignarAtributo("CAPA","segAccesListLayer", "y", String.valueOf(sizeForm + 26));		
		asignarAtributo("CAPA","segAccesListButtonsLayer", "y", String.valueOf(sizeForm + 278));		
	}

	/**
	* Nos permite cargar las combos finales de la jerarquías de dependencia
	*/
	protected void loadLeafDependenceCombos(SegAccesData segAcces) throws Exception{
		...
		SegCanalViewData canaOidCana = new SegCanalViewData();
			...
			canaOidCana, canaOidCana.clone(), "segCanalView", "canaOidCana", "result_ROWSET",false );
		if(segAcces != null && segAcces.getCanaOidCana() != null){
				checkCombo("canaOidCana" , segAcces.getCanaOidCana().getId().toString());
			}
	...
	* @param segAcces La el objetopar la cual se va a realizar la carga
	*/
	protected void loadNonLeafDependenceCombos(SegAccesData segAcces) 
		throws Exception{
	...
	*/
	protected void loadNonLeafDependenceCombosForRequery() throws Exception{
		...
		SegAccesData segAcces = 
			(SegAccesData) conectorParametroSesion(SESSION_ATTRIBUTE_LAST_QUERY_OBJ_FROM);
		
		//Cargamos todos los objetos completos correspodnientes a los valores de los
		//Campos que sean raiz de una jerarquía de combos dependientes
		if(segAcces.getCanaOidCana() != null){
			segAcces.setCanaOidCana((es.indra.belcorp.mso.SegCanalViewData)
			getSingleObject(BUSINESSID_CANAOIDCANA_QUERY, 
			segAcces.getCanaOidCana(),
			segAcces.getCanaOidCana().clone(),
			"segCanalView"));
		}
		
		//Una vez que tenemos completo los datos de los atributos raices de jerarquías de dependencias
		//cargamos las combos y seleccionamos elementos
		loadNonLeafDependenceCombos(segAcces);
		loadLeafDependenceCombos(segAcces);
	}
	...
	*/
	...
	protected void loadLocalizationVars(SegAccesData segAcces) throws Exception{
		GenDetaSiccData mmgLocalizationQuery = null;
		Vector mmgLocalizationDatosResult = null;
		mmgLocalizationQuery = new GenDetaSiccData();
		mmgLocalizationQuery.setEnti("SEG_ACCES");
		mmgLocalizationQuery.setEntiPk(segAcces.getId());
