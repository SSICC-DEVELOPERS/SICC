/*
    INDRA/CAR/mmg
    $Id: FacAsignImpreLPStartUp.java,v 1.1 2009/12/03 18:39:22 pecbazalar Exp $
    DESC
*/
import java.util.*;
...
public class FacAsignImpreLPStartUp extends MMGDruidaBase{
	...
	public static final String BUSINESSID_QUERY = "MMGFacAsignImpreQueryFromToUserPage";
				...
				public static final String BUSINESSID_IMVIOIDIMPR_QUERY ="MMGFacImpreVirtuViewQueryFromToUser";
				public static final String IMVIOIDIMPR_COMBO_CONNECTOR = "FacImpreVirtuViewConectorCombo";
				...
				public static final String BUSINESSID_FORSOIDFORM_QUERY ="MMGFacFormuQueryFromToUser";
				public static final String FORSOIDFORM_COMBO_CONNECTOR = "FacFormuConectorCombo";
	...
	public void ejecucion() throws Exception {
		try{
			...
			asignarAtributo("PAGINA", "FacAsignImprePage", "cod", "FacAsignImpre." + accion + ".label");
			...
			limpiarBotoneraLista("facAsignImpre", accion);
	...
	protected void cmdLov() throws Exception{
		...
		//asignarAtributo("CAPA", "facAsignImpreLovLayer", "visibilidad", "visible");
		asignarAtributo("PAGINA", "FacAsignImprePage", "onblur", "window.focus();");
	...
	*/
	protected void cmdView() throws Exception{
		...
		FacAsignImpreData facAsignImpreQuery= new FacAsignImpreData();
		facAsignImpreQuery.setId(new Long(conectorParametro("idSelection")));
		
		//Asiganmos el valor a la variable de acción anterior para poder volver hacia atras y hacer requery
		asignar("VAR", VAR_ACCION_ANTERIOR, conectorParametro(VAR_ACCION_ANTERIOR));
		
		//Sacamos de la base de datos la entidad a visualizar
		FacAsignImpreData datosResult = (FacAsignImpreData)getSingleObject(BUSINESSID_QUERY, facAsignImpreQuery, 
			facAsignImpreQuery.clone(), "facAsignImpre");
		...
		FacAsignImpreData facAsignImpre = datosResult;
		es.indra.belcorp.mso.FacImpreVirtuViewData imviOidImpr = facAsignImpre.getImviOidImpr();
		if(imviOidImpr != null && imviOidImpr.getId() != null) checkCombo("imviOidImpr" , imviOidImpr.getId().toString());
		es.indra.belcorp.mso.SegSubacData sbacOidSbac = facAsignImpre.getSbacOidSbac();
		if(sbacOidSbac != null && sbacOidSbac.getId() != null) checkCombo("sbacOidSbac" , sbacOidSbac.getId().toString());
		es.indra.belcorp.mso.FacFormuData forsOidForm = facAsignImpre.getForsOidForm();
		if(forsOidForm != null && forsOidForm.getId() != null) checkCombo("forsOidForm" , forsOidForm.getId().toString());
		...
		//Deshabilitamos todos los campos para que no se editen
					...
					asignarAtributo("COMBO", "imviOidImpr", "disabled", "S");
					...
					asignarAtributo("COMBO", "forsOidForm", "disabled", "S");
	...
	protected void cmdCreate() throws Exception{
		...
		asignarAtributo("COMBO", "imviOidImpr", "req", "S");
		asignarAtributo("COMBO", "sbacOidSbac", "req", "S");
		asignarAtributo("COMBO", "forsOidForm", "req", "S");
	...
	protected void cmdUpdateForm() throws Exception{
		...
		//asignarAtributo("FORMULARIO", "facAsignImpreFrm", "oculto", "N");
		...
		//Construimos el mso con los elementos de la búsqueda
		FacAsignImpreData facAsignImpreQuery= new FacAsignImpreData();
		facAsignImpreQuery.setId(new Long(id));
		...
		//asignar("VAR","timestamp",Long.toString(facAsignImpre.jdoGetTimeStamp()));
	...
	*/
	protected void cambiarAcciones() throws Exception{
				asignarAtributo("AREATEXTO", "id", "ontab","ejecutarAccionFocoModificacion(\"facAsignImpreFrm\",1, true)");
				asignarAtributo("AREATEXTO", "id", "onshtab","ejecutarAccionFocoModificacion(\"facAsignImpreFrm\",1, false)");
					asignarAtributo("COMBO","imviOidImpr", "ontab", "ejecutarAccionFocoModificacion(\"facAsignImpreFrm\",2, true)");
					asignarAtributo("COMBO","imviOidImpr", "onshtab", "ejecutarAccionFocoModificacion(\"facAsignImpreFrm\",2, false)");
					asignarAtributo("COMBO","sbacOidSbac_acceOidAcce_canaOidCana", "ontab", "ejecutarAccionFocoModificacion(\"facAsignImpreFrm\",3, true)");
					asignarAtributo("COMBO","sbacOidSbac_acceOidAcce_canaOidCana", "onshtab", "ejecutarAccionFocoModificacion(\"facAsignImpreFrm\",3, false)");
					asignarAtributo("COMBO","sbacOidSbac_acceOidAcce", "ontab", "ejecutarAccionFocoModificacion(\"facAsignImpreFrm\",3, true)");
					asignarAtributo("COMBO","sbacOidSbac_acceOidAcce", "onshtab", "ejecutarAccionFocoModificacion(\"facAsignImpreFrm\",3, false)");
					asignarAtributo("COMBO","sbacOidSbac", "ontab", "ejecutarAccionFocoModificacion(\"facAsignImpreFrm\",3, true)");
					asignarAtributo("COMBO","sbacOidSbac", "onshtab", "ejecutarAccionFocoModificacion(\"facAsignImpreFrm\",3, false)");
					asignarAtributo("COMBO","forsOidForm", "ontab", "ejecutarAccionFocoModificacion(\"facAsignImpreFrm\",4, true)");
					asignarAtributo("COMBO","forsOidForm", "onshtab", "ejecutarAccionFocoModificacion(\"facAsignImpreFrm\",4, false)");
	}
	...
	protected void removeLovElements() throws Exception{
		borrarNodo("TD","nombre","facAsignImpreTdLovNullSelectionButton");
	}
	
	protected void removeQueryElements() throws Exception{
		//Borramos los botones para realizar la búsuqeda
		borrarNodo("TR","nombre","facAsignImpreTrButtons");
	...
	*/
	protected void desactivateNonModifyFields() throws Exception{
			traza("MMG: en desactivateNonModifyFields ");		
				...
				traza("MMG: el atributo es imviOidImpr...");		
				
				traza("MMG: el atributo es sbacOidSbac...");		
				
				traza("MMG: el atributo es forsOidForm...");		
	...
	*/
	protected void establecerSizeCapas(String accion, String origen) throws Exception{
		...
		asignarAtributo("CAPA","facAsignImpreFormLayer", "alto", String.valueOf(sizeForm));		
		
		//Estabelcemos la posicion y de la capa correspondiente a la lista y de su botonera
		asignarAtributo("CAPA","facAsignImpreListLayer", "y", String.valueOf(sizeForm + 26));		
		asignarAtributo("CAPA","facAsignImpreListButtonsLayer", "y", String.valueOf(sizeForm + 278));		
	}

	/**
	* Nos permite cargar las combos finales de la jerarquías de dependencia
	*/
	protected void loadLeafDependenceCombos(FacAsignImpreData facAsignImpre) throws Exception{
		HashMap checkedCombos = new HashMap();
		FacImpreVirtuViewData imviOidImpr = new FacImpreVirtuViewData();
		
		asignarComboResultados(BUSINESSID_IMVIOIDIMPR_QUERY,IMVIOIDIMPR_COMBO_CONNECTOR, 
			imviOidImpr, imviOidImpr.clone(), "facImpreVirtuView", "imviOidImpr", "result_ROWSET" );
        
		if(facAsignImpre != null && facAsignImpre.getImviOidImpr() != null){
			checkCombo("imviOidImpr" , facAsignImpre.getImviOidImpr().getId().toString());
		}
        ...
        /*
         * CONST. - SPLATAS - 12/01/06
         * if(facAsignImpre != null && facAsignImpre.getSbacOidSbac().getAcceOidAcce().getCanaOidCana() != null){
         */
		if(facAsignImpre != null && facAsignImpre.getSbacOidSbac() != null
                && facAsignImpre.getSbacOidSbac().getAcceOidAcce() != null
                && facAsignImpre.getSbacOidSbac().getAcceOidAcce().getCanaOidCana() != null){
			checkCombo("sbacOidSbac_acceOidAcce_canaOidCana" , 
                    facAsignImpre.getSbacOidSbac().getAcceOidAcce().getCanaOidCana().getId().toString());
		}
		...
		if(sbacOidSbac_acceOidAcce_canaOidCanaDefaultValue != null 
			...
			checkedCombos.put("sbacOidSbac_acceOidAcce_canaOidCana",
                    sbacOidSbac_acceOidAcce_canaOidCanaDefaultValue);
			cascadeComboLoad(checkedCombos, "sbacOidSbac_acceOidAcce_canaOidCana");
		}			
        
		FacFormuData forsOidForm = new FacFormuData();
		
		asignarComboResultados(BUSINESSID_FORSOIDFORM_QUERY,FORSOIDFORM_COMBO_CONNECTOR, 
			forsOidForm, forsOidForm.clone(), "facFormu", "forsOidForm", "result_ROWSET" );
		
        if(facAsignImpre != null && facAsignImpre.getForsOidForm() != null){
			checkCombo("forsOidForm" , facAsignImpre.getForsOidForm().getId().toString());
		}
	...
	* @param facAsignImpre La el objetopar la cual se va a realizar la carga
	*/
	protected void loadNonLeafDependenceCombos(FacAsignImpreData facAsignImpre) 
		throws Exception{
		
						if(facAsignImpre.getSbacOidSbac() != null){
			
						SegSubacData sbacOidSbac = new SegSubacData();
			sbacOidSbac.setAcceOidAcce(facAsignImpre.getSbacOidSbac().getAcceOidAcce());
			...
			checkCombo("sbacOidSbac" , facAsignImpre.getSbacOidSbac().getId().toString());
			
						SegAccesData sbacOidSbac_acceOidAcce = new SegAccesData();
			sbacOidSbac_acceOidAcce.setCanaOidCana(facAsignImpre.getSbacOidSbac().getAcceOidAcce().getCanaOidCana());
			...
			checkCombo("sbacOidSbac_acceOidAcce" , facAsignImpre.getSbacOidSbac().getAcceOidAcce().getId().toString());
	...
	*/
	protected void loadNonLeafDependenceCombosForRequery() throws Exception{
		...
		FacAsignImpreData facAsignImpre = 
			(FacAsignImpreData) conectorParametroSesion(SESSION_ATTRIBUTE_LAST_QUERY_OBJ_FROM);
		
		//Cargamos todos los objetos completos correspodnientes a los valores de los
		//Campos que sean raiz de una jerarquía de combos dependientes
		if(facAsignImpre.getImviOidImpr() != null){
			facAsignImpre.setImviOidImpr((es.indra.belcorp.mso.FacImpreVirtuViewData)
			getSingleObject(BUSINESSID_IMVIOIDIMPR_QUERY, 
			facAsignImpre.getImviOidImpr(),
			facAsignImpre.getImviOidImpr().clone(),
			"facImpreVirtuView"));
		}
		if(facAsignImpre.getSbacOidSbac() != null){
			facAsignImpre.setSbacOidSbac((es.indra.belcorp.mso.SegSubacData)
			getSingleObject(BUSINESSID_SBACOIDSBAC_QUERY, 
			facAsignImpre.getSbacOidSbac(),
			facAsignImpre.getSbacOidSbac().clone(),
			"segSubac"));
		}
		if(facAsignImpre.getForsOidForm() != null){
			facAsignImpre.setForsOidForm((es.indra.belcorp.mso.FacFormuData)
			getSingleObject(BUSINESSID_FORSOIDFORM_QUERY, 
			facAsignImpre.getForsOidForm(),
			facAsignImpre.getForsOidForm().clone(),
			"facFormu"));
		}
		
		//Una vez que tenemos completo los datos de los atributos raices de jerarquías de dependencias
		//cargamos las combos y seleccionamos elementos
		loadNonLeafDependenceCombos(facAsignImpre);
		loadLeafDependenceCombos(facAsignImpre);
	}
