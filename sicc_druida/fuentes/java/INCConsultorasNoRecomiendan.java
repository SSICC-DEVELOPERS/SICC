...
public class INCConsultorasNoRecomiendan extends LPSICCBase {
   
  public INCConsultorasNoRecomiendan() {
    super();
  }
  ...
  public void ejecucion() throws Exception {		
    ...
    try {
      ...
      if (accion.equals("generar")){
        ...
        cargarPagina();
        getConfiguracionMenu("INCConsultorasNoRecomiendan");
      }
      
      } catch (Exception ex) {
     			
        //pagina("contenido_informe_consultoras_sin_concurso_generar");
        //getConfiguracionMenu("INCConsultorasNoRecomiendan");
  ...
  private void cargarPagina() throws Exception{
	...
	//Carga Incumplimiento
    DTOBelcorp dtoIncu = new DTOBelcorp();
     
    dtoIncu.setOidIdioma(UtilidadesSession.getIdioma(this));
     
    Vector vecIncu = new Vector();
       
    MareBusinessID idIncumplimiento = new MareBusinessID("REPObtenerIncumplimientos");
          
    vecIncu.add(dtoIncu);
	vecIncu.add(idIncumplimiento);
        
    DTOSalida dtoSalIncu = (DTOSalida)conectar("ConectorGenericos",vecIncu).objeto("dtoSalida");          
    RecordSet incu = (RecordSet)dtoSalIncu.getResultado();
    Vector colINCU = incu.getColumnIdentifiers();
    DruidaConector incus = generarConector("DTOSalida.incu_ROWSET",incu, colINCU);
    asignar("COMBO", "cbMotivosIncumplimiento", incus, "DTOSalida.incu_ROWSET");
    //	------
  ...
  private void generarReporte() throws Exception{
    ...
    String motivosIncumpliento = conectorParametroLimpia("motivosIncumplimiento","",true);
    if(!motivosIncumpliento.equals("")){
      datos.put(ConstantesREP.MOTIVOSINCUMPLIMIENTOS,motivosIncumpliento);
    }
    ...
    datos.put(ConstantesREP.TITULO, "Consultoras que no cumplieron con Concurso Recomendacion");
    ...
    if(region.equals("")){
       reporte = "RD_REP-062_02";    
    }else{
       if(zona.equals("")){
          reporte = "RD_REP-062_01";
       }else{
          reporte = "RD_REP-062";
       }
    }   
