/*
    INDRA/CAR/mmg
    $Id: CobUsuarEtapaCobraCabecLPStartUp.java,v 1.1 2009/12/03 18:40:11 pecbazalar Exp $
    DESC
*/
import java.util.*;
...
public class CobUsuarEtapaCobraCabecLPStartUp extends MMGDruidaBase{
	...
	public static final String BUSINESSID_QUERY = "MMGCobUsuarEtapaCobraCabecQueryFromToUserPage";
				...
				public static final String BUSINESSID_USCOOIDUSUACOBR_QUERY ="MMGCobUsuarCobraViewQueryFromToUser";
				public static final String USCOOIDUSUACOBR_COMBO_CONNECTOR = "CobUsuarCobraViewConectorCombo";
	...
	public CobUsuarEtapaCobraCabecLPStartUp(){
		super();
	}

	public void inicio() throws Exception {
		pagina("CobUsuarEtapaCobraCabecPage");
	}
	...
	public void ejecucion() throws Exception {
		try{
			...
			buildJsPageSize("CobUsuarEtapaCobraCabec");
			...
			asignarAtributo("PAGINA", "CobUsuarEtapaCobraCabecPage", "cod", "CobUsuarEtapaCobraCabec." + accion + ".label");
			...
			limpiarBotoneraLista("cobUsuarEtapaCobraCabec", accion);
	...
	protected void cmdLov() throws Exception{
		...
		//asignarAtributo("CAPA", "cobUsuarEtapaCobraCabecLovLayer", "visibilidad", "visible");
		asignarAtributo("PAGINA", "CobUsuarEtapaCobraCabecPage", "onblur", "window.focus();");
	...
	*/
	protected void cmdView() throws Exception{
		...
		CobUsuarEtapaCobraCabecData cobUsuarEtapaCobraCabecQuery= new CobUsuarEtapaCobraCabecData();
		cobUsuarEtapaCobraCabecQuery.setId(new Long(conectorParametro("idSelection")));
		
		//Asiganmos el valor a la variable de acción anterior para poder volver hacia atras y hacer requery
		asignar("VAR", VAR_ACCION_ANTERIOR, conectorParametro(VAR_ACCION_ANTERIOR));
		
		//Sacamos de la base de datos la entidad a visualizar
		CobUsuarEtapaCobraCabecData datosResult = (CobUsuarEtapaCobraCabecData)getSingleObject(BUSINESSID_QUERY, cobUsuarEtapaCobraCabecQuery, 
			cobUsuarEtapaCobraCabecQuery.clone(), "cobUsuarEtapaCobraCabec");
		...
		CobUsuarEtapaCobraCabecData cobUsuarEtapaCobraCabec = datosResult;
		es.indra.belcorp.mso.CobUsuarCobraViewData uscoOidUsuaCobr = cobUsuarEtapaCobraCabec.getUscoOidUsuaCobr();
		if(uscoOidUsuaCobr != null && uscoOidUsuaCobr.getId() != null) checkCombo("uscoOidUsuaCobr" , uscoOidUsuaCobr.getId().toString());
		es.indra.belcorp.mso.CobEtapaDeudaData etdeOidEtapDeud = cobUsuarEtapaCobraCabec.getEtdeOidEtapDeud();
		if(etdeOidEtapDeud != null && etdeOidEtapDeud.getId() != null) checkCombo("etdeOidEtapDeud" , etdeOidEtapDeud.getId().toString());
		asignar("TEXT", "valObse", FormatUtils.formatObject(cobUsuarEtapaCobraCabec.getValObse(), 
		...
		//Deshabilitamos todos los campos para que no se editen
					...
					asignarAtributo("COMBO", "uscoOidUsuaCobr", "disabled", "S");
					asignarAtributo("COMBO", "etdeOidEtapDeud", "disabled", "S");asignarAtributo("TEXT", "valObse", "disabled", "S");
	...
	protected void cmdCreate() throws Exception{
		...
		asignarAtributo("COMBO", "uscoOidUsuaCobr", "req", "S");
	...
	protected void cmdUpdateForm() throws Exception{
		...
		//asignarAtributo("FORMULARIO", "cobUsuarEtapaCobraCabecFrm", "oculto", "N");
		...
		traza("MMG::Creando CobUsuarEtapaCobraCabec");
		String id = conectorParametro("idSelection");
		traza("MMG:: id del atributo a modificar: " + id);
	
		//Construimos el mso con los elementos de la búsqueda
		CobUsuarEtapaCobraCabecData cobUsuarEtapaCobraCabecQuery= new CobUsuarEtapaCobraCabecData();
		cobUsuarEtapaCobraCabecQuery.setId(new Long(id));
		...
		//asignar("VAR","timestamp",Long.toString(cobUsuarEtapaCobraCabec.jdoGetTimeStamp()));
	...
	*/
	protected void cambiarAcciones() throws Exception{
				asignarAtributo("AREATEXTO", "id", "ontab","ejecutarAccionFocoModificacion(\"cobUsuarEtapaCobraCabecFrm\",1, true)");
				asignarAtributo("AREATEXTO", "id", "onshtab","ejecutarAccionFocoModificacion(\"cobUsuarEtapaCobraCabecFrm\",1, false)");
					asignarAtributo("COMBO","uscoOidUsuaCobr", "ontab", "ejecutarAccionFocoModificacion(\"cobUsuarEtapaCobraCabecFrm\",2, true)");
					asignarAtributo("COMBO","uscoOidUsuaCobr", "onshtab", "ejecutarAccionFocoModificacion(\"cobUsuarEtapaCobraCabecFrm\",2, false)");
					asignarAtributo("COMBO","etdeOidEtapDeud", "ontab", "ejecutarAccionFocoModificacion(\"cobUsuarEtapaCobraCabecFrm\",3, true)");
					asignarAtributo("COMBO","etdeOidEtapDeud", "onshtab", "ejecutarAccionFocoModificacion(\"cobUsuarEtapaCobraCabecFrm\",3, false)");
				asignarAtributo("AREATEXTO", "valObse", "ontab","ejecutarAccionFocoModificacion(\"cobUsuarEtapaCobraCabecFrm\",4, true)");
				asignarAtributo("AREATEXTO", "valObse", "onshtab","ejecutarAccionFocoModificacion(\"cobUsuarEtapaCobraCabecFrm\",4, false)");
	}
	...
	protected void removeLovElements() throws Exception{
		borrarNodo("TD","nombre","cobUsuarEtapaCobraCabecTdLovNullSelectionButton");
	}
	
	protected void removeQueryElements() throws Exception{
		//Borramos los botones para realizar la búsuqeda
		borrarNodo("TR","nombre","cobUsuarEtapaCobraCabecTrButtons");
	...
	*/
	protected void desactivateNonModifyFields() throws Exception{
			traza("MMG: en desactivateNonModifyFields ");		
				...
				traza("MMG: el atributo es uscoOidUsuaCobr...");		
	...
	*/
	protected void establecerSizeCapas(String accion, String origen) throws Exception{
		...
		asignarAtributo("CAPA","cobUsuarEtapaCobraCabecFormLayer", "alto", String.valueOf(sizeForm));		
		
		//Estabelcemos la posicion y de la capa correspondiente a la lista y de su botonera
		asignarAtributo("CAPA","cobUsuarEtapaCobraCabecListLayer", "y", String.valueOf(sizeForm + 26));		
		asignarAtributo("CAPA","cobUsuarEtapaCobraCabecListButtonsLayer", "y", String.valueOf(sizeForm + 278));		
	}

	/**
	* Nos permite cargar las combos finales de la jerarquías de dependencia
	*/
	protected void loadLeafDependenceCombos(CobUsuarEtapaCobraCabecData cobUsuarEtapaCobraCabec) throws Exception{
		HashMap checkedCombos = new HashMap();
		CobUsuarCobraViewData uscoOidUsuaCobr = new CobUsuarCobraViewData();
		
						asignarComboResultados(BUSINESSID_USCOOIDUSUACOBR_QUERY,USCOOIDUSUACOBR_COMBO_CONNECTOR, 
			uscoOidUsuaCobr, uscoOidUsuaCobr.clone(), "cobUsuarCobraView", "uscoOidUsuaCobr", "result_ROWSET" );
		if(cobUsuarEtapaCobraCabec != null && cobUsuarEtapaCobraCabec.getUscoOidUsuaCobr() != null){
				checkCombo("uscoOidUsuaCobr" , cobUsuarEtapaCobraCabec.getUscoOidUsuaCobr().getId().toString());
			}
		...
		if(cobUsuarEtapaCobraCabec != null && cobUsuarEtapaCobraCabec.getEtdeOidEtapDeud() != null){
				checkCombo("etdeOidEtapDeud" , cobUsuarEtapaCobraCabec.getEtdeOidEtapDeud().getId().toString());
			}
	...
	* @param cobUsuarEtapaCobraCabec La el objetopar la cual se va a realizar la carga
	*/
	protected void loadNonLeafDependenceCombos(CobUsuarEtapaCobraCabecData cobUsuarEtapaCobraCabec) 
		throws Exception{
	...
	*/
	protected void loadNonLeafDependenceCombosForRequery() throws Exception{
		...
		CobUsuarEtapaCobraCabecData cobUsuarEtapaCobraCabec = 
			(CobUsuarEtapaCobraCabecData) conectorParametroSesion(SESSION_ATTRIBUTE_LAST_QUERY_OBJ_FROM);
		
		//Cargamos todos los objetos completos correspodnientes a los valores de los
		//Campos que sean raiz de una jerarquía de combos dependientes
		if(cobUsuarEtapaCobraCabec.getUscoOidUsuaCobr() != null){
			cobUsuarEtapaCobraCabec.setUscoOidUsuaCobr((es.indra.belcorp.mso.CobUsuarCobraViewData)
			getSingleObject(BUSINESSID_USCOOIDUSUACOBR_QUERY, 
			cobUsuarEtapaCobraCabec.getUscoOidUsuaCobr(),
			cobUsuarEtapaCobraCabec.getUscoOidUsuaCobr().clone(),
			"cobUsuarCobraView"));
		}
		if(cobUsuarEtapaCobraCabec.getEtdeOidEtapDeud() != null){
			cobUsuarEtapaCobraCabec.setEtdeOidEtapDeud((es.indra.belcorp.mso.CobEtapaDeudaData)
			getSingleObject(BUSINESSID_ETDEOIDETAPDEUD_QUERY, 
			cobUsuarEtapaCobraCabec.getEtdeOidEtapDeud(),
			cobUsuarEtapaCobraCabec.getEtdeOidEtapDeud().clone(),
			"cobEtapaDeuda"));
		}
		
		//Una vez que tenemos completo los datos de los atributos raices de jerarquías de dependencias
		//cargamos las combos y seleccionamos elementos
		loadNonLeafDependenceCombos(cobUsuarEtapaCobraCabec);
		loadLeafDependenceCombos(cobUsuarEtapaCobraCabec);
	}
