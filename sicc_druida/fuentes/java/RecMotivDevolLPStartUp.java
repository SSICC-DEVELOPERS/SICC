/*
    INDRA/CAR/mmg
    $Id: RecMotivDevolLPStartUp.java,v 1.1 2009/12/03 18:41:59 pecbazalar Exp $
    DESC
*/
import java.util.*;
...
public class RecMotivDevolLPStartUp extends MMGDruidaBase{
	...
	public void ejecucion() throws Exception {
		try{
			...
			asignarAtributo("PAGINA", "RecMotivDevolPage", "cod", "RecMotivDevol." + accion + ".label");
			...
			limpiarBotoneraLista("recMotivDevol", accion);
	...
	protected void cmdLov() throws Exception{
		...
		asignarAtributo("TEXT", "codMotiDevo", "validacion", "mensaje = ValidaLongitud(cdos_trim(valor), 1, 2);");
		...
		//asignarAtributo("CAPA", "recMotivDevolLovLayer", "visibilidad", "visible");
		asignarAtributo("PAGINA", "RecMotivDevolPage", "onblur", "window.focus();");
	...
	*/
	protected void cmdView() throws Exception{
		...
		RecMotivDevolData recMotivDevolQuery= new RecMotivDevolData();
		recMotivDevolQuery.setId(new Long(conectorParametro("idSelection")));
		
		//Asiganmos el valor a la variable de acción anterior para poder volver hacia atras y hacer requery
		asignar("VAR", VAR_ACCION_ANTERIOR, conectorParametro(VAR_ACCION_ANTERIOR));
		
		//Sacamos de la base de datos la entidad a visualizar
		RecMotivDevolData datosResult = (RecMotivDevolData)getSingleObject(BUSINESSID_QUERY, recMotivDevolQuery, 
			recMotivDevolQuery.clone(), "recMotivDevol");
		...
		preservaHiddensI18n(new Integer(1), "REC_MOTIV_DEVOL", "recMotivDevolFrm", "Descripcion", true);
		...
		RecMotivDevolData recMotivDevol = datosResult;
		asignar("TEXT", "codMotiDevo", FormatUtils.formatObject(recMotivDevol.getCodMotiDevo(), 
			 MMGDruidaHelper.getUserDecimalFormatPattern(this), decimalFormatSymbols));
		asignar("TEXT", "Descripcion", FormatUtils.formatObject(recMotivDevol.getDescripcion(), 
			 MMGDruidaHelper.getUserDecimalFormatPattern(this), decimalFormatSymbols));
		...
		loadLocalizationVars(recMotivDevol);
		
			
		//Deshabilitamos todos los campos para que no se editen
		asignarAtributo("TEXT", "codMotiDevo", "disabled", "S");
	...
	protected void cmdCreate() throws Exception{
		//Creamos los campos ocultos para la internacionalización
		preservaHiddensI18n(new Integer(1), "REC_MOTIV_DEVOL", "recMotivDevolFrm", "Descripcion", false);
		...
		asignarAtributo("TEXT", "codMotiDevo", "req", "S");
		...
		asignarAtributo("TEXT", "Descripcion", "onblur", "setDefaultLanguageValue('Descripcion', get('recMotivDevolFrm.Descripcion'))");
	...
	protected void cmdUpdateForm() throws Exception{
		...
		//asignarAtributo("FORMULARIO", "recMotivDevolFrm", "oculto", "N");
		...
		//Construimos el mso con los elementos de la búsqueda
		RecMotivDevolData recMotivDevolQuery= new RecMotivDevolData();
		recMotivDevolQuery.setId(new Long(id));
		...
		//asignar("VAR","timestamp",Long.toString(recMotivDevol.jdoGetTimeStamp()));
	...
	*/
	protected void cambiarAcciones() throws Exception{
				asignarAtributo("AREATEXTO", "id", "ontab","ejecutarAccionFocoModificacion(\"recMotivDevolFrm\",1, true)");
				asignarAtributo("AREATEXTO", "id", "onshtab","ejecutarAccionFocoModificacion(\"recMotivDevolFrm\",1, false)");
				asignarAtributo("AREATEXTO", "codMotiDevo", "ontab","ejecutarAccionFocoModificacion(\"recMotivDevolFrm\",2, true)");
				asignarAtributo("AREATEXTO", "codMotiDevo", "onshtab","ejecutarAccionFocoModificacion(\"recMotivDevolFrm\",2, false)");
					asignarAtributo("COMBO","paisOidPais", "ontab", "ejecutarAccionFocoModificacion(\"recMotivDevolFrm\",3, true)");
					asignarAtributo("COMBO","paisOidPais", "onshtab", "ejecutarAccionFocoModificacion(\"recMotivDevolFrm\",3, false)");
				asignarAtributo("AREATEXTO", "Descripcion", "ontab","ejecutarAccionFocoModificacion(\"recMotivDevolFrm\",4, true)");
				asignarAtributo("AREATEXTO", "Descripcion", "onshtab","ejecutarAccionFocoModificacion(\"recMotivDevolFrm\",4, false)");
	}
	...
	protected void removeLovElements() throws Exception{
		borrarNodo("TD","nombre","recMotivDevolTdLovNullSelectionButton");
	}
	
	protected void removeQueryElements() throws Exception{
		//Borramos los botones para realizar la búsuqeda
		borrarNodo("TR","nombre","recMotivDevolTrButtons");
	...
	*/
	protected void establecerSizeCapas(String accion, String origen) throws Exception{
		...
		asignarAtributo("CAPA","recMotivDevolFormLayer", "alto", String.valueOf(sizeForm));		
		
		//Estabelcemos la posicion y de la capa correspondiente a la lista y de su botonera
		asignarAtributo("CAPA","recMotivDevolListLayer", "y", String.valueOf(sizeForm + 26));		
		asignarAtributo("CAPA","recMotivDevolListButtonsLayer", "y", String.valueOf(sizeForm + 278));		
	}

	/**
	* Nos permite cargar las combos finales de la jerarquías de dependencia
	*/
	protected void loadLeafDependenceCombos(RecMotivDevolData recMotivDevol) throws Exception{
		...
		if(recMotivDevol != null && recMotivDevol.getPaisOidPais() != null){
				checkCombo("paisOidPais" , recMotivDevol.getPaisOidPais().getId().toString());
			}
	...
	* @param recMotivDevol La el objetopar la cual se va a realizar la carga
	*/
	protected void loadNonLeafDependenceCombos(RecMotivDevolData recMotivDevol) 
		throws Exception{
	...
	*/
	protected void loadNonLeafDependenceCombosForRequery() throws Exception{
		...
		RecMotivDevolData recMotivDevol = 
			(RecMotivDevolData) conectorParametroSesion(SESSION_ATTRIBUTE_LAST_QUERY_OBJ_FROM);
		
		//Cargamos todos los objetos completos correspodnientes a los valores de los
		//Campos que sean raiz de una jerarquía de combos dependientes
		if(recMotivDevol.getPaisOidPais() != null){
			recMotivDevol.setPaisOidPais((es.indra.belcorp.mso.SegPaisViewData)
			getSingleObject(BUSINESSID_PAISOIDPAIS_QUERY, 
			recMotivDevol.getPaisOidPais(),
			recMotivDevol.getPaisOidPais().clone(),
			"segPaisView"));
		}
		
		//Una vez que tenemos completo los datos de los atributos raices de jerarquías de dependencias
		//cargamos las combos y seleccionamos elementos
		loadNonLeafDependenceCombos(recMotivDevol);
		loadLeafDependenceCombos(recMotivDevol);
	}
	...
	*/
	...
	protected void loadLocalizationVars(RecMotivDevolData recMotivDevol) throws Exception{
		GenDetaSiccData mmgLocalizationQuery = null;
		Vector mmgLocalizationDatosResult = null;
		mmgLocalizationQuery = new GenDetaSiccData();
		mmgLocalizationQuery.setEnti("REC_MOTIV_DEVOL");
		mmgLocalizationQuery.setEntiPk(recMotivDevol.getId());
