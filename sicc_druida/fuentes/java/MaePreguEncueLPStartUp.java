/*
    INDRA/CAR/mmg
    $Id: MaePreguEncueLPStartUp.java,v 1.1 2009/12/03 18:32:46 pecbazalar Exp $
    DESC
*/
import java.util.*;
...
public class MaePreguEncueLPStartUp extends MMGDruidaBase{
	...
	public static final String BUSINESSID_QUERY = "MMGMaePreguEncueQueryFromToUserPage";
				...
				public static final String BUSINESSID_ENSEOIDENCU_QUERY ="MMGMaeEncueNseViewQueryFromToUser";
				public static final String ENSEOIDENCU_COMBO_CONNECTOR = "MaeEncueNseViewConectorCombo";
	...
	public MaePreguEncueLPStartUp(){
		super();
	}

	public void inicio() throws Exception {
		pagina("MaePreguEncuePage");
	}
	...
	public void ejecucion() throws Exception {
		try{
			...
			asignarAtributo("PAGINA", "MaePreguEncuePage", "cod", "MaePreguEncue." + accion + ".label");
			...
			limpiarBotoneraLista("maePreguEncue", accion);
	...
	protected void cmdLov() throws Exception{
		...
		asignarAtributo("TEXT", "codPreg", "validacion", "mensaje = ValidaLongitud(cdos_trim(valor), 1, 2);");
		asignarAtributo("TEXT", "valPesoPreg", "validacion", "mensaje = ValidaInt(cdos_trim(valor), 2, 0, 99);");
		...
		//asignarAtributo("CAPA", "maePreguEncueLovLayer", "visibilidad", "visible");
		asignarAtributo("PAGINA", "MaePreguEncuePage", "onblur", "window.focus();");
	...
	protected void cmdPreView() throws Exception{
		...
		conectorAction("MaePreguEncueLPStartUp");
	...
	*/
	protected void cmdView() throws Exception{
		...
		MaePreguEncueData maePreguEncueQuery= new MaePreguEncueData();
		maePreguEncueQuery.setId(new Long(conectorParametro("idSelection")));
		
		//Asiganmos el valor a la variable de acción anterior para poder volver hacia atras y hacer requery
		asignar("VAR", VAR_ACCION_ANTERIOR, conectorParametro(VAR_ACCION_ANTERIOR));
		
		//Sacamos de la base de datos la entidad a visualizar
		MaePreguEncueData datosResult = (MaePreguEncueData)getSingleObject(BUSINESSID_QUERY, maePreguEncueQuery, 
			maePreguEncueQuery.clone(), "maePreguEncue");
		...
		preservaHiddensI18n(new Integer(1), "MAE_PREGU_ENCUE", "maePreguEncueFrm", "Descripcion", true);
		...
		MaePreguEncueData maePreguEncue = datosResult;
		asignar("TEXT", "codPreg", FormatUtils.formatObject(maePreguEncue.getCodPreg(), 
			 MMGDruidaHelper.getUserDecimalFormatPattern(this), decimalFormatSymbols));
		asignar("TEXT", "valPesoPreg", FormatUtils.formatObject(maePreguEncue.getValPesoPreg(), 
			 MMGDruidaHelper.getUserDecimalFormatPattern(this), decimalFormatSymbols));
		es.indra.belcorp.mso.MaeEncueNseViewData enseOidEncu = maePreguEncue.getEnseOidEncu();
		if(enseOidEncu != null && enseOidEncu.getId() != null) checkCombo("enseOidEncu" , enseOidEncu.getId().toString());
		asignar("TEXT", "Descripcion", FormatUtils.formatObject(maePreguEncue.getDescripcion(), 
			 MMGDruidaHelper.getUserDecimalFormatPattern(this), decimalFormatSymbols));
		...
		loadLocalizationVars(maePreguEncue);
		
			
		//Deshabilitamos todos los campos para que no se editen
		asignarAtributo("TEXT", "codPreg", "disabled", "S");
		asignarAtributo("TEXT", "valPesoPreg", "disabled", "S");
		
					asignarAtributo("COMBO", "enseOidEncu", "disabled", "S");asignarAtributo("TEXT", "Descripcion", "disabled", "S");
	...
	protected void cmdCreate() throws Exception{
		//Creamos los campos ocultos para la internacionalización
		preservaHiddensI18n(new Integer(1), "MAE_PREGU_ENCUE", "maePreguEncueFrm", "Descripcion", false);
		...
		asignarAtributo("TEXT", "codPreg", "req", "S");
		asignarAtributo("TEXT", "valPesoPreg", "req", "S");
		asignarAtributo("COMBO", "enseOidEncu", "req", "S");
		...
		asignarAtributo("TEXT", "Descripcion", "onblur", "setDefaultLanguageValue('Descripcion', get('maePreguEncueFrm.Descripcion'))");
	...
	protected void cmdUpdateForm() throws Exception{
		...
		//asignarAtributo("FORMULARIO", "maePreguEncueFrm", "oculto", "N");
		...
		traza("MMG::Creando MaePreguEncue");
		String id = conectorParametro("idSelection");
		traza("MMG:: id del atributo a modificar: " + id);
	
		//Construimos el mso con los elementos de la búsqueda
		MaePreguEncueData maePreguEncueQuery= new MaePreguEncueData();
		maePreguEncueQuery.setId(new Long(id));
		...
		//asignar("VAR","timestamp",Long.toString(maePreguEncue.jdoGetTimeStamp()));
	...
	*/
	protected void cambiarAcciones() throws Exception{
				asignarAtributo("AREATEXTO", "id", "ontab","ejecutarAccionFocoModificacion(\"maePreguEncueFrm\",1, true)");
				asignarAtributo("AREATEXTO", "id", "onshtab","ejecutarAccionFocoModificacion(\"maePreguEncueFrm\",1, false)");
				asignarAtributo("AREATEXTO", "codPreg", "ontab","ejecutarAccionFocoModificacion(\"maePreguEncueFrm\",2, true)");
				asignarAtributo("AREATEXTO", "codPreg", "onshtab","ejecutarAccionFocoModificacion(\"maePreguEncueFrm\",2, false)");
				asignarAtributo("AREATEXTO", "valPesoPreg", "ontab","ejecutarAccionFocoModificacion(\"maePreguEncueFrm\",3, true)");
				asignarAtributo("AREATEXTO", "valPesoPreg", "onshtab","ejecutarAccionFocoModificacion(\"maePreguEncueFrm\",3, false)");
					asignarAtributo("COMBO","enseOidEncu", "ontab", "ejecutarAccionFocoModificacion(\"maePreguEncueFrm\",4, true)");
					asignarAtributo("COMBO","enseOidEncu", "onshtab", "ejecutarAccionFocoModificacion(\"maePreguEncueFrm\",4, false)");
				asignarAtributo("AREATEXTO", "Descripcion", "ontab","ejecutarAccionFocoModificacion(\"maePreguEncueFrm\",5, true)");
				asignarAtributo("AREATEXTO", "Descripcion", "onshtab","ejecutarAccionFocoModificacion(\"maePreguEncueFrm\",5, false)");
	}
	...
	protected void removeLovElements() throws Exception{
		borrarNodo("TD","nombre","maePreguEncueTdLovNullSelectionButton");
	}
	
	protected void removeQueryElements() throws Exception{
		//Borramos los botones para realizar la búsuqeda
		borrarNodo("TR","nombre","maePreguEncueTrButtons");
	...
	*/
	protected void desactivateNonModifyFields() throws Exception{
			traza("MMG: en desactivateNonModifyFields ");		
				...
				traza("MMG: el atributo es codPreg...");		
				
					traza("MMG: en desactivateNonModifyFields encontrado atributo no editable...");
				
						// borrarCampo("codPreg");
						asignarAtributo ("TEXT", "codPreg", "disabled", "S");
				traza("MMG: el atributo es valPesoPreg...");		
				
				traza("MMG: el atributo es enseOidEncu...");		
	...
	*/
	protected void establecerSizeCapas(String accion, String origen) throws Exception{
		...
		asignarAtributo("CAPA","maePreguEncueFormLayer", "alto", String.valueOf(sizeForm));		
		
		//Estabelcemos la posicion y de la capa correspondiente a la lista y de su botonera
		asignarAtributo("CAPA","maePreguEncueListLayer", "y", String.valueOf(sizeForm + 26));		
		asignarAtributo("CAPA","maePreguEncueListButtonsLayer", "y", String.valueOf(sizeForm + 278));		
	}

	/**
	* Nos permite cargar las combos finales de la jerarquías de dependencia
	*/
	protected void loadLeafDependenceCombos(MaePreguEncueData maePreguEncue) throws Exception{
		HashMap checkedCombos = new HashMap();
		MaeEncueNseViewData enseOidEncu = new MaeEncueNseViewData();
		
						asignarComboResultados(BUSINESSID_ENSEOIDENCU_QUERY,ENSEOIDENCU_COMBO_CONNECTOR, 
			enseOidEncu, enseOidEncu.clone(), "maeEncueNseView", "enseOidEncu", "result_ROWSET" );
		if(maePreguEncue != null && maePreguEncue.getEnseOidEncu() != null){
				checkCombo("enseOidEncu" , maePreguEncue.getEnseOidEncu().getId().toString());
			}
	...
	* @param maePreguEncue La el objetopar la cual se va a realizar la carga
	*/
	protected void loadNonLeafDependenceCombos(MaePreguEncueData maePreguEncue) 
		throws Exception{
	...
	*/
	protected void loadNonLeafDependenceCombosForRequery() throws Exception{
		...
		MaePreguEncueData maePreguEncue = 
			(MaePreguEncueData) conectorParametroSesion(SESSION_ATTRIBUTE_LAST_QUERY_OBJ_FROM);
		
		//Cargamos todos los objetos completos correspodnientes a los valores de los
		//Campos que sean raiz de una jerarquía de combos dependientes
		if(maePreguEncue.getEnseOidEncu() != null){
			maePreguEncue.setEnseOidEncu((es.indra.belcorp.mso.MaeEncueNseViewData)
			getSingleObject(BUSINESSID_ENSEOIDENCU_QUERY, 
			maePreguEncue.getEnseOidEncu(),
			maePreguEncue.getEnseOidEncu().clone(),
			"maeEncueNseView"));
		}
		
		//Una vez que tenemos completo los datos de los atributos raices de jerarquías de dependencias
		//cargamos las combos y seleccionamos elementos
		loadNonLeafDependenceCombos(maePreguEncue);
		loadLeafDependenceCombos(maePreguEncue);
	}
	...
	*/
	...
	protected void loadLocalizationVars(MaePreguEncueData maePreguEncue) throws Exception{
		GenDetaSiccData mmgLocalizationQuery = null;
		Vector mmgLocalizationDatosResult = null;
		mmgLocalizationQuery = new GenDetaSiccData();
		mmgLocalizationQuery.setEnti("MAE_PREGU_ENCUE");
		mmgLocalizationQuery.setEntiPk(maePreguEncue.getId());
