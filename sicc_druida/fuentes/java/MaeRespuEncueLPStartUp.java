/*
    INDRA/CAR/mmg
    $Id: MaeRespuEncueLPStartUp.java,v 1.1 2009/12/03 18:34:16 pecbazalar Exp $
    DESC
*/
import java.util.*;
...
public class MaeRespuEncueLPStartUp extends MMGDruidaBase{
	...
	public static final short UPDATEFORM_SIZE = 221;
	...
	public static final String BUSINESSID_QUERY = "MMGMaeRespuEncueQueryFromToUserPage";
				...
				public static final String BUSINESSID_PRENOIDPREG_QUERY ="MMGMaePreguEncueQueryFromToUser";
				public static final String PRENOIDPREG_COMBO_CONNECTOR = "MaePreguEncueConectorCombo";
	...
	public MaeRespuEncueLPStartUp(){
		super();
	}

	public void inicio() throws Exception {
		pagina("MaeRespuEncuePage");
	}
	...
	public void ejecucion() throws Exception {
		try{
			...
			asignarAtributo("PAGINA", "MaeRespuEncuePage", "cod", "MaeRespuEncue." + accion + ".label");
			...
			limpiarBotoneraLista("maeRespuEncue", accion);
	...
	protected void cmdLov() throws Exception{
		...
		asignarAtributo("TEXT", "codResp", "validacion", "mensaje = ValidaLongitud(cdos_trim(valor), 1, 2);");
		asignarAtributo("TEXT", "valPeso", "validacion", "mensaje = ValidaInt(cdos_trim(valor), 2, 0, 99);");
		
		//Primero determianmos el nombre del campo en la entidad de oriegen donde se van a cargar los datos
		asignar("VAR", "idAttributeLovOpener", conectorParametro(PARAMETRO_LOV_ATTRIBUTO));
		asignar("VAR", "idEntityLovOpener", conectorParametro(PARAMETRO_LOV_ENTIDAD));
		//asignarAtributo("CAPA", "maeRespuEncueLovLayer", "visibilidad", "visible");
		asignarAtributo("PAGINA", "MaeRespuEncuePage", "onblur", "window.focus();");
	...
	protected void cmdPreView() throws Exception{
		...
		conectorAction("MaeRespuEncueLPStartUp");
	...
	*/
	protected void cmdView() throws Exception{
		...
		MaeRespuEncueData maeRespuEncueQuery= new MaeRespuEncueData();
		maeRespuEncueQuery.setId(new Long(conectorParametro("idSelection")));
		
		//Asiganmos el valor a la variable de acción anterior para poder volver hacia atras y hacer requery
		asignar("VAR", VAR_ACCION_ANTERIOR, conectorParametro(VAR_ACCION_ANTERIOR));
		
		//Sacamos de la base de datos la entidad a visualizar
		MaeRespuEncueData datosResult = (MaeRespuEncueData)getSingleObject(BUSINESSID_QUERY, maeRespuEncueQuery, 
			maeRespuEncueQuery.clone(), "maeRespuEncue");
		...
		preservaHiddensI18n(new Integer(1), "MAE_RESPU_ENCUE", "maeRespuEncueFrm", "Descripcion", true);
		...
		MaeRespuEncueData maeRespuEncue = datosResult;
		es.indra.belcorp.mso.MaePreguEncueData prenOidPreg = maeRespuEncue.getPrenOidPreg();
		if(prenOidPreg != null && prenOidPreg.getId() != null) checkCombo("prenOidPreg" , prenOidPreg.getId().toString());
		asignar("TEXT", "codResp", FormatUtils.formatObject(maeRespuEncue.getCodResp(), 
			 MMGDruidaHelper.getUserDecimalFormatPattern(this), decimalFormatSymbols));
		asignar("TEXT", "valPeso", FormatUtils.formatObject(maeRespuEncue.getValPeso(), 
			 MMGDruidaHelper.getUserDecimalFormatPattern(this), decimalFormatSymbols));
		asignar("TEXT", "Descripcion", FormatUtils.formatObject(maeRespuEncue.getDescripcion(), 
			 MMGDruidaHelper.getUserDecimalFormatPattern(this), decimalFormatSymbols));
		...
		loadLocalizationVars(maeRespuEncue);
		
			
		//Deshabilitamos todos los campos para que no se editen
		
					asignarAtributo("COMBO", "prenOidPreg", "disabled", "S");asignarAtributo("TEXT", "codResp", "disabled", "S");
	...
	protected void cmdCreate() throws Exception{
		//Creamos los campos ocultos para la internacionalización
		preservaHiddensI18n(new Integer(1), "MAE_RESPU_ENCUE", "maeRespuEncueFrm", "Descripcion", false);
		...
		asignarAtributo("COMBO", "prenOidPreg", "req", "S");
		asignarAtributo("TEXT", "codResp", "req", "S");
		asignarAtributo("TEXT", "valPeso", "req", "S");
		...
		asignarAtributo("TEXT", "Descripcion", "onblur", "setDefaultLanguageValue('Descripcion', get('maeRespuEncueFrm.Descripcion'))");
	...
	protected void cmdUpdateForm() throws Exception{
		...
		//asignarAtributo("FORMULARIO", "maeRespuEncueFrm", "oculto", "N");
		...
		traza("MMG::Creando MaeRespuEncue");
		String id = conectorParametro("idSelection");
		traza("MMG:: id del atributo a modificar: " + id);
	
		//Construimos el mso con los elementos de la búsqueda
		MaeRespuEncueData maeRespuEncueQuery= new MaeRespuEncueData();
		maeRespuEncueQuery.setId(new Long(id));
		...
		//asignar("VAR","timestamp",Long.toString(maeRespuEncue.jdoGetTimeStamp()));
	...
	*/
	protected void cambiarAcciones() throws Exception{
				asignarAtributo("AREATEXTO", "id", "ontab","ejecutarAccionFocoModificacion(\"maeRespuEncueFrm\",1, true)");
				asignarAtributo("AREATEXTO", "id", "onshtab","ejecutarAccionFocoModificacion(\"maeRespuEncueFrm\",1, false)");
					asignarAtributo("COMBO","prenOidPreg", "ontab", "ejecutarAccionFocoModificacion(\"maeRespuEncueFrm\",2, true)");
					asignarAtributo("COMBO","prenOidPreg", "onshtab", "ejecutarAccionFocoModificacion(\"maeRespuEncueFrm\",2, false)");
				asignarAtributo("AREATEXTO", "codResp", "ontab","ejecutarAccionFocoModificacion(\"maeRespuEncueFrm\",3, true)");
				asignarAtributo("AREATEXTO", "codResp", "onshtab","ejecutarAccionFocoModificacion(\"maeRespuEncueFrm\",3, false)");
				asignarAtributo("AREATEXTO", "valPeso", "ontab","ejecutarAccionFocoModificacion(\"maeRespuEncueFrm\",4, true)");
				asignarAtributo("AREATEXTO", "valPeso", "onshtab","ejecutarAccionFocoModificacion(\"maeRespuEncueFrm\",4, false)");
				asignarAtributo("AREATEXTO", "Descripcion", "ontab","ejecutarAccionFocoModificacion(\"maeRespuEncueFrm\",5, true)");
				asignarAtributo("AREATEXTO", "Descripcion", "onshtab","ejecutarAccionFocoModificacion(\"maeRespuEncueFrm\",5, false)");
	}
	...
	protected void removeLovElements() throws Exception{
		borrarNodo("TD","nombre","maeRespuEncueTdLovNullSelectionButton");
	}
	
	protected void removeQueryElements() throws Exception{
		//Borramos los botones para realizar la búsuqeda
		borrarNodo("TR","nombre","maeRespuEncueTrButtons");
	...
	*/
	protected void desactivateNonModifyFields() throws Exception{
			traza("MMG: en desactivateNonModifyFields ");		
				...
				traza("MMG: el atributo es prenOidPreg...");		
				
				traza("MMG: el atributo es codResp...");		
	...
	*/
	protected void establecerSizeCapas(String accion, String origen) throws Exception{
		...
		asignarAtributo("CAPA","maeRespuEncueFormLayer", "alto", String.valueOf(sizeForm));		
		
		//Estabelcemos la posicion y de la capa correspondiente a la lista y de su botonera
		asignarAtributo("CAPA","maeRespuEncueListLayer", "y", String.valueOf(sizeForm + 26));		
		asignarAtributo("CAPA","maeRespuEncueListButtonsLayer", "y", String.valueOf(sizeForm + 278));		
	}

	/**
	* Nos permite cargar las combos finales de la jerarquías de dependencia
	*/
	protected void loadLeafDependenceCombos(MaeRespuEncueData maeRespuEncue) throws Exception{
		HashMap checkedCombos = new HashMap();
		MaePreguEncueData prenOidPreg = new MaePreguEncueData();
		
						asignarComboResultados(BUSINESSID_PRENOIDPREG_QUERY,PRENOIDPREG_COMBO_CONNECTOR, 
			prenOidPreg, prenOidPreg.clone(), "maePreguEncue", "prenOidPreg", "result_ROWSET" );
		if(maeRespuEncue != null && maeRespuEncue.getPrenOidPreg() != null){
				checkCombo("prenOidPreg" , maeRespuEncue.getPrenOidPreg().getId().toString());
			}
	...
	* @param maeRespuEncue La el objetopar la cual se va a realizar la carga
	*/
	protected void loadNonLeafDependenceCombos(MaeRespuEncueData maeRespuEncue) 
		throws Exception{
	...
	*/
	protected void loadNonLeafDependenceCombosForRequery() throws Exception{
		...
		MaeRespuEncueData maeRespuEncue = 
			(MaeRespuEncueData) conectorParametroSesion(SESSION_ATTRIBUTE_LAST_QUERY_OBJ_FROM);
		
		//Cargamos todos los objetos completos correspodnientes a los valores de los
		//Campos que sean raiz de una jerarquía de combos dependientes
		if(maeRespuEncue.getPrenOidPreg() != null){
			maeRespuEncue.setPrenOidPreg((es.indra.belcorp.mso.MaePreguEncueData)
			getSingleObject(BUSINESSID_PRENOIDPREG_QUERY, 
			maeRespuEncue.getPrenOidPreg(),
			maeRespuEncue.getPrenOidPreg().clone(),
			"maePreguEncue"));
		}
		
		//Una vez que tenemos completo los datos de los atributos raices de jerarquías de dependencias
		//cargamos las combos y seleccionamos elementos
		loadNonLeafDependenceCombos(maeRespuEncue);
		loadLeafDependenceCombos(maeRespuEncue);
	}
	...
	*/
	...
	protected void loadLocalizationVars(MaeRespuEncueData maeRespuEncue) throws Exception{
		GenDetaSiccData mmgLocalizationQuery = null;
		Vector mmgLocalizationDatosResult = null;
		mmgLocalizationQuery = new GenDetaSiccData();
		mmgLocalizationQuery.setEnti("MAE_RESPU_ENCUE");
		mmgLocalizationQuery.setEntiPk(maeRespuEncue.getId());
