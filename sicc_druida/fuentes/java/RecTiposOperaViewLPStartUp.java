/*
    INDRA/CAR/mmg
    $Id: RecTiposOperaViewLPStartUp.java,v 1.1 2009/12/03 18:35:43 pecbazalar Exp $
    DESC
*/
import java.util.*;
...
public class RecTiposOperaViewLPStartUp extends MMGDruidaBase{
	...
	public RecTiposOperaViewLPStartUp(){
		super();
	}
	...
	public void ejecucion() throws Exception {
		try{
			...
			buildJsPageSize("RecTiposOperaView");
			...
			asignarAtributo("PAGINA", "RecTiposOperaViewPage", "cod", "RecTiposOperaView." + accion + ".label");
			...
			limpiarBotoneraLista("recTiposOperaView", accion);
	...
	protected void cmdLov() throws Exception{
		...
		//asignarAtributo("CAPA", "recTiposOperaViewLovLayer", "visibilidad", "visible");
		asignarAtributo("PAGINA", "RecTiposOperaViewPage", "onblur", "window.focus();");
	...
	*/
	protected void cmdView() throws Exception{
		...
		RecTiposOperaViewData recTiposOperaViewQuery= new RecTiposOperaViewData();
		recTiposOperaViewQuery.setId(new Long(conectorParametro("idSelection")));
		
		//Asiganmos el valor a la variable de acción anterior para poder volver hacia atras y hacer requery
		asignar("VAR", VAR_ACCION_ANTERIOR, conectorParametro(VAR_ACCION_ANTERIOR));
		
		//Sacamos de la base de datos la entidad a visualizar
		RecTiposOperaViewData datosResult = (RecTiposOperaViewData)getSingleObject(BUSINESSID_QUERY, recTiposOperaViewQuery, 
			recTiposOperaViewQuery.clone(), "recTiposOperaView");
		...
		RecTiposOperaViewData recTiposOperaView = datosResult;
		es.indra.belcorp.mso.RecOperaViewData ropeOidOper = recTiposOperaView.getRopeOidOper();
		if(ropeOidOper != null && ropeOidOper.getId() != null) checkCombo("ropeOidOper" , ropeOidOper.getId().toString());
		asignar("TEXT", "valTipoOper", FormatUtils.formatObject(recTiposOperaView.getValTipoOper(), 
		...
		//Deshabilitamos todos los campos para que no se editen
					...
					asignarAtributo("COMBO", "ropeOidOper", "disabled", "S");asignarAtributo("TEXT", "valTipoOper", "disabled", "S");
	...
	protected void cmdUpdateForm() throws Exception{
		...
		//asignarAtributo("FORMULARIO", "recTiposOperaViewFrm", "oculto", "N");
		...
		//Construimos el mso con los elementos de la búsqueda
		RecTiposOperaViewData recTiposOperaViewQuery= new RecTiposOperaViewData();
		recTiposOperaViewQuery.setId(new Long(id));
		...
		//asignar("VAR","timestamp",Long.toString(recTiposOperaView.jdoGetTimeStamp()));
	...
	*/
	protected void cambiarAcciones() throws Exception{
				asignarAtributo("AREATEXTO", "id", "ontab","ejecutarAccionFocoModificacion(\"recTiposOperaViewFrm\",0, true)");
				asignarAtributo("AREATEXTO", "id", "onshtab","ejecutarAccionFocoModificacion(\"recTiposOperaViewFrm\",0, false)");
					asignarAtributo("COMBO","ropeOidOper", "ontab", "ejecutarAccionFocoModificacion(\"recTiposOperaViewFrm\",0, true)");
					asignarAtributo("COMBO","ropeOidOper", "onshtab", "ejecutarAccionFocoModificacion(\"recTiposOperaViewFrm\",0, false)");
				asignarAtributo("AREATEXTO", "valTipoOper", "ontab","ejecutarAccionFocoModificacion(\"recTiposOperaViewFrm\",2, true)");
				asignarAtributo("AREATEXTO", "valTipoOper", "onshtab","ejecutarAccionFocoModificacion(\"recTiposOperaViewFrm\",2, false)");
	}
	...
	protected void removeLovElements() throws Exception{
		borrarNodo("TD","nombre","recTiposOperaViewTdLovNullSelectionButton");
	}
	
	protected void removeQueryElements() throws Exception{
		//Borramos los botones para realizar la búsuqeda
		borrarNodo("TR","nombre","recTiposOperaViewTrButtons");
	...
	*/
	protected void establecerSizeCapas(String accion, String origen) throws Exception{
		...
		asignarAtributo("CAPA","recTiposOperaViewFormLayer", "alto", String.valueOf(sizeForm));		
		
		//Estabelcemos la posicion y de la capa correspondiente a la lista y de su botonera
		asignarAtributo("CAPA","recTiposOperaViewListLayer", "y", String.valueOf(sizeForm + 26));		
		asignarAtributo("CAPA","recTiposOperaViewListButtonsLayer", "y", String.valueOf(sizeForm + 278));		
	}

	/**
	* Nos permite cargar las combos finales de la jerarquías de dependencia
	*/
	protected void loadLeafDependenceCombos(RecTiposOperaViewData recTiposOperaView) throws Exception{
		...
		if(recTiposOperaView != null && recTiposOperaView.getRopeOidOper() != null){
				checkCombo("ropeOidOper" , recTiposOperaView.getRopeOidOper().getId().toString());
			}
	...
	* @param recTiposOperaView La el objetopar la cual se va a realizar la carga
	*/
	protected void loadNonLeafDependenceCombos(RecTiposOperaViewData recTiposOperaView) 
		throws Exception{
	...
	*/
	protected void loadNonLeafDependenceCombosForRequery() throws Exception{
		...
		RecTiposOperaViewData recTiposOperaView = 
			(RecTiposOperaViewData) conectorParametroSesion(SESSION_ATTRIBUTE_LAST_QUERY_OBJ_FROM);
		
		//Cargamos todos los objetos completos correspodnientes a los valores de los
		//Campos que sean raiz de una jerarquía de combos dependientes
		if(recTiposOperaView.getRopeOidOper() != null){
			recTiposOperaView.setRopeOidOper((es.indra.belcorp.mso.RecOperaViewData)
			getSingleObject(BUSINESSID_ROPEOIDOPER_QUERY, 
			recTiposOperaView.getRopeOidOper(),
			recTiposOperaView.getRopeOidOper().clone(),
			"recOperaView"));
		}
		
		//Una vez que tenemos completo los datos de los atributos raices de jerarquías de dependencias
		//cargamos las combos y seleccionamos elementos
		loadNonLeafDependenceCombos(recTiposOperaView);
		loadLeafDependenceCombos(recTiposOperaView);
	}
