/*
    INDRA/CAR/mmg
    $Id: MsgMedioEnvioPaisLPStartUp.java,v 1.1 2009/12/03 18:40:53 pecbazalar Exp $
    DESC
*/
import java.util.*;
...
public class MsgMedioEnvioPaisLPStartUp extends MMGDruidaBase{
	...
	public static final String BUSINESSID_QUERY = "MMGMsgMedioEnvioPaisQueryFromToUserPage";
				...
				public static final String BUSINESSID_MEENOIDMEDIENVI_QUERY ="MMGMsgMedioEnvioQueryFromToUser";
				public static final String MEENOIDMEDIENVI_COMBO_CONNECTOR = "MsgMedioEnvioConectorCombo";
	
		
	//Hashtables con los valores estáticos de radiobuttons y combos
	protected static Hashtable codEstaValues = new Hashtable();
	static{
		codEstaValues.put(new java.lang.String("A"), "Activo");
		codEstaValues.put(new java.lang.String("D"), "Desactivo");
	}
	
	
	// Definicion del constructor
	public MsgMedioEnvioPaisLPStartUp(){
		super();
	}
	...
	public void ejecucion() throws Exception {
		try{
			...
			buildJsPageSize("MsgMedioEnvioPais");
			...
			asignarAtributo("PAGINA", "MsgMedioEnvioPaisPage", "cod", "MsgMedioEnvioPais." + accion + ".label");
			...
			limpiarBotoneraLista("msgMedioEnvioPais", accion);
	...
	protected void cmdLov() throws Exception{
		...
		//asignarAtributo("CAPA", "msgMedioEnvioPaisLovLayer", "visibilidad", "visible");
		asignarAtributo("PAGINA", "MsgMedioEnvioPaisPage", "onblur", "window.focus();");
	...
	*/
	protected void cmdView() throws Exception{
		...
		MsgMedioEnvioPaisData msgMedioEnvioPaisQuery= new MsgMedioEnvioPaisData();
		msgMedioEnvioPaisQuery.setId(new Long(conectorParametro("idSelection")));
		
		//Asiganmos el valor a la variable de acción anterior para poder volver hacia atras y hacer requery
		asignar("VAR", VAR_ACCION_ANTERIOR, conectorParametro(VAR_ACCION_ANTERIOR));
		
		//Sacamos de la base de datos la entidad a visualizar
		MsgMedioEnvioPaisData datosResult = (MsgMedioEnvioPaisData)getSingleObject(BUSINESSID_QUERY, msgMedioEnvioPaisQuery, 
			msgMedioEnvioPaisQuery.clone(), "msgMedioEnvioPais");
		...
		if(datosResult.getCodEsta() != null)
			asignar("VAR", "codEstaCheckValue", datosResult.getCodEsta().toString());
		...
		MsgMedioEnvioPaisData msgMedioEnvioPais = datosResult;
		es.indra.belcorp.mso.MsgMedioEnvioData meenOidMediEnvi = msgMedioEnvioPais.getMeenOidMediEnvi();
		if(meenOidMediEnvi != null && meenOidMediEnvi.getId() != null) checkCombo("meenOidMediEnvi" , meenOidMediEnvi.getId().toString());
		asignarAtributo("RADIOB", "codEsta" , "valor", FormatUtils.formatObject(
			msgMedioEnvioPais.getCodEsta(), 
		...
		//Deshabilitamos todos los campos para que no se editen
					...
					asignarAtributo("COMBO", "meenOidMediEnvi", "disabled", "S");
					asignarAtributo("RADIOB", "codEsta", "disabled", "S");
	...
	protected void cmdCreate() throws Exception{
		...
		asignarAtributo("COMBO", "meenOidMediEnvi", "req", "S");
		asignarAtributo("RADIOB", "codEsta", "req", "S");
	...
	protected void cmdUpdateForm() throws Exception{
		...
		//asignarAtributo("FORMULARIO", "msgMedioEnvioPaisFrm", "oculto", "N");
		...
		//Construimos el mso con los elementos de la búsqueda
		MsgMedioEnvioPaisData msgMedioEnvioPaisQuery= new MsgMedioEnvioPaisData();
		msgMedioEnvioPaisQuery.setId(new Long(id));
		...
		decimalFormatSymbols.setDecimalSeparator(UtilidadesSession.getSeparadorDecimales(this).charAt(0));
		decimalFormatSymbols.setGroupingSeparator(UtilidadesSession.getSeparadorDecimales(this).equals(".") ? ",".charAt(0) : ".".charAt(0));
		es.indra.belcorp.mso.SegPaisViewData paisOidPais = msgMedioEnvioPais.getPaisOidPais();
		...
		//asignar("VAR","timestamp",Long.toString(msgMedioEnvioPais.jdoGetTimeStamp()));
	...
	*/
	protected void cambiarAcciones() throws Exception{
				asignarAtributo("AREATEXTO", "id", "ontab","ejecutarAccionFocoModificacion(\"msgMedioEnvioPaisFrm\",1, true)");
				asignarAtributo("AREATEXTO", "id", "onshtab","ejecutarAccionFocoModificacion(\"msgMedioEnvioPaisFrm\",1, false)");
					asignarAtributo("COMBO","paisOidPais", "ontab", "ejecutarAccionFocoModificacion(\"msgMedioEnvioPaisFrm\",2, true)");
					asignarAtributo("COMBO","paisOidPais", "onshtab", "ejecutarAccionFocoModificacion(\"msgMedioEnvioPaisFrm\",2, false)");
					asignarAtributo("COMBO","meenOidMediEnvi", "ontab", "ejecutarAccionFocoModificacion(\"msgMedioEnvioPaisFrm\",3, true)");
					asignarAtributo("COMBO","meenOidMediEnvi", "onshtab", "ejecutarAccionFocoModificacion(\"msgMedioEnvioPaisFrm\",3, false)");
				asignarAtributo("TEXT", "codEsta", "ontab","ejecutarAccionFocoModificacion(\"msgMedioEnvioPaisFrm\",4, true)");
				asignarAtributo("TEXT", "codEsta", "onshtab","ejecutarAccionFocoModificacion(\"msgMedioEnvioPaisFrm\",4, false)");
	}
	...
	protected void removeLovElements() throws Exception{
		borrarNodo("TD","nombre","msgMedioEnvioPaisTdLovNullSelectionButton");
	}
	
	protected void removeQueryElements() throws Exception{
		//Borramos los botones para realizar la búsuqeda
		borrarNodo("TR","nombre","msgMedioEnvioPaisTrButtons");
	...
	*/
	protected void desactivateNonModifyFields() throws Exception{
			traza("MMG: en desactivateNonModifyFields ");		
				...
				traza("MMG: el atributo es meenOidMediEnvi...");		
				
					traza("MMG: en desactivateNonModifyFields encontrado atributo no editable...");
				
						asignarAtributo("COMBO", "meenOidMediEnvi", "disabled", "S");
				traza("MMG: el atributo es codEsta...");		
	...
	*/
	protected void establecerSizeCapas(String accion, String origen) throws Exception{
		...
		asignarAtributo("CAPA","msgMedioEnvioPaisFormLayer", "alto", String.valueOf(sizeForm));		
		
		//Estabelcemos la posicion y de la capa correspondiente a la lista y de su botonera
		asignarAtributo("CAPA","msgMedioEnvioPaisListLayer", "y", String.valueOf(sizeForm + 26));		
		asignarAtributo("CAPA","msgMedioEnvioPaisListButtonsLayer", "y", String.valueOf(sizeForm + 278));		
	}

	/**
	* Nos permite cargar las combos finales de la jerarquías de dependencia
	*/
	protected void loadLeafDependenceCombos(MsgMedioEnvioPaisData msgMedioEnvioPais) throws Exception{
		...
		if(msgMedioEnvioPais != null && msgMedioEnvioPais.getPaisOidPais() != null){
				checkCombo("paisOidPais" , msgMedioEnvioPais.getPaisOidPais().getId().toString());
			}
		...
		MsgMedioEnvioData meenOidMediEnvi = new MsgMedioEnvioData();
		
						asignarComboResultados(BUSINESSID_MEENOIDMEDIENVI_QUERY,MEENOIDMEDIENVI_COMBO_CONNECTOR, 
			meenOidMediEnvi, meenOidMediEnvi.clone(), "msgMedioEnvio", "meenOidMediEnvi", "result_ROWSET" );
		if(msgMedioEnvioPais != null && msgMedioEnvioPais.getMeenOidMediEnvi() != null){
				checkCombo("meenOidMediEnvi" , msgMedioEnvioPais.getMeenOidMediEnvi().getId().toString());
			}
	...
	* @param msgMedioEnvioPais La el objetopar la cual se va a realizar la carga
	*/
	protected void loadNonLeafDependenceCombos(MsgMedioEnvioPaisData msgMedioEnvioPais) 
		throws Exception{
	...
	*/
	protected void loadNonLeafDependenceCombosForRequery() throws Exception{
		...
		MsgMedioEnvioPaisData msgMedioEnvioPais = 
			(MsgMedioEnvioPaisData) conectorParametroSesion(SESSION_ATTRIBUTE_LAST_QUERY_OBJ_FROM);
		
		//Cargamos todos los objetos completos correspodnientes a los valores de los
		//Campos que sean raiz de una jerarquía de combos dependientes
		if(msgMedioEnvioPais.getPaisOidPais() != null){
			msgMedioEnvioPais.setPaisOidPais((es.indra.belcorp.mso.SegPaisViewData)
			getSingleObject(BUSINESSID_PAISOIDPAIS_QUERY, 
			msgMedioEnvioPais.getPaisOidPais(),
			msgMedioEnvioPais.getPaisOidPais().clone(),
			"segPaisView"));
		}
		if(msgMedioEnvioPais.getMeenOidMediEnvi() != null){
			msgMedioEnvioPais.setMeenOidMediEnvi((es.indra.belcorp.mso.MsgMedioEnvioData)
			getSingleObject(BUSINESSID_MEENOIDMEDIENVI_QUERY, 
			msgMedioEnvioPais.getMeenOidMediEnvi(),
			msgMedioEnvioPais.getMeenOidMediEnvi().clone(),
			"msgMedioEnvio"));
		}
		
		//Una vez que tenemos completo los datos de los atributos raices de jerarquías de dependencias
		//cargamos las combos y seleccionamos elementos
		loadNonLeafDependenceCombos(msgMedioEnvioPais);
		loadLeafDependenceCombos(msgMedioEnvioPais);
	}
