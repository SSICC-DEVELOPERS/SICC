/*
    INDRA/CAR/mmg
    $Id: ZonRegioView2LPStartUp.java,v 1.1 2009/12/03 18:38:15 pecbazalar Exp $
    DESC
*/
import java.util.*;
...
public class ZonRegioView2LPStartUp extends MMGDruidaBase{
	...
	public void ejecucion() throws Exception {
		try{
			...
			asignarAtributo("PAGINA", "ZonRegioView2Page", "cod", "ZonRegioView2." + accion + ".label");
			...
			limpiarBotoneraLista("zonRegioView2", accion);
	...
	protected void cmdLov() throws Exception{
		...
		//asignarAtributo("CAPA", "zonRegioView2LovLayer", "visibilidad", "visible");
		asignarAtributo("PAGINA", "ZonRegioView2Page", "onblur", "window.focus();");
	...
	*/
	protected void cmdView() throws Exception{
		...
		ZonRegioView2Data zonRegioView2Query= new ZonRegioView2Data();
		zonRegioView2Query.setId(new Long(conectorParametro("idSelection")));
		
		//Asiganmos el valor a la variable de acción anterior para poder volver hacia atras y hacer requery
		asignar("VAR", VAR_ACCION_ANTERIOR, conectorParametro(VAR_ACCION_ANTERIOR));
		
		//Sacamos de la base de datos la entidad a visualizar
		ZonRegioView2Data datosResult = (ZonRegioView2Data)getSingleObject(BUSINESSID_QUERY, zonRegioView2Query, 
			zonRegioView2Query.clone(), "zonRegioView2");
		...
		ZonRegioView2Data zonRegioView2 = datosResult;
		asignar("TEXT", "codRegi", FormatUtils.formatObject(zonRegioView2.getCodRegi(), 
			 MMGDruidaHelper.getUserDecimalFormatPattern(this), decimalFormatSymbols));
		es.indra.belcorp.mso.SegCanalViewData canaOidCana = zonRegioView2.getCanaOidCana();
		if(canaOidCana != null && canaOidCana.getId() != null) checkCombo("canaOidCana" , canaOidCana.getId().toString());
		es.indra.belcorp.mso.SegMarcaData marcOidMarc = zonRegioView2.getMarcOidMarc();
	...
	protected void cmdUpdateForm() throws Exception{
		...
		//asignarAtributo("FORMULARIO", "zonRegioView2Frm", "oculto", "N");
		...
		//Construimos el mso con los elementos de la búsqueda
		ZonRegioView2Data zonRegioView2Query= new ZonRegioView2Data();
		zonRegioView2Query.setId(new Long(id));
		...
		//asignar("VAR","timestamp",Long.toString(zonRegioView2.jdoGetTimeStamp()));
	...
	*/
	protected void cambiarAcciones() throws Exception{
				asignarAtributo("AREATEXTO", "id", "ontab","ejecutarAccionFocoModificacion(\"zonRegioView2Frm\",1, true)");
				asignarAtributo("AREATEXTO", "id", "onshtab","ejecutarAccionFocoModificacion(\"zonRegioView2Frm\",1, false)");
				asignarAtributo("AREATEXTO", "codRegi", "ontab","ejecutarAccionFocoModificacion(\"zonRegioView2Frm\",2, true)");
				asignarAtributo("AREATEXTO", "codRegi", "onshtab","ejecutarAccionFocoModificacion(\"zonRegioView2Frm\",2, false)");
					asignarAtributo("COMBO","canaOidCana", "ontab", "ejecutarAccionFocoModificacion(\"zonRegioView2Frm\",3, true)");
					asignarAtributo("COMBO","canaOidCana", "onshtab", "ejecutarAccionFocoModificacion(\"zonRegioView2Frm\",3, false)");
					asignarAtributo("COMBO","marcOidMarc", "ontab", "ejecutarAccionFocoModificacion(\"zonRegioView2Frm\",4, true)");
					asignarAtributo("COMBO","marcOidMarc", "onshtab", "ejecutarAccionFocoModificacion(\"zonRegioView2Frm\",4, false)");
					asignarAtributo("COMBO","paisOidPais", "ontab", "ejecutarAccionFocoModificacion(\"zonRegioView2Frm\",5, true)");
					asignarAtributo("COMBO","paisOidPais", "onshtab", "ejecutarAccionFocoModificacion(\"zonRegioView2Frm\",5, false)");
	}
	...
	protected void removeLovElements() throws Exception{
		borrarNodo("TD","nombre","zonRegioView2TdLovNullSelectionButton");
	}
	
	protected void removeQueryElements() throws Exception{
		//Borramos los botones para realizar la búsuqeda
		borrarNodo("TR","nombre","zonRegioView2TrButtons");
	...
	*/
	protected void establecerSizeCapas(String accion, String origen) throws Exception{
		...
		asignarAtributo("CAPA","zonRegioView2FormLayer", "alto", String.valueOf(sizeForm));		
		
		//Estabelcemos la posicion y de la capa correspondiente a la lista y de su botonera
		asignarAtributo("CAPA","zonRegioView2ListLayer", "y", String.valueOf(sizeForm + 26));		
		asignarAtributo("CAPA","zonRegioView2ListButtonsLayer", "y", String.valueOf(sizeForm + 278));		
	}

	/**
	* Nos permite cargar las combos finales de la jerarquías de dependencia
	*/
	protected void loadLeafDependenceCombos(ZonRegioView2Data zonRegioView2) throws Exception{
		...
		if(zonRegioView2 != null && zonRegioView2.getCanaOidCana() != null){
				checkCombo("canaOidCana" , zonRegioView2.getCanaOidCana().getId().toString());
			}
		...
		if(zonRegioView2 != null && zonRegioView2.getMarcOidMarc() != null){
				checkCombo("marcOidMarc" , zonRegioView2.getMarcOidMarc().getId().toString());
			}
		...
		if(zonRegioView2 != null && zonRegioView2.getPaisOidPais() != null){
				checkCombo("paisOidPais" , zonRegioView2.getPaisOidPais().getId().toString());
			}
	...
	* @param zonRegioView2 La el objetopar la cual se va a realizar la carga
	*/
	protected void loadNonLeafDependenceCombos(ZonRegioView2Data zonRegioView2) 
		throws Exception{
	...
	*/
	protected void loadNonLeafDependenceCombosForRequery() throws Exception{
		...
		ZonRegioView2Data zonRegioView2 = 
			(ZonRegioView2Data) conectorParametroSesion(SESSION_ATTRIBUTE_LAST_QUERY_OBJ_FROM);
		
		//Cargamos todos los objetos completos correspodnientes a los valores de los
		//Campos que sean raiz de una jerarquía de combos dependientes
		if(zonRegioView2.getCanaOidCana() != null){
			zonRegioView2.setCanaOidCana((es.indra.belcorp.mso.SegCanalViewData)
			getSingleObject(BUSINESSID_CANAOIDCANA_QUERY, 
			zonRegioView2.getCanaOidCana(),
			zonRegioView2.getCanaOidCana().clone(),
			"segCanalView"));
		}
		if(zonRegioView2.getMarcOidMarc() != null){
			zonRegioView2.setMarcOidMarc((es.indra.belcorp.mso.SegMarcaData)
			getSingleObject(BUSINESSID_MARCOIDMARC_QUERY, 
			zonRegioView2.getMarcOidMarc(),
			zonRegioView2.getMarcOidMarc().clone(),
			"segMarca"));
		}
		if(zonRegioView2.getPaisOidPais() != null){
			zonRegioView2.setPaisOidPais((es.indra.belcorp.mso.SegPaisViewData)
			getSingleObject(BUSINESSID_PAISOIDPAIS_QUERY, 
			zonRegioView2.getPaisOidPais(),
			zonRegioView2.getPaisOidPais().clone(),
			"segPaisView"));
		}
		
		//Una vez que tenemos completo los datos de los atributos raices de jerarquías de dependencias
		//cargamos las combos y seleccionamos elementos
		loadNonLeafDependenceCombos(zonRegioView2);
		loadLeafDependenceCombos(zonRegioView2);
	}
