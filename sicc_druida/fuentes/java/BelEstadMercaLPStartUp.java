/*
    INDRA/CAR/mmg
    $Id: BelEstadMercaLPStartUp.java,v 1.1 2009/12/03 18:37:14 pecbazalar Exp $
    DESC
*/
import java.util.*;
...
public class BelEstadMercaLPStartUp extends MMGDruidaBase{
	...
	public void ejecucion() throws Exception {
		try{
			...
			asignarAtributo("PAGINA", "BelEstadMercaPage", "cod", "BelEstadMerca." + accion + ".label");
			...
			limpiarBotoneraLista("belEstadMerca", accion);
	...
	protected void cmdLov() throws Exception{
		...
		asignarAtributo("TEXT", "codEsta", "validacion", "mensaje = ValidaLongitud(cdos_trim(valor), 1, 3);");
		...
		//asignarAtributo("CAPA", "belEstadMercaLovLayer", "visibilidad", "visible");
		asignarAtributo("PAGINA", "BelEstadMercaPage", "onblur", "window.focus();");
	...
	*/
	protected void cmdView() throws Exception{
		...
		BelEstadMercaData belEstadMercaQuery= new BelEstadMercaData();
		belEstadMercaQuery.setId(new Long(conectorParametro("idSelection")));
		
		//Asiganmos el valor a la variable de acción anterior para poder volver hacia atras y hacer requery
		asignar("VAR", VAR_ACCION_ANTERIOR, conectorParametro(VAR_ACCION_ANTERIOR));
		
		//Sacamos de la base de datos la entidad a visualizar
		BelEstadMercaData datosResult = (BelEstadMercaData)getSingleObject(BUSINESSID_QUERY, belEstadMercaQuery, 
			belEstadMercaQuery.clone(), "belEstadMerca");
		...
		preservaHiddensI18n(new Integer(1), "BEL_ESTAD_MERCA", "belEstadMercaFrm", "Descripcion", true);
		...
		BelEstadMercaData belEstadMerca = datosResult;
		asignar("TEXT", "codEsta", FormatUtils.formatObject(belEstadMerca.getCodEsta(), 
			 MMGDruidaHelper.getUserDecimalFormatPattern(this), decimalFormatSymbols));
		asignar("TEXT", "Descripcion", FormatUtils.formatObject(belEstadMerca.getDescripcion(), 
			 MMGDruidaHelper.getUserDecimalFormatPattern(this), decimalFormatSymbols));
		...
		loadLocalizationVars(belEstadMerca);
		
			
		//Deshabilitamos todos los campos para que no se editen
		asignarAtributo("TEXT", "codEsta", "disabled", "S");
	...
	protected void cmdCreate() throws Exception{
		//Creamos los campos ocultos para la internacionalización
		preservaHiddensI18n(new Integer(1), "BEL_ESTAD_MERCA", "belEstadMercaFrm", "Descripcion", false);
		...
		asignarAtributo("TEXT", "codEsta", "req", "S");
		...
		asignarAtributo("TEXT", "Descripcion", "onblur", "setDefaultLanguageValue('Descripcion', get('belEstadMercaFrm.Descripcion'))");
	...
	protected void cmdUpdateForm() throws Exception{
		...
		//asignarAtributo("FORMULARIO", "belEstadMercaFrm", "oculto", "N");
		...
		//Construimos el mso con los elementos de la búsqueda
		BelEstadMercaData belEstadMercaQuery= new BelEstadMercaData();
		belEstadMercaQuery.setId(new Long(id));
		...
		//asignar("VAR","timestamp",Long.toString(belEstadMerca.jdoGetTimeStamp()));
	...
	*/
	protected void cambiarAcciones() throws Exception{
				asignarAtributo("AREATEXTO", "id", "ontab","ejecutarAccionFocoModificacion(\"belEstadMercaFrm\",1, true)");
				asignarAtributo("AREATEXTO", "id", "onshtab","ejecutarAccionFocoModificacion(\"belEstadMercaFrm\",1, false)");
					asignarAtributo("COMBO","paisOidPais", "ontab", "ejecutarAccionFocoModificacion(\"belEstadMercaFrm\",2, true)");
					asignarAtributo("COMBO","paisOidPais", "onshtab", "ejecutarAccionFocoModificacion(\"belEstadMercaFrm\",2, false)");
				asignarAtributo("AREATEXTO", "codEsta", "ontab","ejecutarAccionFocoModificacion(\"belEstadMercaFrm\",3, true)");
				asignarAtributo("AREATEXTO", "codEsta", "onshtab","ejecutarAccionFocoModificacion(\"belEstadMercaFrm\",3, false)");
				asignarAtributo("AREATEXTO", "Descripcion", "ontab","ejecutarAccionFocoModificacion(\"belEstadMercaFrm\",4, true)");
				asignarAtributo("AREATEXTO", "Descripcion", "onshtab","ejecutarAccionFocoModificacion(\"belEstadMercaFrm\",4, false)");
	}
	...
	protected void removeLovElements() throws Exception{
		borrarNodo("TD","nombre","belEstadMercaTdLovNullSelectionButton");
	}
	
	protected void removeQueryElements() throws Exception{
		//Borramos los botones para realizar la búsuqeda
		borrarNodo("TR","nombre","belEstadMercaTrButtons");
	...
	*/
	protected void establecerSizeCapas(String accion, String origen) throws Exception{
		...
		asignarAtributo("CAPA","belEstadMercaFormLayer", "alto", String.valueOf(sizeForm));		
		
		//Estabelcemos la posicion y de la capa correspondiente a la lista y de su botonera
		asignarAtributo("CAPA","belEstadMercaListLayer", "y", String.valueOf(sizeForm + 26));		
		asignarAtributo("CAPA","belEstadMercaListButtonsLayer", "y", String.valueOf(sizeForm + 278));		
	}

	/**
	* Nos permite cargar las combos finales de la jerarquías de dependencia
	*/
	protected void loadLeafDependenceCombos(BelEstadMercaData belEstadMerca) throws Exception{
		...
		if(belEstadMerca != null && belEstadMerca.getPaisOidPais() != null){
				checkCombo("paisOidPais" , belEstadMerca.getPaisOidPais().getId().toString());
			}
	...
	* @param belEstadMerca La el objetopar la cual se va a realizar la carga
	*/
	protected void loadNonLeafDependenceCombos(BelEstadMercaData belEstadMerca) 
		throws Exception{
	...
	*/
	protected void loadNonLeafDependenceCombosForRequery() throws Exception{
		...
		BelEstadMercaData belEstadMerca = 
			(BelEstadMercaData) conectorParametroSesion(SESSION_ATTRIBUTE_LAST_QUERY_OBJ_FROM);
		
		//Cargamos todos los objetos completos correspodnientes a los valores de los
		//Campos que sean raiz de una jerarquía de combos dependientes
		if(belEstadMerca.getPaisOidPais() != null){
			belEstadMerca.setPaisOidPais((es.indra.belcorp.mso.SegPaisViewData)
			getSingleObject(BUSINESSID_PAISOIDPAIS_QUERY, 
			belEstadMerca.getPaisOidPais(),
			belEstadMerca.getPaisOidPais().clone(),
			"segPaisView"));
		}
		
		//Una vez que tenemos completo los datos de los atributos raices de jerarquías de dependencias
		//cargamos las combos y seleccionamos elementos
		loadNonLeafDependenceCombos(belEstadMerca);
		loadLeafDependenceCombos(belEstadMerca);
	}
	...
	*/
	...
	protected void loadLocalizationVars(BelEstadMercaData belEstadMerca) throws Exception{
		GenDetaSiccData mmgLocalizationQuery = null;
		Vector mmgLocalizationDatosResult = null;
		mmgLocalizationQuery = new GenDetaSiccData();
		mmgLocalizationQuery.setEnti("BEL_ESTAD_MERCA");
		mmgLocalizationQuery.setEntiPk(belEstadMerca.getId());
