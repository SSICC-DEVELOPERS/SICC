/*
    INDRA/CAR/mmg
    $Id: EduHistoCursoLPStartUp.java,v 1.1 2009/12/03 18:36:22 pecbazalar Exp $
    DESC
*/
import java.util.*;
...
public class EduHistoCursoLPStartUp extends MMGDruidaBase{
	...
	public void ejecucion() throws Exception {
		try{
			...
			asignarAtributo("PAGINA", "EduHistoCursoPage", "cod", "EduHistoCurso." + accion + ".label");
			...
			limpiarBotoneraLista("eduHistoCurso", accion);
	...
	protected void cmdLov() throws Exception{
		...
		asignarAtributo("TEXT", "indApta", "validacion", "ValidaInt(cdos_trim(valor), 1, 0, 9)");
		asignarAtributo("TEXT", "indInvit", "validacion", "ValidaInt(cdos_trim(valor), 1, 0, 9)");
		...
		//asignarAtributo("CAPA", "eduHistoCursoLovLayer", "visibilidad", "visible");
		asignarAtributo("PAGINA", "EduHistoCursoPage", "onblur", "window.focus();");
	...
	protected void cmdView() throws Exception{
		...
		EduHistoCursoData eduHistoCursoQuery= new EduHistoCursoData();
		eduHistoCursoQuery.setId(new Integer(conectorParametro("idSelection")));
		
		//Asiganmos el valor a la variable de acción anterior para poder volver hacia atras y hacer requery
		asignar("VAR", VAR_ACCION_ANTERIOR, conectorParametro(VAR_ACCION_ANTERIOR));
		
		//Sacamos de la base de datos la entidad a visualizar
		EduHistoCursoData datosResult = (EduHistoCursoData)getSingleObject(BUSINESSID_QUERY, eduHistoCursoQuery, 
			eduHistoCursoQuery.clone(), "eduHistoCurso");
		...
		EduHistoCursoData eduHistoCurso = datosResult;
	
	
		asignarAtributo("LABELC", "lblViewIndApta", "valor", 
				FormatUtils.formatObject(eduHistoCurso.getIndApta(), 
				 MMGDruidaHelper.getUserDecimalFormatPattern(this), MMGDruidaHelper.getUserDecimalFormatSymbols(this)));			
	
		asignarAtributo("LABELC", "lblViewIndInvit", "valor", 
				FormatUtils.formatObject(eduHistoCurso.getIndInvit(), 
				 MMGDruidaHelper.getUserDecimalFormatPattern(this), MMGDruidaHelper.getUserDecimalFormatSymbols(this)));			
	
		asignarAtributo("LABELC", "lblViewIndAsis", "valor", 
				FormatUtils.formatObject(eduHistoCurso.getIndAsis(), 
				 MMGDruidaHelper.getUserDecimalFormatPattern(this), MMGDruidaHelper.getUserDecimalFormatSymbols(this)));			
	...
	protected void cmdUpdateForm() throws Exception{
		...
		//asignarAtributo("FORMULARIO", "eduHistoCursoFrm", "oculto", "N");
		...
		EduHistoCursoData eduHistoCursoQuery= new EduHistoCursoData();
		eduHistoCursoQuery.setId(new Integer(id));
		...
		es.indra.belcorp.mso.MaeClienTipoSubtiData ctsuOidClieTipoSubt = eduHistoCurso.getCtsuOidClieTipoSubt();
		...
		es.indra.belcorp.mso.EduMatriCursoData mcurOidCurs = eduHistoCurso.getMcurOidCurs();
		if(mcurOidCurs != null && mcurOidCurs.getId() != null) checkCombo("mcurOidCurs" , mcurOidCurs.getId().toString());
	
		asignar("TEXT", "indApta", FormatUtils.formatObject(eduHistoCurso.getIndApta(), 
			 MMGDruidaHelper.getUserDecimalFormatPattern(this), MMGDruidaHelper.getUserDecimalFormatSymbols(this)));
	
		asignar("TEXT", "indInvit", FormatUtils.formatObject(eduHistoCurso.getIndInvit(), 
			 MMGDruidaHelper.getUserDecimalFormatPattern(this), MMGDruidaHelper.getUserDecimalFormatSymbols(this)));
	
		asignar("TEXT", "indAsis", FormatUtils.formatObject(eduHistoCurso.getIndAsis(), 
			 MMGDruidaHelper.getUserDecimalFormatPattern(this), MMGDruidaHelper.getUserDecimalFormatSymbols(this)));
		...
		//asignar("VAR","timestamp",Long.toString(eduHistoCurso.jdoGetTimeStamp()));
	...
	protected void removeLovElements() throws Exception{
		borrarNodo("TD","nombre","eduHistoCursoTdLovNullSelectionButton");
	}
	
	protected void removeQueryElements() throws Exception{
		//Borramos los botones para realizar la búsuqeda
		borrarNodo("TR","nombre","eduHistoCursoTrButtons");
	...
	*/
	protected void establecerSizeCapas(String accion, String origen) throws Exception{
		...
		asignarAtributo("CAPA","eduHistoCursoFormLayer", "alto", String.valueOf(sizeForm));		
		
		//Estabelcemos la posicion y de la capa correspondiente a la lista y de su botonera
		asignarAtributo("CAPA","eduHistoCursoPreListLayer", "y", String.valueOf(sizeForm + 24));
		asignarAtributo("CAPA","eduHistoCursoListLayer", "y", String.valueOf(sizeForm + 46));		
		asignarAtributo("CAPA","eduHistoCursoListButtonsLayer", "y", String.valueOf(sizeForm + 299));		
	}
	...
	* @param eduHistoCurso La el objetopar la cual se va a realizar la carga
	*/
	protected void loadNonLeafDependenceCombos(EduHistoCursoData eduHistoCurso) 
		throws Exception{
		if(eduHistoCurso.getCtsuOidClieTipoSubt() != null){
			checkCombo("ctsuOidClieTipoSubt" , eduHistoCurso.getCtsuOidClieTipoSubt().getId().toString());
			}
		if(eduHistoCurso.getMcurOidCurs() != null){
			checkCombo("mcurOidCurs" , eduHistoCurso.getMcurOidCurs().getId().toString());
			}
	...
	*/
	protected void setDependenceComboLabels(EduHistoCursoData eduHistoCurso) 
		throws Exception{
		asignarAtributo("LABELC", "lblViewCtsuOidClieTipoSubt", "valor",FormatUtils.formatObject(eduHistoCurso.getCtsuOidClieTipoSubt()));
		asignarAtributo("LABELC", "lblViewMcurOidCurs", "valor",FormatUtils.formatObject(eduHistoCurso.getMcurOidCurs()));
	...
	*/
	protected void loadNonLeafDependenceCombosForRequery() throws Exception{
		...
		EduHistoCursoData eduHistoCurso = 
			(EduHistoCursoData) conectorParametroSesion(SESSION_ATTRIBUTE_LAST_QUERY_OBJ_FROM);
		
		//Cargamos todos los objetos completos correspodnientes a los valores de los
		//Campos que sean raiz de una jerarquía de combos dependientes
		if(eduHistoCurso.getCtsuOidClieTipoSubt() != null){
			eduHistoCurso.setCtsuOidClieTipoSubt((es.indra.belcorp.mso.MaeClienTipoSubtiData)
			getSingleObject(BUSINESSID_CTSUOIDCLIETIPOSUBT_QUERY, 
			eduHistoCurso.getCtsuOidClieTipoSubt(),
			eduHistoCurso.getCtsuOidClieTipoSubt().clone(),
			"maeClienTipoSubti"));
		}
		if(eduHistoCurso.getMcurOidCurs() != null){
			eduHistoCurso.setMcurOidCurs((es.indra.belcorp.mso.EduMatriCursoData)
			getSingleObject(BUSINESSID_MCUROIDCURS_QUERY, 
			eduHistoCurso.getMcurOidCurs(),
			eduHistoCurso.getMcurOidCurs().clone(),
			"eduMatriCurso"));
		}
		
		//Una vez que tenemos completo los datos de los atributos raices de jerarquías de dependencias
		//cargamos las combos y seleccionamos elementos
		loadNonLeafDependenceCombos(eduHistoCurso);
	}
	...
	*/
	protected void removeViewLabels() throws Exception{
		...
		borrarNodo("TR", "nombre", "indAptaViewLabelTrId");
		borrarNodo("TR", "nombre", "indInvitViewLabelTrId");
	...
	*/
	protected void removeWidgets() throws Exception{
		...
		borrarNodo("TR", "nombre", "indAptaWidgetTrId");
		borrarNodo("TR", "nombre", "indInvitWidgetTrId");
