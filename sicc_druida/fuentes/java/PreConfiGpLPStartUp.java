/*
    INDRA/CAR/mmg
    $Id: PreConfiGpLPStartUp.java,v 1.1 2009/12/03 18:39:10 pecbazalar Exp $
    DESC
*/
import java.util.*;
...
public class PreConfiGpLPStartUp extends MMGDruidaBase{
	...
	public void ejecucion() throws Exception {
		try{
			...
			asignarAtributo("PAGINA", "PreConfiGpPage", "cod", "PreConfiGp." + accion + ".label");
			...
			limpiarBotoneraLista("preConfiGp", accion);
	...
	protected void cmdLov() throws Exception{
		...
		//asignarAtributo("CAPA", "preConfiGpLovLayer", "visibilidad", "visible");
		asignarAtributo("PAGINA", "PreConfiGpPage", "onblur", "window.focus();");
	...
	*/
	protected void cmdView() throws Exception{
		...
		PreConfiGpData preConfiGpQuery= new PreConfiGpData();
		preConfiGpQuery.setId(new Long(conectorParametro("idSelection")));
		
		//Asiganmos el valor a la variable de acción anterior para poder volver hacia atras y hacer requery
		asignar("VAR", VAR_ACCION_ANTERIOR, conectorParametro(VAR_ACCION_ANTERIOR));
		
		//Sacamos de la base de datos la entidad a visualizar
		PreConfiGpData datosResult = (PreConfiGpData)getSingleObject(BUSINESSID_QUERY, preConfiGpQuery, 
			preConfiGpQuery.clone(), "preConfiGp");
		...
		PreConfiGpData preConfiGp = datosResult;
		es.indra.belcorp.mso.SegMarcaData marcOidMarc = preConfiGp.getMarcOidMarc();
		if(marcOidMarc != null && marcOidMarc.getId() != null) checkCombo("marcOidMarc" , marcOidMarc.getId().toString());
		es.indra.belcorp.mso.SegCanalViewData canaOidCana = preConfiGp.getCanaOidCana();
		if(canaOidCana != null && canaOidCana.getId() != null) checkCombo("canaOidCana" , canaOidCana.getId().toString());
		asignar("TEXT", "valIncr", FormatUtils.formatObject(preConfiGp.getValIncr(), 
		...
		//Deshabilitamos todos los campos para que no se editen
					...
					asignarAtributo("COMBO", "canaOidCana", "disabled", "S");asignarAtributo("TEXT", "valIncr", "disabled", "S");
	...
	protected void cmdCreate() throws Exception{
		...
		asignarAtributo("TEXT", "valIncr", "req", "S");
		...
		asignarAtributo("TEXT", "valIncr", "validacion", "mensaje = ValidaInt(cdos_trim(valor), 4, 0, 9999);");
	...
	protected void cmdUpdateForm() throws Exception{
		...
		//asignarAtributo("FORMULARIO", "preConfiGpFrm", "oculto", "N");
		...
		//Construimos el mso con los elementos de la búsqueda
		PreConfiGpData preConfiGpQuery= new PreConfiGpData();
		preConfiGpQuery.setId(new Long(id));
		...
		//asignar("VAR","timestamp",Long.toString(preConfiGp.jdoGetTimeStamp()));
	...
	*/
	protected void cambiarAcciones() throws Exception{
				asignarAtributo("AREATEXTO", "id", "ontab","ejecutarAccionFocoModificacion(\"preConfiGpFrm\",1, true)");
				asignarAtributo("AREATEXTO", "id", "onshtab","ejecutarAccionFocoModificacion(\"preConfiGpFrm\",1, false)");
					asignarAtributo("COMBO","paisOidPais", "ontab", "ejecutarAccionFocoModificacion(\"preConfiGpFrm\",2, true)");
					asignarAtributo("COMBO","paisOidPais", "onshtab", "ejecutarAccionFocoModificacion(\"preConfiGpFrm\",2, false)");
					asignarAtributo("COMBO","marcOidMarc", "ontab", "ejecutarAccionFocoModificacion(\"preConfiGpFrm\",3, true)");
					asignarAtributo("COMBO","marcOidMarc", "onshtab", "ejecutarAccionFocoModificacion(\"preConfiGpFrm\",3, false)");
					asignarAtributo("COMBO","canaOidCana", "ontab", "ejecutarAccionFocoModificacion(\"preConfiGpFrm\",4, true)");
					asignarAtributo("COMBO","canaOidCana", "onshtab", "ejecutarAccionFocoModificacion(\"preConfiGpFrm\",4, false)");
				asignarAtributo("AREATEXTO", "valIncr", "ontab","ejecutarAccionFocoModificacion(\"preConfiGpFrm\",5, true)");
				asignarAtributo("AREATEXTO", "valIncr", "onshtab","ejecutarAccionFocoModificacion(\"preConfiGpFrm\",5, false)");
	}
	...
	protected void removeLovElements() throws Exception{
		borrarNodo("TD","nombre","preConfiGpTdLovNullSelectionButton");
	}
	
	protected void removeQueryElements() throws Exception{
		//Borramos los botones para realizar la búsuqeda
		borrarNodo("TR","nombre","preConfiGpTrButtons");
	...
	*/
	protected void desactivateNonModifyFields() throws Exception{
			traza("MMG: en desactivateNonModifyFields ");		
				...
				traza("MMG: el atributo es valIncr...");		
	...
	*/
	protected void establecerSizeCapas(String accion, String origen) throws Exception{
		...
		asignarAtributo("CAPA","preConfiGpFormLayer", "alto", String.valueOf(sizeForm));		
		
		//Estabelcemos la posicion y de la capa correspondiente a la lista y de su botonera
		asignarAtributo("CAPA","preConfiGpListLayer", "y", String.valueOf(sizeForm + 26));		
		asignarAtributo("CAPA","preConfiGpListButtonsLayer", "y", String.valueOf(sizeForm + 278));		
	}

	/**
	* Nos permite cargar las combos finales de la jerarquías de dependencia
	*/
	protected void loadLeafDependenceCombos(PreConfiGpData preConfiGp) throws Exception{
		...
		if(preConfiGp != null && preConfiGp.getPaisOidPais() != null){
				checkCombo("paisOidPais" , preConfiGp.getPaisOidPais().getId().toString());
			}
		...
		if(preConfiGp != null && preConfiGp.getMarcOidMarc() != null){
				checkCombo("marcOidMarc" , preConfiGp.getMarcOidMarc().getId().toString());
			}
		...
		if(preConfiGp != null && preConfiGp.getCanaOidCana() != null){
				checkCombo("canaOidCana" , preConfiGp.getCanaOidCana().getId().toString());
			}
	...
	* @param preConfiGp La el objetopar la cual se va a realizar la carga
	*/
	protected void loadNonLeafDependenceCombos(PreConfiGpData preConfiGp) 
		throws Exception{
	...
	*/
	protected void loadNonLeafDependenceCombosForRequery() throws Exception{
		...
		PreConfiGpData preConfiGp = 
			(PreConfiGpData) conectorParametroSesion(SESSION_ATTRIBUTE_LAST_QUERY_OBJ_FROM);
		
		//Cargamos todos los objetos completos correspodnientes a los valores de los
		//Campos que sean raiz de una jerarquía de combos dependientes
		if(preConfiGp.getPaisOidPais() != null){
			preConfiGp.setPaisOidPais((es.indra.belcorp.mso.SegPaisViewData)
			getSingleObject(BUSINESSID_PAISOIDPAIS_QUERY, 
			preConfiGp.getPaisOidPais(),
			preConfiGp.getPaisOidPais().clone(),
			"segPaisView"));
		}
		if(preConfiGp.getMarcOidMarc() != null){
			preConfiGp.setMarcOidMarc((es.indra.belcorp.mso.SegMarcaData)
			getSingleObject(BUSINESSID_MARCOIDMARC_QUERY, 
			preConfiGp.getMarcOidMarc(),
			preConfiGp.getMarcOidMarc().clone(),
			"segMarca"));
		}
		if(preConfiGp.getCanaOidCana() != null){
			preConfiGp.setCanaOidCana((es.indra.belcorp.mso.SegCanalViewData)
			getSingleObject(BUSINESSID_CANAOIDCANA_QUERY, 
			preConfiGp.getCanaOidCana(),
			preConfiGp.getCanaOidCana().clone(),
			"segCanalView"));
		}
		
		//Una vez que tenemos completo los datos de los atributos raices de jerarquías de dependencias
		//cargamos las combos y seleccionamos elementos
		loadNonLeafDependenceCombos(preConfiGp);
		loadLeafDependenceCombos(preConfiGp);
	}
