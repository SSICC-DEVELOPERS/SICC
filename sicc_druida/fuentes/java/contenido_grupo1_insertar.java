
import org.w3c.dom.*;
import java.util.ArrayList;

public class contenido_grupo1_insertar  implements es.indra.druida.base.ObjetoXML {
   private ArrayList v = new ArrayList();

   public Element getXML (Document doc){
      

      getXML0(doc);
         

      getXML90(doc);
         

      getXML180(doc);
         

      getXML270(doc);
         

      getXML360(doc);
         

      getXML450(doc);
         

      getXML540(doc);
         

      getXML630(doc);
         

      getXML720(doc);
         

      getXML810(doc);
         
      return (Element)v.get(0);
      
   }

   
/* Primer nodo */
   

   private void getXML0(Document doc) {
      v.add(doc.createElement("PAGINA"));
      ((Element)v.get(0)).setAttribute("nombre","contenido_grupo1_insertar" );
      ((Element)v.get(0)).setAttribute("cod","0309" );
      ((Element)v.get(0)).setAttribute("titulo","Insertar Clientes" );
      ((Element)v.get(0)).setAttribute("estilos","estilosB3.css" );
      ((Element)v.get(0)).setAttribute("colorf","#F0F0F0" );
      ((Element)v.get(0)).setAttribute("msgle","Insertar Clientes" );
      ((Element)v.get(0)).setAttribute("onload","onLoadPag()" );
      ((Element)v.get(0)).setAttribute("xml:lang","es" );
      ((Element)v.get(0)).setAttribute("repeat","N" );

      /* Empieza nodo:1 / Elemento padre: 0   */
      v.add(doc.createElement("JS"));
      ((Element)v.get(1)).setAttribute("src","sicc_util.js" );
      ((Element)v.get(0)).appendChild((Element)v.get(1));
      /* Termina nodo:1   */

      /* Empieza nodo:2 / Elemento padre: 0   */
      v.add(doc.createElement("JS"));
      ((Element)v.get(2)).setAttribute("src","menu_basicas.js" );
      ((Element)v.get(0)).appendChild((Element)v.get(2));
      /* Termina nodo:2   */

      /* Empieza nodo:3 / Elemento padre: 0   */
      v.add(doc.createElement("JS"));
      ((Element)v.get(3)).setAttribute("src","scripts_clientes.js" );
      ((Element)v.get(0)).appendChild((Element)v.get(3));
      /* Termina nodo:3   */

      /* Empieza nodo:4 / Elemento padre: 0   */
      v.add(doc.createElement("JAVASCRIPT"));
      ((Element)v.get(0)).appendChild((Element)v.get(4));

      /* Elemento padre:4 / Elemento actual: 5   */
      v.add(doc.createTextNode("\r           \r\r                  \r         var formName = 'frmInsertarCliente';\rfunction onLoadPag(){\r         if(!fMostrarMensajeError() ){\r                  configurarMenuSecundario(\"frmInsertarCliente\");\r                  DrdEnsanchaConMargenDcho('listado1',12);	\r                  eval (ON_RSZ); \r                                    cargaComboPersonaEmpresa();\r         \r                                    var div=document.getElementById(\"InsertarP1\");\r             div.style.setAttribute(\"backgroundColor\",\"#496A9A\");\r             div.style.setAttribute(\"color\",\"#FFFFFF\");\r\r                  var datosP1 = get(formName + \".hidListaIdentificacion\");\r                  if(datosP1==\"\"){\r                                                                        }else{\r                                                                                                                                       recargarValores(datosP1);\r                  }\r\r         }                  document.getElementById(\"comboTipoDocId_0\").setActive();\r}\r\r         /*******************************************************************/\rfunction cargarComboPersonaEmpresa(){\r                           var opciones = new Array();\r         opciones[0]=['-1',\"\"];\r         opciones[1]=['P',GestionarMensaje('1505',null,null,null)];\r         opciones[2]=['E',GestionarMensaje('1506',null,null,null)];\r         \r         for(var i=0;i<listado1.datos.length;i++){                  var elCombo =  document.getElementById(\"comboPersonaEmpresa_\" + i);\r                  var optis = elCombo.options;\r             var option =document.createElement(\"OPTION\");\r             optis.add(option);\r             option.innerText=opciones[0][1];\r             option.value= opciones[0][0];\r                  var option2 =document.createElement(\"OPTION\");\r             optis.add(option2);\r             option2.innerText=opciones[1][1];\r             option2.value= opciones[1][0];\r                  var option2 =document.createElement(\"OPTION\");\r             optis.add(option2);\r             option2.innerText=opciones[2][1];\r             option2.value= opciones[2][0];\r         }	\r}\rfunction cambioDocumento(){\r                  var docu=window.event.srcElement;\r         var docuname=window.event.srcElement.getAttribute(\"NAME\");\r         var temparray =  docuname.split(\"_\");\r\r         var princIndex = get(formName + \".ultimoChequeado\");\r         if(temparray[1] == princIndex){\r                                    actualizaCodigoCliente(docu.value);\r         }\r}\r\rfunction actualizaCodigoCliente(newCod){\r         var codigoClienteAutomatico = get(formName + \".hidCodigoAutomatico\");\r         if(codigoClienteAutomatico == \"false\"){\r                  var labCodClie = document.getElementById(\"lblCodClienteActual\");\r                  txt_to('lblCodClienteActual',newCod);	\r         }\r}\r\rfunction flagRadio(){\r                  var radio=window.event.srcElement;\r         var radioname=window.event.srcElement.getAttribute(\"NAME\");\r         var temparray =  radioname.split(\"_\");\r\r         if(get(formName + \".ultimoChequeado\")!=temparray[1]){                                    var tabla=document.getElementById(\"tblDatlistado1\");\r                  var td2 = tabla.all.tags(\"INPUT\");\r                  for(var i=0;i<td2.length;i++){                           if(td2.item(i).name.substring(0,3)==\"RbD\"){\r                                    td2.item(i).checked=false;\r                           }\r                  }	\r                                    radio.checked=true;\r         \r                  var docText = document.getElementById(\"TextoNDoc_\"+ Math.abs(temparray[1]) );\r                  actualizaCodigoCliente(docText.value);\r                  \r                  set(formName + \".ultimoChequeado\", temparray[1]);\r         }}\rfunction cargaComboPersonaEmpresa()  {\r   var lacolumna=4;   var elcontenido=[ ['-1',''],['P',GestionarMensaje('1505',null,null,null)] , ['E',GestionarMensaje('1506',null,null,null)] ];\r\r   listado1.tiposCol[lacolumna][2]=elcontenido;\r   listado1.repinta();\r   }\rfunction accionInsertarP2(){\r         \r         if(accionValidar()){\r                  var datosStr = recolectarValores();\r                  var ind = \"indices_iden=\";\r                  for(var c=0;c<listado1.datos.length;c++){\r                           if(tieneDatosLinea(c)){\r                                    if(c!=0){\r                                             ind = ind + \",\";\r                                    }\r\r                                    ind = ind + c;\r                           }\r                  }\r                  ind = ind + \";\";\r                  datosStr = ind + datosStr ;\r                                                      set('frmInsertarCliente.hidListaIdentificacion', datosStr );\r                                    var labCodClie = document.getElementById(\"lblCodClienteActual\");\r                  set('frmInsertarCliente.hidtextCodCliente', labCodClie.innerText);\r                  set('frmInsertarCliente.accion','Validar');\r                  if(get('frmInsertarCliente.casoDeUso')=='ModificarCliente'){\r                           set('frmInsertarCliente.conectorAction','LPModificarCliente');\r                  }else{\r                           set('frmInsertarCliente.conectorAction','LPInsertarCliente');\r                  }\r                     enviaSICC('frmInsertarCliente');\r         }\r}\r\rfunction accionValidar(){\r                  \r                                    var hayDirPrincipal = false;\r         var tabla=document.getElementById(\"tblDatlistado1\");\r         var td2 = tabla.all.tags(\"INPUT\");\r         for(var i=0;i<td2.length;i++){                  if(td2.item(i).name.substring(0,3)==\"RbD\"){\r                           if(td2.item(i).checked==true){\r                                    hayDirPrincipal = true;\r                                    break;\r                           }\r                  }\r         }	\r         if(!hayDirPrincipal){\r                  alert('MAE0004 (Debe seleccionar una de las direcciones como dirección principal) ');\r                  return false;\r         }\r                           for(var i=0;i<listado1.datos.length;i++){\r                  var docu = document.getElementById(\"TextoNDoc_\" + i);\r                  var docuconfi = document.getElementById(\"TextoConfirmacionDoc_\" + i);\r                  if(docu.value!=docuconfi.value){\r                           alert('MAE-0008 (El número de documento debe coincidir con la confirmación) ');\r                           docuconfi.setActive();\r                           return false;			\r                  }\r         }\r                                    for(var i=0;i<listado1.datos.length;i++){\r                  if(tieneDatosLinea(i)){\r                                                      if(!validaObligatorios(i)){\r                                    return false;\r                           }\r                  }\r         }\r                                             return true;\r}\rfunction validaObligatorios(index){\r                           var tipoDoc = document.getElementById(\"comboTipoDocId_\" + index);\r         var numDoc = document.getElementById(\"TextoNDoc_\" + index);\r         var numDocConf = document.getElementById(\"TextoConfirmacionDoc_\" + index);\r         var princip = document.getElementById(\"RbDocPrincipal_\" + index);\r         var tipoPersona = document.getElementById(\"comboPersonaEmpresa_\" + index);\r         if(tipoDoc.value==-1){\r                  alert(\"MAE-0009 (Debe introducir todos los campos requeridos para el tipo y subtipo de cliente elegidos)tipoDoc\");\r                  tipoDoc.setActive();\r                  return false;\r         }\r         if(numDoc.value==\"\"){\r                  alert(\"MAE-0009 (Debe introducir todos los campos requeridos para el tipo y subtipo de cliente elegidos)numDoc\");\r                  numDoc.setActive();\r                  return false;\r         }	\r         if(numDocConf.value==\"\"){\r                  alert(\"MAE-0009 (Debe introducir todos los campos requeridos para el tipo y subtipo de cliente elegidos)numDocConf\");\r                  numDocConf.setActive();\r                  return false;\r         }\r         if(tipoPersona.value==-1){\r                  alert(\"MAE-0009 (Debe introducir todos los campos requeridos para el tipo y subtipo de cliente elegidos)tipoPersona\");\r                  tipoPersona.setActive();\r                  return false;\r         }\r         return true;\r}\rfunction tieneDatosLinea(index){\r                           var tipoDoc = document.getElementById(\"comboTipoDocId_\" + index);\r         var numDoc = document.getElementById(\"TextoNDoc_\" + index);\r         var numDocConf = document.getElementById(\"TextoConfirmacionDoc_\" + index);\r         var princip = document.getElementById(\"RbDocPrincipal_\" + index);\r         var tipoPersona = document.getElementById(\"comboPersonaEmpresa_\" + index);\r\r         if(tipoDoc.value!=-1){\r                  return true;\r         }\r         if(numDoc.value!=\"\"){\r                  return true;\r         }	\r         if(numDocConf.value!=\"\"){\r                  return true;\r         }\r         if(princip.checked){\r                  return true;\r         }\r         if(tipoPersona.value!=-1){\r                  return true;\r         }\r         return false;\r}\r\r/**********************************************************************************************/\r\r   \r  "));
      ((Element)v.get(4)).appendChild((Text)v.get(5));

      /* Termina nodo Texto:5   */
      /* Termina nodo:4   */

      /* Empieza nodo:6 / Elemento padre: 0   */
      v.add(doc.createElement("FORMULARIO"));
      ((Element)v.get(6)).setAttribute("nombre","frmInsertarCliente" );
      ((Element)v.get(0)).appendChild((Element)v.get(6));

      /* Empieza nodo:7 / Elemento padre: 6   */
      v.add(doc.createElement("VAR"));
      ((Element)v.get(7)).setAttribute("nombre","accion" );
      ((Element)v.get(7)).setAttribute("valor","" );
      ((Element)v.get(6)).appendChild((Element)v.get(7));
      /* Termina nodo:7   */

      /* Empieza nodo:8 / Elemento padre: 6   */
      v.add(doc.createElement("VAR"));
      ((Element)v.get(8)).setAttribute("nombre","conectorAction" );
      ((Element)v.get(8)).setAttribute("valor","" );
      ((Element)v.get(6)).appendChild((Element)v.get(8));
      /* Termina nodo:8   */

      /* Empieza nodo:9 / Elemento padre: 6   */
      v.add(doc.createElement("VAR"));
      ((Element)v.get(9)).setAttribute("nombre","casoDeUso" );
      ((Element)v.get(9)).setAttribute("valor","" );
      ((Element)v.get(6)).appendChild((Element)v.get(9));
      /* Termina nodo:9   */

      /* Empieza nodo:10 / Elemento padre: 6   */
      v.add(doc.createElement("VAR"));
      ((Element)v.get(10)).setAttribute("nombre","nameComboSelected" );
      ((Element)v.get(10)).setAttribute("valor","" );
      ((Element)v.get(6)).appendChild((Element)v.get(10));
      /* Termina nodo:10   */

      /* Empieza nodo:11 / Elemento padre: 6   */
      v.add(doc.createElement("VAR"));
      ((Element)v.get(11)).setAttribute("nombre","valueComboSelected" );
      ((Element)v.get(11)).setAttribute("valor","" );
      ((Element)v.get(6)).appendChild((Element)v.get(11));
      /* Termina nodo:11   */

      /* Empieza nodo:12 / Elemento padre: 6   */
      v.add(doc.createElement("VAR"));
      ((Element)v.get(12)).setAttribute("nombre","errCodigo" );
      ((Element)v.get(12)).setAttribute("valor","" );
      ((Element)v.get(6)).appendChild((Element)v.get(12));
      /* Termina nodo:12   */

      /* Empieza nodo:13 / Elemento padre: 6   */
      v.add(doc.createElement("VAR"));
      ((Element)v.get(13)).setAttribute("nombre","errDescripcion" );
      ((Element)v.get(13)).setAttribute("valor","" );
      ((Element)v.get(6)).appendChild((Element)v.get(13));
      /* Termina nodo:13   */

      /* Empieza nodo:14 / Elemento padre: 6   */
      v.add(doc.createElement("VAR"));
      ((Element)v.get(14)).setAttribute("nombre","ultimoChequeado" );
      ((Element)v.get(14)).setAttribute("valor","" );
      ((Element)v.get(6)).appendChild((Element)v.get(14));
      /* Termina nodo:14   */

      /* Empieza nodo:15 / Elemento padre: 6   */
      v.add(doc.createElement("VAR"));
      ((Element)v.get(15)).setAttribute("nombre","hFormatoFechaPais" );
      ((Element)v.get(15)).setAttribute("valor","" );
      ((Element)v.get(6)).appendChild((Element)v.get(15));
      /* Termina nodo:15   */

      /* Empieza nodo:16 / Elemento padre: 6   */
      v.add(doc.createElement("VAR"));
      ((Element)v.get(16)).setAttribute("nombre","elementos" );
      ((Element)v.get(16)).setAttribute("valor","" );
      ((Element)v.get(6)).appendChild((Element)v.get(16));
      /* Termina nodo:16   */

      /* Empieza nodo:17 / Elemento padre: 6   */
      v.add(doc.createElement("VAR"));
      ((Element)v.get(17)).setAttribute("nombre","cbTipo" );
      ((Element)v.get(17)).setAttribute("valor","" );
      ((Element)v.get(6)).appendChild((Element)v.get(17));
      /* Termina nodo:17   */

      /* Empieza nodo:18 / Elemento padre: 6   */
      v.add(doc.createElement("VAR"));
      ((Element)v.get(18)).setAttribute("nombre","cbSTipo" );
      ((Element)v.get(18)).setAttribute("valor","" );
      ((Element)v.get(6)).appendChild((Element)v.get(18));
      /* Termina nodo:18   */

      /* Empieza nodo:19 / Elemento padre: 6   */
      v.add(doc.createElement("VAR"));
      ((Element)v.get(19)).setAttribute("nombre","hidCodigoAutomatico" );
      ((Element)v.get(19)).setAttribute("valor","" );
      ((Element)v.get(6)).appendChild((Element)v.get(19));
      /* Termina nodo:19   */

      /* Empieza nodo:20 / Elemento padre: 6   */
      v.add(doc.createElement("VAR"));
      ((Element)v.get(20)).setAttribute("nombre","hidDigitoControl" );
      ((Element)v.get(20)).setAttribute("valor","" );
      ((Element)v.get(6)).appendChild((Element)v.get(20));
      /* Termina nodo:20   */

      /* Empieza nodo:21 / Elemento padre: 6   */
      v.add(doc.createElement("VAR"));
      ((Element)v.get(21)).setAttribute("nombre","hidtextCodCliente" );
      ((Element)v.get(21)).setAttribute("valor","" );
      ((Element)v.get(6)).appendChild((Element)v.get(21));
      /* Termina nodo:21   */

      /* Empieza nodo:22 / Elemento padre: 6   */
      v.add(doc.createElement("VAR"));
      ((Element)v.get(22)).setAttribute("nombre","hidListaIdentificacion" );
      ((Element)v.get(22)).setAttribute("valor","" );
      ((Element)v.get(6)).appendChild((Element)v.get(22));
      /* Termina nodo:22   */

      /* Empieza nodo:23 / Elemento padre: 6   */
   }

   private void getXML90(Document doc) {
      v.add(doc.createElement("VAR"));
      ((Element)v.get(23)).setAttribute("nombre","elementosP2" );
      ((Element)v.get(23)).setAttribute("valor","" );
      ((Element)v.get(6)).appendChild((Element)v.get(23));
      /* Termina nodo:23   */

      /* Empieza nodo:24 / Elemento padre: 6   */
      v.add(doc.createElement("VAR"));
      ((Element)v.get(24)).setAttribute("nombre","hidFechaIngreso" );
      ((Element)v.get(24)).setAttribute("valor","" );
      ((Element)v.get(6)).appendChild((Element)v.get(24));
      /* Termina nodo:24   */

      /* Empieza nodo:25 / Elemento padre: 6   */
      v.add(doc.createElement("VAR"));
      ((Element)v.get(25)).setAttribute("nombre","hidtextApellido1" );
      ((Element)v.get(25)).setAttribute("valor","" );
      ((Element)v.get(6)).appendChild((Element)v.get(25));
      /* Termina nodo:25   */

      /* Empieza nodo:26 / Elemento padre: 6   */
      v.add(doc.createElement("VAR"));
      ((Element)v.get(26)).setAttribute("nombre","hidtextApellido2" );
      ((Element)v.get(26)).setAttribute("valor","" );
      ((Element)v.get(6)).appendChild((Element)v.get(26));
      /* Termina nodo:26   */

      /* Empieza nodo:27 / Elemento padre: 6   */
      v.add(doc.createElement("VAR"));
      ((Element)v.get(27)).setAttribute("nombre","hidtextApellidoCasada" );
      ((Element)v.get(27)).setAttribute("valor","" );
      ((Element)v.get(6)).appendChild((Element)v.get(27));
      /* Termina nodo:27   */

      /* Empieza nodo:28 / Elemento padre: 6   */
      v.add(doc.createElement("VAR"));
      ((Element)v.get(28)).setAttribute("nombre","hidtextNombre1" );
      ((Element)v.get(28)).setAttribute("valor","" );
      ((Element)v.get(6)).appendChild((Element)v.get(28));
      /* Termina nodo:28   */

      /* Empieza nodo:29 / Elemento padre: 6   */
      v.add(doc.createElement("VAR"));
      ((Element)v.get(29)).setAttribute("nombre","hidtextNombre2" );
      ((Element)v.get(29)).setAttribute("valor","" );
      ((Element)v.get(6)).appendChild((Element)v.get(29));
      /* Termina nodo:29   */

      /* Empieza nodo:30 / Elemento padre: 6   */
      v.add(doc.createElement("VAR"));
      ((Element)v.get(30)).setAttribute("nombre","hidcbTratamiento" );
      ((Element)v.get(30)).setAttribute("valor","" );
      ((Element)v.get(6)).appendChild((Element)v.get(30));
      /* Termina nodo:30   */

      /* Empieza nodo:31 / Elemento padre: 6   */
      v.add(doc.createElement("VAR"));
      ((Element)v.get(31)).setAttribute("nombre","hidrbSexo" );
      ((Element)v.get(31)).setAttribute("valor","" );
      ((Element)v.get(6)).appendChild((Element)v.get(31));
      /* Termina nodo:31   */

      /* Empieza nodo:32 / Elemento padre: 6   */
      v.add(doc.createElement("VAR"));
      ((Element)v.get(32)).setAttribute("nombre","hidcbFormaPago" );
      ((Element)v.get(32)).setAttribute("valor","" );
      ((Element)v.get(6)).appendChild((Element)v.get(32));
      /* Termina nodo:32   */

      /* Empieza nodo:33 / Elemento padre: 6   */
      v.add(doc.createElement("VAR"));
      ((Element)v.get(33)).setAttribute("nombre","hidtextFechaNacimiento" );
      ((Element)v.get(33)).setAttribute("valor","" );
      ((Element)v.get(6)).appendChild((Element)v.get(33));
      /* Termina nodo:33   */

      /* Empieza nodo:34 / Elemento padre: 6   */
      v.add(doc.createElement("VAR"));
      ((Element)v.get(34)).setAttribute("nombre","hidEdad" );
      ((Element)v.get(34)).setAttribute("valor","" );
      ((Element)v.get(6)).appendChild((Element)v.get(34));
      /* Termina nodo:34   */

      /* Empieza nodo:35 / Elemento padre: 6   */
      v.add(doc.createElement("VAR"));
      ((Element)v.get(35)).setAttribute("nombre","hidtextCodEmpleado" );
      ((Element)v.get(35)).setAttribute("valor","" );
      ((Element)v.get(6)).appendChild((Element)v.get(35));
      /* Termina nodo:35   */

      /* Empieza nodo:36 / Elemento padre: 6   */
      v.add(doc.createElement("VAR"));
      ((Element)v.get(36)).setAttribute("nombre","hidcbNacionalidad" );
      ((Element)v.get(36)).setAttribute("valor","" );
      ((Element)v.get(6)).appendChild((Element)v.get(36));
      /* Termina nodo:36   */

      /* Empieza nodo:37 / Elemento padre: 6   */
      v.add(doc.createElement("VAR"));
      ((Element)v.get(37)).setAttribute("nombre","hidcbEstadoCivil" );
      ((Element)v.get(37)).setAttribute("valor","" );
      ((Element)v.get(6)).appendChild((Element)v.get(37));
      /* Termina nodo:37   */

      /* Empieza nodo:38 / Elemento padre: 6   */
      v.add(doc.createElement("VAR"));
      ((Element)v.get(38)).setAttribute("nombre","hidtextOcupacion" );
      ((Element)v.get(38)).setAttribute("valor","" );
      ((Element)v.get(6)).appendChild((Element)v.get(38));
      /* Termina nodo:38   */

      /* Empieza nodo:39 / Elemento padre: 6   */
      v.add(doc.createElement("VAR"));
      ((Element)v.get(39)).setAttribute("nombre","hidtextProfesion" );
      ((Element)v.get(39)).setAttribute("valor","" );
      ((Element)v.get(6)).appendChild((Element)v.get(39));
      /* Termina nodo:39   */

      /* Empieza nodo:40 / Elemento padre: 6   */
      v.add(doc.createElement("VAR"));
      ((Element)v.get(40)).setAttribute("nombre","hidtextCentroTrabajo" );
      ((Element)v.get(40)).setAttribute("valor","" );
      ((Element)v.get(6)).appendChild((Element)v.get(40));
      /* Termina nodo:40   */

      /* Empieza nodo:41 / Elemento padre: 6   */
      v.add(doc.createElement("VAR"));
      ((Element)v.get(41)).setAttribute("nombre","hidtextCargoDesempena" );
      ((Element)v.get(41)).setAttribute("valor","" );
      ((Element)v.get(6)).appendChild((Element)v.get(41));
      /* Termina nodo:41   */

      /* Empieza nodo:42 / Elemento padre: 6   */
      v.add(doc.createElement("VAR"));
      ((Element)v.get(42)).setAttribute("nombre","hidcbNivelEstudios" );
      ((Element)v.get(42)).setAttribute("valor","" );
      ((Element)v.get(6)).appendChild((Element)v.get(42));
      /* Termina nodo:42   */

      /* Empieza nodo:43 / Elemento padre: 6   */
      v.add(doc.createElement("VAR"));
      ((Element)v.get(43)).setAttribute("nombre","hidtextCentroEstudios" );
      ((Element)v.get(43)).setAttribute("valor","" );
      ((Element)v.get(6)).appendChild((Element)v.get(43));
      /* Termina nodo:43   */

      /* Empieza nodo:44 / Elemento padre: 6   */
      v.add(doc.createElement("VAR"));
      ((Element)v.get(44)).setAttribute("nombre","hidtextNHijos" );
      ((Element)v.get(44)).setAttribute("valor","" );
      ((Element)v.get(6)).appendChild((Element)v.get(44));
      /* Termina nodo:44   */

      /* Empieza nodo:45 / Elemento padre: 6   */
      v.add(doc.createElement("VAR"));
      ((Element)v.get(45)).setAttribute("nombre","hidtextNPersonasDependientes" );
   }

   private void getXML180(Document doc) {
      ((Element)v.get(45)).setAttribute("valor","" );
      ((Element)v.get(6)).appendChild((Element)v.get(45));
      /* Termina nodo:45   */

      /* Empieza nodo:46 / Elemento padre: 6   */
      v.add(doc.createElement("VAR"));
      ((Element)v.get(46)).setAttribute("nombre","hidcbNivelSocioEconomico" );
      ((Element)v.get(46)).setAttribute("valor","" );
      ((Element)v.get(6)).appendChild((Element)v.get(46));
      /* Termina nodo:46   */

      /* Empieza nodo:47 / Elemento padre: 6   */
      v.add(doc.createElement("VAR"));
      ((Element)v.get(47)).setAttribute("nombre","hidcbCicloVidaFamiliar" );
      ((Element)v.get(47)).setAttribute("valor","" );
      ((Element)v.get(6)).appendChild((Element)v.get(47));
      /* Termina nodo:47   */

      /* Empieza nodo:48 / Elemento padre: 6   */
      v.add(doc.createElement("VAR"));
      ((Element)v.get(48)).setAttribute("nombre","hidrbDeseaCorrespondencia" );
      ((Element)v.get(48)).setAttribute("valor","" );
      ((Element)v.get(6)).appendChild((Element)v.get(48));
      /* Termina nodo:48   */

      /* Empieza nodo:49 / Elemento padre: 6   */
      v.add(doc.createElement("VAR"));
      ((Element)v.get(49)).setAttribute("nombre","hidtextImporteIngresoFamiliar" );
      ((Element)v.get(49)).setAttribute("valor","" );
      ((Element)v.get(6)).appendChild((Element)v.get(49));
      /* Termina nodo:49   */

      /* Empieza nodo:50 / Elemento padre: 6   */
      v.add(doc.createElement("VAR"));
      ((Element)v.get(50)).setAttribute("nombre","hidcbPaisContactado" );
      ((Element)v.get(50)).setAttribute("valor","" );
      ((Element)v.get(6)).appendChild((Element)v.get(50));
      /* Termina nodo:50   */

      /* Empieza nodo:51 / Elemento padre: 6   */
      v.add(doc.createElement("VAR"));
      ((Element)v.get(51)).setAttribute("nombre","hidtextCodClienteContactado" );
      ((Element)v.get(51)).setAttribute("valor","" );
      ((Element)v.get(6)).appendChild((Element)v.get(51));
      /* Termina nodo:51   */

      /* Empieza nodo:52 / Elemento padre: 6   */
      v.add(doc.createElement("VAR"));
      ((Element)v.get(52)).setAttribute("nombre","hidcbTipoClienteContactado" );
      ((Element)v.get(52)).setAttribute("valor","" );
      ((Element)v.get(6)).appendChild((Element)v.get(52));
      /* Termina nodo:52   */

      /* Empieza nodo:53 / Elemento padre: 6   */
      v.add(doc.createElement("VAR"));
      ((Element)v.get(53)).setAttribute("nombre","hidcbCodTipoContactado" );
      ((Element)v.get(53)).setAttribute("valor","" );
      ((Element)v.get(6)).appendChild((Element)v.get(53));
      /* Termina nodo:53   */

      /* Empieza nodo:54 / Elemento padre: 6   */
      v.add(doc.createElement("VAR"));
      ((Element)v.get(54)).setAttribute("nombre","hidtextFechaContacto" );
      ((Element)v.get(54)).setAttribute("valor","" );
      ((Element)v.get(6)).appendChild((Element)v.get(54));
      /* Termina nodo:54   */

      /* Empieza nodo:55 / Elemento padre: 6   */
      v.add(doc.createElement("VAR"));
      ((Element)v.get(55)).setAttribute("nombre","hidtextFecha1PedidoContacto" );
      ((Element)v.get(55)).setAttribute("valor","" );
      ((Element)v.get(6)).appendChild((Element)v.get(55));
      /* Termina nodo:55   */

      /* Empieza nodo:56 / Elemento padre: 6   */
      v.add(doc.createElement("VAR"));
      ((Element)v.get(56)).setAttribute("nombre","hidtextFechaSiguienteContacto" );
      ((Element)v.get(56)).setAttribute("valor","" );
      ((Element)v.get(6)).appendChild((Element)v.get(56));
      /* Termina nodo:56   */

      /* Empieza nodo:57 / Elemento padre: 6   */
      v.add(doc.createElement("VAR"));
      ((Element)v.get(57)).setAttribute("nombre","hidtxtFechaIngresoActual" );
      ((Element)v.get(57)).setAttribute("valor","" );
      ((Element)v.get(6)).appendChild((Element)v.get(57));
      /* Termina nodo:57   */

      /* Empieza nodo:58 / Elemento padre: 6   */
      v.add(doc.createElement("VAR"));
      ((Element)v.get(58)).setAttribute("nombre","hidListaVinculo" );
      ((Element)v.get(58)).setAttribute("valor","" );
      ((Element)v.get(6)).appendChild((Element)v.get(58));
      /* Termina nodo:58   */

      /* Empieza nodo:59 / Elemento padre: 6   */
      v.add(doc.createElement("VAR"));
      ((Element)v.get(59)).setAttribute("nombre","hidListaObservacion" );
      ((Element)v.get(59)).setAttribute("valor","" );
      ((Element)v.get(6)).appendChild((Element)v.get(59));
      /* Termina nodo:59   */

      /* Empieza nodo:60 / Elemento padre: 6   */
      v.add(doc.createElement("VAR"));
      ((Element)v.get(60)).setAttribute("nombre","hidListaPrefe" );
      ((Element)v.get(60)).setAttribute("valor","" );
      ((Element)v.get(6)).appendChild((Element)v.get(60));
      /* Termina nodo:60   */

      /* Empieza nodo:61 / Elemento padre: 6   */
      v.add(doc.createElement("VAR"));
      ((Element)v.get(61)).setAttribute("nombre","hOidCliente" );
      ((Element)v.get(61)).setAttribute("valor","" );
      ((Element)v.get(6)).appendChild((Element)v.get(61));
      /* Termina nodo:61   */

      /* Empieza nodo:62 / Elemento padre: 6   */
      v.add(doc.createElement("VAR"));
      ((Element)v.get(62)).setAttribute("nombre","hidMarcaContacto" );
      ((Element)v.get(62)).setAttribute("valor","" );
      ((Element)v.get(6)).appendChild((Element)v.get(62));
      /* Termina nodo:62   */

      /* Empieza nodo:63 / Elemento padre: 6   */
      v.add(doc.createElement("VAR"));
      ((Element)v.get(63)).setAttribute("nombre","hidCanalContacto" );
      ((Element)v.get(63)).setAttribute("valor","" );
      ((Element)v.get(6)).appendChild((Element)v.get(63));
      /* Termina nodo:63   */

      /* Empieza nodo:64 / Elemento padre: 6   */
      v.add(doc.createElement("VAR"));
      ((Element)v.get(64)).setAttribute("nombre","hidPeriodoContacto" );
      ((Element)v.get(64)).setAttribute("valor","" );
      ((Element)v.get(6)).appendChild((Element)v.get(64));
      /* Termina nodo:64   */

      /* Empieza nodo:65 / Elemento padre: 6   */
      v.add(doc.createElement("VAR"));
      ((Element)v.get(65)).setAttribute("nombre","elementosP3" );
      ((Element)v.get(65)).setAttribute("valor","" );
      ((Element)v.get(6)).appendChild((Element)v.get(65));
      /* Termina nodo:65   */

      /* Empieza nodo:66 / Elemento padre: 6   */
      v.add(doc.createElement("VAR"));
      ((Element)v.get(66)).setAttribute("nombre","hidListaDirecciones" );
      ((Element)v.get(66)).setAttribute("valor","" );
      ((Element)v.get(6)).appendChild((Element)v.get(66));
      /* Termina nodo:66   */

      /* Empieza nodo:67 / Elemento padre: 6   */
      v.add(doc.createElement("VAR"));
      ((Element)v.get(67)).setAttribute("nombre","hidListaComunicacion" );
      ((Element)v.get(67)).setAttribute("valor","" );
      ((Element)v.get(6)).appendChild((Element)v.get(67));
      /* Termina nodo:67   */

      /* Empieza nodo:68 / Elemento padre: 6   */
   }

   private void getXML270(Document doc) {
      v.add(doc.createElement("VAR"));
      ((Element)v.get(68)).setAttribute("nombre","hidListaMarca" );
      ((Element)v.get(68)).setAttribute("valor","" );
      ((Element)v.get(6)).appendChild((Element)v.get(68));
      /* Termina nodo:68   */

      /* Empieza nodo:69 / Elemento padre: 6   */
      v.add(doc.createElement("VAR"));
      ((Element)v.get(69)).setAttribute("nombre","hidMarcaValor" );
      ((Element)v.get(69)).setAttribute("valor","" );
      ((Element)v.get(6)).appendChild((Element)v.get(69));
      /* Termina nodo:69   */

      /* Empieza nodo:70 / Elemento padre: 6   */
      v.add(doc.createElement("VAR"));
      ((Element)v.get(70)).setAttribute("nombre","hidOidClienteCont" );
      ((Element)v.get(70)).setAttribute("valor","" );
      ((Element)v.get(6)).appendChild((Element)v.get(70));
      /* Termina nodo:70   */

      /* Empieza nodo:71 / Elemento padre: 6   */
      v.add(doc.createElement("VAR"));
      ((Element)v.get(71)).setAttribute("nombre","usaGeoreferenciador" );
      ((Element)v.get(71)).setAttribute("valor","" );
      ((Element)v.get(6)).appendChild((Element)v.get(71));
      /* Termina nodo:71   */

      /* Empieza nodo:72 / Elemento padre: 6   */
      v.add(doc.createElement("VAR"));
      ((Element)v.get(72)).setAttribute("nombre","elementosP4" );
      ((Element)v.get(72)).setAttribute("valor","" );
      ((Element)v.get(6)).appendChild((Element)v.get(72));
      /* Termina nodo:72   */

      /* Empieza nodo:73 / Elemento padre: 6   */
      v.add(doc.createElement("VAR"));
      ((Element)v.get(73)).setAttribute("nombre","hidListaTarjetas" );
      ((Element)v.get(73)).setAttribute("valor","" );
      ((Element)v.get(6)).appendChild((Element)v.get(73));
      /* Termina nodo:73   */

      /* Empieza nodo:74 / Elemento padre: 6   */
      v.add(doc.createElement("VAR"));
      ((Element)v.get(74)).setAttribute("nombre","hidListaClasificacion" );
      ((Element)v.get(74)).setAttribute("valor","" );
      ((Element)v.get(6)).appendChild((Element)v.get(74));
      /* Termina nodo:74   */

      /* Empieza nodo:75 / Elemento padre: 6   */
      v.add(doc.createElement("VAR"));
      ((Element)v.get(75)).setAttribute("nombre","hidListaProbSolu" );
      ((Element)v.get(75)).setAttribute("valor","" );
      ((Element)v.get(6)).appendChild((Element)v.get(75));
      /* Termina nodo:75   */

      /* Empieza nodo:76 / Elemento padre: 6   */
      v.add(doc.createElement("DIV"));
      ((Element)v.get(76)).setAttribute("ID","InsertarP1" );
      ((Element)v.get(76)).setAttribute("style","width:80px;border-style:solid; border-color:#496A9A;  border-width:2px;position:absolute;left:630px;top:3px;text-align:center;color:#496A9A; font-family: Arial, Helvetica, sans-serif;font-size: 11px;font-weight: bold; visibility:visible" );
      ((Element)v.get(6)).appendChild((Element)v.get(76));

      /* Elemento padre:76 / Elemento actual: 77   */
      v.add(doc.createTextNode("Pestanya 1"));
      ((Element)v.get(76)).appendChild((Text)v.get(77));

      /* Termina nodo Texto:77   */
      /* Termina nodo:76   */

      /* Empieza nodo:78 / Elemento padre: 6   */
      v.add(doc.createElement("DIV"));
      ((Element)v.get(78)).setAttribute("ID","InsertarP2" );
      ((Element)v.get(78)).setAttribute("style","width:80px;border-style:solid; border-color:#496A9A;border-width:2px;position:absolute;left:720px;top:3px;text-align:center;color:#496A9A; font-family: Arial, Helvetica, sans-serif;font-size: 11px;font-weight: bold; visibility:visible" );
      ((Element)v.get(78)).setAttribute("onmouseover","manejarPestanas()" );
      ((Element)v.get(78)).setAttribute("onmouseout","manejarPestanas2()" );
      ((Element)v.get(78)).setAttribute("onclick","accionInsertarP2()" );
      ((Element)v.get(6)).appendChild((Element)v.get(78));

      /* Elemento padre:78 / Elemento actual: 79   */
      v.add(doc.createTextNode("Pestanya 2"));
      ((Element)v.get(78)).appendChild((Text)v.get(79));

      /* Termina nodo Texto:79   */
      /* Termina nodo:78   */

      /* Empieza nodo:80 / Elemento padre: 6   */
      v.add(doc.createElement("DIV"));
      ((Element)v.get(80)).setAttribute("ID","InsertarP3" );
      ((Element)v.get(80)).setAttribute("style","width:80px;border-style:solid; border-color:#496A9A; border-width:2px;position:absolute;left:810px;top:3px;text-align:center;color:#496A9A;  font-family: Arial, Helvetica, sans-serif;font-size: 11px;font-weight: bold; visibility:visible" );
      ((Element)v.get(6)).appendChild((Element)v.get(80));

      /* Elemento padre:80 / Elemento actual: 81   */
      v.add(doc.createTextNode("Pestanya 3"));
      ((Element)v.get(80)).appendChild((Text)v.get(81));

      /* Termina nodo Texto:81   */
      /* Termina nodo:80   */

      /* Empieza nodo:82 / Elemento padre: 6   */
      v.add(doc.createElement("DIV"));
      ((Element)v.get(82)).setAttribute("ID","InsertarP4" );
      ((Element)v.get(82)).setAttribute("style","width:80px;border-style:solid; border-color:#496A9A; border-width:2px;position:absolute;left:900px;top:3px;text-align:center;color:#496A9A;  font-family: Arial, Helvetica, sans-serif;font-size: 11px;font-weight: bold; visibility:visible" );
      ((Element)v.get(6)).appendChild((Element)v.get(82));

      /* Elemento padre:82 / Elemento actual: 83   */
      v.add(doc.createTextNode("Pestanya 4"));
      ((Element)v.get(82)).appendChild((Text)v.get(83));

      /* Termina nodo Texto:83   */
      /* Termina nodo:82   */

      /* Empieza nodo:84 / Elemento padre: 6   */
      v.add(doc.createElement("table"));
      ((Element)v.get(84)).setAttribute("width","100%" );
      ((Element)v.get(84)).setAttribute("border","0" );
      ((Element)v.get(84)).setAttribute("cellspacing","0" );
      ((Element)v.get(84)).setAttribute("cellpadding","0" );
      ((Element)v.get(6)).appendChild((Element)v.get(84));

      /* Empieza nodo:85 / Elemento padre: 84   */
      v.add(doc.createElement("tr"));
      ((Element)v.get(84)).appendChild((Element)v.get(85));

      /* Empieza nodo:86 / Elemento padre: 85   */
      v.add(doc.createElement("td"));
      ((Element)v.get(86)).setAttribute("width","12" );
      ((Element)v.get(86)).setAttribute("align","center" );
      ((Element)v.get(85)).appendChild((Element)v.get(86));

      /* Empieza nodo:87 / Elemento padre: 86   */
      v.add(doc.createElement("IMG"));
      ((Element)v.get(87)).setAttribute("src","b.gif" );
      ((Element)v.get(87)).setAttribute("width","12" );
      ((Element)v.get(87)).setAttribute("height","22" );
      ((Element)v.get(86)).appendChild((Element)v.get(87));
      /* Termina nodo:87   */
      /* Termina nodo:86   */

      /* Empieza nodo:88 / Elemento padre: 85   */
      v.add(doc.createElement("td"));
      ((Element)v.get(88)).setAttribute("width","750" );
      ((Element)v.get(85)).appendChild((Element)v.get(88));

      /* Empieza nodo:89 / Elemento padre: 88   */
      v.add(doc.createElement("IMG"));
      ((Element)v.get(89)).setAttribute("src","b.gif" );
      ((Element)v.get(88)).appendChild((Element)v.get(89));
      /* Termina nodo:89   */
      /* Termina nodo:88   */

      /* Empieza nodo:90 / Elemento padre: 85   */
      v.add(doc.createElement("td"));
      ((Element)v.get(90)).setAttribute("width","12" );
      ((Element)v.get(85)).appendChild((Element)v.get(90));

      /* Empieza nodo:91 / Elemento padre: 90   */
      v.add(doc.createElement("IMG"));
      ((Element)v.get(91)).setAttribute("src","b.gif" );
      ((Element)v.get(91)).setAttribute("width","12" );
      ((Element)v.get(91)).setAttribute("height","1" );
      ((Element)v.get(90)).appendChild((Element)v.get(91));
      /* Termina nodo:91   */
      /* Termina nodo:90   */
      /* Termina nodo:85   */

      /* Empieza nodo:92 / Elemento padre: 84   */
   }

   private void getXML360(Document doc) {
      v.add(doc.createElement("tr"));
      ((Element)v.get(84)).appendChild((Element)v.get(92));

      /* Empieza nodo:93 / Elemento padre: 92   */
      v.add(doc.createElement("td"));
      ((Element)v.get(92)).appendChild((Element)v.get(93));

      /* Empieza nodo:94 / Elemento padre: 93   */
      v.add(doc.createElement("IMG"));
      ((Element)v.get(94)).setAttribute("src","b.gif" );
      ((Element)v.get(93)).appendChild((Element)v.get(94));
      /* Termina nodo:94   */
      /* Termina nodo:93   */

      /* Empieza nodo:95 / Elemento padre: 92   */
      v.add(doc.createElement("td"));
      ((Element)v.get(92)).appendChild((Element)v.get(95));

      /* Empieza nodo:96 / Elemento padre: 95   */
      v.add(doc.createElement("fieldset"));
      ((Element)v.get(95)).appendChild((Element)v.get(96));

      /* Empieza nodo:97 / Elemento padre: 96   */
      v.add(doc.createElement("legend"));
      ((Element)v.get(97)).setAttribute("class","legend" );
      ((Element)v.get(96)).appendChild((Element)v.get(97));

      /* Empieza nodo:98 / Elemento padre: 97   */
      v.add(doc.createElement("LABELC"));
      ((Element)v.get(98)).setAttribute("nombre","lblTitDatosComunes" );
      ((Element)v.get(98)).setAttribute("ancho","105" );
      ((Element)v.get(98)).setAttribute("alto","17" );
      ((Element)v.get(98)).setAttribute("filas","1" );
      ((Element)v.get(98)).setAttribute("valor","" );
      ((Element)v.get(98)).setAttribute("cod","00159" );
      ((Element)v.get(98)).setAttribute("id","legend" );
      ((Element)v.get(97)).appendChild((Element)v.get(98));
      /* Termina nodo:98   */
      /* Termina nodo:97   */

      /* Empieza nodo:99 / Elemento padre: 96   */
      v.add(doc.createElement("table"));
      ((Element)v.get(99)).setAttribute("width","100%" );
      ((Element)v.get(99)).setAttribute("border","0" );
      ((Element)v.get(99)).setAttribute("align","center" );
      ((Element)v.get(99)).setAttribute("cellspacing","0" );
      ((Element)v.get(99)).setAttribute("cellpadding","0" );
      ((Element)v.get(96)).appendChild((Element)v.get(99));

      /* Empieza nodo:100 / Elemento padre: 99   */
      v.add(doc.createElement("tr"));
      ((Element)v.get(99)).appendChild((Element)v.get(100));

      /* Empieza nodo:101 / Elemento padre: 100   */
      v.add(doc.createElement("td"));
      ((Element)v.get(101)).setAttribute("colspan","4" );
      ((Element)v.get(100)).appendChild((Element)v.get(101));

      /* Empieza nodo:102 / Elemento padre: 101   */
      v.add(doc.createElement("IMG"));
      ((Element)v.get(102)).setAttribute("src","b.gif" );
      ((Element)v.get(102)).setAttribute("width","8" );
      ((Element)v.get(102)).setAttribute("height","8" );
      ((Element)v.get(101)).appendChild((Element)v.get(102));
      /* Termina nodo:102   */
      /* Termina nodo:101   */
      /* Termina nodo:100   */

      /* Empieza nodo:103 / Elemento padre: 99   */
      v.add(doc.createElement("tr"));
      ((Element)v.get(99)).appendChild((Element)v.get(103));

      /* Empieza nodo:104 / Elemento padre: 103   */
      v.add(doc.createElement("td"));
      ((Element)v.get(103)).appendChild((Element)v.get(104));

      /* Empieza nodo:105 / Elemento padre: 104   */
      v.add(doc.createElement("IMG"));
      ((Element)v.get(105)).setAttribute("src","b.gif" );
      ((Element)v.get(105)).setAttribute("width","8" );
      ((Element)v.get(105)).setAttribute("height","8" );
      ((Element)v.get(104)).appendChild((Element)v.get(105));
      /* Termina nodo:105   */
      /* Termina nodo:104   */

      /* Empieza nodo:106 / Elemento padre: 103   */
      v.add(doc.createElement("td"));
      ((Element)v.get(103)).appendChild((Element)v.get(106));

      /* Empieza nodo:107 / Elemento padre: 106   */
      v.add(doc.createElement("LABELC"));
      ((Element)v.get(107)).setAttribute("nombre","lblCodCliente" );
      ((Element)v.get(107)).setAttribute("ancho","105" );
      ((Element)v.get(107)).setAttribute("alto","17" );
      ((Element)v.get(107)).setAttribute("filas","1" );
      ((Element)v.get(107)).setAttribute("valor","" );
      ((Element)v.get(107)).setAttribute("id","datosTitle" );
      ((Element)v.get(107)).setAttribute("cod","263" );
      ((Element)v.get(106)).appendChild((Element)v.get(107));
      /* Termina nodo:107   */
      /* Termina nodo:106   */

      /* Empieza nodo:108 / Elemento padre: 103   */
      v.add(doc.createElement("td"));
      ((Element)v.get(108)).setAttribute("width","100%" );
      ((Element)v.get(103)).appendChild((Element)v.get(108));

      /* Empieza nodo:109 / Elemento padre: 108   */
      v.add(doc.createElement("IMG"));
      ((Element)v.get(109)).setAttribute("src","b.gif" );
      ((Element)v.get(109)).setAttribute("width","545" );
      ((Element)v.get(109)).setAttribute("height","8" );
      ((Element)v.get(108)).appendChild((Element)v.get(109));
      /* Termina nodo:109   */
      /* Termina nodo:108   */
      /* Termina nodo:103   */

      /* Empieza nodo:110 / Elemento padre: 99   */
      v.add(doc.createElement("tr"));
      ((Element)v.get(99)).appendChild((Element)v.get(110));

      /* Empieza nodo:111 / Elemento padre: 110   */
      v.add(doc.createElement("td"));
      ((Element)v.get(110)).appendChild((Element)v.get(111));

      /* Empieza nodo:112 / Elemento padre: 111   */
      v.add(doc.createElement("IMG"));
      ((Element)v.get(112)).setAttribute("src","b.gif" );
      ((Element)v.get(112)).setAttribute("width","8" );
      ((Element)v.get(112)).setAttribute("height","8" );
      ((Element)v.get(111)).appendChild((Element)v.get(112));
      /* Termina nodo:112   */
      /* Termina nodo:111   */

      /* Empieza nodo:113 / Elemento padre: 110   */
      v.add(doc.createElement("td"));
      ((Element)v.get(110)).appendChild((Element)v.get(113));

      /* Empieza nodo:114 / Elemento padre: 113   */
      v.add(doc.createElement("LABELC"));
      ((Element)v.get(114)).setAttribute("nombre","lblCodClienteActual" );
      ((Element)v.get(114)).setAttribute("ancho","107" );
      ((Element)v.get(114)).setAttribute("alto","17" );
      ((Element)v.get(114)).setAttribute("filas","1" );
      ((Element)v.get(114)).setAttribute("valor","xxxxxxxxxxxxxxx" );
      ((Element)v.get(114)).setAttribute("id","datosCampos" );
      ((Element)v.get(113)).appendChild((Element)v.get(114));
      /* Termina nodo:114   */
      /* Termina nodo:113   */

      /* Empieza nodo:115 / Elemento padre: 110   */
      v.add(doc.createElement("td"));
      ((Element)v.get(115)).setAttribute("width","100%" );
      ((Element)v.get(110)).appendChild((Element)v.get(115));

      /* Empieza nodo:116 / Elemento padre: 115   */
   }

   private void getXML450(Document doc) {
      v.add(doc.createElement("IMG"));
      ((Element)v.get(116)).setAttribute("src","b.gif" );
      ((Element)v.get(116)).setAttribute("width","8" );
      ((Element)v.get(116)).setAttribute("height","8" );
      ((Element)v.get(115)).appendChild((Element)v.get(116));
      /* Termina nodo:116   */
      /* Termina nodo:115   */
      /* Termina nodo:110   */

      /* Empieza nodo:117 / Elemento padre: 99   */
      v.add(doc.createElement("tr"));
      ((Element)v.get(99)).appendChild((Element)v.get(117));

      /* Empieza nodo:118 / Elemento padre: 117   */
      v.add(doc.createElement("td"));
      ((Element)v.get(118)).setAttribute("colspan","4" );
      ((Element)v.get(117)).appendChild((Element)v.get(118));

      /* Empieza nodo:119 / Elemento padre: 118   */
      v.add(doc.createElement("IMG"));
      ((Element)v.get(119)).setAttribute("src","b.gif" );
      ((Element)v.get(119)).setAttribute("width","8" );
      ((Element)v.get(119)).setAttribute("height","8" );
      ((Element)v.get(118)).appendChild((Element)v.get(119));
      /* Termina nodo:119   */
      /* Termina nodo:118   */
      /* Termina nodo:117   */
      /* Termina nodo:99   */
      /* Termina nodo:96   */
      /* Termina nodo:95   */

      /* Empieza nodo:120 / Elemento padre: 92   */
      v.add(doc.createElement("td"));
      ((Element)v.get(92)).appendChild((Element)v.get(120));

      /* Empieza nodo:121 / Elemento padre: 120   */
      v.add(doc.createElement("IMG"));
      ((Element)v.get(121)).setAttribute("src","b.gif" );
      ((Element)v.get(121)).setAttribute("width","8" );
      ((Element)v.get(121)).setAttribute("height","12" );
      ((Element)v.get(120)).appendChild((Element)v.get(121));
      /* Termina nodo:121   */
      /* Termina nodo:120   */
      /* Termina nodo:92   */

      /* Empieza nodo:122 / Elemento padre: 84   */
      v.add(doc.createElement("tr"));
      ((Element)v.get(84)).appendChild((Element)v.get(122));

      /* Empieza nodo:123 / Elemento padre: 122   */
      v.add(doc.createElement("td"));
      ((Element)v.get(123)).setAttribute("width","12" );
      ((Element)v.get(123)).setAttribute("align","center" );
      ((Element)v.get(122)).appendChild((Element)v.get(123));

      /* Empieza nodo:124 / Elemento padre: 123   */
      v.add(doc.createElement("IMG"));
      ((Element)v.get(124)).setAttribute("src","b.gif" );
      ((Element)v.get(124)).setAttribute("width","12" );
      ((Element)v.get(124)).setAttribute("height","24" );
      ((Element)v.get(123)).appendChild((Element)v.get(124));
      /* Termina nodo:124   */
      /* Termina nodo:123   */

      /* Empieza nodo:125 / Elemento padre: 122   */
      v.add(doc.createElement("td"));
      ((Element)v.get(125)).setAttribute("width","750" );
      ((Element)v.get(122)).appendChild((Element)v.get(125));

      /* Empieza nodo:126 / Elemento padre: 125   */
      v.add(doc.createElement("IMG"));
      ((Element)v.get(126)).setAttribute("src","b.gif" );
      ((Element)v.get(125)).appendChild((Element)v.get(126));
      /* Termina nodo:126   */
      /* Termina nodo:125   */

      /* Empieza nodo:127 / Elemento padre: 122   */
      v.add(doc.createElement("td"));
      ((Element)v.get(127)).setAttribute("width","12" );
      ((Element)v.get(122)).appendChild((Element)v.get(127));

      /* Empieza nodo:128 / Elemento padre: 127   */
      v.add(doc.createElement("IMG"));
      ((Element)v.get(128)).setAttribute("src","b.gif" );
      ((Element)v.get(128)).setAttribute("width","12" );
      ((Element)v.get(128)).setAttribute("height","24" );
      ((Element)v.get(127)).appendChild((Element)v.get(128));
      /* Termina nodo:128   */
      /* Termina nodo:127   */
      /* Termina nodo:122   */
      /* Termina nodo:84   */

      /* Empieza nodo:129 / Elemento padre: 6   */
      v.add(doc.createElement("LISTAEDITABLE"));
      ((Element)v.get(129)).setAttribute("nombre","listado1" );
      ((Element)v.get(129)).setAttribute("ancho","656" );
      ((Element)v.get(129)).setAttribute("alto","275" );
      ((Element)v.get(129)).setAttribute("x","12" );
      ((Element)v.get(129)).setAttribute("y","129" );
      ((Element)v.get(129)).setAttribute("colorFondo","#CECFCE" );
      ((Element)v.get(129)).setAttribute("msgDebugJS","S" );
      ((Element)v.get(6)).appendChild((Element)v.get(129));

      /* Empieza nodo:130 / Elemento padre: 129   */
      v.add(doc.createElement("IMGBOTONES"));
      ((Element)v.get(130)).setAttribute("precarga","S" );
      ((Element)v.get(130)).setAttribute("conROver","S" );
      ((Element)v.get(129)).appendChild((Element)v.get(130));

      /* Empieza nodo:131 / Elemento padre: 130   */
      v.add(doc.createElement("BTNSELECCION"));
      ((Element)v.get(131)).setAttribute("normal","btnLista2N.gif" );
      ((Element)v.get(131)).setAttribute("rollOver","btnLista2S.gif" );
      ((Element)v.get(131)).setAttribute("seleccionado","btnLista2M.gif" );
      ((Element)v.get(131)).setAttribute("desactivado","btnLista2D.gif" );
      ((Element)v.get(130)).appendChild((Element)v.get(131));
      /* Termina nodo:131   */

      /* Empieza nodo:132 / Elemento padre: 130   */
      v.add(doc.createElement("BTNMINIMIZAR"));
      ((Element)v.get(132)).setAttribute("minimizar","bot_pliega_columna_on.gif" );
      ((Element)v.get(132)).setAttribute("minimROver","bot_pliega_columna_over.gif" );
      ((Element)v.get(132)).setAttribute("maximizar","bot_despliega_columna_on.gif" );
      ((Element)v.get(132)).setAttribute("maximROver","bot_despliega_columna_over.gif" );
      ((Element)v.get(130)).appendChild((Element)v.get(132));
      /* Termina nodo:132   */
      /* Termina nodo:130   */

      /* Empieza nodo:133 / Elemento padre: 129   */
      v.add(doc.createElement("LINEAS"));
      ((Element)v.get(129)).appendChild((Element)v.get(133));

      /* Empieza nodo:134 / Elemento padre: 133   */
      v.add(doc.createElement("GROSOR"));
      ((Element)v.get(134)).setAttribute("borde","1" );
      ((Element)v.get(134)).setAttribute("horizDatos","1" );
      ((Element)v.get(134)).setAttribute("horizCabecera","1" );
      ((Element)v.get(134)).setAttribute("vertical","0" );
      ((Element)v.get(133)).appendChild((Element)v.get(134));
      /* Termina nodo:134   */

      /* Empieza nodo:135 / Elemento padre: 133   */
      v.add(doc.createElement("COLOR"));
      ((Element)v.get(135)).setAttribute("borde","#999999" );
      ((Element)v.get(135)).setAttribute("vertCabecera","#999999" );
      ((Element)v.get(135)).setAttribute("vertDatos","#999999" );
      ((Element)v.get(135)).setAttribute("horizDatos","#999999" );
      ((Element)v.get(135)).setAttribute("horizCabecera","#999999" );
      ((Element)v.get(133)).appendChild((Element)v.get(135));
      /* Termina nodo:135   */
      /* Termina nodo:133   */

      /* Empieza nodo:136 / Elemento padre: 129   */
      v.add(doc.createElement("COLUMNAS"));
      ((Element)v.get(136)).setAttribute("ajustarMinimo","S" );
      ((Element)v.get(136)).setAttribute("permiteOrdenar","N" );
   }

   private void getXML540(Document doc) {
      ((Element)v.get(136)).setAttribute("blancosAInsertar","1" );
      ((Element)v.get(136)).setAttribute("sinSaltoLinea","S" );
      ((Element)v.get(136)).setAttribute("AnchoMinimizadas","20" );
      ((Element)v.get(129)).appendChild((Element)v.get(136));

      /* Empieza nodo:137 / Elemento padre: 136   */
      v.add(doc.createElement("COL"));
      ((Element)v.get(137)).setAttribute("ancho","160" );
      ((Element)v.get(137)).setAttribute("minimizable","S" );
      ((Element)v.get(137)).setAttribute("minimizada","N" );
      ((Element)v.get(136)).appendChild((Element)v.get(137));
      /* Termina nodo:137   */

      /* Empieza nodo:138 / Elemento padre: 136   */
      v.add(doc.createElement("COL"));
      ((Element)v.get(138)).setAttribute("ancho","150" );
      ((Element)v.get(138)).setAttribute("minimizable","S" );
      ((Element)v.get(138)).setAttribute("minimizada","N" );
      ((Element)v.get(136)).appendChild((Element)v.get(138));
      /* Termina nodo:138   */

      /* Empieza nodo:139 / Elemento padre: 136   */
      v.add(doc.createElement("COL"));
      ((Element)v.get(139)).setAttribute("ancho","150" );
      ((Element)v.get(139)).setAttribute("minimizable","S" );
      ((Element)v.get(139)).setAttribute("minimizada","N" );
      ((Element)v.get(136)).appendChild((Element)v.get(139));
      /* Termina nodo:139   */

      /* Empieza nodo:140 / Elemento padre: 136   */
      v.add(doc.createElement("COL"));
      ((Element)v.get(140)).setAttribute("ancho","85" );
      ((Element)v.get(140)).setAttribute("minimizable","S" );
      ((Element)v.get(140)).setAttribute("minimizada","N" );
      ((Element)v.get(136)).appendChild((Element)v.get(140));
      /* Termina nodo:140   */

      /* Empieza nodo:141 / Elemento padre: 136   */
      v.add(doc.createElement("COL"));
      ((Element)v.get(141)).setAttribute("ancho","105" );
      ((Element)v.get(141)).setAttribute("minimizable","S" );
      ((Element)v.get(141)).setAttribute("minimizada","N" );
      ((Element)v.get(136)).appendChild((Element)v.get(141));
      /* Termina nodo:141   */
      /* Termina nodo:136   */

      /* Empieza nodo:142 / Elemento padre: 129   */
      v.add(doc.createElement("CABECERA"));
      ((Element)v.get(142)).setAttribute("alto","20" );
      ((Element)v.get(142)).setAttribute("IDScroll","EstCab" );
      ((Element)v.get(142)).setAttribute("imgFondo","" );
      ((Element)v.get(142)).setAttribute("colFondo","#CCCCCC" );
      ((Element)v.get(129)).appendChild((Element)v.get(142));

      /* Empieza nodo:143 / Elemento padre: 142   */
      v.add(doc.createElement("COL"));
      ((Element)v.get(143)).setAttribute("colFondo","" );
      ((Element)v.get(143)).setAttribute("ID","EstCab" );
      ((Element)v.get(143)).setAttribute("align","center" );
      ((Element)v.get(143)).setAttribute("cod","1126" );
      ((Element)v.get(142)).appendChild((Element)v.get(143));
      /* Termina nodo:143   */

      /* Empieza nodo:144 / Elemento padre: 142   */
      v.add(doc.createElement("COL"));
      ((Element)v.get(144)).setAttribute("colFondo","" );
      ((Element)v.get(144)).setAttribute("ID","EstCab" );
      ((Element)v.get(144)).setAttribute("align","center" );
      ((Element)v.get(144)).setAttribute("cod","1364" );
      ((Element)v.get(142)).appendChild((Element)v.get(144));
      /* Termina nodo:144   */

      /* Empieza nodo:145 / Elemento padre: 142   */
      v.add(doc.createElement("COL"));
      ((Element)v.get(145)).setAttribute("colFondo","" );
      ((Element)v.get(145)).setAttribute("ID","EstCab" );
      ((Element)v.get(145)).setAttribute("align","center" );
      ((Element)v.get(145)).setAttribute("cod","1332" );
      ((Element)v.get(142)).appendChild((Element)v.get(145));
      /* Termina nodo:145   */

      /* Empieza nodo:146 / Elemento padre: 142   */
      v.add(doc.createElement("COL"));
      ((Element)v.get(146)).setAttribute("colFondo","" );
      ((Element)v.get(146)).setAttribute("ID","EstCab" );
      ((Element)v.get(146)).setAttribute("align","center" );
      ((Element)v.get(146)).setAttribute("cod","1073" );
      ((Element)v.get(142)).appendChild((Element)v.get(146));
      /* Termina nodo:146   */

      /* Empieza nodo:147 / Elemento padre: 142   */
      v.add(doc.createElement("COL"));
      ((Element)v.get(147)).setAttribute("colFondo","" );
      ((Element)v.get(147)).setAttribute("ID","EstCab" );
      ((Element)v.get(147)).setAttribute("align","center" );
      ((Element)v.get(147)).setAttribute("cod","1113" );
      ((Element)v.get(142)).appendChild((Element)v.get(147));
      /* Termina nodo:147   */
      /* Termina nodo:142   */

      /* Empieza nodo:148 / Elemento padre: 129   */
      v.add(doc.createElement("DATOS"));
      ((Element)v.get(148)).setAttribute("alto","22" );
      ((Element)v.get(148)).setAttribute("accion","" );
      ((Element)v.get(148)).setAttribute("tipoEnvio","edicion" );
      ((Element)v.get(148)).setAttribute("formaEnvio","xml" );
      ((Element)v.get(148)).setAttribute("maxSel","0" );
      ((Element)v.get(148)).setAttribute("msgErrMaxSel","" );
      ((Element)v.get(148)).setAttribute("coloresScrollNativo","#D4D0C8,black,white,#D4D0C8,#D4D0C8,#ECE9E4,black" );
      ((Element)v.get(148)).setAttribute("onLoad","" );
      ((Element)v.get(129)).appendChild((Element)v.get(148));

      /* Empieza nodo:149 / Elemento padre: 148   */
      v.add(doc.createElement("COL"));
      ((Element)v.get(149)).setAttribute("tipo","combo" );
      ((Element)v.get(149)).setAttribute("nombre","comboTipoDocId" );
      ((Element)v.get(149)).setAttribute("IDOBJ","EstDatForm" );
      ((Element)v.get(149)).setAttribute("ID","EstDat2" );
      ((Element)v.get(149)).setAttribute("req","N" );
      ((Element)v.get(149)).setAttribute("size","1" );
      ((Element)v.get(149)).setAttribute("multiple","N" );
      ((Element)v.get(148)).appendChild((Element)v.get(149));

      /* Empieza nodo:150 / Elemento padre: 149   */
      v.add(doc.createElement("ROWSET"));
      ((Element)v.get(149)).appendChild((Element)v.get(150));
      /* Termina nodo:150   */
      /* Termina nodo:149   */

      /* Empieza nodo:151 / Elemento padre: 148   */
      v.add(doc.createElement("COL"));
      ((Element)v.get(151)).setAttribute("tipo","cajatexto" );
      ((Element)v.get(151)).setAttribute("nombre","TextoNDoc" );
      ((Element)v.get(151)).setAttribute("onchange","cambioDocumento();" );
   }

   private void getXML630(Document doc) {
      ((Element)v.get(151)).setAttribute("size","21" );
      ((Element)v.get(151)).setAttribute("max","15" );
      ((Element)v.get(151)).setAttribute("IDOBJ","EstCajaDatForm" );
      ((Element)v.get(151)).setAttribute("ID","EstDat" );
      ((Element)v.get(148)).appendChild((Element)v.get(151));
      /* Termina nodo:151   */

      /* Empieza nodo:152 / Elemento padre: 148   */
      v.add(doc.createElement("COL"));
      ((Element)v.get(152)).setAttribute("tipo","cajatexto" );
      ((Element)v.get(152)).setAttribute("nombre","TextoConfirmacionDoc" );
      ((Element)v.get(152)).setAttribute("size","21" );
      ((Element)v.get(152)).setAttribute("max","15" );
      ((Element)v.get(152)).setAttribute("IDOBJ","EstCajaDatForm" );
      ((Element)v.get(152)).setAttribute("ID","EstDat2" );
      ((Element)v.get(148)).appendChild((Element)v.get(152));
      /* Termina nodo:152   */

      /* Empieza nodo:153 / Elemento padre: 148   */
      v.add(doc.createElement("COL"));
      ((Element)v.get(153)).setAttribute("tipo","checkbox" );
      ((Element)v.get(153)).setAttribute("nombre","RbDocPrincipal" );
      ((Element)v.get(153)).setAttribute("onchange","flagRadio();" );
      ((Element)v.get(153)).setAttribute("ID","EstDat" );
      ((Element)v.get(153)).setAttribute("req","N" );
      ((Element)v.get(148)).appendChild((Element)v.get(153));

      /* Empieza nodo:154 / Elemento padre: 153   */
      v.add(doc.createElement("RBINPUT"));
      ((Element)v.get(154)).setAttribute("valor","N" );
      ((Element)v.get(153)).appendChild((Element)v.get(154));
      /* Termina nodo:154   */
      /* Termina nodo:153   */

      /* Empieza nodo:155 / Elemento padre: 148   */
      v.add(doc.createElement("COL"));
      ((Element)v.get(155)).setAttribute("tipo","combo" );
      ((Element)v.get(155)).setAttribute("nombre","comboPersonaEmpresa" );
      ((Element)v.get(155)).setAttribute("IDOBJ","EstDatForm" );
      ((Element)v.get(155)).setAttribute("ID","EstDat2" );
      ((Element)v.get(155)).setAttribute("req","N" );
      ((Element)v.get(155)).setAttribute("size","1" );
      ((Element)v.get(155)).setAttribute("multiple","N" );
      ((Element)v.get(148)).appendChild((Element)v.get(155));

      /* Empieza nodo:156 / Elemento padre: 155   */
      v.add(doc.createElement("ROWSET"));
      ((Element)v.get(155)).appendChild((Element)v.get(156));
      /* Termina nodo:156   */
      /* Termina nodo:155   */
      /* Termina nodo:148   */

      /* Empieza nodo:157 / Elemento padre: 129   */
      v.add(doc.createElement("ROWSET"));
      ((Element)v.get(129)).appendChild((Element)v.get(157));

      /* Empieza nodo:158 / Elemento padre: 157   */
      v.add(doc.createElement("ROW"));
      ((Element)v.get(158)).setAttribute("ID","1" );
      ((Element)v.get(157)).appendChild((Element)v.get(158));
      /* Termina nodo:158   */

      /* Empieza nodo:159 / Elemento padre: 157   */
      v.add(doc.createElement("ROW"));
      ((Element)v.get(159)).setAttribute("ID","1" );
      ((Element)v.get(157)).appendChild((Element)v.get(159));
      /* Termina nodo:159   */

      /* Empieza nodo:160 / Elemento padre: 157   */
      v.add(doc.createElement("ROW"));
      ((Element)v.get(160)).setAttribute("ID","1" );
      ((Element)v.get(157)).appendChild((Element)v.get(160));
      /* Termina nodo:160   */

      /* Empieza nodo:161 / Elemento padre: 157   */
      v.add(doc.createElement("ROW"));
      ((Element)v.get(161)).setAttribute("ID","1" );
      ((Element)v.get(157)).appendChild((Element)v.get(161));
      /* Termina nodo:161   */

      /* Empieza nodo:162 / Elemento padre: 157   */
      v.add(doc.createElement("ROW"));
      ((Element)v.get(162)).setAttribute("ID","1" );
      ((Element)v.get(157)).appendChild((Element)v.get(162));
      /* Termina nodo:162   */

      /* Empieza nodo:163 / Elemento padre: 157   */
      v.add(doc.createElement("ROW"));
      ((Element)v.get(163)).setAttribute("ID","1" );
      ((Element)v.get(157)).appendChild((Element)v.get(163));
      /* Termina nodo:163   */

      /* Empieza nodo:164 / Elemento padre: 157   */
      v.add(doc.createElement("ROW"));
      ((Element)v.get(164)).setAttribute("ID","1" );
      ((Element)v.get(157)).appendChild((Element)v.get(164));
      /* Termina nodo:164   */

      /* Empieza nodo:165 / Elemento padre: 157   */
      v.add(doc.createElement("ROW"));
      ((Element)v.get(165)).setAttribute("ID","1" );
      ((Element)v.get(157)).appendChild((Element)v.get(165));
      /* Termina nodo:165   */

      /* Empieza nodo:166 / Elemento padre: 157   */
      v.add(doc.createElement("ROW"));
      ((Element)v.get(166)).setAttribute("ID","1" );
      ((Element)v.get(157)).appendChild((Element)v.get(166));
      /* Termina nodo:166   */

      /* Empieza nodo:167 / Elemento padre: 157   */
      v.add(doc.createElement("ROW"));
      ((Element)v.get(167)).setAttribute("ID","1" );
      ((Element)v.get(157)).appendChild((Element)v.get(167));
      /* Termina nodo:167   */
      /* Termina nodo:157   */

      /* Empieza nodo:168 / Elemento padre: 129   */
      v.add(doc.createElement("PAGINADO"));
      ((Element)v.get(168)).setAttribute("nombre","mipgndo" );
      ((Element)v.get(168)).setAttribute("ancho","656" );
      ((Element)v.get(168)).setAttribute("sep","$" );
      ((Element)v.get(168)).setAttribute("x","12" );
      ((Element)v.get(168)).setAttribute("class","botonera" );
      ((Element)v.get(168)).setAttribute("y","385" );
      ((Element)v.get(168)).setAttribute("control","|" );
      ((Element)v.get(168)).setAttribute("conector","conector_campos" );
      ((Element)v.get(168)).setAttribute("rowset","" );
      ((Element)v.get(168)).setAttribute("cargainicial","N" );
      ((Element)v.get(129)).appendChild((Element)v.get(168));

      /* Empieza nodo:169 / Elemento padre: 168   */
      v.add(doc.createElement("BOTON"));
      ((Element)v.get(169)).setAttribute("nombre","masDatos" );
      ((Element)v.get(169)).setAttribute("x","13" );
      ((Element)v.get(169)).setAttribute("y","381" );
      ((Element)v.get(169)).setAttribute("ID","botonContenido" );
      ((Element)v.get(169)).setAttribute("tipo","html" );
      ((Element)v.get(169)).setAttribute("estado","false" );
      ((Element)v.get(169)).setAttribute("cod","1093" );
      ((Element)v.get(169)).setAttribute("accion","nuevaFila();" );
      ((Element)v.get(168)).appendChild((Element)v.get(169));
      /* Termina nodo:169   */

      /* Empieza nodo:170 / Elemento padre: 168   */
      v.add(doc.createElement("BOTON"));
      ((Element)v.get(170)).setAttribute("nombre","Validar" );
   }

   private void getXML720(Document doc) {
      ((Element)v.get(170)).setAttribute("x","88" );
      ((Element)v.get(170)).setAttribute("y","381" );
      ((Element)v.get(170)).setAttribute("ID","botonContenido" );
      ((Element)v.get(170)).setAttribute("tipo","html" );
      ((Element)v.get(170)).setAttribute("estado","false" );
      ((Element)v.get(170)).setAttribute("cod","1138" );
      ((Element)v.get(170)).setAttribute("accion","accionInsertarP2();" );
      ((Element)v.get(168)).appendChild((Element)v.get(170));
      /* Termina nodo:170   */
      /* Termina nodo:168   */
      /* Termina nodo:129   */

      /* Empieza nodo:171 / Elemento padre: 6   */
      v.add(doc.createElement("CAPA"));
      ((Element)v.get(171)).setAttribute("nombre","capaTituloLista" );
      ((Element)v.get(171)).setAttribute("alto","50" );
      ((Element)v.get(171)).setAttribute("ancho","100%" );
      ((Element)v.get(171)).setAttribute("colorf","" );
      ((Element)v.get(171)).setAttribute("borde","0" );
      ((Element)v.get(171)).setAttribute("imagenf","" );
      ((Element)v.get(171)).setAttribute("repeat","" );
      ((Element)v.get(171)).setAttribute("padding","" );
      ((Element)v.get(171)).setAttribute("visibilidad","visible" );
      ((Element)v.get(171)).setAttribute("contravsb","" );
      ((Element)v.get(171)).setAttribute("x","0" );
      ((Element)v.get(171)).setAttribute("y","104" );
      ((Element)v.get(171)).setAttribute("zindex","" );
      ((Element)v.get(6)).appendChild((Element)v.get(171));

      /* Empieza nodo:172 / Elemento padre: 171   */
      v.add(doc.createElement("table"));
      ((Element)v.get(172)).setAttribute("width","100%" );
      ((Element)v.get(172)).setAttribute("border","0" );
      ((Element)v.get(172)).setAttribute("align","center" );
      ((Element)v.get(172)).setAttribute("cellspacing","0" );
      ((Element)v.get(172)).setAttribute("cellpadding","0" );
      ((Element)v.get(171)).appendChild((Element)v.get(172));

      /* Empieza nodo:173 / Elemento padre: 172   */
      v.add(doc.createElement("tr"));
      ((Element)v.get(172)).appendChild((Element)v.get(173));

      /* Empieza nodo:174 / Elemento padre: 173   */
      v.add(doc.createElement("td"));
      ((Element)v.get(174)).setAttribute("width","12" );
      ((Element)v.get(174)).setAttribute("align","center" );
      ((Element)v.get(173)).appendChild((Element)v.get(174));

      /* Empieza nodo:175 / Elemento padre: 174   */
      v.add(doc.createElement("IMG"));
      ((Element)v.get(175)).setAttribute("src","b.gif" );
      ((Element)v.get(175)).setAttribute("width","12" );
      ((Element)v.get(175)).setAttribute("height","1" );
      ((Element)v.get(174)).appendChild((Element)v.get(175));
      /* Termina nodo:175   */
      /* Termina nodo:174   */

      /* Empieza nodo:176 / Elemento padre: 173   */
      v.add(doc.createElement("td"));
      ((Element)v.get(176)).setAttribute("width","750" );
      ((Element)v.get(173)).appendChild((Element)v.get(176));

      /* Empieza nodo:177 / Elemento padre: 176   */
      v.add(doc.createElement("IMG"));
      ((Element)v.get(177)).setAttribute("src","b.gif" );
      ((Element)v.get(176)).appendChild((Element)v.get(177));
      /* Termina nodo:177   */
      /* Termina nodo:176   */

      /* Empieza nodo:178 / Elemento padre: 173   */
      v.add(doc.createElement("td"));
      ((Element)v.get(178)).setAttribute("width","12" );
      ((Element)v.get(173)).appendChild((Element)v.get(178));

      /* Empieza nodo:179 / Elemento padre: 178   */
      v.add(doc.createElement("IMG"));
      ((Element)v.get(179)).setAttribute("src","b.gif" );
      ((Element)v.get(179)).setAttribute("width","12" );
      ((Element)v.get(179)).setAttribute("height","1" );
      ((Element)v.get(178)).appendChild((Element)v.get(179));
      /* Termina nodo:179   */
      /* Termina nodo:178   */
      /* Termina nodo:173   */

      /* Empieza nodo:180 / Elemento padre: 172   */
      v.add(doc.createElement("tr"));
      ((Element)v.get(172)).appendChild((Element)v.get(180));

      /* Empieza nodo:181 / Elemento padre: 180   */
      v.add(doc.createElement("td"));
      ((Element)v.get(180)).appendChild((Element)v.get(181));

      /* Empieza nodo:182 / Elemento padre: 181   */
      v.add(doc.createElement("IMG"));
      ((Element)v.get(182)).setAttribute("src","b.gif" );
      ((Element)v.get(181)).appendChild((Element)v.get(182));
      /* Termina nodo:182   */
      /* Termina nodo:181   */

      /* Empieza nodo:183 / Elemento padre: 180   */
      v.add(doc.createElement("td"));
      ((Element)v.get(180)).appendChild((Element)v.get(183));

      /* Empieza nodo:184 / Elemento padre: 183   */
      v.add(doc.createElement("fieldset"));
      ((Element)v.get(183)).appendChild((Element)v.get(184));

      /* Empieza nodo:185 / Elemento padre: 184   */
      v.add(doc.createElement("table"));
      ((Element)v.get(185)).setAttribute("width","100%" );
      ((Element)v.get(185)).setAttribute("border","0" );
      ((Element)v.get(185)).setAttribute("align","center" );
      ((Element)v.get(185)).setAttribute("cellspacing","0" );
      ((Element)v.get(185)).setAttribute("cellpadding","0" );
      ((Element)v.get(184)).appendChild((Element)v.get(185));

      /* Empieza nodo:186 / Elemento padre: 185   */
      v.add(doc.createElement("tr"));
      ((Element)v.get(185)).appendChild((Element)v.get(186));

      /* Empieza nodo:187 / Elemento padre: 186   */
      v.add(doc.createElement("td"));
      ((Element)v.get(187)).setAttribute("bgcolor","#CCCCCC" );
      ((Element)v.get(187)).setAttribute("width","4" );
      ((Element)v.get(186)).appendChild((Element)v.get(187));

      /* Empieza nodo:188 / Elemento padre: 187   */
      v.add(doc.createElement("IMG"));
      ((Element)v.get(188)).setAttribute("src","b.gif" );
      ((Element)v.get(188)).setAttribute("width","4" );
      ((Element)v.get(188)).setAttribute("height","4" );
      ((Element)v.get(187)).appendChild((Element)v.get(188));
      /* Termina nodo:188   */
      /* Termina nodo:187   */

      /* Empieza nodo:189 / Elemento padre: 186   */
      v.add(doc.createElement("td"));
      ((Element)v.get(189)).setAttribute("bgcolor","#CCCCCC" );
      ((Element)v.get(189)).setAttribute("valign","middle" );
      ((Element)v.get(189)).setAttribute("height","20" );
      ((Element)v.get(186)).appendChild((Element)v.get(189));

      /* Empieza nodo:190 / Elemento padre: 189   */
      v.add(doc.createElement("LABELC"));
   }

   private void getXML810(Document doc) {
      ((Element)v.get(190)).setAttribute("nombre","lbltitulo" );
      ((Element)v.get(190)).setAttribute("ancho","649" );
      ((Element)v.get(190)).setAttribute("alto","13" );
      ((Element)v.get(190)).setAttribute("filas","1" );
      ((Element)v.get(190)).setAttribute("valor","" );
      ((Element)v.get(190)).setAttribute("id","datosTitle" );
      ((Element)v.get(190)).setAttribute("cod","0053" );
      ((Element)v.get(189)).appendChild((Element)v.get(190));
      /* Termina nodo:190   */
      /* Termina nodo:189   */
      /* Termina nodo:186   */
      /* Termina nodo:185   */
      /* Termina nodo:184   */
      /* Termina nodo:183   */

      /* Empieza nodo:191 / Elemento padre: 180   */
      v.add(doc.createElement("td"));
      ((Element)v.get(191)).setAttribute("width","12" );
      ((Element)v.get(180)).appendChild((Element)v.get(191));

      /* Empieza nodo:192 / Elemento padre: 191   */
      v.add(doc.createElement("IMG"));
      ((Element)v.get(192)).setAttribute("src","b.gif" );
      ((Element)v.get(192)).setAttribute("width","12" );
      ((Element)v.get(192)).setAttribute("height","12" );
      ((Element)v.get(191)).appendChild((Element)v.get(192));
      /* Termina nodo:192   */
      /* Termina nodo:191   */
      /* Termina nodo:180   */

      /* Empieza nodo:193 / Elemento padre: 172   */
      v.add(doc.createElement("tr"));
      ((Element)v.get(172)).appendChild((Element)v.get(193));

      /* Empieza nodo:194 / Elemento padre: 193   */
      v.add(doc.createElement("td"));
      ((Element)v.get(194)).setAttribute("width","12" );
      ((Element)v.get(194)).setAttribute("align","center" );
      ((Element)v.get(193)).appendChild((Element)v.get(194));

      /* Empieza nodo:195 / Elemento padre: 194   */
      v.add(doc.createElement("IMG"));
      ((Element)v.get(195)).setAttribute("src","b.gif" );
      ((Element)v.get(195)).setAttribute("width","12" );
      ((Element)v.get(195)).setAttribute("height","1" );
      ((Element)v.get(194)).appendChild((Element)v.get(195));
      /* Termina nodo:195   */
      /* Termina nodo:194   */

      /* Empieza nodo:196 / Elemento padre: 193   */
      v.add(doc.createElement("td"));
      ((Element)v.get(196)).setAttribute("width","750" );
      ((Element)v.get(193)).appendChild((Element)v.get(196));

      /* Empieza nodo:197 / Elemento padre: 196   */
      v.add(doc.createElement("IMG"));
      ((Element)v.get(197)).setAttribute("src","b.gif" );
      ((Element)v.get(196)).appendChild((Element)v.get(197));
      /* Termina nodo:197   */
      /* Termina nodo:196   */

      /* Empieza nodo:198 / Elemento padre: 193   */
      v.add(doc.createElement("td"));
      ((Element)v.get(198)).setAttribute("width","12" );
      ((Element)v.get(193)).appendChild((Element)v.get(198));

      /* Empieza nodo:199 / Elemento padre: 198   */
      v.add(doc.createElement("IMG"));
      ((Element)v.get(199)).setAttribute("src","b.gif" );
      ((Element)v.get(199)).setAttribute("width","12" );
      ((Element)v.get(199)).setAttribute("height","1" );
      ((Element)v.get(198)).appendChild((Element)v.get(199));
      /* Termina nodo:199   */
      /* Termina nodo:198   */
      /* Termina nodo:193   */
      /* Termina nodo:172   */
      /* Termina nodo:171   */

      /* Empieza nodo:200 / Elemento padre: 6   */
      v.add(doc.createElement("CAPA"));
      ((Element)v.get(200)).setAttribute("nombre","capaFinal" );
      ((Element)v.get(200)).setAttribute("alto","1" );
      ((Element)v.get(200)).setAttribute("ancho","2" );
      ((Element)v.get(200)).setAttribute("colorf","#FFF0F0" );
      ((Element)v.get(200)).setAttribute("borde","0" );
      ((Element)v.get(200)).setAttribute("x","0" );
      ((Element)v.get(200)).setAttribute("y","311" );
      ((Element)v.get(6)).appendChild((Element)v.get(200));
      /* Termina nodo:200   */
      /* Termina nodo:6   */


   }

}
