...
public class BELHojaEspecialista extends LPSICCBase {           
   public BELHojaEspecialista() {	 
     super();
  }
     ...
     try {
        ...
        if (accion.equals("generar")){
          ...
          //cargarPagina();
           ...
           cargarPagina();
           getConfiguracionMenu("BELHojaEspecialista");
        }
  ...
  private void cargarPagina() throws Exception{
     pagina("contenido_hoja_especialista_obtener");   
     ...
     DTOCaja dtoC = new DTOCaja();
     dtoC.setOidPais(UtilidadesSession.getPais(this));
     MareBusinessID idC = new MareBusinessID("BELObtenerCajas");
     Vector vecC= new Vector();
     vecC.add(dtoC);
     vecC.add(idC);
     DTOSalida dtoCa = (DTOSalida) conectar("ConectorUA",vecC).objeto("dtoSalida"); 
     RecordSet caja = (RecordSet)dtoCa.getResultado();
     Vector colCaja = caja.getColumnIdentifiers();
     DruidaConector cajas = generarConector("DTOSalida.cajas_ROWSET",caja, colCaja);
     traza("colNumero "+dtoCa.getResultado());
     traza("colNumero "+dtoCa.getResultado().getColumnIdentifiers());
     this.crearListaLP(colCaja, caja.getColumnCount(), "CapaCaja","ListaCaja", "14", "200");
     asignar("LISTA", "ListaCaja", cajas, "DTOSalida.cajas_ROWSET"); 
 ...
 private void generarReporte() throws Exception{
     ...
     String caja        = conectorParametroLimpia("caja", "", true);     
     if(!caja.equals("")){
        datos.put(ConstantesREP.CAJAS, caja);          
     }
     ...
     datos.put(ConstantesREP.TITULO, "Hoja por Especialista");     
     traza("++++++Antes de enviar a reportes " + datos); 
     traza("el formaro es  " + formato);
     //ejecutarReporte(datos, "REPGenerarReporte", "RD_REP-097", formato);
     ejecutarReporte2(datos, "REPGenerarReporteBatch", "RD_REP-097", formato);
