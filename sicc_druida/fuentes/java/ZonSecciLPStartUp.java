/*
    INDRA/CAR/mmg
    $Id: ZonSecciLPStartUp.java,v 1.1 2009/12/03 18:32:58 pecbazalar Exp $
    DESC
*/
import java.util.*;
...
public class ZonSecciLPStartUp extends MMGDruidaBase{
	...
	public static final String BUSINESSID_ZZONOIDZONA_QUERY ="MMGZonZonaQueryFromToUserStructuralSecurity";
	public static final String ZZONOIDZONA_COMBO_CONNECTOR = "ZonZonaConectorCombo";
	...
	public void ejecucion() throws Exception {
		try{
			...
			asignarAtributo("PAGINA", "ZonSecciPage", "cod", "ZonSecci." + accion + ".label");
			...
			limpiarBotoneraLista("zonSecci", accion);
	...
	protected void cmdLov() throws Exception{
		...
		asignarAtributo("TEXT", "codSecc", "validacion", "ValidaLongitud(cdos_trim(valor), 1, 1)");
		...
		asignarAtributo("TEXT", "desSecci", "validacion", "ValidaLongitud(cdos_trim(valor), 0, 40)");
		...
		//asignarAtributo("CAPA", "zonSecciLovLayer", "visibilidad", "visible");
		asignarAtributo("PAGINA", "ZonSecciPage", "onblur", "window.focus();");
	...
	protected void cmdView() throws Exception{
		...
		ZonSecciData zonSecciQuery= new ZonSecciData();
		zonSecciQuery.setId(new Integer(conectorParametro("idSelection")));
		
		//Asiganmos el valor a la variable de acción anterior para poder volver hacia atras y hacer requery
		asignar("VAR", VAR_ACCION_ANTERIOR, conectorParametro(VAR_ACCION_ANTERIOR));
		
		//Sacamos de la base de datos la entidad a visualizar
		ZonSecciData datosResult = (ZonSecciData)getSingleObject(BUSINESSID_QUERY, zonSecciQuery, 
			zonSecciQuery.clone(), "zonSecci", false);
		...
		ZonSecciData zonSecci = datosResult;
		es.indra.belcorp.mso.ZonZonaData zzonOidZona = zonSecci.getZzonOidZona();
		if(zzonOidZona != null && zzonOidZona.getId() != null) checkCombo("zzonOidZona" , zzonOidZona.getId().toString());
		asignar("TEXT", "codSecc", FormatUtils.formatObject(zonSecci.getCodSecc(), 
			 MMGDruidaHelper.getUserDecimalFormatPattern(this), MMGDruidaHelper.getUserDecimalFormatSymbols(this)));
		asignar("TEXT", "indActi", FormatUtils.formatObject(zonSecci.getIndActi(), 
			 MMGDruidaHelper.getUserDecimalFormatPattern(this), MMGDruidaHelper.getUserDecimalFormatSymbols(this)));
		asignar("TEXT", "indBorr", FormatUtils.formatObject(zonSecci.getIndBorr(), 
			 MMGDruidaHelper.getUserDecimalFormatPattern(this), MMGDruidaHelper.getUserDecimalFormatSymbols(this)));
		es.indra.belcorp.mso.MaeClienData clieOidClie = zonSecci.getClieOidClie();
		if(clieOidClie != null && clieOidClie.getId() != null) checkCombo("clieOidClie" , clieOidClie.getId().toString());
		asignar("TEXT", "codNse1", FormatUtils.formatObject(zonSecci.getCodNse1(), 
			 MMGDruidaHelper.getUserDecimalFormatPattern(this), MMGDruidaHelper.getUserDecimalFormatSymbols(this)));
		asignar("TEXT", "codNse2", FormatUtils.formatObject(zonSecci.getCodNse2(), 
			 MMGDruidaHelper.getUserDecimalFormatPattern(this), MMGDruidaHelper.getUserDecimalFormatSymbols(this)));
		asignar("TEXT", "codNse3", FormatUtils.formatObject(zonSecci.getCodNse3(), 
			 MMGDruidaHelper.getUserDecimalFormatPattern(this), MMGDruidaHelper.getUserDecimalFormatSymbols(this)));
		asignar("TEXT", "desSecci", FormatUtils.formatObject(zonSecci.getDesSecci(), 
			 MMGDruidaHelper.getUserDecimalFormatPattern(this), MMGDruidaHelper.getUserDecimalFormatSymbols(this)));
		asignar("TEXT", "fecRezo", FormatUtils.formatObject(zonSecci.getFecRezo(), 
	...
	protected void cmdUpdateForm() throws Exception{
		...
		//asignarAtributo("FORMULARIO", "zonSecciFrm", "oculto", "N");
		...
		//Construimos el mso con los elementos de la búsqueda
		ZonSecciData zonSecciQuery= new ZonSecciData();
		zonSecciQuery.setId(new Integer(id));
		...
		//asignar("VAR","timestamp",Long.toString(zonSecci.jdoGetTimeStamp()));
	...
	protected void removeLovElements() throws Exception{
		borrarNodo("TD","nombre","zonSecciTdLovNullSelectionButton");
	}
	
	protected void removeQueryElements() throws Exception{
		//Borramos los botones para realizar la búsuqeda
		borrarNodo("TR","nombre","zonSecciTrButtons");
	...
	*/
	protected void establecerSizeCapas(String accion, String origen) throws Exception{
		...
		asignarAtributo("CAPA","zonSecciFormLayer", "alto", String.valueOf(sizeForm));		
		
		//Estabelcemos la posicion y de la capa correspondiente a la lista y de su botonera
		asignarAtributo("CAPA","zonSecciListLayer", "y", String.valueOf(sizeForm + 26));		
		asignarAtributo("CAPA","zonSecciListButtonsLayer", "y", String.valueOf(sizeForm + 278));		
	}

	/**
	* Nos permite cargar las combos finales de la jerarquías de dependencia
	*/
	protected void loadLeafDependenceCombos(ZonSecciData zonSecci) throws Exception{
		HashMap checkedCombos = new HashMap();
		ZonZonaData zzonOidZona = new ZonZonaData();
		asignarComboResultados(BUSINESSID_ZZONOIDZONA_QUERY,ZZONOIDZONA_COMBO_CONNECTOR, 
			zzonOidZona, zzonOidZona.clone(), "zonZona", "zzonOidZona", "result_ROWSET",false );
		if(zonSecci != null && zonSecci.getZzonOidZona() != null){
				checkCombo("zzonOidZona" , zonSecci.getZzonOidZona().getId().toString());
			}
		String zzonOidZonaDefaultValue = MMGDruidaHelper.gerUserStructuralEntityValue(this, "ZonZona");
		if(zzonOidZonaDefaultValue != null && !zzonOidZonaDefaultValue.trim().equals("")){
			checkCombo("zzonOidZona", zzonOidZonaDefaultValue);
			checkedCombos.put("zzonOidZona",zzonOidZonaDefaultValue);
			cascadeComboLoad(checkedCombos, "zzonOidZona");
		}						 
		...
		if(zonSecci != null && zonSecci.getClieOidClie() != null){
				checkCombo("clieOidClie" , zonSecci.getClieOidClie().getId().toString());
			}
	...
	* @param zonSecci La el objetopar la cual se va a realizar la carga
	*/
	protected void loadNonLeafDependenceCombos(ZonSecciData zonSecci) 
		throws Exception{
	...
	*/
	protected void loadNonLeafDependenceCombosForRequery() throws Exception{
		...
		ZonSecciData zonSecci = 
			(ZonSecciData) conectorParametroSesion(SESSION_ATTRIBUTE_LAST_QUERY_OBJ_FROM);
		
		//Cargamos todos los objetos completos correspodnientes a los valores de los
		//Campos que sean raiz de una jerarquía de combos dependientes
		if(zonSecci.getZzonOidZona() != null){
			zonSecci.setZzonOidZona((es.indra.belcorp.mso.ZonZonaData)
			getSingleObject(BUSINESSID_ZZONOIDZONA_QUERY, 
			zonSecci.getZzonOidZona(),
			zonSecci.getZzonOidZona().clone(),
			"zonZona"));
		}
		if(zonSecci.getClieOidClie() != null){
			zonSecci.setClieOidClie((es.indra.belcorp.mso.MaeClienData)
			getSingleObject(BUSINESSID_CLIEOIDCLIE_QUERY, 
			zonSecci.getClieOidClie(),
			zonSecci.getClieOidClie().clone(),
			"maeClien"));
		}
		
		//Una vez que tenemos completo los datos de los atributos raices de jerarquías de dependencias
		//cargamos las combos y seleccionamos elementos
		loadNonLeafDependenceCombos(zonSecci);
		loadLeafDependenceCombos(zonSecci);
	}
