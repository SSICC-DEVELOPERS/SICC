...
public class CCCAplicacionesCC extends LPSICCBase {           
   public CCCAplicacionesCC() {	 
     super();
  }
     ...
     try {
        ...
        if (accion.equals("generar")){
          ...
          //cargarPagina();
           ...
           cargarPagina();
           getConfiguracionMenu("CCCAplicacionesCC");
        }
  ...
  private void cargarPagina() throws Exception{
     pagina("contenido_informe_aplicaciones_CCC_generar");   
     ...
     MareBusinessID id = new MareBusinessID("CCCObtenerProcesosActivos");
     ...
     asignar("COMBO", "cbProceso", mcs, "DTOSalida.mc_ROWSET");  
     
     
     MareBusinessID idNum = new MareBusinessID("CCCObtenerTipoCargoProcesos");
     ...
     this.crearListaLP(colNumero, numero.getColumnCount(), "CapaTC","ListaTC", "14", "200");
     asignar("LISTA", "ListaTC", numeros, "DTOSalida.numeros_ROWSET"); 
 ...
 private void generarReporte() throws Exception{
	 ...
	 if(!proceso.equals("")){
        datos.put(ConstantesREP.PROCESOS, proceso);
	 }
     
     String tipoCargo = conectorParametroLimpia("tipoCargo", "", true);
	  String tipoCargoDesc = conectorParametroLimpia("tipoCargoDesc", "", true);
	 if(!tipoCargo.equals("")){
        datos.put(ConstantesREP.TIPOCARGOABONOS, tipoCargo);
		  //falta agregar la constante de abajo.
		  datos.put(ConstantesREP.TIPOCARGOABONOSDESC, tipoCargoDesc);

	 }
     
     String fechaProcesoDesde = conectorParametroLimpia("fechaProcesoDesde", "", true);
     datos.put(ConstantesREP.FECHADESDE, fechaProcesoDesde);
     
     String fechaProcesoHasta = conectorParametroLimpia("fechaProcesoHasta", "", true);
     datos.put(ConstantesREP.FECHAHASTA, fechaProcesoHasta);
        
     String comprobante = conectorParametroLimpia("numeroComprobante", "", true);
     if(!comprobante.equals("")){
        datos.put(ConstantesREP.NUMEROCOMPROBANTE, comprobante);
     }
            

     datos.put(ConstantesREP.TITULO, "Aplicaciones en Cuenta Corriente");     
     traza("++++++Antes de enviar a reportes " + datos); 
       
          
     //ejecutarReporte(datos, "REPGenerarReporte", "RD_REP-138", null);
     ejecutarReporte2(datos, "REPGenerarReporteBatch", "RD_REP-138", "pdf");
