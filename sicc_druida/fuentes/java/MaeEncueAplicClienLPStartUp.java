/*
    INDRA/CAR/mmg
    $Id: MaeEncueAplicClienLPStartUp.java,v 1.1 2009/12/03 18:42:52 pecbazalar Exp $
    DESC
*/
import java.util.*;
...
public class MaeEncueAplicClienLPStartUp extends MMGDruidaBase{
	...
	public static final String BUSINESSID_QUERY = "MMGMaeEncueAplicClienQueryFromToUserPage";
				...
				public static final String BUSINESSID_REENOIDRESP_PRENOIDPREG_QUERY ="MMGMaePreguEncueQueryFromToUser";
				public static final String BUSINESSID_REENOIDRESP_QUERY ="MMGMaeRespuEncueQueryFromToUser";
				public static final String REENOIDRESP_PRENOIDPREG_COMBO_CONNECTOR = "MaePreguEncueConectorCombo";
	
				public static final String REENOIDRESP_COMBO_CONNECTOR = "MaeRespuEncueConectorCombo";
	...
	public MaeEncueAplicClienLPStartUp(){
		super();
	}

	public void inicio() throws Exception {
		pagina("MaeEncueAplicClienPage");
	}
	...
	public void ejecucion() throws Exception {
		try{
			...
			buildJsPageSize("MaeEncueAplicClien");
			...
			asignarAtributo("PAGINA", "MaeEncueAplicClienPage", "cod", "MaeEncueAplicClien." + accion + ".label");
			...
			limpiarBotoneraLista("maeEncueAplicClien", accion);
	...
	protected void cmdLov() throws Exception{
		...
		//asignarAtributo("CAPA", "maeEncueAplicClienLovLayer", "visibilidad", "visible");
		asignarAtributo("PAGINA", "MaeEncueAplicClienPage", "onblur", "window.focus();");
	...
	protected void cmdPreView() throws Exception{
		...
		conectorAction("MaeEncueAplicClienLPStartUp");
	...
	*/
	protected void cmdView() throws Exception{
		...
		MaeEncueAplicClienData maeEncueAplicClienQuery= new MaeEncueAplicClienData();
		maeEncueAplicClienQuery.setId(new Long(conectorParametro("idSelection")));
		
		//Asiganmos el valor a la variable de acción anterior para poder volver hacia atras y hacer requery
		asignar("VAR", VAR_ACCION_ANTERIOR, conectorParametro(VAR_ACCION_ANTERIOR));
		
		//Sacamos de la base de datos la entidad a visualizar
		MaeEncueAplicClienData datosResult = (MaeEncueAplicClienData)getSingleObject(BUSINESSID_QUERY, maeEncueAplicClienQuery, 
			maeEncueAplicClienQuery.clone(), "maeEncueAplicClien");
		...
		MaeEncueAplicClienData maeEncueAplicClien = datosResult;
		es.indra.belcorp.mso.MaeClienData clieOidClie = maeEncueAplicClien.getClieOidClie();
		if(clieOidClie != null && clieOidClie.getId() != null) checkCombo("clieOidClie" , clieOidClie.getId().toString());
		es.indra.belcorp.mso.MaeRespuEncueData reenOidResp = maeEncueAplicClien.getReenOidResp();
		if(reenOidResp != null && reenOidResp.getId() != null) checkCombo("reenOidResp" , reenOidResp.getId().toString());
		...
		//Deshabilitamos todos los campos para que no se editen
					...
					asignarAtributo("COMBO", "clieOidClie", "disabled", "S");
									asignarAtributo("COMBO", "reenOidResp_prenOidPreg", "disabled", "S");

					asignarAtributo("COMBO", "reenOidResp", "disabled", "S");
	...
	protected void cmdCreate() throws Exception{
		...
		asignarAtributo("COMBO", "reenOidResp", "req", "S");
	...
	protected void cmdUpdateForm() throws Exception{
		...
		//asignarAtributo("FORMULARIO", "maeEncueAplicClienFrm", "oculto", "N");
		...
		traza("MMG::Creando MaeEncueAplicClien");
		String id = conectorParametro("idSelection");
		traza("MMG:: id del atributo a modificar: " + id);
	
		//Construimos el mso con los elementos de la búsqueda
		MaeEncueAplicClienData maeEncueAplicClienQuery= new MaeEncueAplicClienData();
		maeEncueAplicClienQuery.setId(new Long(id));
		...
		//asignar("VAR","timestamp",Long.toString(maeEncueAplicClien.jdoGetTimeStamp()));
	...
	*/
	protected void cambiarAcciones() throws Exception{
				asignarAtributo("AREATEXTO", "id", "ontab","ejecutarAccionFocoModificacion(\"maeEncueAplicClienFrm\",1, true)");
				asignarAtributo("AREATEXTO", "id", "onshtab","ejecutarAccionFocoModificacion(\"maeEncueAplicClienFrm\",1, false)");
					asignarAtributo("COMBO","clieOidClie", "ontab", "ejecutarAccionFocoModificacion(\"maeEncueAplicClienFrm\",2, true)");
					asignarAtributo("COMBO","clieOidClie", "onshtab", "ejecutarAccionFocoModificacion(\"maeEncueAplicClienFrm\",2, false)");
					asignarAtributo("COMBO","reenOidResp_prenOidPreg", "ontab", "ejecutarAccionFocoModificacion(\"maeEncueAplicClienFrm\",4, true)");
					asignarAtributo("COMBO","reenOidResp_prenOidPreg", "onshtab", "ejecutarAccionFocoModificacion(\"maeEncueAplicClienFrm\",4, false)");
					asignarAtributo("COMBO","reenOidResp", "ontab", "ejecutarAccionFocoModificacion(\"maeEncueAplicClienFrm\",4, true)");
					asignarAtributo("COMBO","reenOidResp", "onshtab", "ejecutarAccionFocoModificacion(\"maeEncueAplicClienFrm\",4, false)");
	}
	...
	protected void removeLovElements() throws Exception{
		borrarNodo("TD","nombre","maeEncueAplicClienTdLovNullSelectionButton");
	}
	
	protected void removeQueryElements() throws Exception{
		//Borramos los botones para realizar la búsuqeda
		borrarNodo("TR","nombre","maeEncueAplicClienTrButtons");
			...
			traza("MMG: en desactivateNonModifyFields ");		
				...
				traza("MMG: el atributo es clieOidClie...");		
					...
					traza("MMG: en desactivateNonModifyFields encontrado atributo no editable...");
						...
						asignarAtributo("COMBO", "clieOidClie", "disabled", "S");
				traza("MMG: el atributo es reenOidResp...");		
	...
	*/
	protected void establecerSizeCapas(String accion, String origen) throws Exception{
		...
		asignarAtributo("CAPA","maeEncueAplicClienFormLayer", "alto", String.valueOf(sizeForm));		
		
		//Estabelcemos la posicion y de la capa correspondiente a la lista y de su botonera
		asignarAtributo("CAPA","maeEncueAplicClienListLayer", "y", String.valueOf(sizeForm + 26));		
		asignarAtributo("CAPA","maeEncueAplicClienListButtonsLayer", "y", String.valueOf(sizeForm + 278));		
	}

	/**
	* Nos permite cargar las combos finales de la jerarquías de dependencia
	*/
	protected void loadLeafDependenceCombos(MaeEncueAplicClienData maeEncueAplicClien) throws Exception{
		...
		if(maeEncueAplicClien != null && maeEncueAplicClien.getClieOidClie() != null){
				checkCombo("clieOidClie" , maeEncueAplicClien.getClieOidClie().getId().toString());
			}
		MaePreguEncueData reenOidResp_prenOidPreg = new MaePreguEncueData();
		
						asignarComboResultados(BUSINESSID_REENOIDRESP_PRENOIDPREG_QUERY,REENOIDRESP_PRENOIDPREG_COMBO_CONNECTOR, 
			reenOidResp_prenOidPreg, reenOidResp_prenOidPreg.clone(), "maePreguEncue", "reenOidResp_prenOidPreg", "result_ROWSET" );
		if(maeEncueAplicClien != null && maeEncueAplicClien.getReenOidResp().getPrenOidPreg() != null){
				checkCombo("reenOidResp_prenOidPreg" , maeEncueAplicClien.getReenOidResp().getPrenOidPreg().getId().toString());
			}
	...
	*/
	public void cascadeComboLoad(HashMap checkedCombos, String comboName) throws Exception{
						...
						if(comboName.trim().equals("reenOidResp_prenOidPreg") ){
			String reenOidResp_prenOidPregDefaultValue = (String)checkedCombos.get("reenOidResp_prenOidPreg");
			if(reenOidResp_prenOidPregDefaultValue != null){
				MaeRespuEncueData reenOidResp= new MaeRespuEncueData();
				es.indra.belcorp.mso.MaePreguEncueData reenOidRespPrenOidPreg = new es.indra.belcorp.mso.MaePreguEncueData();
				reenOidRespPrenOidPreg.setId(new Long(reenOidResp_prenOidPregDefaultValue ));
				reenOidResp.setPrenOidPreg(reenOidRespPrenOidPreg);
				asignarComboResultados(BUSINESSID_REENOIDRESP_QUERY,REENOIDRESP_COMBO_CONNECTOR, 
					reenOidResp, reenOidResp.clone(), "maeRespuEncue", "reenOidResp", "result_ROWSET",false );
			}
		}
	...
	* @param maeEncueAplicClien La el objetopar la cual se va a realizar la carga
	*/
	protected void loadNonLeafDependenceCombos(MaeEncueAplicClienData maeEncueAplicClien) 
		throws Exception{
		
						if(maeEncueAplicClien.getReenOidResp() != null){
			
						MaeRespuEncueData reenOidResp = new MaeRespuEncueData();
			reenOidResp.setPrenOidPreg(maeEncueAplicClien.getReenOidResp().getPrenOidPreg());
			asignarComboResultados(BUSINESSID_REENOIDRESP_QUERY,REENOIDRESP_COMBO_CONNECTOR, reenOidResp, reenOidResp.clone(), "maeRespuEncue", "reenOidResp", "result_ROWSET");
			checkCombo("reenOidResp" , maeEncueAplicClien.getReenOidResp().getId().toString());
	...
	*/
	protected void loadNonLeafDependenceCombosForRequery() throws Exception{
		...
		MaeEncueAplicClienData maeEncueAplicClien = 
			(MaeEncueAplicClienData) conectorParametroSesion(SESSION_ATTRIBUTE_LAST_QUERY_OBJ_FROM);
		
		//Cargamos todos los objetos completos correspodnientes a los valores de los
		//Campos que sean raiz de una jerarquía de combos dependientes
		if(maeEncueAplicClien.getClieOidClie() != null){
			maeEncueAplicClien.setClieOidClie((es.indra.belcorp.mso.MaeClienData)
			getSingleObject(BUSINESSID_CLIEOIDCLIE_QUERY, 
			maeEncueAplicClien.getClieOidClie(),
			maeEncueAplicClien.getClieOidClie().clone(),
			"maeClien"));
		}
		if(maeEncueAplicClien.getReenOidResp() != null){
			maeEncueAplicClien.setReenOidResp((es.indra.belcorp.mso.MaeRespuEncueData)
			getSingleObject(BUSINESSID_REENOIDRESP_QUERY, 
			maeEncueAplicClien.getReenOidResp(),
			maeEncueAplicClien.getReenOidResp().clone(),
			"maeRespuEncue"));
		}
		
		//Una vez que tenemos completo los datos de los atributos raices de jerarquías de dependencias
		//cargamos las combos y seleccionamos elementos
		loadNonLeafDependenceCombos(maeEncueAplicClien);
		loadLeafDependenceCombos(maeEncueAplicClien);
	}
