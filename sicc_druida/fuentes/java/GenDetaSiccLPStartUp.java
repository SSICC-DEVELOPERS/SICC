/*
    INDRA/CAR/mmg
    $Id: GenDetaSiccLPStartUp.java,v 1.1 2009/12/03 18:40:01 pecbazalar Exp $
    DESC
*/
import java.util.*;
...
public class GenDetaSiccLPStartUp extends MMGDruidaBase{
	...
	public GenDetaSiccLPStartUp(){
		super();
	}

	public void inicio() throws Exception {
		pagina("GenDetaSiccPage");
	}
	...
	public void ejecucion() throws Exception {
		try{
			...
			asignarAtributo("PAGINA", "GenDetaSiccPage", "cod", "GenDetaSicc." + accion + ".label");
			...
			limpiarBotoneraLista("genDetaSicc", accion);
	...
	protected void cmdLov() throws Exception{
		...
		//asignarAtributo("CAPA", "genDetaSiccLovLayer", "visibilidad", "visible");
		asignarAtributo("PAGINA", "GenDetaSiccPage", "onblur", "window.focus();");
	...
	protected void cmdPreView() throws Exception{
		...
		conectorAction("GenDetaSiccLPStartUp");
	...
	*/
	protected void cmdView() throws Exception{
		...
		GenDetaSiccData genDetaSiccQuery= new GenDetaSiccData();
		genDetaSiccQuery.setId(new Long(conectorParametro("idSelection")));
		
		//Asiganmos el valor a la variable de acción anterior para poder volver hacia atras y hacer requery
		asignar("VAR", VAR_ACCION_ANTERIOR, conectorParametro(VAR_ACCION_ANTERIOR));
		
		//Sacamos de la base de datos la entidad a visualizar
		GenDetaSiccData datosResult = (GenDetaSiccData)getSingleObject(BUSINESSID_QUERY, genDetaSiccQuery, 
			genDetaSiccQuery.clone(), "genDetaSicc");
		...
		GenDetaSiccData genDetaSicc = datosResult;
		asignar("TEXT", "enti", FormatUtils.formatObject(genDetaSicc.getEnti(), 
			 MMGDruidaHelper.getUserDecimalFormatPattern(this), decimalFormatSymbols));
		asignar("TEXT", "entiPk", FormatUtils.formatObject(genDetaSicc.getEntiPk(), 
			 MMGDruidaHelper.getUserDecimalFormatPattern(this), decimalFormatSymbols));
		asignar("TEXT", "atri", FormatUtils.formatObject(genDetaSicc.getAtri(), 
			 MMGDruidaHelper.getUserDecimalFormatPattern(this), decimalFormatSymbols));
		asignar("TEXT", "deta", FormatUtils.formatObject(genDetaSicc.getDeta(), 
			 MMGDruidaHelper.getUserDecimalFormatPattern(this), decimalFormatSymbols));
		es.indra.belcorp.mso.SegIdiomData idioOidIdio = genDetaSicc.getIdioOidIdio();
	...
	protected void cmdUpdateForm() throws Exception{
		...
		//asignarAtributo("FORMULARIO", "genDetaSiccFrm", "oculto", "N");
		...
		traza("MMG::Creando GenDetaSicc");
		String id = conectorParametro("idSelection");
		traza("MMG:: id del atributo a modificar: " + id);
	
		//Construimos el mso con los elementos de la búsqueda
		GenDetaSiccData genDetaSiccQuery= new GenDetaSiccData();
		genDetaSiccQuery.setId(new Long(id));
		...
		//asignar("VAR","timestamp",Long.toString(genDetaSicc.jdoGetTimeStamp()));
	...
	*/
	protected void cambiarAcciones() throws Exception{
				asignarAtributo("AREATEXTO", "id", "ontab","ejecutarAccionFocoModificacion(\"genDetaSiccFrm\",0, true)");
				asignarAtributo("AREATEXTO", "id", "onshtab","ejecutarAccionFocoModificacion(\"genDetaSiccFrm\",0, false)");
				asignarAtributo("AREATEXTO", "enti", "ontab","ejecutarAccionFocoModificacion(\"genDetaSiccFrm\",0, true)");
				asignarAtributo("AREATEXTO", "enti", "onshtab","ejecutarAccionFocoModificacion(\"genDetaSiccFrm\",0, false)");
				asignarAtributo("AREATEXTO", "entiPk", "ontab","ejecutarAccionFocoModificacion(\"genDetaSiccFrm\",0, true)");
				asignarAtributo("AREATEXTO", "entiPk", "onshtab","ejecutarAccionFocoModificacion(\"genDetaSiccFrm\",0, false)");
				asignarAtributo("AREATEXTO", "atri", "ontab","ejecutarAccionFocoModificacion(\"genDetaSiccFrm\",0, true)");
				asignarAtributo("AREATEXTO", "atri", "onshtab","ejecutarAccionFocoModificacion(\"genDetaSiccFrm\",0, false)");
				asignarAtributo("AREATEXTO", "deta", "ontab","ejecutarAccionFocoModificacion(\"genDetaSiccFrm\",0, true)");
				asignarAtributo("AREATEXTO", "deta", "onshtab","ejecutarAccionFocoModificacion(\"genDetaSiccFrm\",0, false)");
					asignarAtributo("COMBO","idioOidIdio", "ontab", "ejecutarAccionFocoModificacion(\"genDetaSiccFrm\",0, true)");
					asignarAtributo("COMBO","idioOidIdio", "onshtab", "ejecutarAccionFocoModificacion(\"genDetaSiccFrm\",0, false)");
	}
	...
	protected void removeLovElements() throws Exception{
		borrarNodo("TD","nombre","genDetaSiccTdLovNullSelectionButton");
	}
	
	protected void removeQueryElements() throws Exception{
		//Borramos los botones para realizar la búsuqeda
		borrarNodo("TR","nombre","genDetaSiccTrButtons");
	...
	*/
	protected void establecerSizeCapas(String accion, String origen) throws Exception{
		...
		asignarAtributo("CAPA","genDetaSiccFormLayer", "alto", String.valueOf(sizeForm));		
		
		//Estabelcemos la posicion y de la capa correspondiente a la lista y de su botonera
		asignarAtributo("CAPA","genDetaSiccListLayer", "y", String.valueOf(sizeForm + 26));		
		asignarAtributo("CAPA","genDetaSiccListButtonsLayer", "y", String.valueOf(sizeForm + 278));		
	}

	/**
	* Nos permite cargar las combos finales de la jerarquías de dependencia
	*/
	protected void loadLeafDependenceCombos(GenDetaSiccData genDetaSicc) throws Exception{
		...
		if(genDetaSicc != null && genDetaSicc.getIdioOidIdio() != null){
				checkCombo("idioOidIdio" , genDetaSicc.getIdioOidIdio().getId().toString());
			}
	...
	* @param genDetaSicc La el objetopar la cual se va a realizar la carga
	*/
	protected void loadNonLeafDependenceCombos(GenDetaSiccData genDetaSicc) 
		throws Exception{
	...
	*/
	protected void loadNonLeafDependenceCombosForRequery() throws Exception{
		...
		GenDetaSiccData genDetaSicc = 
			(GenDetaSiccData) conectorParametroSesion(SESSION_ATTRIBUTE_LAST_QUERY_OBJ_FROM);
		
		//Cargamos todos los objetos completos correspodnientes a los valores de los
		//Campos que sean raiz de una jerarquía de combos dependientes
		if(genDetaSicc.getIdioOidIdio() != null){
			genDetaSicc.setIdioOidIdio((es.indra.belcorp.mso.SegIdiomData)
			getSingleObject(BUSINESSID_IDIOOIDIDIO_QUERY, 
			genDetaSicc.getIdioOidIdio(),
			genDetaSicc.getIdioOidIdio().clone(),
			"segIdiom"));
		}
		
		//Una vez que tenemos completo los datos de los atributos raices de jerarquías de dependencias
		//cargamos las combos y seleccionamos elementos
		loadNonLeafDependenceCombos(genDetaSicc);
		loadLeafDependenceCombos(genDetaSicc);
	}
