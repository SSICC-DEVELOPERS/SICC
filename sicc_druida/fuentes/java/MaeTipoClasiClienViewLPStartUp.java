/*
    INDRA/CAR/mmg
    $Id: MaeTipoClasiClienViewLPStartUp.java,v 1.1 2009/12/03 18:33:57 pecbazalar Exp $
    DESC
*/
import java.util.*;
...
public class MaeTipoClasiClienViewLPStartUp extends MMGDruidaBase{
	...
	public MaeTipoClasiClienViewLPStartUp(){
		super();
	}
	...
	public void ejecucion() throws Exception {
		try{
			...
			buildJsPageSize("MaeTipoClasiClienView");
			...
			asignarAtributo("PAGINA", "MaeTipoClasiClienViewPage", "cod", "MaeTipoClasiClienView." + accion + ".label");
			...
			limpiarBotoneraLista("maeTipoClasiClienView", accion);
	...
	protected void cmdLov() throws Exception{
		...
		asignarAtributo("TEXT", "codTipoClas", "validacion", "mensaje = ValidaLongitud(cdos_trim(valor), 1, 2);");
		...
		//asignarAtributo("CAPA", "maeTipoClasiClienViewLovLayer", "visibilidad", "visible");
		asignarAtributo("PAGINA", "MaeTipoClasiClienViewPage", "onblur", "window.focus();");
	...
	*/
	protected void cmdView() throws Exception{
		...
		MaeTipoClasiClienViewData maeTipoClasiClienViewQuery= new MaeTipoClasiClienViewData();
		maeTipoClasiClienViewQuery.setId(new Long(conectorParametro("idSelection")));
		
		//Asiganmos el valor a la variable de acción anterior para poder volver hacia atras y hacer requery
		asignar("VAR", VAR_ACCION_ANTERIOR, conectorParametro(VAR_ACCION_ANTERIOR));
		
		//Sacamos de la base de datos la entidad a visualizar
		MaeTipoClasiClienViewData datosResult = (MaeTipoClasiClienViewData)getSingleObject(BUSINESSID_QUERY, maeTipoClasiClienViewQuery, 
			maeTipoClasiClienViewQuery.clone(), "maeTipoClasiClienView");
		...
		preservaHiddensI18n(new Integer(1), "MAE_TIPO_CLASI_CLIEN", "maeTipoClasiClienViewFrm", "Descripcion", true);
		...
		MaeTipoClasiClienViewData maeTipoClasiClienView = datosResult;
		asignar("TEXT", "codTipoClas", FormatUtils.formatObject(maeTipoClasiClienView.getCodTipoClas(), 
			 MMGDruidaHelper.getUserDecimalFormatPattern(this), decimalFormatSymbols));
		es.indra.belcorp.mso.MaeSubtiClienData sbtiOidSubtClie = maeTipoClasiClienView.getSbtiOidSubtClie();
		...
		loadLocalizationVars(maeTipoClasiClienView);
		
			
		//Deshabilitamos todos los campos para que no se editen
		asignarAtributo("TEXT", "codTipoClas", "disabled", "S");
	...
	protected void cmdCreate() throws Exception{
		//Creamos los campos ocultos para la internacionalización
		preservaHiddensI18n(new Integer(1), "MAE_TIPO_CLASI_CLIEN", "maeTipoClasiClienViewFrm", "Descripcion", false);
		...
		asignarAtributo("TEXT", "codTipoClas", "req", "S");
		asignarAtributo("TEXT", "Descripcion", "req", "S");
		asignarAtributo("COMBO", "sbtiOidSubtClie", "req", "S");
		
		
		//Asiganamos a los campos internacionalizables la función de onblur para actualizar el 
		//valor del idioma por defecto
		asignarAtributo("TEXT", "Descripcion", "onblur", "setDefaultLanguageValue('Descripcion', get('maeTipoClasiClienViewFrm.Descripcion'))");
	...
	protected void cmdUpdateForm() throws Exception{
		...
		//asignarAtributo("FORMULARIO", "maeTipoClasiClienViewFrm", "oculto", "N");
		...
		//Construimos el mso con los elementos de la búsqueda
		MaeTipoClasiClienViewData maeTipoClasiClienViewQuery= new MaeTipoClasiClienViewData();
		maeTipoClasiClienViewQuery.setId(new Long(id));
		...
		//asignar("VAR","timestamp",Long.toString(maeTipoClasiClienView.jdoGetTimeStamp()));
	...
	*/
	protected void cambiarAcciones() throws Exception{
				asignarAtributo("AREATEXTO", "id", "ontab","ejecutarAccionFocoModificacion(\"maeTipoClasiClienViewFrm\",1, true)");
				asignarAtributo("AREATEXTO", "id", "onshtab","ejecutarAccionFocoModificacion(\"maeTipoClasiClienViewFrm\",1, false)");
				asignarAtributo("AREATEXTO", "codTipoClas", "ontab","ejecutarAccionFocoModificacion(\"maeTipoClasiClienViewFrm\",3, true)");
				asignarAtributo("AREATEXTO", "codTipoClas", "onshtab","ejecutarAccionFocoModificacion(\"maeTipoClasiClienViewFrm\",3, false)");
				asignarAtributo("AREATEXTO", "Descripcion", "ontab","ejecutarAccionFocoModificacion(\"maeTipoClasiClienViewFrm\",4, true)");
				asignarAtributo("AREATEXTO", "Descripcion", "onshtab","ejecutarAccionFocoModificacion(\"maeTipoClasiClienViewFrm\",4, false)");
					asignarAtributo("COMBO","sbtiOidSubtClie_ticlOidTipoClie", "ontab", "ejecutarAccionFocoModificacion(\"maeTipoClasiClienViewFrm\",40, true)");
					asignarAtributo("COMBO","sbtiOidSubtClie_ticlOidTipoClie", "onshtab", "ejecutarAccionFocoModificacion(\"maeTipoClasiClienViewFrm\",40, false)");
					asignarAtributo("COMBO","sbtiOidSubtClie", "ontab", "ejecutarAccionFocoModificacion(\"maeTipoClasiClienViewFrm\",40, true)");
					asignarAtributo("COMBO","sbtiOidSubtClie", "onshtab", "ejecutarAccionFocoModificacion(\"maeTipoClasiClienViewFrm\",40, false)");
	}
	...
	protected void removeLovElements() throws Exception{
		borrarNodo("TD","nombre","maeTipoClasiClienViewTdLovNullSelectionButton");
	}
	
	protected void removeQueryElements() throws Exception{
		//Borramos los botones para realizar la búsuqeda
		borrarNodo("TR","nombre","maeTipoClasiClienViewTrButtons");
	...
	*/
	protected void establecerSizeCapas(String accion, String origen) throws Exception{
		...
		asignarAtributo("CAPA","maeTipoClasiClienViewFormLayer", "alto", String.valueOf(sizeForm));		
		
		//Estabelcemos la posicion y de la capa correspondiente a la lista y de su botonera
		asignarAtributo("CAPA","maeTipoClasiClienViewListLayer", "y", String.valueOf(sizeForm + 26));		
		asignarAtributo("CAPA","maeTipoClasiClienViewListButtonsLayer", "y", String.valueOf(sizeForm + 278));		
	}

	/**
	* Nos permite cargar las combos finales de la jerarquías de dependencia
	*/
	protected void loadLeafDependenceCombos(MaeTipoClasiClienViewData maeTipoClasiClienView) throws Exception{
		...
		if(maeTipoClasiClienView != null && maeTipoClasiClienView.getSbtiOidSubtClie().getTiclOidTipoClie() != null){
				checkCombo("sbtiOidSubtClie_ticlOidTipoClie" , maeTipoClasiClienView.getSbtiOidSubtClie().getTiclOidTipoClie().getId().toString());
			}
	...
	* @param maeTipoClasiClienView La el objetopar la cual se va a realizar la carga
	*/
	protected void loadNonLeafDependenceCombos(MaeTipoClasiClienViewData maeTipoClasiClienView) 
		throws Exception{
		
						if(maeTipoClasiClienView.getSbtiOidSubtClie() != null){
			
						MaeSubtiClienData sbtiOidSubtClie = new MaeSubtiClienData();
			sbtiOidSubtClie.setTiclOidTipoClie(maeTipoClasiClienView.getSbtiOidSubtClie().getTiclOidTipoClie());
			...
			checkCombo("sbtiOidSubtClie" , maeTipoClasiClienView.getSbtiOidSubtClie().getId().toString());
	...
	*/
	protected void loadNonLeafDependenceCombosForRequery() throws Exception{
		...
		MaeTipoClasiClienViewData maeTipoClasiClienView = 
			(MaeTipoClasiClienViewData) conectorParametroSesion(SESSION_ATTRIBUTE_LAST_QUERY_OBJ_FROM);
		
		//Cargamos todos los objetos completos correspodnientes a los valores de los
		//Campos que sean raiz de una jerarquía de combos dependientes
		if(maeTipoClasiClienView.getSbtiOidSubtClie() != null){
			maeTipoClasiClienView.setSbtiOidSubtClie((es.indra.belcorp.mso.MaeSubtiClienData)
			getSingleObject(BUSINESSID_SBTIOIDSUBTCLIE_QUERY, 
			maeTipoClasiClienView.getSbtiOidSubtClie(),
			maeTipoClasiClienView.getSbtiOidSubtClie().clone(),
			"maeSubtiClien"));
		}
		
		//Una vez que tenemos completo los datos de los atributos raices de jerarquías de dependencias
		//cargamos las combos y seleccionamos elementos
		loadNonLeafDependenceCombos(maeTipoClasiClienView);
		loadLeafDependenceCombos(maeTipoClasiClienView);
	}
	...
	*/
	...
	protected void loadLocalizationVars(MaeTipoClasiClienViewData maeTipoClasiClienView) throws Exception{
		GenDetaSiccData mmgLocalizationQuery = null;
		Vector mmgLocalizationDatosResult = null;
		mmgLocalizationQuery = new GenDetaSiccData();
		mmgLocalizationQuery.setEnti("MAE_TIPO_CLASI_CLIEN");
		mmgLocalizationQuery.setEntiPk(maeTipoClasiClienView.getId());
