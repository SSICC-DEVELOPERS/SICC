/*
    INDRA/CAR/mmg
    $Id: EduRegalLPStartUp.java,v 1.1 2009/12/03 18:39:30 pecbazalar Exp $
    DESC
*/
import java.util.*;
...
public class EduRegalLPStartUp extends MMGDruidaBase{
	...
	public static final String BUSINESSID_PRODOIDPROD_QUERY ="MMGMaeProduQueryFromToUser";
	public static final String PRODOIDPROD_COMBO_CONNECTOR = "MaeProduConectorCombo";
	...
	public void ejecucion() throws Exception {
		try{
			...
			asignarAtributo("PAGINA", "EduRegalPage", "cod", "EduRegal." + accion + ".label");
			...
			limpiarBotoneraLista("eduRegal", accion);
	...
	protected void cmdLov() throws Exception{
		...
		asignarAtributo("TEXT", "codRega", "validacion", "ValidaInt(cdos_trim(valor), 3, 0, 999)");
		...
		//asignarAtributo("CAPA", "eduRegalLovLayer", "visibilidad", "visible");
		asignarAtributo("PAGINA", "EduRegalPage", "onblur", "window.focus();");
	...
	protected void cmdView() throws Exception{
		...
		EduRegalData eduRegalQuery= new EduRegalData();
		eduRegalQuery.setId(new Integer(conectorParametro("idSelection")));
		
		//Asiganmos el valor a la variable de acción anterior para poder volver hacia atras y hacer requery
		asignar("VAR", VAR_ACCION_ANTERIOR, conectorParametro(VAR_ACCION_ANTERIOR));
		
		//Sacamos de la base de datos la entidad a visualizar
		EduRegalData datosResult = (EduRegalData)getSingleObject(BUSINESSID_QUERY, eduRegalQuery, 
			eduRegalQuery.clone(), "eduRegal");
		...
		preservaHiddensI18n(new Integer(1), "EDU_REGAL", "eduRegalFrm", "Descripcion", true);
		...
		EduRegalData eduRegal = datosResult;
		asignar("TEXT", "codRega", FormatUtils.formatObject(eduRegal.getCodRega(), 
			 MMGDruidaHelper.getUserDecimalFormatPattern(this), MMGDruidaHelper.getUserDecimalFormatSymbols(this)));
		es.indra.belcorp.mso.MaeProduData prodOidProd = eduRegal.getProdOidProd();
		if(prodOidProd != null && prodOidProd.getId() != null) checkCombo("prodOidProd" , prodOidProd.getId().toString());
		asignar("TEXT", "Descripcion", FormatUtils.formatObject(eduRegal.getDescripcion(), 
			 MMGDruidaHelper.getUserDecimalFormatPattern(this), MMGDruidaHelper.getUserDecimalFormatSymbols(this)));
		...
		loadLocalizationVars(eduRegal);
		
			
		//Deshabilitamos todos los campos para que no se editen
		asignarAtributo("TEXT", "codRega", "disabled", "S");
	...
	protected void cmdCreate() throws Exception{
		//Creamos los campos ocultos para la internacionalización
		preservaHiddensI18n(new Integer(1), "EDU_REGAL", "eduRegalFrm", "Descripcion", false);
		...
		asignarAtributo("TEXT", "codRega", "req", "S");
		...
		asignarAtributo("TEXT", "Descripcion", "onblur", "setDefaultLanguageValue('Descripcion', get('eduRegalFrm.Descripcion'))");
	...
	protected void cmdUpdateForm() throws Exception{
		...
		//asignarAtributo("FORMULARIO", "eduRegalFrm", "oculto", "N");
		...
		//Construimos el mso con los elementos de la búsqueda
		EduRegalData eduRegalQuery= new EduRegalData();
		eduRegalQuery.setId(new Integer(id));
		...
		//asignar("VAR","timestamp",Long.toString(eduRegal.jdoGetTimeStamp()));
	...
	protected void removeLovElements() throws Exception{
		borrarNodo("TD","nombre","eduRegalTdLovNullSelectionButton");
	}
	
	protected void removeQueryElements() throws Exception{
		//Borramos los botones para realizar la búsuqeda
		borrarNodo("TR","nombre","eduRegalTrButtons");
	...
	*/
	protected void establecerSizeCapas(String accion, String origen) throws Exception{
		...
		asignarAtributo("CAPA","eduRegalFormLayer", "alto", String.valueOf(sizeForm));		
		
		//Estabelcemos la posicion y de la capa correspondiente a la lista y de su botonera
		asignarAtributo("CAPA","eduRegalListLayer", "y", String.valueOf(sizeForm + 26));		
		asignarAtributo("CAPA","eduRegalListButtonsLayer", "y", String.valueOf(sizeForm + 278));		
	}

	/**
	* Nos permite cargar las combos finales de la jerarquías de dependencia
	*/
	protected void loadLeafDependenceCombos(EduRegalData eduRegal) throws Exception{
		HashMap checkedCombos = new HashMap();
		MaeProduData prodOidProd = new MaeProduData();
		asignarComboResultados(BUSINESSID_PRODOIDPROD_QUERY,PRODOIDPROD_COMBO_CONNECTOR, 
			prodOidProd, prodOidProd.clone(), "maeProdu", "prodOidProd", "result_ROWSET" );
		if(eduRegal != null && eduRegal.getProdOidProd() != null){
				checkCombo("prodOidProd" , eduRegal.getProdOidProd().getId().toString());
			}
	...
	* @param eduRegal La el objetopar la cual se va a realizar la carga
	*/
	protected void loadNonLeafDependenceCombos(EduRegalData eduRegal) 
		throws Exception{
	...
	*/
	protected void loadNonLeafDependenceCombosForRequery() throws Exception{
		...
		EduRegalData eduRegal = 
			(EduRegalData) conectorParametroSesion(SESSION_ATTRIBUTE_LAST_QUERY_OBJ_FROM);
		
		//Cargamos todos los objetos completos correspodnientes a los valores de los
		//Campos que sean raiz de una jerarquía de combos dependientes
		if(eduRegal.getProdOidProd() != null){
			eduRegal.setProdOidProd((es.indra.belcorp.mso.MaeProduData)
			getSingleObject(BUSINESSID_PRODOIDPROD_QUERY, 
			eduRegal.getProdOidProd(),
			eduRegal.getProdOidProd().clone(),
			"maeProdu"));
		}
		
		//Una vez que tenemos completo los datos de los atributos raices de jerarquías de dependencias
		//cargamos las combos y seleccionamos elementos
		loadNonLeafDependenceCombos(eduRegal);
		loadLeafDependenceCombos(eduRegal);
	}
	...
	*/
	...
	protected void loadLocalizationVars(EduRegalData eduRegal) throws Exception{
		GenDetaSiccData mmgLocalizationQuery = null;
		Vector mmgLocalizationDatosResult = null;
		mmgLocalizationQuery = new GenDetaSiccData();
		mmgLocalizationQuery.setEnti("EDU_REGAL");
		mmgLocalizationQuery.setEntiPk(eduRegal.getId());
