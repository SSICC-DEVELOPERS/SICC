/*
    INDRA/CAR/mmg
    $Id: ComCongeLPStartUp.java,v 1.1 2009/12/03 18:34:47 pecbazalar Exp $
    DESC
*/
import java.util.*;
...
public class ComCongeLPStartUp extends MMGDruidaBase{
	...
	public static final short VIEWFORM_SIZE = 313;
	...
	protected static Hashtable indDescAutoValues = new Hashtable();
	static{
		indDescAutoValues.put(new java.lang.Long(0), "No");
		indDescAutoValues.put(new java.lang.Long(1), "Si");
	}
	
	
	// Definicion del constructor
	public ComCongeLPStartUp(){
		super();
	}
	...
	public void ejecucion() throws Exception {
		try{
			...
			asignarAtributo("PAGINA", "ComCongePage", "cod", "ComConge." + accion + ".label");
			...
			limpiarBotoneraLista("comConge", accion);
	...
	protected void cmdLov() throws Exception{
		...
		asignarAtributo("TEXT", "impCong", "validacion", "mensaje = ValidaNumeroMMG(cdos_trim(valor), " + (12 - MMGDruidaHelper.getUserDecimalPrecision(this)) + ", " + MMGDruidaHelper.getUserDecimalPrecision(this)+ ", 'ComConge.impCong', 0, " + MMGDruidaHelper.getMaxValueForDruidaValidaFloat((12 - MMGDruidaHelper.getUserDecimalPrecision(this)), this) + ");");
		...
		//asignarAtributo("CAPA", "comCongeLovLayer", "visibilidad", "visible");
		asignarAtributo("PAGINA", "ComCongePage", "onblur", "window.focus();");
	...
	*/
	protected void cmdView() throws Exception{
		...
		ComCongeData comCongeQuery= new ComCongeData();
		comCongeQuery.setId(new Long(conectorParametro("idSelection")));
		
		//Asiganmos el valor a la variable de acción anterior para poder volver hacia atras y hacer requery
		asignar("VAR", VAR_ACCION_ANTERIOR, conectorParametro(VAR_ACCION_ANTERIOR));
		
		//Sacamos de la base de datos la entidad a visualizar
		ComCongeData datosResult = (ComCongeData)getSingleObject(BUSINESSID_QUERY, comCongeQuery, 
			comCongeQuery.clone(), "comConge");
		...
		if(datosResult.getIndDescAuto() != null)
			asignar("VAR", "indDescAutoCheckValue", datosResult.getIndDescAuto().toString());
		...
		ComCongeData comConge = datosResult;
		asignar("TEXT", "codCong", FormatUtils.formatObject(comConge.getCodCong(), 
			 MMGDruidaHelper.getUserDecimalFormatPattern(this), decimalFormatSymbols));
		asignar("TEXT", "fecDesd", FormatUtils.formatObject(comConge.getFecDesd(), 
			 UtilidadesSession.getFormatoFecha(this), decimalFormatSymbols));
		asignar("TEXT", "fecHast", FormatUtils.formatObject(comConge.getFecHast(), 
			 UtilidadesSession.getFormatoFecha(this), decimalFormatSymbols));
		asignar("TEXT", "impCong", FormatUtils.formatObject(comConge.getImpCong(), 
			 MMGDruidaHelper.getUserDecimalFormatPattern(this), decimalFormatSymbols));
		asignarAtributo("RADIOB", "indDescAuto" , "valor", FormatUtils.formatObject(
			comConge.getIndDescAuto(), 
			 MMGDruidaHelper.getUserDecimalFormatPattern(this), decimalFormatSymbols));
		es.indra.belcorp.mso.MaeClienViewData clieOidClie = comConge.getClieOidClie();
		...
		asignarAtributo("TEXT", "codCong", "disabled", "S");
		asignarAtributo("TEXT", "fecDesd", "disabled", "S");
		asignarAtributo("TEXT", "fecHast", "disabled", "S");
		asignarAtributo("TEXT", "impCong", "disabled", "S");
		
					asignarAtributo("RADIOB", "indDescAuto", "disabled", "S");
	...
	protected void cmdCreate() throws Exception{
		...
		asignarAtributo("TEXT", "impCong", "req", "S");
		asignarAtributo("RADIOB", "indDescAuto", "req", "S");
	...
	protected void cmdUpdateForm() throws Exception{
		...
		//asignarAtributo("FORMULARIO", "comCongeFrm", "oculto", "N");
		...
		//Construimos el mso con los elementos de la búsqueda
		ComCongeData comCongeQuery= new ComCongeData();
		comCongeQuery.setId(new Long(id));
		...
		//asignar("VAR","timestamp",Long.toString(comConge.jdoGetTimeStamp()));
	...
	*/
	protected void cambiarAcciones() throws Exception{
				asignarAtributo("AREATEXTO", "id", "ontab","ejecutarAccionFocoModificacion(\"comCongeFrm\",1, true)");
				asignarAtributo("AREATEXTO", "id", "onshtab","ejecutarAccionFocoModificacion(\"comCongeFrm\",1, false)");
				asignarAtributo("AREATEXTO", "codCong", "ontab","ejecutarAccionFocoModificacion(\"comCongeFrm\",2, true)");
				asignarAtributo("AREATEXTO", "codCong", "onshtab","ejecutarAccionFocoModificacion(\"comCongeFrm\",2, false)");
				asignarAtributo("AREATEXTO", "fecDesd", "ontab","ejecutarAccionFocoModificacion(\"comCongeFrm\",3, true)");
				asignarAtributo("AREATEXTO", "fecDesd", "onshtab","ejecutarAccionFocoModificacion(\"comCongeFrm\",3, false)");
				asignarAtributo("AREATEXTO", "fecHast", "ontab","ejecutarAccionFocoModificacion(\"comCongeFrm\",4, true)");
				asignarAtributo("AREATEXTO", "fecHast", "onshtab","ejecutarAccionFocoModificacion(\"comCongeFrm\",4, false)");
				asignarAtributo("AREATEXTO", "impCong", "ontab","ejecutarAccionFocoModificacion(\"comCongeFrm\",5, true)");
				asignarAtributo("AREATEXTO", "impCong", "onshtab","ejecutarAccionFocoModificacion(\"comCongeFrm\",5, false)");
				asignarAtributo("TEXT", "indDescAuto", "ontab","ejecutarAccionFocoModificacion(\"comCongeFrm\",6, true)");
				asignarAtributo("TEXT", "indDescAuto", "onshtab","ejecutarAccionFocoModificacion(\"comCongeFrm\",6, false)");
					asignarAtributo("COMBO","clieOidClie", "ontab", "ejecutarAccionFocoModificacion(\"comCongeFrm\",7, true)");
					asignarAtributo("COMBO","clieOidClie", "onshtab", "ejecutarAccionFocoModificacion(\"comCongeFrm\",7, false)");
	}
	...
	protected void removeLovElements() throws Exception{
		borrarNodo("TD","nombre","comCongeTdLovNullSelectionButton");
	}
	
	protected void removeQueryElements() throws Exception{
		//Borramos los botones para realizar la búsuqeda
		borrarNodo("TR","nombre","comCongeTrButtons");
	...
	*/
	protected void desactivateNonModifyFields() throws Exception{
			traza("MMG: en desactivateNonModifyFields ");		
				...
				traza("MMG: el atributo es codCong...");		
				
				traza("MMG: el atributo es fecDesd...");		
				
				traza("MMG: el atributo es fecHast...");		
				
				traza("MMG: el atributo es impCong...");		
				
				traza("MMG: el atributo es indDescAuto...");		
	...
	*/
	protected void establecerSizeCapas(String accion, String origen) throws Exception{
		...
		asignarAtributo("CAPA","comCongeFormLayer", "alto", String.valueOf(sizeForm));		
		
		//Estabelcemos la posicion y de la capa correspondiente a la lista y de su botonera
		asignarAtributo("CAPA","comCongeListLayer", "y", String.valueOf(sizeForm + 26));		
		asignarAtributo("CAPA","comCongeListButtonsLayer", "y", String.valueOf(sizeForm + 278));		
	}

	/**
	* Nos permite cargar las combos finales de la jerarquías de dependencia
	*/
	protected void loadLeafDependenceCombos(ComCongeData comConge) throws Exception{
		...
		if(comConge != null && comConge.getClieOidClie() != null){
				checkCombo("clieOidClie" , comConge.getClieOidClie().getId().toString());
			}
	...
	* @param comConge La el objetopar la cual se va a realizar la carga
	*/
	protected void loadNonLeafDependenceCombos(ComCongeData comConge) 
		throws Exception{
	...
	*/
	protected void loadNonLeafDependenceCombosForRequery() throws Exception{
		...
		ComCongeData comConge = 
			(ComCongeData) conectorParametroSesion(SESSION_ATTRIBUTE_LAST_QUERY_OBJ_FROM);
		
		//Cargamos todos los objetos completos correspodnientes a los valores de los
		//Campos que sean raiz de una jerarquía de combos dependientes
		if(comConge.getClieOidClie() != null){
			comConge.setClieOidClie((es.indra.belcorp.mso.MaeClienViewData)
			getSingleObject(BUSINESSID_CLIEOIDCLIE_QUERY, 
			comConge.getClieOidClie(),
			comConge.getClieOidClie().clone(),
			"maeClienView"));
		}
		
		//Una vez que tenemos completo los datos de los atributos raices de jerarquías de dependencias
		//cargamos las combos y seleccionamos elementos
		loadNonLeafDependenceCombos(comConge);
		loadLeafDependenceCombos(comConge);
	}
