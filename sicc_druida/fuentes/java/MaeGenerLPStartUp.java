/*
    INDRA/CAR/mmg
    $Id: MaeGenerLPStartUp.java,v 1.1 2009/12/03 18:33:10 pecbazalar Exp $
    DESC
*/
import java.util.*;
...
public class MaeGenerLPStartUp extends MMGDruidaBase{
	...
	public void ejecucion() throws Exception {
		try{
			...
			asignarAtributo("PAGINA", "MaeGenerPage", "cod", "MaeGener." + accion + ".label");
			...
			limpiarBotoneraLista("maeGener", accion);
	...
	protected void cmdLov() throws Exception{
		...
		asignarAtributo("TEXT", "codGene", "validacion", "mensaje = ValidaLongitud(cdos_trim(valor), 1, 9);");
		...
		//asignarAtributo("CAPA", "maeGenerLovLayer", "visibilidad", "visible");
		asignarAtributo("PAGINA", "MaeGenerPage", "onblur", "window.focus();");
	...
	*/
	protected void cmdView() throws Exception{
		...
		MaeGenerData maeGenerQuery= new MaeGenerData();
		maeGenerQuery.setId(new Long(conectorParametro("idSelection")));
		
		//Asiganmos el valor a la variable de acción anterior para poder volver hacia atras y hacer requery
		asignar("VAR", VAR_ACCION_ANTERIOR, conectorParametro(VAR_ACCION_ANTERIOR));
		
		//Sacamos de la base de datos la entidad a visualizar
		MaeGenerData datosResult = (MaeGenerData)getSingleObject(BUSINESSID_QUERY, maeGenerQuery, 
			maeGenerQuery.clone(), "maeGener");
		...
		preservaHiddensI18n(new Integer(1), "MAE_GENER", "maeGenerFrm", "Descripcion", true);
		...
		MaeGenerData maeGener = datosResult;
		asignar("TEXT", "codGene", FormatUtils.formatObject(maeGener.getCodGene(), 
		...
		loadLocalizationVars(maeGener);
		
			
		//Deshabilitamos todos los campos para que no se editen
		asignarAtributo("TEXT", "codGene", "disabled", "S");
	...
	protected void cmdCreate() throws Exception{
		//Creamos los campos ocultos para la internacionalización
		preservaHiddensI18n(new Integer(1), "MAE_GENER", "maeGenerFrm", "Descripcion", false);
		...
		asignarAtributo("TEXT", "codGene", "req", "S");
		...
		asignarAtributo("TEXT", "Descripcion", "onblur", "setDefaultLanguageValue('Descripcion', get('maeGenerFrm.Descripcion'))");
	...
	protected void cmdUpdateForm() throws Exception{
		...
		//asignarAtributo("FORMULARIO", "maeGenerFrm", "oculto", "N");
		...
		//Construimos el mso con los elementos de la búsqueda
		MaeGenerData maeGenerQuery= new MaeGenerData();
		maeGenerQuery.setId(new Long(id));
		...
		//asignar("VAR","timestamp",Long.toString(maeGener.jdoGetTimeStamp()));
	...
	*/
	protected void cambiarAcciones() throws Exception{
				asignarAtributo("AREATEXTO", "id", "ontab","ejecutarAccionFocoModificacion(\"maeGenerFrm\",1, true)");
				asignarAtributo("AREATEXTO", "id", "onshtab","ejecutarAccionFocoModificacion(\"maeGenerFrm\",1, false)");
					asignarAtributo("COMBO","paisOidPais", "ontab", "ejecutarAccionFocoModificacion(\"maeGenerFrm\",2, true)");
					asignarAtributo("COMBO","paisOidPais", "onshtab", "ejecutarAccionFocoModificacion(\"maeGenerFrm\",2, false)");
				asignarAtributo("AREATEXTO", "codGene", "ontab","ejecutarAccionFocoModificacion(\"maeGenerFrm\",3, true)");
				asignarAtributo("AREATEXTO", "codGene", "onshtab","ejecutarAccionFocoModificacion(\"maeGenerFrm\",3, false)");
				asignarAtributo("AREATEXTO", "Descripcion", "ontab","ejecutarAccionFocoModificacion(\"maeGenerFrm\",4, true)");
				asignarAtributo("AREATEXTO", "Descripcion", "onshtab","ejecutarAccionFocoModificacion(\"maeGenerFrm\",4, false)");
	}
	...
	protected void removeLovElements() throws Exception{
		borrarNodo("TD","nombre","maeGenerTdLovNullSelectionButton");
	}
	
	protected void removeQueryElements() throws Exception{
		//Borramos los botones para realizar la búsuqeda
		borrarNodo("TR","nombre","maeGenerTrButtons");
	...
	*/
	protected void desactivateNonModifyFields() throws Exception{
			traza("MMG: en desactivateNonModifyFields ");		
				...
				traza("MMG: el atributo es codGene...");		
	...
	*/
	protected void establecerSizeCapas(String accion, String origen) throws Exception{
		...
		asignarAtributo("CAPA","maeGenerFormLayer", "alto", String.valueOf(sizeForm));		
		
		//Estabelcemos la posicion y de la capa correspondiente a la lista y de su botonera
		asignarAtributo("CAPA","maeGenerListLayer", "y", String.valueOf(sizeForm + 26));		
		asignarAtributo("CAPA","maeGenerListButtonsLayer", "y", String.valueOf(sizeForm + 278));		
	}

	/**
	* Nos permite cargar las combos finales de la jerarquías de dependencia
	*/
	protected void loadLeafDependenceCombos(MaeGenerData maeGener) throws Exception{
		...
		if(maeGener != null && maeGener.getPaisOidPais() != null){
				checkCombo("paisOidPais" , maeGener.getPaisOidPais().getId().toString());
			}
	...
	* @param maeGener La el objetopar la cual se va a realizar la carga
	*/
	protected void loadNonLeafDependenceCombos(MaeGenerData maeGener) 
		throws Exception{
	...
	*/
	protected void loadNonLeafDependenceCombosForRequery() throws Exception{
		...
		MaeGenerData maeGener = 
			(MaeGenerData) conectorParametroSesion(SESSION_ATTRIBUTE_LAST_QUERY_OBJ_FROM);
		
		//Cargamos todos los objetos completos correspodnientes a los valores de los
		//Campos que sean raiz de una jerarquía de combos dependientes
		if(maeGener.getPaisOidPais() != null){
			maeGener.setPaisOidPais((es.indra.belcorp.mso.SegPaisViewData)
			getSingleObject(BUSINESSID_PAISOIDPAIS_QUERY, 
			maeGener.getPaisOidPais(),
			maeGener.getPaisOidPais().clone(),
			"segPaisView"));
		}
		
		//Una vez que tenemos completo los datos de los atributos raices de jerarquías de dependencias
		//cargamos las combos y seleccionamos elementos
		loadNonLeafDependenceCombos(maeGener);
		loadLeafDependenceCombos(maeGener);
	}
	...
	*/
	...
	protected void loadLocalizationVars(MaeGenerData maeGener) throws Exception{
		GenDetaSiccData mmgLocalizationQuery = null;
		Vector mmgLocalizationDatosResult = null;
		mmgLocalizationQuery = new GenDetaSiccData();
		mmgLocalizationQuery.setEnti("MAE_GENER");
		mmgLocalizationQuery.setEntiPk(maeGener.getId());
