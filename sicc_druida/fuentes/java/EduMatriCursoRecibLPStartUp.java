/*
    INDRA/CAR/mmg
    $Id: EduMatriCursoRecibLPStartUp.java,v 1.1 2009/12/03 18:34:26 pecbazalar Exp $
    DESC
*/
import java.util.*;
...
public class EduMatriCursoRecibLPStartUp extends MMGDruidaBase{
	...
	public static final String BUSINESSID_MCUROIDCURSREQU_QUERY ="MMGEduMatriCursoQueryFromToUser";public static final String MCUROIDCURSREQU_COMBO_CONNECTOR = "EduMatriCursoConectorCombo";
	...
	public EduMatriCursoRecibLPStartUp(){
		super();
	}
	...
	public void ejecucion() throws Exception {
		try{
			...
			buildJsPageSize("EduMatriCursoRecib");
			...
			asignarAtributo("PAGINA", "EduMatriCursoRecibPage", "cod", "EduMatriCursoRecib." + accion + ".label");
			...
			limpiarBotoneraLista("eduMatriCursoRecib", accion);
	...
	protected void cmdLov() throws Exception{
		...
		//asignarAtributo("CAPA", "eduMatriCursoRecibLovLayer", "visibilidad", "visible");
		asignarAtributo("PAGINA", "EduMatriCursoRecibPage", "onblur", "window.focus();");
	...
	protected void cmdView() throws Exception{
		...
		EduMatriCursoRecibData eduMatriCursoRecibQuery= new EduMatriCursoRecibData();
		eduMatriCursoRecibQuery.setId(new Integer(conectorParametro("idSelection")));
		
		//Asiganmos el valor a la variable de acción anterior para poder volver hacia atras y hacer requery
		asignar("VAR", VAR_ACCION_ANTERIOR, conectorParametro(VAR_ACCION_ANTERIOR));
		
		//Sacamos de la base de datos la entidad a visualizar
		EduMatriCursoRecibData datosResult = (EduMatriCursoRecibData)getSingleObject(BUSINESSID_QUERY, eduMatriCursoRecibQuery, 
			eduMatriCursoRecibQuery.clone(), "eduMatriCursoRecib");
		...
		EduMatriCursoRecibData eduMatriCursoRecib = datosResult;
	...
	protected void cmdCreate() throws Exception{
		...
		asignarAtributo("COMBO", "mcurOidCursRequ", "req", "S");
	...
	protected void cmdUpdateForm() throws Exception{
		...
		//asignarAtributo("FORMULARIO", "eduMatriCursoRecibFrm", "oculto", "N");
		...
		EduMatriCursoRecibData eduMatriCursoRecibQuery= new EduMatriCursoRecibData();
		eduMatriCursoRecibQuery.setId(new Integer(id));
		...
		es.indra.belcorp.mso.EduMatriCursoData mcurOidCurs = eduMatriCursoRecib.getMcurOidCurs();
		if(mcurOidCurs != null && mcurOidCurs.getId() != null) checkCombo("mcurOidCurs" , mcurOidCurs.getId().toString());
	
		es.indra.belcorp.mso.EduMatriCursoData mcurOidCursRequ = eduMatriCursoRecib.getMcurOidCursRequ();
		if(mcurOidCursRequ != null && mcurOidCursRequ.getId() != null) checkCombo("mcurOidCursRequ" , mcurOidCursRequ.getId().toString());
		...
		//asignar("VAR","timestamp",Long.toString(eduMatriCursoRecib.jdoGetTimeStamp()));
	...
	protected void removeLovElements() throws Exception{
		borrarNodo("TD","nombre","eduMatriCursoRecibTdLovNullSelectionButton");
	}
	
	protected void removeQueryElements() throws Exception{
		//Borramos los botones para realizar la búsuqeda
		borrarNodo("TR","nombre","eduMatriCursoRecibTrButtons");
	...
	*/
	protected void establecerSizeCapas(String accion, String origen) throws Exception{
		...
		asignarAtributo("CAPA","eduMatriCursoRecibFormLayer", "alto", String.valueOf(sizeForm));		
		
		//Estabelcemos la posicion y de la capa correspondiente a la lista y de su botonera
		asignarAtributo("CAPA","eduMatriCursoRecibPreListLayer", "y", String.valueOf(sizeForm + 24));
		asignarAtributo("CAPA","eduMatriCursoRecibListLayer", "y", String.valueOf(sizeForm + 46));		
		asignarAtributo("CAPA","eduMatriCursoRecibListButtonsLayer", "y", String.valueOf(sizeForm + 299));		
	}
	...
	*/
	protected void loadLeafDependenceCombos() throws Exception{
		...
		asignarComboResultados(BUSINESSID_MCUROIDCURSREQU_QUERY,MCUROIDCURSREQU_COMBO_CONNECTOR, new EduMatriCursoData(), new EduMatriCursoData(), "eduMatriCurso", "mcurOidCursRequ", "result_ROWSET" );
	...
	* @param eduMatriCursoRecib La el objetopar la cual se va a realizar la carga
	*/
	protected void loadNonLeafDependenceCombos(EduMatriCursoRecibData eduMatriCursoRecib) 
		throws Exception{
		if(eduMatriCursoRecib.getMcurOidCurs() != null){
			checkCombo("mcurOidCurs" , eduMatriCursoRecib.getMcurOidCurs().getId().toString());
			}
		if(eduMatriCursoRecib.getMcurOidCursRequ() != null){
			checkCombo("mcurOidCursRequ" , eduMatriCursoRecib.getMcurOidCursRequ().getId().toString());
			}
	...
	*/
	protected void setDependenceComboLabels(EduMatriCursoRecibData eduMatriCursoRecib) 
		throws Exception{
		asignarAtributo("LABELC", "lblViewMcurOidCurs", "valor",FormatUtils.formatObject(eduMatriCursoRecib.getMcurOidCurs()));
		asignarAtributo("LABELC", "lblViewMcurOidCursRequ", "valor",FormatUtils.formatObject(eduMatriCursoRecib.getMcurOidCursRequ()));
	...
	*/
	protected void loadNonLeafDependenceCombosForRequery() throws Exception{
		...
		EduMatriCursoRecibData eduMatriCursoRecib = 
			(EduMatriCursoRecibData) conectorParametroSesion(SESSION_ATTRIBUTE_LAST_QUERY_OBJ_FROM);
		
		//Cargamos todos los objetos completos correspodnientes a los valores de los
		//Campos que sean raiz de una jerarquía de combos dependientes
		if(eduMatriCursoRecib.getMcurOidCurs() != null){
			eduMatriCursoRecib.setMcurOidCurs((es.indra.belcorp.mso.EduMatriCursoData)
			getSingleObject(BUSINESSID_MCUROIDCURS_QUERY, 
			eduMatriCursoRecib.getMcurOidCurs(),
			eduMatriCursoRecib.getMcurOidCurs().clone(),
			"eduMatriCurso"));
		}
		if(eduMatriCursoRecib.getMcurOidCursRequ() != null){
			eduMatriCursoRecib.setMcurOidCursRequ((es.indra.belcorp.mso.EduMatriCursoData)
			getSingleObject(BUSINESSID_MCUROIDCURSREQU_QUERY, 
			eduMatriCursoRecib.getMcurOidCursRequ(),
			eduMatriCursoRecib.getMcurOidCursRequ().clone(),
			"eduMatriCurso"));
		}
		
		//Una vez que tenemos completo los datos de los atributos raices de jerarquías de dependencias
		//cargamos las combos y seleccionamos elementos
		loadNonLeafDependenceCombos(eduMatriCursoRecib);
	}
	...
	*/
	protected void removeViewLabels() throws Exception{
		...
		borrarNodo("TR", "nombre", "mcurOidCursRequViewLabelTrId");
	...
	*/
	protected void removeWidgets() throws Exception{
		...
		borrarNodo("TR", "nombre", "mcurOidCursRequWidgetTrId");
