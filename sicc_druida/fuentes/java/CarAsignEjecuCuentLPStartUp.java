/*
    INDRA/CAR/mmg
    $Id: CarAsignEjecuCuentLPStartUp.java,v 1.1 2009/12/03 18:33:13 pecbazalar Exp $
    DESC
*/
import java.util.*;
...
public class CarAsignEjecuCuentLPStartUp extends MMGDruidaBase{
	//Constantes que definen la altura de los formularios según el modo
	public static final short QUERYFORM_SIZE = 258;
	public static final short CREATEFORM_SIZE = 258;
	public static final short UPDATEFORM_SIZE = 258;
	public static final short VIEWFORM_SIZE = 258;
	...
	public CarAsignEjecuCuentLPStartUp(){
		super();
	}
	...
	public void ejecucion() throws Exception {
		try{
			...
			buildJsPageSize("CarAsignEjecuCuent");
			...
			asignarAtributo("PAGINA", "CarAsignEjecuCuentPage", "cod", "CarAsignEjecuCuent." + accion + ".label");
			...
			limpiarBotoneraLista("carAsignEjecuCuent", accion);
	...
	protected void cmdLov() throws Exception{
		...
		//asignarAtributo("CAPA", "carAsignEjecuCuentLovLayer", "visibilidad", "visible");
		asignarAtributo("PAGINA", "CarAsignEjecuCuentPage", "onblur", "window.focus();");
	...
	protected void cmdView() throws Exception{
		...
		CarAsignEjecuCuentData carAsignEjecuCuentQuery= new CarAsignEjecuCuentData();
		carAsignEjecuCuentQuery.setId(new Integer(conectorParametro("idSelection")));
		
		//Asiganmos el valor a la variable de acción anterior para poder volver hacia atras y hacer requery
		asignar("VAR", VAR_ACCION_ANTERIOR, conectorParametro(VAR_ACCION_ANTERIOR));
		
		//Sacamos de la base de datos la entidad a visualizar
		CarAsignEjecuCuentData datosResult = (CarAsignEjecuCuentData)getSingleObject(BUSINESSID_QUERY, carAsignEjecuCuentQuery, 
			carAsignEjecuCuentQuery.clone(), "carAsignEjecuCuent");
		...
		CarAsignEjecuCuentData carAsignEjecuCuent = datosResult;
	...
	protected void cmdUpdateForm() throws Exception{
		...
		//asignarAtributo("FORMULARIO", "carAsignEjecuCuentFrm", "oculto", "N");
		...
		CarAsignEjecuCuentData carAsignEjecuCuentQuery= new CarAsignEjecuCuentData();
		carAsignEjecuCuentQuery.setId(new Integer(id));
		...
		es.indra.belcorp.mso.CarNivelRiesgData niriOidNiveRies = carAsignEjecuCuent.getNiriOidNiveRies();
		if(niriOidNiveRies != null && niriOidNiveRies.getId() != null) checkCombo("niriOidNiveRies" , niriOidNiveRies.getId().toString());
	
		es.indra.belcorp.mso.ZonSubGerenVentaData zsgvOidSubgVent = carAsignEjecuCuent.getZsgvOidSubgVent();
		if(zsgvOidSubgVent != null && zsgvOidSubgVent.getId() != null) checkCombo("zsgvOidSubgVent" , zsgvOidSubgVent.getId().toString());
	
		es.indra.belcorp.mso.ZonRegioData zorgOidRegi = carAsignEjecuCuent.getZorgOidRegi();
		if(zorgOidRegi != null && zorgOidRegi.getId() != null) checkCombo("zorgOidRegi" , zorgOidRegi.getId().toString());
	
		es.indra.belcorp.mso.ZonZonaData zzonOidZona = carAsignEjecuCuent.getZzonOidZona();
		if(zzonOidZona != null && zzonOidZona.getId() != null) checkCombo("zzonOidZona" , zzonOidZona.getId().toString());
	
		es.indra.belcorp.mso.CarEjecuCuentData ejcuOidEjecCuen = carAsignEjecuCuent.getEjcuOidEjecCuen();
		...
		//asignar("VAR","timestamp",Long.toString(carAsignEjecuCuent.jdoGetTimeStamp()));
	...
	protected void removeLovElements() throws Exception{
		borrarNodo("TD","nombre","carAsignEjecuCuentTdLovNullSelectionButton");
	}
	
	protected void removeQueryElements() throws Exception{
		//Borramos los botones para realizar la búsuqeda
		borrarNodo("TR","nombre","carAsignEjecuCuentTrButtons");
	...
	*/
	protected void establecerSizeCapas(String accion, String origen) throws Exception{
		...
		asignarAtributo("CAPA","carAsignEjecuCuentFormLayer", "alto", String.valueOf(sizeForm));		
		
		//Estabelcemos la posicion y de la capa correspondiente a la lista y de su botonera
		asignarAtributo("CAPA","carAsignEjecuCuentPreListLayer", "y", String.valueOf(sizeForm + 24));
		asignarAtributo("CAPA","carAsignEjecuCuentListLayer", "y", String.valueOf(sizeForm + 46));		
		asignarAtributo("CAPA","carAsignEjecuCuentListButtonsLayer", "y", String.valueOf(sizeForm + 299));		
	}
	...
	* @param carAsignEjecuCuent La el objetopar la cual se va a realizar la carga
	*/
	protected void loadNonLeafDependenceCombos(CarAsignEjecuCuentData carAsignEjecuCuent) 
		throws Exception{
		if(carAsignEjecuCuent.getNiriOidNiveRies() != null){
			checkCombo("niriOidNiveRies" , carAsignEjecuCuent.getNiriOidNiveRies().getId().toString());
			}
		if(carAsignEjecuCuent.getZsgvOidSubgVent() != null){
			checkCombo("zsgvOidSubgVent" , carAsignEjecuCuent.getZsgvOidSubgVent().getId().toString());
			}
		if(carAsignEjecuCuent.getZorgOidRegi() != null){
			checkCombo("zorgOidRegi" , carAsignEjecuCuent.getZorgOidRegi().getId().toString());
			}
		if(carAsignEjecuCuent.getZzonOidZona() != null){
			checkCombo("zzonOidZona" , carAsignEjecuCuent.getZzonOidZona().getId().toString());
			}
		if(carAsignEjecuCuent.getEjcuOidEjecCuen() != null){
			checkCombo("ejcuOidEjecCuen" , carAsignEjecuCuent.getEjcuOidEjecCuen().getId().toString());
			}
	...
	*/
	protected void setDependenceComboLabels(CarAsignEjecuCuentData carAsignEjecuCuent) 
		throws Exception{
		asignarAtributo("LABELC", "lblViewNiriOidNiveRies", "valor",FormatUtils.formatObject(carAsignEjecuCuent.getNiriOidNiveRies()));
		asignarAtributo("LABELC", "lblViewZsgvOidSubgVent", "valor",FormatUtils.formatObject(carAsignEjecuCuent.getZsgvOidSubgVent()));
		asignarAtributo("LABELC", "lblViewZorgOidRegi", "valor",FormatUtils.formatObject(carAsignEjecuCuent.getZorgOidRegi()));
		asignarAtributo("LABELC", "lblViewZzonOidZona", "valor",FormatUtils.formatObject(carAsignEjecuCuent.getZzonOidZona()));
		asignarAtributo("LABELC", "lblViewEjcuOidEjecCuen", "valor",FormatUtils.formatObject(carAsignEjecuCuent.getEjcuOidEjecCuen()));
	...
	*/
	protected void loadNonLeafDependenceCombosForRequery() throws Exception{
		...
		CarAsignEjecuCuentData carAsignEjecuCuent = 
			(CarAsignEjecuCuentData) conectorParametroSesion(SESSION_ATTRIBUTE_LAST_QUERY_OBJ_FROM);
		
		//Cargamos todos los objetos completos correspodnientes a los valores de los
		//Campos que sean raiz de una jerarquía de combos dependientes
		if(carAsignEjecuCuent.getNiriOidNiveRies() != null){
			carAsignEjecuCuent.setNiriOidNiveRies((es.indra.belcorp.mso.CarNivelRiesgData)
			getSingleObject(BUSINESSID_NIRIOIDNIVERIES_QUERY, 
			carAsignEjecuCuent.getNiriOidNiveRies(),
			carAsignEjecuCuent.getNiriOidNiveRies().clone(),
			"carNivelRiesg"));
		}
		if(carAsignEjecuCuent.getZsgvOidSubgVent() != null){
			carAsignEjecuCuent.setZsgvOidSubgVent((es.indra.belcorp.mso.ZonSubGerenVentaData)
			getSingleObject(BUSINESSID_ZSGVOIDSUBGVENT_QUERY, 
			carAsignEjecuCuent.getZsgvOidSubgVent(),
			carAsignEjecuCuent.getZsgvOidSubgVent().clone(),
			"zonSubGerenVenta"));
		}
		if(carAsignEjecuCuent.getZorgOidRegi() != null){
			carAsignEjecuCuent.setZorgOidRegi((es.indra.belcorp.mso.ZonRegioData)
			getSingleObject(BUSINESSID_ZORGOIDREGI_QUERY, 
			carAsignEjecuCuent.getZorgOidRegi(),
			carAsignEjecuCuent.getZorgOidRegi().clone(),
			"zonRegio"));
		}
		if(carAsignEjecuCuent.getZzonOidZona() != null){
			carAsignEjecuCuent.setZzonOidZona((es.indra.belcorp.mso.ZonZonaData)
			getSingleObject(BUSINESSID_ZZONOIDZONA_QUERY, 
			carAsignEjecuCuent.getZzonOidZona(),
			carAsignEjecuCuent.getZzonOidZona().clone(),
			"zonZona"));
		}
		if(carAsignEjecuCuent.getEjcuOidEjecCuen() != null){
			carAsignEjecuCuent.setEjcuOidEjecCuen((es.indra.belcorp.mso.CarEjecuCuentData)
			getSingleObject(BUSINESSID_EJCUOIDEJECCUEN_QUERY, 
			carAsignEjecuCuent.getEjcuOidEjecCuen(),
			carAsignEjecuCuent.getEjcuOidEjecCuen().clone(),
			"carEjecuCuent"));
		}
		
		//Una vez que tenemos completo los datos de los atributos raices de jerarquías de dependencias
		//cargamos las combos y seleccionamos elementos
		loadNonLeafDependenceCombos(carAsignEjecuCuent);
	}
