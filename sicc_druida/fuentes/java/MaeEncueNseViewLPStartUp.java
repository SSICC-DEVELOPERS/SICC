/*
    INDRA/CAR/mmg
    $Id: MaeEncueNseViewLPStartUp.java,v 1.1 2009/12/03 18:32:22 pecbazalar Exp $
    DESC
*/
import java.util.*;
...
public class MaeEncueNseViewLPStartUp extends MMGDruidaBase{
	...
	public MaeEncueNseViewLPStartUp(){
		super();
	}

	public void inicio() throws Exception {
		pagina("MaeEncueNseViewPage");
	}
	...
	public void ejecucion() throws Exception {
		try{
			...
			buildJsPageSize("MaeEncueNseView");
			...
			asignarAtributo("PAGINA", "MaeEncueNseViewPage", "cod", "MaeEncueNseView." + accion + ".label");
			...
			limpiarBotoneraLista("maeEncueNseView", accion);
	...
	protected void cmdLov() throws Exception{
		...
		asignarAtributo("TEXT", "codTipoEncu", "validacion", "mensaje = ValidaLongitud(cdos_trim(valor), 1, 3);");
		...
		//asignarAtributo("CAPA", "maeEncueNseViewLovLayer", "visibilidad", "visible");
		asignarAtributo("PAGINA", "MaeEncueNseViewPage", "onblur", "window.focus();");
	...
	protected void cmdPreView() throws Exception{
		...
		conectorAction("MaeEncueNseViewLPStartUp");
	...
	*/
	protected void cmdView() throws Exception{
		...
		MaeEncueNseViewData maeEncueNseViewQuery= new MaeEncueNseViewData();
		maeEncueNseViewQuery.setId(new Long(conectorParametro("idSelection")));
		
		//Asiganmos el valor a la variable de acción anterior para poder volver hacia atras y hacer requery
		asignar("VAR", VAR_ACCION_ANTERIOR, conectorParametro(VAR_ACCION_ANTERIOR));
		
		//Sacamos de la base de datos la entidad a visualizar
		MaeEncueNseViewData datosResult = (MaeEncueNseViewData)getSingleObject(BUSINESSID_QUERY, maeEncueNseViewQuery, 
			maeEncueNseViewQuery.clone(), "maeEncueNseView");
		...
		preservaHiddensI18n(new Integer(1), "MAE_ENCUE_NSE", "maeEncueNseViewFrm", "Descripcion", true);
		...
		MaeEncueNseViewData maeEncueNseView = datosResult;
		asignar("TEXT", "codTipoEncu", FormatUtils.formatObject(maeEncueNseView.getCodTipoEncu(), 
			 MMGDruidaHelper.getUserDecimalFormatPattern(this), decimalFormatSymbols));
		asignar("TEXT", "Descripcion", FormatUtils.formatObject(maeEncueNseView.getDescripcion(), 
			 MMGDruidaHelper.getUserDecimalFormatPattern(this), decimalFormatSymbols));
		es.indra.belcorp.mso.SegMarcaData marcOidMarc = maeEncueNseView.getMarcOidMarc();
		...
		loadLocalizationVars(maeEncueNseView);
		
			
		//Deshabilitamos todos los campos para que no se editen
		asignarAtributo("TEXT", "codTipoEncu", "disabled", "S");
	...
	protected void cmdCreate() throws Exception{
		//Creamos los campos ocultos para la internacionalización
		preservaHiddensI18n(new Integer(1), "MAE_ENCUE_NSE", "maeEncueNseViewFrm", "Descripcion", false);
		...
		asignarAtributo("TEXT", "codTipoEncu", "req", "S");
		...
		asignarAtributo("TEXT", "Descripcion", "onblur", "setDefaultLanguageValue('Descripcion', get('maeEncueNseViewFrm.Descripcion'))");
	...
	protected void cmdUpdateForm() throws Exception{
		...
		//asignarAtributo("FORMULARIO", "maeEncueNseViewFrm", "oculto", "N");
		...
		traza("MMG::Creando MaeEncueNseView");
		String id = conectorParametro("idSelection");
		traza("MMG:: id del atributo a modificar: " + id);
	
		//Construimos el mso con los elementos de la búsqueda
		MaeEncueNseViewData maeEncueNseViewQuery= new MaeEncueNseViewData();
		maeEncueNseViewQuery.setId(new Long(id));
		...
		//asignar("VAR","timestamp",Long.toString(maeEncueNseView.jdoGetTimeStamp()));
	...
	*/
	protected void cambiarAcciones() throws Exception{
				asignarAtributo("AREATEXTO", "id", "ontab","ejecutarAccionFocoModificacion(\"maeEncueNseViewFrm\",1, true)");
				asignarAtributo("AREATEXTO", "id", "onshtab","ejecutarAccionFocoModificacion(\"maeEncueNseViewFrm\",1, false)");
					asignarAtributo("COMBO","paisOidPais", "ontab", "ejecutarAccionFocoModificacion(\"maeEncueNseViewFrm\",2, true)");
					asignarAtributo("COMBO","paisOidPais", "onshtab", "ejecutarAccionFocoModificacion(\"maeEncueNseViewFrm\",2, false)");
				asignarAtributo("AREATEXTO", "codTipoEncu", "ontab","ejecutarAccionFocoModificacion(\"maeEncueNseViewFrm\",3, true)");
				asignarAtributo("AREATEXTO", "codTipoEncu", "onshtab","ejecutarAccionFocoModificacion(\"maeEncueNseViewFrm\",3, false)");
				asignarAtributo("AREATEXTO", "Descripcion", "ontab","ejecutarAccionFocoModificacion(\"maeEncueNseViewFrm\",4, true)");
				asignarAtributo("AREATEXTO", "Descripcion", "onshtab","ejecutarAccionFocoModificacion(\"maeEncueNseViewFrm\",4, false)");
					asignarAtributo("COMBO","marcOidMarc", "ontab", "ejecutarAccionFocoModificacion(\"maeEncueNseViewFrm\",7, true)");
					asignarAtributo("COMBO","marcOidMarc", "onshtab", "ejecutarAccionFocoModificacion(\"maeEncueNseViewFrm\",7, false)");
	}
	...
	protected void removeLovElements() throws Exception{
		borrarNodo("TD","nombre","maeEncueNseViewTdLovNullSelectionButton");
	}
	
	protected void removeQueryElements() throws Exception{
		//Borramos los botones para realizar la búsuqeda
		borrarNodo("TR","nombre","maeEncueNseViewTrButtons");
	...
	*/
	protected void desactivateNonModifyFields() throws Exception{
			traza("MMG: en desactivateNonModifyFields ");		
				...
				traza("MMG: el atributo es codTipoEncu...");		
	...
	*/
	protected void establecerSizeCapas(String accion, String origen) throws Exception{
		...
		asignarAtributo("CAPA","maeEncueNseViewFormLayer", "alto", String.valueOf(sizeForm));		
		
		//Estabelcemos la posicion y de la capa correspondiente a la lista y de su botonera
		asignarAtributo("CAPA","maeEncueNseViewListLayer", "y", String.valueOf(sizeForm + 26));		
		asignarAtributo("CAPA","maeEncueNseViewListButtonsLayer", "y", String.valueOf(sizeForm + 278));		
	}

	/**
	* Nos permite cargar las combos finales de la jerarquías de dependencia
	*/
	protected void loadLeafDependenceCombos(MaeEncueNseViewData maeEncueNseView) throws Exception{
		...
		if(maeEncueNseView != null && maeEncueNseView.getPaisOidPais() != null){
				checkCombo("paisOidPais" , maeEncueNseView.getPaisOidPais().getId().toString());
			}
		...
		if(maeEncueNseView != null && maeEncueNseView.getMarcOidMarc() != null){
				checkCombo("marcOidMarc" , maeEncueNseView.getMarcOidMarc().getId().toString());
			}
	...
	* @param maeEncueNseView La el objetopar la cual se va a realizar la carga
	*/
	protected void loadNonLeafDependenceCombos(MaeEncueNseViewData maeEncueNseView) 
		throws Exception{
	...
	*/
	protected void loadNonLeafDependenceCombosForRequery() throws Exception{
		...
		MaeEncueNseViewData maeEncueNseView = 
			(MaeEncueNseViewData) conectorParametroSesion(SESSION_ATTRIBUTE_LAST_QUERY_OBJ_FROM);
		
		//Cargamos todos los objetos completos correspodnientes a los valores de los
		//Campos que sean raiz de una jerarquía de combos dependientes
		if(maeEncueNseView.getPaisOidPais() != null){
			maeEncueNseView.setPaisOidPais((es.indra.belcorp.mso.SegPaisViewData)
			getSingleObject(BUSINESSID_PAISOIDPAIS_QUERY, 
			maeEncueNseView.getPaisOidPais(),
			maeEncueNseView.getPaisOidPais().clone(),
			"segPaisView"));
		}
		if(maeEncueNseView.getMarcOidMarc() != null){
			maeEncueNseView.setMarcOidMarc((es.indra.belcorp.mso.SegMarcaData)
			getSingleObject(BUSINESSID_MARCOIDMARC_QUERY, 
			maeEncueNseView.getMarcOidMarc(),
			maeEncueNseView.getMarcOidMarc().clone(),
			"segMarca"));
		}
		
		//Una vez que tenemos completo los datos de los atributos raices de jerarquías de dependencias
		//cargamos las combos y seleccionamos elementos
		loadNonLeafDependenceCombos(maeEncueNseView);
		loadLeafDependenceCombos(maeEncueNseView);
	}
	...
	*/
	...
	protected void loadLocalizationVars(MaeEncueNseViewData maeEncueNseView) throws Exception{
		GenDetaSiccData mmgLocalizationQuery = null;
		Vector mmgLocalizationDatosResult = null;
		mmgLocalizationQuery = new GenDetaSiccData();
		mmgLocalizationQuery.setEnti("MAE_ENCUE_NSE");
		mmgLocalizationQuery.setEntiPk(maeEncueNseView.getId());
