/*
    INDRA/CAR/mmg
    $Id: CccMarcaSituaLPStartUp.java,v 1.1 2009/12/03 18:33:50 pecbazalar Exp $
    DESC
*/
import java.util.*;
...
public class CccMarcaSituaLPStartUp extends MMGDruidaBase{
	...
	protected static Hashtable indCuenCastValues = new Hashtable();
	static{
		indCuenCastValues.put(new java.lang.String("1"), "Si");
		indCuenCastValues.put(new java.lang.String("0"), "No");
	}
	
	
	// Definicion del constructor
	public CccMarcaSituaLPStartUp(){
		super();
	}
	...
	public void ejecucion() throws Exception {
		try{
			...
			asignarAtributo("PAGINA", "CccMarcaSituaPage", "cod", "CccMarcaSitua." + accion + ".label");
			...
			limpiarBotoneraLista("cccMarcaSitua", accion);
	...
	protected void cmdLov() throws Exception{
		...
		asignarAtributo("TEXT", "codMarcSitu", "validacion", "mensaje = ValidaLongitud(cdos_trim(valor), 1, 2);");
		...
		//asignarAtributo("CAPA", "cccMarcaSituaLovLayer", "visibilidad", "visible");
		asignarAtributo("PAGINA", "CccMarcaSituaPage", "onblur", "window.focus();");
	...
	*/
	protected void cmdView() throws Exception{
		...
		CccMarcaSituaData cccMarcaSituaQuery= new CccMarcaSituaData();
		cccMarcaSituaQuery.setId(new Long(conectorParametro("idSelection")));
		
		//Asiganmos el valor a la variable de acción anterior para poder volver hacia atras y hacer requery
		asignar("VAR", VAR_ACCION_ANTERIOR, conectorParametro(VAR_ACCION_ANTERIOR));
		
		//Sacamos de la base de datos la entidad a visualizar
		CccMarcaSituaData datosResult = (CccMarcaSituaData)getSingleObject(BUSINESSID_QUERY, cccMarcaSituaQuery, 
			cccMarcaSituaQuery.clone(), "cccMarcaSitua");
		...
		preservaHiddensI18n(new Integer(1), "CCC_MARCA_SITUA", "cccMarcaSituaFrm", "Descripcion", true);
		
		
		//Guardamos los valores de los radiobuttons
		if(datosResult.getIndCuenCast() != null)
			asignar("VAR", "indCuenCastCheckValue", datosResult.getIndCuenCast().toString());
		...
		CccMarcaSituaData cccMarcaSitua = datosResult;
		asignar("TEXT", "codMarcSitu", FormatUtils.formatObject(cccMarcaSitua.getCodMarcSitu(), 
			 MMGDruidaHelper.getUserDecimalFormatPattern(this), decimalFormatSymbols));
		asignar("TEXT", "Descripcion", FormatUtils.formatObject(cccMarcaSitua.getDescripcion(), 
			 MMGDruidaHelper.getUserDecimalFormatPattern(this), decimalFormatSymbols));
		asignarAtributo("RADIOB", "indCuenCast" , "valor", FormatUtils.formatObject(
			cccMarcaSitua.getIndCuenCast(), 
			 MMGDruidaHelper.getUserDecimalFormatPattern(this), decimalFormatSymbols));
		asignar("AREATEXTO", "valObse", FormatUtils.formatObject(cccMarcaSitua.getValObse(), 
			 MMGDruidaHelper.getUserDecimalFormatPattern(this), decimalFormatSymbols));
		...
		loadLocalizationVars(cccMarcaSitua);
		
			
		//Deshabilitamos todos los campos para que no se editen
		asignarAtributo("TEXT", "codMarcSitu", "disabled", "S");
		asignarAtributo("TEXT", "Descripcion", "disabled", "S");
		
					asignarAtributo("RADIOB", "indCuenCast", "disabled", "S");
	...
	protected void cmdCreate() throws Exception{
		//Creamos los campos ocultos para la internacionalización
		preservaHiddensI18n(new Integer(1), "CCC_MARCA_SITUA", "cccMarcaSituaFrm", "Descripcion", false);
		...
		asignarAtributo("TEXT", "codMarcSitu", "req", "S");
		asignarAtributo("TEXT", "Descripcion", "req", "S");
		asignarAtributo("RADIOB", "indCuenCast", "req", "S");
		
		
		//Asiganamos a los campos internacionalizables la función de onblur para actualizar el 
		//valor del idioma por defecto
		asignarAtributo("TEXT", "Descripcion", "onblur", "setDefaultLanguageValue('Descripcion', get('cccMarcaSituaFrm.Descripcion'))");
	...
	protected void cmdUpdateForm() throws Exception{
		...
		//asignarAtributo("FORMULARIO", "cccMarcaSituaFrm", "oculto", "N");
		...
		//Construimos el mso con los elementos de la búsqueda
		CccMarcaSituaData cccMarcaSituaQuery= new CccMarcaSituaData();
		cccMarcaSituaQuery.setId(new Long(id));
		...
		//asignar("VAR","timestamp",Long.toString(cccMarcaSitua.jdoGetTimeStamp()));
	...
	*/
	protected void cambiarAcciones() throws Exception{
				asignarAtributo("AREATEXTO", "id", "ontab","ejecutarAccionFocoModificacion(\"cccMarcaSituaFrm\",1, true)");
				asignarAtributo("AREATEXTO", "id", "onshtab","ejecutarAccionFocoModificacion(\"cccMarcaSituaFrm\",1, false)");
					asignarAtributo("COMBO","paisOidPais", "ontab", "ejecutarAccionFocoModificacion(\"cccMarcaSituaFrm\",2, true)");
					asignarAtributo("COMBO","paisOidPais", "onshtab", "ejecutarAccionFocoModificacion(\"cccMarcaSituaFrm\",2, false)");
				asignarAtributo("AREATEXTO", "codMarcSitu", "ontab","ejecutarAccionFocoModificacion(\"cccMarcaSituaFrm\",3, true)");
				asignarAtributo("AREATEXTO", "codMarcSitu", "onshtab","ejecutarAccionFocoModificacion(\"cccMarcaSituaFrm\",3, false)");
				asignarAtributo("AREATEXTO", "Descripcion", "ontab","ejecutarAccionFocoModificacion(\"cccMarcaSituaFrm\",4, true)");
				asignarAtributo("AREATEXTO", "Descripcion", "onshtab","ejecutarAccionFocoModificacion(\"cccMarcaSituaFrm\",4, false)");
				asignarAtributo("TEXT", "indCuenCast", "ontab","ejecutarAccionFocoModificacion(\"cccMarcaSituaFrm\",5, true)");
				asignarAtributo("TEXT", "indCuenCast", "onshtab","ejecutarAccionFocoModificacion(\"cccMarcaSituaFrm\",5, false)");
				asignarAtributo("AREATEXTO", "valObse", "ontab","ejecutarAccionFocoModificacion(\"cccMarcaSituaFrm\",6, true)");
				asignarAtributo("AREATEXTO", "valObse", "onshtab","ejecutarAccionFocoModificacion(\"cccMarcaSituaFrm\",6, false)");
	}
	...
	protected void removeLovElements() throws Exception{
		borrarNodo("TD","nombre","cccMarcaSituaTdLovNullSelectionButton");
	}
	
	protected void removeQueryElements() throws Exception{
		//Borramos los botones para realizar la búsuqeda
		borrarNodo("TR","nombre","cccMarcaSituaTrButtons");
	...
	*/
	protected void establecerSizeCapas(String accion, String origen) throws Exception{
		...
		asignarAtributo("CAPA","cccMarcaSituaFormLayer", "alto", String.valueOf(sizeForm));		
		
		//Estabelcemos la posicion y de la capa correspondiente a la lista y de su botonera
		asignarAtributo("CAPA","cccMarcaSituaListLayer", "y", String.valueOf(sizeForm + 26));		
		asignarAtributo("CAPA","cccMarcaSituaListButtonsLayer", "y", String.valueOf(sizeForm + 278));		
	}

	/**
	* Nos permite cargar las combos finales de la jerarquías de dependencia
	*/
	protected void loadLeafDependenceCombos(CccMarcaSituaData cccMarcaSitua) throws Exception{
		...
		if(cccMarcaSitua != null && cccMarcaSitua.getPaisOidPais() != null){
				checkCombo("paisOidPais" , cccMarcaSitua.getPaisOidPais().getId().toString());
			}
	...
	* @param cccMarcaSitua La el objetopar la cual se va a realizar la carga
	*/
	protected void loadNonLeafDependenceCombos(CccMarcaSituaData cccMarcaSitua) 
		throws Exception{
	...
	*/
	protected void loadNonLeafDependenceCombosForRequery() throws Exception{
		...
		CccMarcaSituaData cccMarcaSitua = 
			(CccMarcaSituaData) conectorParametroSesion(SESSION_ATTRIBUTE_LAST_QUERY_OBJ_FROM);
		
		//Cargamos todos los objetos completos correspodnientes a los valores de los
		//Campos que sean raiz de una jerarquía de combos dependientes
		if(cccMarcaSitua.getPaisOidPais() != null){
			cccMarcaSitua.setPaisOidPais((es.indra.belcorp.mso.SegPaisViewData)
			getSingleObject(BUSINESSID_PAISOIDPAIS_QUERY, 
			cccMarcaSitua.getPaisOidPais(),
			cccMarcaSitua.getPaisOidPais().clone(),
			"segPaisView"));
		}
		
		//Una vez que tenemos completo los datos de los atributos raices de jerarquías de dependencias
		//cargamos las combos y seleccionamos elementos
		loadNonLeafDependenceCombos(cccMarcaSitua);
		loadLeafDependenceCombos(cccMarcaSitua);
	}
	...
	*/
	...
	protected void loadLocalizationVars(CccMarcaSituaData cccMarcaSitua) throws Exception{
		GenDetaSiccData mmgLocalizationQuery = null;
		Vector mmgLocalizationDatosResult = null;
		mmgLocalizationQuery = new GenDetaSiccData();
		mmgLocalizationQuery.setEnti("CCC_MARCA_SITUA");
		mmgLocalizationQuery.setEntiPk(cccMarcaSitua.getId());
