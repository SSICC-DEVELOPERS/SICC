...
/*
    INDRA/CAR/mmg
    $Id: SegCanalViewListFormatter.java,v 1.1 2009/12/03 18:37:32 pecbazalar Exp $
    DESC
*/
 ...
 * @author Indra
 */
public class SegCanalViewListFormatter extends MMGDruidaFormatoObjeto {
	
	public SegCanalViewListFormatter() {
	}
		...
		for (int i = 0; i < segCanalViewList.size(); i++) {
			...
			row.add((segCanalViewData.getDescripcion() != null ? 
				FormatUtils.formatObject(segCanalViewData.getDescripcion(), 
				 MMGDruidaHelper.getUserDecimalFormatPattern(this), 
				symbols) : ""));
			row.add((segCanalViewData.getTipeOidTipoPeri() != null ? 
				FormatUtils.formatObject(segCanalViewData.getTipeOidTipoPeri(), 
				 MMGDruidaHelper.getUserDecimalFormatPattern(this), 
				symbols) : ""));
