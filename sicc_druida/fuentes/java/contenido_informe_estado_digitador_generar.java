...
public class contenido_informe_estado_digitador_generar  implements es.indra.druida.base.ObjetoXML {
...
/* Primer nodo */
   ...
   private void getXML0(Document doc) {
      ...
      ((Element)v.get(0)).setAttribute("nombre","contenido_informe_estado_digitador_generar" );
      ((Element)v.get(0)).setAttribute("cod","0854" );
      ((Element)v.get(0)).setAttribute("titulo","Generar Informe Estado de Digitación Informe por Digitador" );
      ((Element)v.get(0)).setAttribute("estilos","estilosB3.css" );
      ((Element)v.get(0)).setAttribute("colorf","#F0F0F0" );
      ((Element)v.get(0)).setAttribute("msgle","Generar Informe Estado de Digitación Informe por Digitador" );
      ...
      v.add(doc.createTextNode("\r   \r   \r   function onLoad(){\r      configurarMenuSecundario('Formulario');\r      focaliza('Formulario.cbMarca');\r   }\r\r   function quitarItemVacio(elemento){\r      var elementoADevolver = new Array();\r     \r      for(var k=0;k<elemento.length;k++) {\r          if (elemento[k] != ''){                            \r             elementoADevolver[elementoADevolver.length] = elemento[k];\r          }\r      }\r         \r      return elementoADevolver;\r      \r   }\r   function generar(){\r      \r      if(sicc_validaciones_generales()){\r        var fecha1;\r        var fecha2;\r         tipoP = get('Formulario.cbPeriodo','V');   \r         for(var k=0;k<lstPeriodo.datos.longitud;k++) {\r            for(var y=0;y<tipoM.length;y++){           \r                 if ((lstPeriodo.datos.ij(k,0) == tipoP[y])){ 			 \r                                       fecha1 = lstPeriodo.datos.ij(k,4);\r                    fecha2 = lstPeriodo.datos.ij(k,5);\r                 }              \r            }\r          }\r          if(validarangofechas(fecha1,fecha2,get('Formulario.txtFecha'))){\r              /*recolectarParametros();\r              set('Formulario.conectorAction', 'PEDEstadoDigitacionInformeDigitador');           \r              set('Formulario.accion', 'generar');\r              enviaSICC('Formulario');*/\r              var a = new Object();\r              a.accion = 'generar';\r              a.marca= get('Formulario.cbMarca');\r              a.canal= get('Formulario.cbCanal');\r              a.periodo= get('Formulario.cbPeriodo');\r              a.accesos= quitarItemVacio(get('Formulario.cbAcceso'));\r              a.subaccesos= quitarItemVacio(get('Formulario.cbSubacceso'));\r              a.fecha= get('Formulario.txtFecha');           \r              mostrarModalSICC('PEDEstadoDigitacionInformeDigitador','generar', a);  \r           }\r    \r      }\r\r   }\r\r   function recolectarParametros(){     \r      set('Formulario.marca', get('Formulario.cbMarca'));\r      set('Formulario.canal', get('Formulario.cbCanal'));\r      set('Formulario.periodo', get('Formulario.cbPeriodo'));\r      set('Formulario.accesos', get('Formulario.cbAcceso'))\r      set('Formulario.subaccesos', get('Formulario.cbSubacceso'));\r      set('Formulario.fecha', get('Formulario.txtFecha'));\r      \r   }\r\rfunction validarangofechas(fecha1, fecha2, fecha){\r  if (fecha=='') return true;\r   \r    if(EsFechaValida(fecha1,fecha,'Formulario')==3){      \r      cdos_mostrarAlert(GestionarMensaje('UIREP001',fecha1,fecha2));      \r      return false;\r    }else {\r      if(EsFechaValida(fecha,fecha2,'Formulario')==3){\r        cdos_mostrarAlert(GestionarMensaje('UIREP001',fecha1,fecha2));        \r        return false;  \r      } else {      \r        return true;\r      }\r    }\r}    \r\r   function fLimpiar(){\r      var aCombo = new Array(new Array(\"\",\"\"));\r      set_combo('Formulario.cbPeriodo', aCombo, []);\r      set_combo('Formulario.cbAcceso', aCombo, []);\r      set_combo('Formulario.cbSubacceso', aCombo, []);      \r      \r      focaliza('Formulario.cbMarca'); \r   }\r   \r    function onchangeMarcaCanal(){\r     tipoM = get('Formulario.cbMarca','V');\r     tipoC = get('Formulario.cbCanal','V');\r\r          if(tipoM != '' && tipoC != ''){\r         var arr = new Array();     \r         pais   = get('Formulario.pais');\r         idioma = get('Formulario.idioma');\r          \r         arr[arr.length] = [\"oidIdioma\", idioma]; \r         arr[arr.length] = [\"oidPais\", pais]; \r         arr[arr.length] = [\"marca\", tipoM];\r         arr[arr.length] = [\"canal\", tipoC];\r     \r         recargaCombo(\"Formulario.cbPeriodo\",\"REPCargarPeriodos\",\"es.indra.sicc.dtos.cra.DTOPeriodo\",arr);\r          \r     } else {\r         var aCombo = new Array(new Array(\"\",\"\"));\r         set_combo('Formulario.cbPeriodo', aCombo, []); \r     }\r         } \r    function onchangeCanal(){\r      onchangeMarcaCanal();\r      cargaAccesos();\r    }\r\r    function cargaAccesos(){\r        \r       canal  = get('Formulario.cbCanal');\r       pais   = get('Formulario.pais');\r       idioma = get('Formulario.idioma');\r        \r       if(canal != ''){\r       \r          var arr = new Array();\r               \r          arr[arr.length] = [\"oidIdioma\", idioma]; \r          arr[arr.length] = [\"oidPais\", pais]; \r                    arr[arr.length] = [\"oid\", canal];\r             \r          recargaCombo(\"Formulario.cbAcceso\",\"SEGObtieneAccesosPorCanal\",\"es.indra.sicc.util.DTOOID\",arr);\r          \r          var aCombo = new Array(new Array(\"\",\"\"));\r          set_combo('Formulario.cbSubacceso', aCombo, []);\r          \r       }else{\r          var aCombo = new Array(new Array(\"\",\"\"));\r          set_combo('Formulario.cbAcceso', aCombo, []);\r          set_combo('Formulario.cbSubacceso', aCombo, []);\r       }\r   \r    } \r   \r   function cambioAcceso(){\r     \r          opciones = new Array();\r     set_combo('Formulario.cbSubacceso',opciones);        \r     tipo = get('Formulario.cbAcceso','V');\r     combo_add('Formulario.cbSubacceso', '', '');\r       \r     for(var k=0;k<lstSubacceso.datos.longitud;k++) {\r        for(var y=0;y<tipo.length;y++){\r                     if (lstSubacceso.datos.ij(k,2) == tipo[y]){ 			 \r              combo_add('Formulario.cbSubacceso',lstSubacceso.datos.ij(k,0),lstSubacceso.datos.ij(k,1));\r    \r           }\r        }\r    }\r  }\r        function arrayComponentes(){\r              var arr = new Array();\r              arr[arr.length]= new Array(\"cbMarca\",\"\");\r              arr[arr.length]= new Array(\"cbCanal\",\"\");\r              arr[arr.length]= new Array(\"cbAcceso\",\"\");\r              arr[arr.length]= new Array(\"cbSubacceso\",\"\");\r              arr[arr.length]= new Array(\"cbPeriodo\",\"\");\r              arr[arr.length]= new Array(\"txtFecha\",\"\");\r              arr[arr.length]= new Array(\"btnGenerarInforme\",\"BOTON\");\r              \r              return arr;\r          }\r          function tabGenerico(nombreComp, tipotab){       \r            var arr = arrayComponentes();\r            var tabular = false;\r            var nombreFormulario = getFormularioSICC();\r            var j = 0;\r            var k = arr.length;\r            if (tipotab != \"sh\") {\r                j = j-1;                \r                k = k-1;\r            }\r\r            while(j<k){\r                var t;\r                if(tipotab == \"sh\"){\r                  k--;\r                  t = k;\r                } else{\r                  j++;\r                  t = j;                  \r                }\r                  \r                var nombre = arr[t][0];\r                var tipo = arr[t][1];\r                if (tabular==false){\r                  if (nombreComp==nombre){\r                      tabular=true;\r                      if (tipotab == \"sh\"){\r                        if (k==0) k= arr.length;\r                      } else {\r                        if ((j+1)==arr.length)  j = -1;\r                      }\r                  }                \r                } else {\r                  try {\r                    var name = nombreFormulario+\".\"+nombre;\r                    if (tipo==\"BOTONXY\"){\r                       focalizaBotonHTML_XY(nombre);\r                    } else if (tipo==\"BOTON\") {\r                       focalizaBotonHTML(\"botonContenido\",nombre)\r                    } else {\r                       focaliza(name);\r                    }\r                    return;\r                  } catch (e) {\r                  }\r                }\r            }\r          } \r   \r"));
      ...
      v.add(doc.createElement("ELEMENTO"));
      ((Element)v.get(10)).setAttribute("name","txtFecha" );
      ((Element)v.get(10)).setAttribute("required","true" );
      ((Element)v.get(10)).setAttribute("cod","188" );
      ...
      ((Element)v.get(13)).setAttribute("valor","PEDEstadoDigitacionInformeDigitador" );
   ...
   private void getXML90(Document doc) {
      ...
      v.add(doc.createElement("VAR"));
      ((Element)v.get(24)).setAttribute("nombre","fecha" );
   ...
   private void getXML360(Document doc) {
      ...
      v.add(doc.createElement("LABELC"));
      ((Element)v.get(109)).setAttribute("nombre","lblPeriodo" );
      ((Element)v.get(109)).setAttribute("filas","1" );
      ((Element)v.get(109)).setAttribute("id","datosTitle" );
      ((Element)v.get(109)).setAttribute("cod","276" );
      ...
      v.add(doc.createElement("LABELC"));
      ((Element)v.get(113)).setAttribute("nombre","lblFecha" );
      ((Element)v.get(113)).setAttribute("filas","1" );
      ((Element)v.get(113)).setAttribute("id","datosTitle" );
      ((Element)v.get(113)).setAttribute("cod","188" );
   ...
   private void getXML450(Document doc) {
      ...
      v.add(doc.createElement("COMBO"));
      ((Element)v.get(120)).setAttribute("nombre","cbPeriodo" );
      ...
      ((Element)v.get(120)).setAttribute("ontab","tabGenerico('cbPeriodo');" );
      ((Element)v.get(120)).setAttribute("onshtab","tabGenerico('cbPeriodo','sh');" );
      ...
      ((Element)v.get(125)).setAttribute("nombre","txtFecha" );
      ((Element)v.get(125)).setAttribute("id","datosCampos" );
      ((Element)v.get(125)).setAttribute("max","10" );
      ...
      ((Element)v.get(125)).setAttribute("ontab","tabGenerico('txtFecha');" );
      ((Element)v.get(125)).setAttribute("onshtab","tabGenerico('txtFecha','sh');" );
      ...
      /* Empieza nodo:131 / Elemento padre: 34   */
      v.add(doc.createElement("td"));
      ((Element)v.get(34)).appendChild((Element)v.get(131));
      ...
      /* Empieza nodo:133 / Elemento padre: 26   */
      v.add(doc.createElement("tr"));
      ((Element)v.get(26)).appendChild((Element)v.get(133));
   ...
   private void getXML540(Document doc) {
      ...
      v.add(doc.createElement("CAPA"));
      ((Element)v.get(151)).setAttribute("nombre","capaSubacceso" );
      ...
      ((Element)v.get(152)).setAttribute("nombre","lstSubacceso" );
   ...
   private void getXML630(Document doc) {
      ...
      ((Element)v.get(157)).setAttribute("ancho","440" );
      ...
      ((Element)v.get(159)).setAttribute("ancho","590" );
      ((Element)v.get(159)).setAttribute("filas","1" );
      ((Element)v.get(159)).setAttribute("bloquesid","['datosCamposCenterImpar', 'datosCamposCenterPar']" );
      ...
      v.add(doc.createElement("COL"));
      ((Element)v.get(162)).setAttribute("ancho","440" );
      ((Element)v.get(162)).setAttribute("caracteres","30" );
      ...
      v.add(doc.createElement("CAPA"));
      ((Element)v.get(164)).setAttribute("nombre","capaPeriodo" );
      ((Element)v.get(164)).setAttribute("alto","3" );
      ((Element)v.get(164)).setAttribute("ancho","624" );
      ...
      ((Element)v.get(164)).setAttribute("y","165" );
      ((Element)v.get(164)).setAttribute("zindex","" );
      ((Element)v.get(11)).appendChild((Element)v.get(164));

      /* Empieza nodo:165 / Elemento padre: 164   */
      v.add(doc.createElement("LISTA"));
      ((Element)v.get(165)).setAttribute("seleccion","boton" );
      ((Element)v.get(165)).setAttribute("nombre","lstPeriodo" );
      ((Element)v.get(165)).setAttribute("x","12" );
      ((Element)v.get(165)).setAttribute("y","10" );
      ((Element)v.get(165)).setAttribute("ancho","600" );
      ((Element)v.get(165)).setAttribute("alto","3" );
      ((Element)v.get(165)).setAttribute("multisel","-1" );
      ((Element)v.get(165)).setAttribute("incy","10" );
      ((Element)v.get(165)).setAttribute("incx","10" );
      ((Element)v.get(165)).setAttribute("colorf","#EFEFEF" );
      ((Element)v.get(165)).setAttribute("imagenoff","close_no.gif" );
      ((Element)v.get(165)).setAttribute("imagenon","close_up.gif" );
      ((Element)v.get(165)).setAttribute("accion","" );
      ((Element)v.get(165)).setAttribute("sep","|" );
      ((Element)v.get(165)).setAttribute("pixelsborde","2" );
      ((Element)v.get(165)).setAttribute("colorborde","#CCCCCC" );
      ((Element)v.get(164)).appendChild((Element)v.get(165));

      /* Empieza nodo:166 / Elemento padre: 165   */
      v.add(doc.createElement("CABECERA"));
      ((Element)v.get(166)).setAttribute("nombre","cab1" );
      ((Element)v.get(166)).setAttribute("height","20" );
   ...
   private void getXML720(Document doc) {
      ...
      v.add(doc.createElement("PRESENTACION"));
      ((Element)v.get(179)).setAttribute("ancho","590" );
      ((Element)v.get(179)).setAttribute("filas","1" );
      ((Element)v.get(179)).setAttribute("bloquesid","['datosCamposCenterImpar', 'datosCamposCenterPar']" );
      ((Element)v.get(165)).appendChild((Element)v.get(179));

      /* Empieza nodo:180 / Elemento padre: 179   */
      v.add(doc.createElement("COL"));
      ((Element)v.get(180)).setAttribute("ancho","40" );
      ((Element)v.get(180)).setAttribute("caracteres","20" );
      ...
      ((Element)v.get(181)).setAttribute("caracteres","20" );
      ...
      ((Element)v.get(184)).setAttribute("caracteres","30" );
      ((Element)v.get(179)).appendChild((Element)v.get(184));
      /* Termina nodo:184   */

      /* Empieza nodo:185 / Elemento padre: 179   */
      v.add(doc.createElement("COL"));
      ((Element)v.get(185)).setAttribute("ancho","440" );
      ((Element)v.get(185)).setAttribute("caracteres","30" );
