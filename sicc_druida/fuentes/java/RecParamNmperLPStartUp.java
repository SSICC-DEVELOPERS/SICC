/*
    INDRA/CAR/mmg
    $Id: RecParamNmperLPStartUp.java,v 1.1 2009/12/03 18:33:28 pecbazalar Exp $
    DESC
*/
import java.util.*;
...
public class RecParamNmperLPStartUp extends MMGDruidaBase{
	...
	public static final String BUSINESSID_QUERY = "MMGRecParamNmperQueryFromToUserPage";
				...
				public static final String BUSINESSID_PPEROIDPRECPERD_QUERY ="MMGRecPreciPerdiQueryFromToUser";
				public static final String PPEROIDPRECPERD_COMBO_CONNECTOR = "RecPreciPerdiConectorCombo";
	...
	protected static Hashtable indParaModiValues = new Hashtable();
	static{
		indParaModiValues.put(new java.lang.Long(1), "Si");
		indParaModiValues.put(new java.lang.Long(0), "No");
	}
	
	
	// Definicion del constructor
	public RecParamNmperLPStartUp(){
		super();
	}
	...
	public void ejecucion() throws Exception {
		try{
			...
			asignarAtributo("PAGINA", "RecParamNmperPage", "cod", "RecParamNmper." + accion + ".label");
			...
			limpiarBotoneraLista("recParamNmper", accion);
	...
	protected void cmdLov() throws Exception{
		...
		asignarAtributo("TEXT", "valPorcReca", "validacion", "mensaje = ValidaNumeroMMG(cdos_trim(valor), " + (5 - MMGDruidaHelper.getUserDecimalPrecision(this)) + ", " + MMGDruidaHelper.getUserDecimalPrecision(this)+ ", 'RecParamNmper.valPorcReca', -9.99999999999E11, " + MMGDruidaHelper.getMaxValueForDruidaValidaFloat((5 - MMGDruidaHelper.getUserDecimalPrecision(this)), this) + ");");
		
		
		//Primero determianmos el nombre del campo en la entidad de oriegen donde se van a cargar los datos
		asignar("VAR", "idAttributeLovOpener", conectorParametro(PARAMETRO_LOV_ATTRIBUTO));
		asignar("VAR", "idEntityLovOpener", conectorParametro(PARAMETRO_LOV_ENTIDAD));
		//asignarAtributo("CAPA", "recParamNmperLovLayer", "visibilidad", "visible");
		asignarAtributo("PAGINA", "RecParamNmperPage", "onblur", "window.focus();");
	...
	*/
	protected void cmdView() throws Exception{
		...
		RecParamNmperData recParamNmperQuery= new RecParamNmperData();
		recParamNmperQuery.setId(new Long(conectorParametro("idSelection")));
		
		//Asiganmos el valor a la variable de acción anterior para poder volver hacia atras y hacer requery
		asignar("VAR", VAR_ACCION_ANTERIOR, conectorParametro(VAR_ACCION_ANTERIOR));
		
		//Sacamos de la base de datos la entidad a visualizar
		RecParamNmperData datosResult = (RecParamNmperData)getSingleObject(BUSINESSID_QUERY, recParamNmperQuery, 
			recParamNmperQuery.clone(), "recParamNmper");
		...
		RecParamNmperData recParamNmper = datosResult;
		es.indra.belcorp.mso.MaeTipoClienData ticlOidTipoClie = recParamNmper.getTiclOidTipoClie();
		if(ticlOidTipoClie != null && ticlOidTipoClie.getId() != null) checkCombo("ticlOidTipoClie" , ticlOidTipoClie.getId().toString());
		es.indra.belcorp.mso.RecPreciPerdiData pperOidPrecPerd = recParamNmper.getPperOidPrecPerd();
		if(pperOidPrecPerd != null && pperOidPrecPerd.getId() != null) checkCombo("pperOidPrecPerd" , pperOidPrecPerd.getId().toString());
		asignar("TEXT", "valPorcReca", FormatUtils.formatObject(recParamNmper.getValPorcReca(), 
			 MMGDruidaHelper.getUserDecimalFormatPattern(this), decimalFormatSymbols));
		if(recParamNmper.getIndParaModi() != null){	
			checkCombo("indParaModi", FormatUtils.formatObject(recParamNmper.getIndParaModi(), 
			 MMGDruidaHelper.getUserDecimalFormatPattern(this), decimalFormatSymbols));
		}
		...
		//Deshabilitamos todos los campos para que no se editen
					...
					asignarAtributo("COMBO", "pperOidPrecPerd", "disabled", "S");asignarAtributo("TEXT", "valPorcReca", "disabled", "S");
		
					asignarAtributo("COMBO", "indParaModi", "disabled", "S");
	...
	protected void cmdCreate() throws Exception{
		...
		asignarAtributo("COMBO", "pperOidPrecPerd", "req", "S");
		asignarAtributo("TEXT", "valPorcReca", "req", "S");
	...
	protected void cmdUpdateForm() throws Exception{
		...
		//asignarAtributo("FORMULARIO", "recParamNmperFrm", "oculto", "N");
		...
		//Construimos el mso con los elementos de la búsqueda
		RecParamNmperData recParamNmperQuery= new RecParamNmperData();
		recParamNmperQuery.setId(new Long(id));
		...
		//asignar("VAR","timestamp",Long.toString(recParamNmper.jdoGetTimeStamp()));
	...
	*/
	protected void cambiarAcciones() throws Exception{
				asignarAtributo("AREATEXTO", "id", "ontab","ejecutarAccionFocoModificacion(\"recParamNmperFrm\",1, true)");
				asignarAtributo("AREATEXTO", "id", "onshtab","ejecutarAccionFocoModificacion(\"recParamNmperFrm\",1, false)");
					asignarAtributo("COMBO","ticlOidTipoClie", "ontab", "ejecutarAccionFocoModificacion(\"recParamNmperFrm\",2, true)");
					asignarAtributo("COMBO","ticlOidTipoClie", "onshtab", "ejecutarAccionFocoModificacion(\"recParamNmperFrm\",2, false)");
					asignarAtributo("COMBO","pperOidPrecPerd", "ontab", "ejecutarAccionFocoModificacion(\"recParamNmperFrm\",3, true)");
					asignarAtributo("COMBO","pperOidPrecPerd", "onshtab", "ejecutarAccionFocoModificacion(\"recParamNmperFrm\",3, false)");
				asignarAtributo("AREATEXTO", "valPorcReca", "ontab","ejecutarAccionFocoModificacion(\"recParamNmperFrm\",4, true)");
				asignarAtributo("AREATEXTO", "valPorcReca", "onshtab","ejecutarAccionFocoModificacion(\"recParamNmperFrm\",4, false)");
				asignarAtributo("TEXT", "indParaModi", "ontab","ejecutarAccionFocoModificacion(\"recParamNmperFrm\",5, true)");
				asignarAtributo("TEXT", "indParaModi", "onshtab","ejecutarAccionFocoModificacion(\"recParamNmperFrm\",5, false)");
					asignarAtributo("COMBO","paisOidPais", "ontab", "ejecutarAccionFocoModificacion(\"recParamNmperFrm\",6, true)");
					asignarAtributo("COMBO","paisOidPais", "onshtab", "ejecutarAccionFocoModificacion(\"recParamNmperFrm\",6, false)");
	}
	...
	protected void removeLovElements() throws Exception{
		borrarNodo("TD","nombre","recParamNmperTdLovNullSelectionButton");
	}
	
	protected void removeQueryElements() throws Exception{
		//Borramos los botones para realizar la búsuqeda
		borrarNodo("TR","nombre","recParamNmperTrButtons");
			...
			traza("MMG: en desactivateNonModifyFields ");		
				...
				traza("MMG: el atributo es ticlOidTipoClie...");		
					...
					traza("MMG: en desactivateNonModifyFields encontrado atributo no editable...");
						...
						asignarAtributo("COMBO", "ticlOidTipoClie", "disabled", "S");
				traza("MMG: el atributo es pperOidPrecPerd...");		
				
				traza("MMG: el atributo es valPorcReca...");		
				
				traza("MMG: el atributo es indParaModi...");		
	...
	*/
	protected void establecerSizeCapas(String accion, String origen) throws Exception{
		...
		asignarAtributo("CAPA","recParamNmperFormLayer", "alto", String.valueOf(sizeForm));		
		
		//Estabelcemos la posicion y de la capa correspondiente a la lista y de su botonera
		asignarAtributo("CAPA","recParamNmperListLayer", "y", String.valueOf(sizeForm + 26));		
		asignarAtributo("CAPA","recParamNmperListButtonsLayer", "y", String.valueOf(sizeForm + 278));		
	}

	/**
	* Nos permite cargar las combos finales de la jerarquías de dependencia
	*/
	protected void loadLeafDependenceCombos(RecParamNmperData recParamNmper) throws Exception{
		...
		if(recParamNmper != null && recParamNmper.getTiclOidTipoClie() != null){
				checkCombo("ticlOidTipoClie" , recParamNmper.getTiclOidTipoClie().getId().toString());
			}
		RecPreciPerdiData pperOidPrecPerd = new RecPreciPerdiData();
		
						asignarComboResultados(BUSINESSID_PPEROIDPRECPERD_QUERY,PPEROIDPRECPERD_COMBO_CONNECTOR, 
			pperOidPrecPerd, pperOidPrecPerd.clone(), "recPreciPerdi", "pperOidPrecPerd", "result_ROWSET" );
		if(recParamNmper != null && recParamNmper.getPperOidPrecPerd() != null){
				checkCombo("pperOidPrecPerd" , recParamNmper.getPperOidPrecPerd().getId().toString());
			}
		...
		if(recParamNmper != null && recParamNmper.getPaisOidPais() != null){
				checkCombo("paisOidPais" , recParamNmper.getPaisOidPais().getId().toString());
			}
	...
	* @param recParamNmper La el objetopar la cual se va a realizar la carga
	*/
	protected void loadNonLeafDependenceCombos(RecParamNmperData recParamNmper) 
		throws Exception{
	...
	*/
	protected void loadNonLeafDependenceCombosForRequery() throws Exception{
		...
		RecParamNmperData recParamNmper = 
			(RecParamNmperData) conectorParametroSesion(SESSION_ATTRIBUTE_LAST_QUERY_OBJ_FROM);
		
		//Cargamos todos los objetos completos correspodnientes a los valores de los
		//Campos que sean raiz de una jerarquía de combos dependientes
		if(recParamNmper.getTiclOidTipoClie() != null){
			recParamNmper.setTiclOidTipoClie((es.indra.belcorp.mso.MaeTipoClienData)
			getSingleObject(BUSINESSID_TICLOIDTIPOCLIE_QUERY, 
			recParamNmper.getTiclOidTipoClie(),
			recParamNmper.getTiclOidTipoClie().clone(),
			"maeTipoClien"));
		}
		if(recParamNmper.getPperOidPrecPerd() != null){
			recParamNmper.setPperOidPrecPerd((es.indra.belcorp.mso.RecPreciPerdiData)
			getSingleObject(BUSINESSID_PPEROIDPRECPERD_QUERY, 
			recParamNmper.getPperOidPrecPerd(),
			recParamNmper.getPperOidPrecPerd().clone(),
			"recPreciPerdi"));
		}
		if(recParamNmper.getPaisOidPais() != null){
			recParamNmper.setPaisOidPais((es.indra.belcorp.mso.SegPaisViewData)
			getSingleObject(BUSINESSID_PAISOIDPAIS_QUERY, 
			recParamNmper.getPaisOidPais(),
			recParamNmper.getPaisOidPais().clone(),
			"segPaisView"));
		}
		
		//Una vez que tenemos completo los datos de los atributos raices de jerarquías de dependencias
		//cargamos las combos y seleccionamos elementos
		loadNonLeafDependenceCombos(recParamNmper);
		loadLeafDependenceCombos(recParamNmper);
	}
