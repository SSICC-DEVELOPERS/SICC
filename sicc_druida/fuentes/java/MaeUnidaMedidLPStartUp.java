/*
    INDRA/CAR/mmg
    $Id: MaeUnidaMedidLPStartUp.java,v 1.1 2009/12/03 18:33:58 pecbazalar Exp $
    DESC
*/
import java.util.*;
...
public class MaeUnidaMedidLPStartUp extends MMGDruidaBase{
	...
	public static final String BUSINESSID_QUERY = "MMGMaeUnidaMedidQueryFromToUserPage";
				...
				public static final String BUSINESSID_MAGNOIDMAGN_QUERY ="MMGMaeMagniQueryFromToUser";
				public static final String MAGNOIDMAGN_COMBO_CONNECTOR = "MaeMagniConectorCombo";
	
		
	//Hashtables con los valores estáticos de radiobuttons y combos
	protected static Hashtable indUmStndValues = new Hashtable();
	static{
		indUmStndValues.put(new java.lang.String("1"), "Si");
		indUmStndValues.put(new java.lang.String("0"), "No");
	}
	
	
	// Definicion del constructor
	public MaeUnidaMedidLPStartUp(){
		super();
	}
	...
	public void ejecucion() throws Exception {
		try{
			...
			asignarAtributo("PAGINA", "MaeUnidaMedidPage", "cod", "MaeUnidaMedid." + accion + ".label");
			...
			limpiarBotoneraLista("maeUnidaMedid", accion);
	...
	protected void cmdLov() throws Exception{
		...
		asignarAtributo("TEXT", "codUnidMedi", "validacion", "mensaje = ValidaLongitud(cdos_trim(valor), 1, 3);");
		asignarAtributo("TEXT", "Descripcion", "validacion", "mensaje = ValidaLongitud(cdos_trim(valor), 1, 40);");
		asignarAtributo("TEXT", "desAbre", "validacion", "mensaje = ValidaLongitud(cdos_trim(valor), 0, 5);");
		...
		//asignarAtributo("CAPA", "maeUnidaMedidLovLayer", "visibilidad", "visible");
		asignarAtributo("PAGINA", "MaeUnidaMedidPage", "onblur", "window.focus();");
	...
	*/
	protected void cmdView() throws Exception{
		...
		MaeUnidaMedidData maeUnidaMedidQuery= new MaeUnidaMedidData();
		maeUnidaMedidQuery.setId(new Long(conectorParametro("idSelection")));
		
		//Asiganmos el valor a la variable de acción anterior para poder volver hacia atras y hacer requery
		asignar("VAR", VAR_ACCION_ANTERIOR, conectorParametro(VAR_ACCION_ANTERIOR));
		
		//Sacamos de la base de datos la entidad a visualizar
		MaeUnidaMedidData datosResult = (MaeUnidaMedidData)getSingleObject(BUSINESSID_QUERY, maeUnidaMedidQuery, 
			maeUnidaMedidQuery.clone(), "maeUnidaMedid");
		...
		preservaHiddensI18n(new Integer(1), "MAE_UNIDA_MEDID", "maeUnidaMedidFrm", "Descripcion", true);
		
		
		//Guardamos los valores de los radiobuttons
		if(datosResult.getIndUmStnd() != null)
			asignar("VAR", "indUmStndCheckValue", datosResult.getIndUmStnd().toString());
		...
		MaeUnidaMedidData maeUnidaMedid = datosResult;
		asignar("TEXT", "codUnidMedi", FormatUtils.formatObject(maeUnidaMedid.getCodUnidMedi(), 
			 MMGDruidaHelper.getUserDecimalFormatPattern(this), decimalFormatSymbols));
		asignar("TEXT", "desAbre", FormatUtils.formatObject(maeUnidaMedid.getDesAbre(), 
			 MMGDruidaHelper.getUserDecimalFormatPattern(this), decimalFormatSymbols));
		es.indra.belcorp.mso.MaeMagniData magnOidMagn = maeUnidaMedid.getMagnOidMagn();
		if(magnOidMagn != null && magnOidMagn.getId() != null) checkCombo("magnOidMagn" , magnOidMagn.getId().toString());
		asignarAtributo("RADIOB", "indUmStnd" , "valor", FormatUtils.formatObject(
			maeUnidaMedid.getIndUmStnd(), 
			 MMGDruidaHelper.getUserDecimalFormatPattern(this), decimalFormatSymbols));
		...
		loadLocalizationVars(maeUnidaMedid);
		
			
		//Deshabilitamos todos los campos para que no se editen
		asignarAtributo("TEXT", "codUnidMedi", "disabled", "S");
		asignarAtributo("TEXT", "Descripcion", "disabled", "S");
		asignarAtributo("TEXT", "desAbre", "disabled", "S");
		
					asignarAtributo("COMBO", "magnOidMagn", "disabled", "S");
					asignarAtributo("RADIOB", "indUmStnd", "disabled", "S");
	...
	protected void cmdCreate() throws Exception{
		//Creamos los campos ocultos para la internacionalización
		preservaHiddensI18n(new Integer(1), "MAE_UNIDA_MEDID", "maeUnidaMedidFrm", "Descripcion", false);
		...
		asignarAtributo("TEXT", "codUnidMedi", "req", "S");
		asignarAtributo("TEXT", "Descripcion", "req", "S");
		asignarAtributo("COMBO", "magnOidMagn", "req", "S");
		asignarAtributo("RADIOB", "indUmStnd", "req", "S");
		
		
		//Asiganamos a los campos internacionalizables la función de onblur para actualizar el 
		//valor del idioma por defecto
		asignarAtributo("TEXT", "Descripcion", "onblur", "setDefaultLanguageValue('Descripcion', get('maeUnidaMedidFrm.Descripcion'))");
	...
	protected void cmdUpdateForm() throws Exception{
		...
		//asignarAtributo("FORMULARIO", "maeUnidaMedidFrm", "oculto", "N");
		...
		//Construimos el mso con los elementos de la búsqueda
		MaeUnidaMedidData maeUnidaMedidQuery= new MaeUnidaMedidData();
		maeUnidaMedidQuery.setId(new Long(id));
		...
		//asignar("VAR","timestamp",Long.toString(maeUnidaMedid.jdoGetTimeStamp()));
	...
	*/
	protected void cambiarAcciones() throws Exception{
				asignarAtributo("AREATEXTO", "id", "ontab","ejecutarAccionFocoModificacion(\"maeUnidaMedidFrm\",1, true)");
				asignarAtributo("AREATEXTO", "id", "onshtab","ejecutarAccionFocoModificacion(\"maeUnidaMedidFrm\",1, false)");
					asignarAtributo("COMBO","paisOidPais", "ontab", "ejecutarAccionFocoModificacion(\"maeUnidaMedidFrm\",2, true)");
					asignarAtributo("COMBO","paisOidPais", "onshtab", "ejecutarAccionFocoModificacion(\"maeUnidaMedidFrm\",2, false)");
				asignarAtributo("AREATEXTO", "codUnidMedi", "ontab","ejecutarAccionFocoModificacion(\"maeUnidaMedidFrm\",3, true)");
				asignarAtributo("AREATEXTO", "codUnidMedi", "onshtab","ejecutarAccionFocoModificacion(\"maeUnidaMedidFrm\",3, false)");
				asignarAtributo("AREATEXTO", "Descripcion", "ontab","ejecutarAccionFocoModificacion(\"maeUnidaMedidFrm\",4, true)");
				asignarAtributo("AREATEXTO", "Descripcion", "onshtab","ejecutarAccionFocoModificacion(\"maeUnidaMedidFrm\",4, false)");
				asignarAtributo("AREATEXTO", "desAbre", "ontab","ejecutarAccionFocoModificacion(\"maeUnidaMedidFrm\",5, true)");
				asignarAtributo("AREATEXTO", "desAbre", "onshtab","ejecutarAccionFocoModificacion(\"maeUnidaMedidFrm\",5, false)");
					asignarAtributo("COMBO","magnOidMagn", "ontab", "ejecutarAccionFocoModificacion(\"maeUnidaMedidFrm\",6, true)");
					asignarAtributo("COMBO","magnOidMagn", "onshtab", "ejecutarAccionFocoModificacion(\"maeUnidaMedidFrm\",6, false)");
				asignarAtributo("TEXT", "indUmStnd", "ontab","ejecutarAccionFocoModificacion(\"maeUnidaMedidFrm\",7, true)");
				asignarAtributo("TEXT", "indUmStnd", "onshtab","ejecutarAccionFocoModificacion(\"maeUnidaMedidFrm\",7, false)");
	}
	...
	protected void removeLovElements() throws Exception{
		borrarNodo("TD","nombre","maeUnidaMedidTdLovNullSelectionButton");
	}
	
	protected void removeQueryElements() throws Exception{
		//Borramos los botones para realizar la búsuqeda
		borrarNodo("TR","nombre","maeUnidaMedidTrButtons");
	...
	*/
	protected void desactivateNonModifyFields() throws Exception{
			traza("MMG: en desactivateNonModifyFields ");		
				...
				traza("MMG: el atributo es codUnidMedi...");		
				
					traza("MMG: en desactivateNonModifyFields encontrado atributo no editable...");
				
						// borrarCampo("codUnidMedi");
						asignarAtributo ("TEXT", "codUnidMedi", "disabled", "S");
				traza("MMG: el atributo es Descripcion...");		
				
				traza("MMG: el atributo es desAbre...");		
				
				traza("MMG: el atributo es magnOidMagn...");		
				
				traza("MMG: el atributo es indUmStnd...");		
	...
	*/
	protected void establecerSizeCapas(String accion, String origen) throws Exception{
		...
		asignarAtributo("CAPA","maeUnidaMedidFormLayer", "alto", String.valueOf(sizeForm));		
		
		//Estabelcemos la posicion y de la capa correspondiente a la lista y de su botonera
		asignarAtributo("CAPA","maeUnidaMedidListLayer", "y", String.valueOf(sizeForm + 26));		
		asignarAtributo("CAPA","maeUnidaMedidListButtonsLayer", "y", String.valueOf(sizeForm + 278));		
	}

	/**
	* Nos permite cargar las combos finales de la jerarquías de dependencia
	*/
	protected void loadLeafDependenceCombos(MaeUnidaMedidData maeUnidaMedid) throws Exception{
		...
		if(maeUnidaMedid != null && maeUnidaMedid.getPaisOidPais() != null){
				checkCombo("paisOidPais" , maeUnidaMedid.getPaisOidPais().getId().toString());
			}
		...
		MaeMagniData magnOidMagn = new MaeMagniData();
		
						asignarComboResultados(BUSINESSID_MAGNOIDMAGN_QUERY,MAGNOIDMAGN_COMBO_CONNECTOR, 
			magnOidMagn, magnOidMagn.clone(), "maeMagni", "magnOidMagn", "result_ROWSET" );
		if(maeUnidaMedid != null && maeUnidaMedid.getMagnOidMagn() != null){
				checkCombo("magnOidMagn" , maeUnidaMedid.getMagnOidMagn().getId().toString());
			}
	...
	* @param maeUnidaMedid La el objetopar la cual se va a realizar la carga
	*/
	protected void loadNonLeafDependenceCombos(MaeUnidaMedidData maeUnidaMedid) 
		throws Exception{
	...
	*/
	protected void loadNonLeafDependenceCombosForRequery() throws Exception{
		...
		MaeUnidaMedidData maeUnidaMedid = 
			(MaeUnidaMedidData) conectorParametroSesion(SESSION_ATTRIBUTE_LAST_QUERY_OBJ_FROM);
		
		//Cargamos todos los objetos completos correspodnientes a los valores de los
		//Campos que sean raiz de una jerarquía de combos dependientes
		if(maeUnidaMedid.getPaisOidPais() != null){
			maeUnidaMedid.setPaisOidPais((es.indra.belcorp.mso.SegPaisViewData)
			getSingleObject(BUSINESSID_PAISOIDPAIS_QUERY, 
			maeUnidaMedid.getPaisOidPais(),
			maeUnidaMedid.getPaisOidPais().clone(),
			"segPaisView"));
		}
		if(maeUnidaMedid.getMagnOidMagn() != null){
			maeUnidaMedid.setMagnOidMagn((es.indra.belcorp.mso.MaeMagniData)
			getSingleObject(BUSINESSID_MAGNOIDMAGN_QUERY, 
			maeUnidaMedid.getMagnOidMagn(),
			maeUnidaMedid.getMagnOidMagn().clone(),
			"maeMagni"));
		}
		
		//Una vez que tenemos completo los datos de los atributos raices de jerarquías de dependencias
		//cargamos las combos y seleccionamos elementos
		loadNonLeafDependenceCombos(maeUnidaMedid);
		loadLeafDependenceCombos(maeUnidaMedid);
	}
	...
	*/
	...
	protected void loadLocalizationVars(MaeUnidaMedidData maeUnidaMedid) throws Exception{
		GenDetaSiccData mmgLocalizationQuery = null;
		Vector mmgLocalizationDatosResult = null;
		mmgLocalizationQuery = new GenDetaSiccData();
		mmgLocalizationQuery.setEnti("MAE_UNIDA_MEDID");
		mmgLocalizationQuery.setEntiPk(maeUnidaMedid.getId());
