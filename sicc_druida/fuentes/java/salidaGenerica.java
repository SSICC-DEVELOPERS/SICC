...
public class salidaGenerica  implements es.indra.druida.base.ObjetoXML {
...
/* Primer nodo */
   ...
   private void getXML0(Document doc) {
      ...
      v.add(doc.createTextNode("\r	\r		function fInit(){\r			var deb = eval(get(\"frmFormulario.debugWindow\"));\r			if(deb){\r				debugger;\r			}\r\r			var cerrar = get(\"frmFormulario.cerrarVentana\")+\"\";\r			if (cerrar == \"true\" || cerrar == \"1\"){\r				cerrar = true;\r			}else{\r				cerrar = false;\r			}\r			if(gestionarError()){\r				try{\r					if(opener && !opener.closed){\r						opener.focus();\r						var formularioSiccjs = opener.getFormularioSICC();\r						opener.configurarMenuSecundario(formularioSiccjs);\r						var codigo = get(\"frmFormulario.ejecutarError\")+\"\";\r						if(codigo!=null && codigo.split(\" \").join(\"\")!=\"\"){\r							eval(\"opener.\"+codigo);\r						}\r					}\r				}catch(e){\r				}\r			}else{\r				try{\r					if(opener){\r						if (!opener.closed){\r							var formularioSiccjs = opener.getFormularioSICC();\r							opener.configurarMenuSecundario(formularioSiccjs);						\r							var codigo = (get(\"frmFormulario.ejecutar\")+\"\").split(\"\\\\\").join(\"\");\r							if(codigo!=null && codigo.split(\" \").join(\"\")!=\"\"){\r								try{\r									eval(\"opener.\"+codigo);\r								}catch(e1){\r									alert(\"no se pudo ejecutar: \"+ codigo +\"\\ndebido a: \"+e1.description);\r								}\r							}\r							try{\r								opener.trasnsaccionPerf();\r							}catch(e){}\r							if(cerrar){\r								opener.close();\r							}\r						}\r					}\r				}catch(e){\r				}\r			}\r\r			window.close();\r		}\r\r		function gestionarError(){\r			try{\r				if(opener && !opener.closed){\r					var sicc_formularioOpener = opener.getFormularioSICC();\r					opener.set(sicc_formularioOpener+\".errCodigo\",notNull(get(\"frmFormulario.errCodigo\")));\r					opener.set(sicc_formularioOpener+\".errDescripcion\",notNull(get(\"frmFormulario.errDescripcion\")));\r					opener.set(sicc_formularioOpener+\".errSeverity\",notNull(get(\"frmFormulario.errSeverity\")));\r					try{\r						opener.focus();\r					}catch(e){\r						;\r					}\r					return opener.fMostrarMensajeError();\r				}/*esto fue agregado para que ande el caso de uso DescargarGuia de Productos, por JPB*/\r				else if (get(\"frmFormulario.generarGuiaProducto\")=='true'){\r					fMostrarMensajeError();\r					set(\"frmFormulario.conectorAction\",\"LPDescargarGP\");\r					set(\"frmFormulario.accion\",\"\");\r					enviaSICC(\"frmFormulario\");\r				}/*fin de chanchada*/\r			}catch(e){\r				;\r			}\r			return false;\r		}\r\r		function notNull(strValor){\r			if(strValor==null){\r				return \"\";\r			}else{\r				return strValor;\r			}\r		}\r   \r"));
