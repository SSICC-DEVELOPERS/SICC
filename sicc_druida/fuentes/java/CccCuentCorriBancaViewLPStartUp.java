/*
    INDRA/CAR/mmg
    $Id: CccCuentCorriBancaViewLPStartUp.java,v 1.1 2009/12/03 18:38:36 pecbazalar Exp $
    DESC
*/
import java.util.*;
...
public class CccCuentCorriBancaViewLPStartUp extends MMGDruidaBase{
	...
	public CccCuentCorriBancaViewLPStartUp(){
		super();
	}
	...
	public void ejecucion() throws Exception {
		try{
			...
			buildJsPageSize("CccCuentCorriBancaView");
			...
			asignarAtributo("PAGINA", "CccCuentCorriBancaViewPage", "cod", "CccCuentCorriBancaView." + accion + ".label");
			...
			limpiarBotoneraLista("cccCuentCorriBancaView", accion);
	...
	protected void cmdLov() throws Exception{
		...
		//asignarAtributo("CAPA", "cccCuentCorriBancaViewLovLayer", "visibilidad", "visible");
		asignarAtributo("PAGINA", "CccCuentCorriBancaViewPage", "onblur", "window.focus();");
	...
	*/
	protected void cmdView() throws Exception{
		...
		CccCuentCorriBancaViewData cccCuentCorriBancaViewQuery= new CccCuentCorriBancaViewData();
		cccCuentCorriBancaViewQuery.setId(new Long(conectorParametro("idSelection")));
		
		//Asiganmos el valor a la variable de acción anterior para poder volver hacia atras y hacer requery
		asignar("VAR", VAR_ACCION_ANTERIOR, conectorParametro(VAR_ACCION_ANTERIOR));
		
		//Sacamos de la base de datos la entidad a visualizar
		CccCuentCorriBancaViewData datosResult = (CccCuentCorriBancaViewData)getSingleObject(BUSINESSID_QUERY, cccCuentCorriBancaViewQuery, 
			cccCuentCorriBancaViewQuery.clone(), "cccCuentCorriBancaView");
		...
		CccCuentCorriBancaViewData cccCuentCorriBancaView = datosResult;
		es.indra.belcorp.mso.SegSocieData sociOidSoci = cccCuentCorriBancaView.getSociOidSoci();
		if(sociOidSoci != null && sociOidSoci.getId() != null) checkCombo("sociOidSoci" , sociOidSoci.getId().toString());
		asignar("TEXT", "desCc", FormatUtils.formatObject(cccCuentCorriBancaView.getDesCc(), 
	...
	protected void cmdUpdateForm() throws Exception{
		...
		//asignarAtributo("FORMULARIO", "cccCuentCorriBancaViewFrm", "oculto", "N");
		...
		//Construimos el mso con los elementos de la búsqueda
		CccCuentCorriBancaViewData cccCuentCorriBancaViewQuery= new CccCuentCorriBancaViewData();
		cccCuentCorriBancaViewQuery.setId(new Long(id));
		...
		//asignar("VAR","timestamp",Long.toString(cccCuentCorriBancaView.jdoGetTimeStamp()));
	...
	*/
	protected void cambiarAcciones() throws Exception{
				asignarAtributo("AREATEXTO", "id", "ontab","ejecutarAccionFocoModificacion(\"cccCuentCorriBancaViewFrm\",1, true)");
				asignarAtributo("AREATEXTO", "id", "onshtab","ejecutarAccionFocoModificacion(\"cccCuentCorriBancaViewFrm\",1, false)");
					asignarAtributo("COMBO","sociOidSoci", "ontab", "ejecutarAccionFocoModificacion(\"cccCuentCorriBancaViewFrm\",2, true)");
					asignarAtributo("COMBO","sociOidSoci", "onshtab", "ejecutarAccionFocoModificacion(\"cccCuentCorriBancaViewFrm\",2, false)");
				asignarAtributo("AREATEXTO", "desCc", "ontab","ejecutarAccionFocoModificacion(\"cccCuentCorriBancaViewFrm\",4, true)");
				asignarAtributo("AREATEXTO", "desCc", "onshtab","ejecutarAccionFocoModificacion(\"cccCuentCorriBancaViewFrm\",4, false)");
					asignarAtributo("COMBO","paisOidPais", "ontab", "ejecutarAccionFocoModificacion(\"cccCuentCorriBancaViewFrm\",15, true)");
					asignarAtributo("COMBO","paisOidPais", "onshtab", "ejecutarAccionFocoModificacion(\"cccCuentCorriBancaViewFrm\",15, false)");
	}
	...
	protected void removeLovElements() throws Exception{
		borrarNodo("TD","nombre","cccCuentCorriBancaViewTdLovNullSelectionButton");
	}
	
	protected void removeQueryElements() throws Exception{
		//Borramos los botones para realizar la búsuqeda
		borrarNodo("TR","nombre","cccCuentCorriBancaViewTrButtons");
	...
	*/
	protected void establecerSizeCapas(String accion, String origen) throws Exception{
		...
		asignarAtributo("CAPA","cccCuentCorriBancaViewFormLayer", "alto", String.valueOf(sizeForm));		
		
		//Estabelcemos la posicion y de la capa correspondiente a la lista y de su botonera
		asignarAtributo("CAPA","cccCuentCorriBancaViewListLayer", "y", String.valueOf(sizeForm + 26));		
		asignarAtributo("CAPA","cccCuentCorriBancaViewListButtonsLayer", "y", String.valueOf(sizeForm + 278));		
	}

	/**
	* Nos permite cargar las combos finales de la jerarquías de dependencia
	*/
	protected void loadLeafDependenceCombos(CccCuentCorriBancaViewData cccCuentCorriBancaView) throws Exception{
		...
		if(cccCuentCorriBancaView != null && cccCuentCorriBancaView.getSociOidSoci() != null){
				checkCombo("sociOidSoci" , cccCuentCorriBancaView.getSociOidSoci().getId().toString());
			}
		...
		if(cccCuentCorriBancaView != null && cccCuentCorriBancaView.getPaisOidPais() != null){
				checkCombo("paisOidPais" , cccCuentCorriBancaView.getPaisOidPais().getId().toString());
			}
	...
	* @param cccCuentCorriBancaView La el objetopar la cual se va a realizar la carga
	*/
	protected void loadNonLeafDependenceCombos(CccCuentCorriBancaViewData cccCuentCorriBancaView) 
		throws Exception{
	...
	*/
	protected void loadNonLeafDependenceCombosForRequery() throws Exception{
		...
		CccCuentCorriBancaViewData cccCuentCorriBancaView = 
			(CccCuentCorriBancaViewData) conectorParametroSesion(SESSION_ATTRIBUTE_LAST_QUERY_OBJ_FROM);
		
		//Cargamos todos los objetos completos correspodnientes a los valores de los
		//Campos que sean raiz de una jerarquía de combos dependientes
		if(cccCuentCorriBancaView.getSociOidSoci() != null){
			cccCuentCorriBancaView.setSociOidSoci((es.indra.belcorp.mso.SegSocieData)
			getSingleObject(BUSINESSID_SOCIOIDSOCI_QUERY, 
			cccCuentCorriBancaView.getSociOidSoci(),
			cccCuentCorriBancaView.getSociOidSoci().clone(),
			"segSocie"));
		}
		if(cccCuentCorriBancaView.getPaisOidPais() != null){
			cccCuentCorriBancaView.setPaisOidPais((es.indra.belcorp.mso.SegPaisViewData)
			getSingleObject(BUSINESSID_PAISOIDPAIS_QUERY, 
			cccCuentCorriBancaView.getPaisOidPais(),
			cccCuentCorriBancaView.getPaisOidPais().clone(),
			"segPaisView"));
		}
		
		//Una vez que tenemos completo los datos de los atributos raices de jerarquías de dependencias
		//cargamos las combos y seleccionamos elementos
		loadNonLeafDependenceCombos(cccCuentCorriBancaView);
		loadLeafDependenceCombos(cccCuentCorriBancaView);
	}
