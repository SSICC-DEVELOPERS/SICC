/*
    INDRA/CAR/mmg
    $Id: CobUsuarGrupoUsuarLPStartUp.java,v 1.1 2009/12/03 18:43:25 pecbazalar Exp $
    DESC
*/
import java.util.*;
...
public class CobUsuarGrupoUsuarLPStartUp extends MMGDruidaBase{
	...
	public static final String BUSINESSID_QUERY = "MMGCobUsuarGrupoUsuarQueryFromToUserPage";
				...
				public static final String BUSINESSID_GUCOOIDGRUPUSUACOBR_QUERY ="MMGCobGrupoUsuarCobraQueryFromToUser";
				public static final String GUCOOIDGRUPUSUACOBR_COMBO_CONNECTOR = "CobGrupoUsuarCobraConectorCombo";
	...
	public CobUsuarGrupoUsuarLPStartUp(){
		super();
	}

	public void inicio() throws Exception {
		pagina("CobUsuarGrupoUsuarPage");
	}
	...
	public void ejecucion() throws Exception {
		try{
			...
			buildJsPageSize("CobUsuarGrupoUsuar");
			...
			asignarAtributo("PAGINA", "CobUsuarGrupoUsuarPage", "cod", "CobUsuarGrupoUsuar." + accion + ".label");
			...
			limpiarBotoneraLista("cobUsuarGrupoUsuar", accion);
	...
	protected void cmdLov() throws Exception{
		...
		//asignarAtributo("CAPA", "cobUsuarGrupoUsuarLovLayer", "visibilidad", "visible");
		asignarAtributo("PAGINA", "CobUsuarGrupoUsuarPage", "onblur", "window.focus();");
	...
	protected void cmdPreView() throws Exception{
		...
		conectorAction("CobUsuarGrupoUsuarLPStartUp");
	...
	*/
	protected void cmdView() throws Exception{
		...
		CobUsuarGrupoUsuarData cobUsuarGrupoUsuarQuery= new CobUsuarGrupoUsuarData();
		cobUsuarGrupoUsuarQuery.setId(new Long(conectorParametro("idSelection")));
		
		//Asiganmos el valor a la variable de acción anterior para poder volver hacia atras y hacer requery
		asignar("VAR", VAR_ACCION_ANTERIOR, conectorParametro(VAR_ACCION_ANTERIOR));
		
		//Sacamos de la base de datos la entidad a visualizar
		CobUsuarGrupoUsuarData datosResult = (CobUsuarGrupoUsuarData)getSingleObject(BUSINESSID_QUERY, cobUsuarGrupoUsuarQuery, 
			cobUsuarGrupoUsuarQuery.clone(), "cobUsuarGrupoUsuar");
		...
		CobUsuarGrupoUsuarData cobUsuarGrupoUsuar = datosResult;
		es.indra.belcorp.mso.CobUsuarCobraViewData uscoOidUsuaCobr = cobUsuarGrupoUsuar.getUscoOidUsuaCobr();
		if(uscoOidUsuaCobr != null && uscoOidUsuaCobr.getId() != null) checkCombo("uscoOidUsuaCobr" , uscoOidUsuaCobr.getId().toString());
		es.indra.belcorp.mso.CobGrupoUsuarCobraData gucoOidGrupUsuaCobr = cobUsuarGrupoUsuar.getGucoOidGrupUsuaCobr();
		if(gucoOidGrupUsuaCobr != null && gucoOidGrupUsuaCobr.getId() != null) checkCombo("gucoOidGrupUsuaCobr" , gucoOidGrupUsuaCobr.getId().toString());
		...
		//Deshabilitamos todos los campos para que no se editen
					...
					asignarAtributo("COMBO", "gucoOidGrupUsuaCobr", "disabled", "S");
	...
	protected void cmdCreate() throws Exception{
		...
		asignarAtributo("COMBO", "gucoOidGrupUsuaCobr", "req", "S");
	...
	protected void cmdUpdateForm() throws Exception{
		...
		//asignarAtributo("FORMULARIO", "cobUsuarGrupoUsuarFrm", "oculto", "N");
		...
		traza("MMG::Creando CobUsuarGrupoUsuar");
		String id = conectorParametro("idSelection");
		traza("MMG:: id del atributo a modificar: " + id);
	
		//Construimos el mso con los elementos de la búsqueda
		CobUsuarGrupoUsuarData cobUsuarGrupoUsuarQuery= new CobUsuarGrupoUsuarData();
		cobUsuarGrupoUsuarQuery.setId(new Long(id));
		...
		//asignar("VAR","timestamp",Long.toString(cobUsuarGrupoUsuar.jdoGetTimeStamp()));
	...
	*/
	protected void cambiarAcciones() throws Exception{
					asignarAtributo("COMBO","uscoOidUsuaCobr", "ontab", "ejecutarAccionFocoModificacion(\"cobUsuarGrupoUsuarFrm\",2, true)");
					asignarAtributo("COMBO","uscoOidUsuaCobr", "onshtab", "ejecutarAccionFocoModificacion(\"cobUsuarGrupoUsuarFrm\",2, false)");
					asignarAtributo("COMBO","gucoOidGrupUsuaCobr", "ontab", "ejecutarAccionFocoModificacion(\"cobUsuarGrupoUsuarFrm\",3, true)");
					asignarAtributo("COMBO","gucoOidGrupUsuaCobr", "onshtab", "ejecutarAccionFocoModificacion(\"cobUsuarGrupoUsuarFrm\",3, false)");
	}
	...
	protected void removeLovElements() throws Exception{
		borrarNodo("TD","nombre","cobUsuarGrupoUsuarTdLovNullSelectionButton");
	}
	
	protected void removeQueryElements() throws Exception{
		//Borramos los botones para realizar la búsuqeda
		borrarNodo("TR","nombre","cobUsuarGrupoUsuarTrButtons");
	...
	*/
	protected void desactivateNonModifyFields() throws Exception{
			traza("MMG: en desactivateNonModifyFields ");		
				...
				traza("MMG: el atributo es gucoOidGrupUsuaCobr...");		
	...
	*/
	protected void establecerSizeCapas(String accion, String origen) throws Exception{
		...
		asignarAtributo("CAPA","cobUsuarGrupoUsuarFormLayer", "alto", String.valueOf(sizeForm));		
		
		//Estabelcemos la posicion y de la capa correspondiente a la lista y de su botonera
		asignarAtributo("CAPA","cobUsuarGrupoUsuarListLayer", "y", String.valueOf(sizeForm + 26));		
		asignarAtributo("CAPA","cobUsuarGrupoUsuarListButtonsLayer", "y", String.valueOf(sizeForm + 278));		
	}

	/**
	* Nos permite cargar las combos finales de la jerarquías de dependencia
	*/
	protected void loadLeafDependenceCombos(CobUsuarGrupoUsuarData cobUsuarGrupoUsuar) throws Exception{
		...
		if(cobUsuarGrupoUsuar != null && cobUsuarGrupoUsuar.getUscoOidUsuaCobr() != null){
				checkCombo("uscoOidUsuaCobr" , cobUsuarGrupoUsuar.getUscoOidUsuaCobr().getId().toString());
			}
		CobGrupoUsuarCobraData gucoOidGrupUsuaCobr = new CobGrupoUsuarCobraData();
		
						asignarComboResultados(BUSINESSID_GUCOOIDGRUPUSUACOBR_QUERY,GUCOOIDGRUPUSUACOBR_COMBO_CONNECTOR, 
			gucoOidGrupUsuaCobr, gucoOidGrupUsuaCobr.clone(), "cobGrupoUsuarCobra", "gucoOidGrupUsuaCobr", "result_ROWSET" );
		if(cobUsuarGrupoUsuar != null && cobUsuarGrupoUsuar.getGucoOidGrupUsuaCobr() != null){
				checkCombo("gucoOidGrupUsuaCobr" , cobUsuarGrupoUsuar.getGucoOidGrupUsuaCobr().getId().toString());
			}
	...
	* @param cobUsuarGrupoUsuar La el objetopar la cual se va a realizar la carga
	*/
	protected void loadNonLeafDependenceCombos(CobUsuarGrupoUsuarData cobUsuarGrupoUsuar) 
		throws Exception{
	...
	*/
	protected void loadNonLeafDependenceCombosForRequery() throws Exception{
		...
		CobUsuarGrupoUsuarData cobUsuarGrupoUsuar = 
			(CobUsuarGrupoUsuarData) conectorParametroSesion(SESSION_ATTRIBUTE_LAST_QUERY_OBJ_FROM);
		
		//Cargamos todos los objetos completos correspodnientes a los valores de los
		//Campos que sean raiz de una jerarquía de combos dependientes
		if(cobUsuarGrupoUsuar.getUscoOidUsuaCobr() != null){
			cobUsuarGrupoUsuar.setUscoOidUsuaCobr((es.indra.belcorp.mso.CobUsuarCobraViewData)
			getSingleObject(BUSINESSID_USCOOIDUSUACOBR_QUERY, 
			cobUsuarGrupoUsuar.getUscoOidUsuaCobr(),
			cobUsuarGrupoUsuar.getUscoOidUsuaCobr().clone(),
			"cobUsuarCobraView"));
		}
		if(cobUsuarGrupoUsuar.getGucoOidGrupUsuaCobr() != null){
			cobUsuarGrupoUsuar.setGucoOidGrupUsuaCobr((es.indra.belcorp.mso.CobGrupoUsuarCobraData)
			getSingleObject(BUSINESSID_GUCOOIDGRUPUSUACOBR_QUERY, 
			cobUsuarGrupoUsuar.getGucoOidGrupUsuaCobr(),
			cobUsuarGrupoUsuar.getGucoOidGrupUsuaCobr().clone(),
			"cobGrupoUsuarCobra"));
		}
		
		//Una vez que tenemos completo los datos de los atributos raices de jerarquías de dependencias
		//cargamos las combos y seleccionamos elementos
		loadNonLeafDependenceCombos(cobUsuarGrupoUsuar);
		loadLeafDependenceCombos(cobUsuarGrupoUsuar);
	}
