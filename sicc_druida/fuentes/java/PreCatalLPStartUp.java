/*
    INDRA/CAR/mmg
    $Id: PreCatalLPStartUp.java,v 1.1 2009/12/03 18:35:22 pecbazalar Exp $
    DESC
*/
import java.util.*;
...
public class PreCatalLPStartUp extends MMGDruidaBase{
	...
	public void ejecucion() throws Exception {
		try{
			...
			asignarAtributo("PAGINA", "PreCatalPage", "cod", "PreCatal." + accion + ".label");
			...
			limpiarBotoneraLista("preCatal", accion);
	...
	protected void cmdLov() throws Exception{
		...
		asignarAtributo("TEXT", "codCata", "validacion", "mensaje = ValidaInt(cdos_trim(valor), 5, 0, 99999);");
		asignarAtributo("TEXT", "desCata", "validacion", "mensaje = ValidaLongitud(cdos_trim(valor), 1, 50);");
		
		//Primero determianmos el nombre del campo en la entidad de oriegen donde se van a cargar los datos
		asignar("VAR", "idAttributeLovOpener", conectorParametro(PARAMETRO_LOV_ATTRIBUTO));
		asignar("VAR", "idEntityLovOpener", conectorParametro(PARAMETRO_LOV_ENTIDAD));
		//asignarAtributo("CAPA", "preCatalLovLayer", "visibilidad", "visible");
		asignarAtributo("PAGINA", "PreCatalPage", "onblur", "window.focus();");
	...
	protected void cmdPreView() throws Exception{
		...
		conectorAction("PreCatalLPStartUp");
	...
	*/
	protected void cmdView() throws Exception{
		...
		PreCatalData preCatalQuery= new PreCatalData();
		preCatalQuery.setId(new Long(conectorParametro("idSelection")));
		
		//Asiganmos el valor a la variable de acción anterior para poder volver hacia atras y hacer requery
		asignar("VAR", VAR_ACCION_ANTERIOR, conectorParametro(VAR_ACCION_ANTERIOR));
		
		//Sacamos de la base de datos la entidad a visualizar
		PreCatalData datosResult = (PreCatalData)getSingleObject(BUSINESSID_QUERY, preCatalQuery, 
			preCatalQuery.clone(), "preCatal");
		...
		preservaHiddensI18n(new Integer(1), "PRE_CATAL", "preCatalFrm", "desCata", true);
		...
		PreCatalData preCatal = datosResult;
		asignar("TEXT", "codCata", FormatUtils.formatObject(preCatal.getCodCata(), 
			 MMGDruidaHelper.getUserDecimalFormatPattern(this), decimalFormatSymbols));
		asignar("TEXT", "desCata", FormatUtils.formatObject(preCatal.getDesCata(), 
			 MMGDruidaHelper.getUserDecimalFormatPattern(this), decimalFormatSymbols));
		...
		loadLocalizationVars(preCatal);
		
			
		//Deshabilitamos todos los campos para que no se editen
		asignarAtributo("TEXT", "codCata", "disabled", "S");
		asignarAtributo("TEXT", "desCata", "disabled", "S");
	...
	protected void cmdCreate() throws Exception{
		//Creamos los campos ocultos para la internacionalización
		preservaHiddensI18n(new Integer(1), "PRE_CATAL", "preCatalFrm", "desCata", false);
		...
		asignarAtributo("TEXT", "codCata", "req", "S");
		asignarAtributo("TEXT", "desCata", "req", "S");
		
		
		//Asiganamos a los campos internacionalizables la función de onblur para actualizar el 
		//valor del idioma por defecto
		asignarAtributo("TEXT", "desCata", "onblur", "setDefaultLanguageValue('desCata', get('preCatalFrm.desCata'))");
	...
	protected void cmdUpdateForm() throws Exception{
		...
		//asignarAtributo("FORMULARIO", "preCatalFrm", "oculto", "N");
		...
		traza("MMG::Creando PreCatal");
		String id = conectorParametro("idSelection");
		traza("MMG:: id del atributo a modificar: " + id);
	
		//Construimos el mso con los elementos de la búsqueda
		PreCatalData preCatalQuery= new PreCatalData();
		preCatalQuery.setId(new Long(id));
		...
		//asignar("VAR","timestamp",Long.toString(preCatal.jdoGetTimeStamp()));
	...
	*/
	protected void cambiarAcciones() throws Exception{
				asignarAtributo("AREATEXTO", "id", "ontab","ejecutarAccionFocoModificacion(\"preCatalFrm\",1, true)");
				asignarAtributo("AREATEXTO", "id", "onshtab","ejecutarAccionFocoModificacion(\"preCatalFrm\",1, false)");
				asignarAtributo("AREATEXTO", "codCata", "ontab","ejecutarAccionFocoModificacion(\"preCatalFrm\",2, true)");
				asignarAtributo("AREATEXTO", "codCata", "onshtab","ejecutarAccionFocoModificacion(\"preCatalFrm\",2, false)");
				asignarAtributo("AREATEXTO", "desCata", "ontab","ejecutarAccionFocoModificacion(\"preCatalFrm\",3, true)");
				asignarAtributo("AREATEXTO", "desCata", "onshtab","ejecutarAccionFocoModificacion(\"preCatalFrm\",3, false)");
					asignarAtributo("COMBO","paisOidPais", "ontab", "ejecutarAccionFocoModificacion(\"preCatalFrm\",4, true)");
					asignarAtributo("COMBO","paisOidPais", "onshtab", "ejecutarAccionFocoModificacion(\"preCatalFrm\",4, false)");
	}
	
	/**Nos permite eliminar de la página todos los elementos correspondiente
	a las internacionalizacion*/
	protected void removeLocalizationElements() throws Exception{
				borrarNodo("TD","nombre","desCataTdLocalizationButtonGap");
				borrarNodo("TD","nombre","desCataTdLocalizationButtonGapHeader");
				borrarNodo("TD","nombre","desCataTdLocalizationButton");
	}
	
	protected void removeLovElements() throws Exception{
		borrarNodo("TD","nombre","preCatalTdLovNullSelectionButton");
	}
	
	protected void removeQueryElements() throws Exception{
		//Borramos los botones para realizar la búsuqeda
		borrarNodo("TR","nombre","preCatalTrButtons");
	...
	*/
	protected void desactivateNonModifyFields() throws Exception{
			traza("MMG: en desactivateNonModifyFields ");		
				...
				traza("MMG: el atributo es codCata...");		
				
				traza("MMG: el atributo es desCata...");		
	...
	*/
	protected void establecerSizeCapas(String accion, String origen) throws Exception{
		...
		asignarAtributo("CAPA","preCatalFormLayer", "alto", String.valueOf(sizeForm));		
		
		//Estabelcemos la posicion y de la capa correspondiente a la lista y de su botonera
		asignarAtributo("CAPA","preCatalListLayer", "y", String.valueOf(sizeForm + 26));		
		asignarAtributo("CAPA","preCatalListButtonsLayer", "y", String.valueOf(sizeForm + 278));		
	}

	/**
	* Nos permite cargar las combos finales de la jerarquías de dependencia
	*/
	protected void loadLeafDependenceCombos(PreCatalData preCatal) throws Exception{
		...
		if(preCatal != null && preCatal.getPaisOidPais() != null){
				checkCombo("paisOidPais" , preCatal.getPaisOidPais().getId().toString());
			}
	...
	* @param preCatal La el objetopar la cual se va a realizar la carga
	*/
	protected void loadNonLeafDependenceCombos(PreCatalData preCatal) 
		throws Exception{
	...
	*/
	protected void loadNonLeafDependenceCombosForRequery() throws Exception{
		...
		PreCatalData preCatal = 
			(PreCatalData) conectorParametroSesion(SESSION_ATTRIBUTE_LAST_QUERY_OBJ_FROM);
		
		//Cargamos todos los objetos completos correspodnientes a los valores de los
		//Campos que sean raiz de una jerarquía de combos dependientes
		if(preCatal.getPaisOidPais() != null){
			preCatal.setPaisOidPais((es.indra.belcorp.mso.SegPaisViewData)
			getSingleObject(BUSINESSID_PAISOIDPAIS_QUERY, 
			preCatal.getPaisOidPais(),
			preCatal.getPaisOidPais().clone(),
			"segPaisView"));
		}
		
		//Una vez que tenemos completo los datos de los atributos raices de jerarquías de dependencias
		//cargamos las combos y seleccionamos elementos
		loadNonLeafDependenceCombos(preCatal);
		loadLeafDependenceCombos(preCatal);
	}
	...
	*/
	...
	protected void loadLocalizationVars(PreCatalData preCatal) throws Exception{
		GenDetaSiccData mmgLocalizationQuery = null;
		Vector mmgLocalizationDatosResult = null;
		mmgLocalizationQuery = new GenDetaSiccData();
		mmgLocalizationQuery.setEnti("PRE_CATAL");
		mmgLocalizationQuery.setEntiPk(preCatal.getId());
