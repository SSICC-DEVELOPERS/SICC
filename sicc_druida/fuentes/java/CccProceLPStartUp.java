/*
    INDRA/CAR/mmg
    $Id: CccProceLPStartUp.java,v 1.1 2009/12/03 18:39:46 pecbazalar Exp $
    DESC
*/
import java.util.*;
...
public class CccProceLPStartUp extends MMGDruidaBase{
	...
	public void ejecucion() throws Exception {
		try{
			...
			asignarAtributo("PAGINA", "CccProcePage", "cod", "CccProce." + accion + ".label");
			...
			limpiarBotoneraLista("cccProce", accion);
	...
	protected void cmdLov() throws Exception{
		...
		asignarAtributo("TEXT", "codProc", "validacion", "mensaje = ValidaLongitud(cdos_trim(valor), 1, 6);");
		asignarAtributo("TEXT", "desProc", "validacion", "mensaje = ValidaLongitud(cdos_trim(valor), 1, 40);");
		
		//Primero determianmos el nombre del campo en la entidad de oriegen donde se van a cargar los datos
		asignar("VAR", "idAttributeLovOpener", conectorParametro(PARAMETRO_LOV_ATTRIBUTO));
		asignar("VAR", "idEntityLovOpener", conectorParametro(PARAMETRO_LOV_ENTIDAD));
		//asignarAtributo("CAPA", "cccProceLovLayer", "visibilidad", "visible");
		asignarAtributo("PAGINA", "CccProcePage", "onblur", "window.focus();");
	...
	*/
	protected void cmdView() throws Exception{
		...
		CccProceData cccProceQuery= new CccProceData();
		cccProceQuery.setId(new Long(conectorParametro("idSelection")));
		
		//Asiganmos el valor a la variable de acción anterior para poder volver hacia atras y hacer requery
		asignar("VAR", VAR_ACCION_ANTERIOR, conectorParametro(VAR_ACCION_ANTERIOR));
		
		//Sacamos de la base de datos la entidad a visualizar
		CccProceData datosResult = (CccProceData)getSingleObject(BUSINESSID_QUERY, cccProceQuery, 
			cccProceQuery.clone(), "cccProce");
		...
		CccProceData cccProce = datosResult;
		asignar("TEXT", "codProc", FormatUtils.formatObject(cccProce.getCodProc(), 
			 MMGDruidaHelper.getUserDecimalFormatPattern(this), decimalFormatSymbols));
		asignar("TEXT", "desProc", FormatUtils.formatObject(cccProce.getDesProc(), 
			 MMGDruidaHelper.getUserDecimalFormatPattern(this), decimalFormatSymbols));
		asignar("AREATEXTO", "valObse", FormatUtils.formatObject(cccProce.getValObse(), 
			 MMGDruidaHelper.getUserDecimalFormatPattern(this), decimalFormatSymbols));
		...
		asignarAtributo("TEXT", "desProc", "disabled", "S");
	...
	protected void cmdCreate() throws Exception{
		...
		asignarAtributo("TEXT", "desProc", "req", "S");
	...
	protected void cmdUpdateForm() throws Exception{
		...
		//asignarAtributo("FORMULARIO", "cccProceFrm", "oculto", "N");
		...
		//Construimos el mso con los elementos de la búsqueda
		CccProceData cccProceQuery= new CccProceData();
		cccProceQuery.setId(new Long(id));
		...
		asignar("TEXT", "desProc", FormatUtils.formatObject(cccProce.getDesProc(), 
			 MMGDruidaHelper.getUserDecimalFormatPattern(this), decimalFormatSymbols));
		asignarAtributo("CHECKBOX", "indCvDire" , "check", FormatUtils.formatObject(
			cccProce.getIndCvDire(), 
		...
		//asignar("VAR","timestamp",Long.toString(cccProce.jdoGetTimeStamp()));
	...
	*/
	protected void cambiarAcciones() throws Exception{
				asignarAtributo("AREATEXTO", "id", "ontab","ejecutarAccionFocoModificacion(\"cccProceFrm\",1, true)");
				asignarAtributo("AREATEXTO", "id", "onshtab","ejecutarAccionFocoModificacion(\"cccProceFrm\",1, false)");
					asignarAtributo("COMBO","paisOidPais", "ontab", "ejecutarAccionFocoModificacion(\"cccProceFrm\",2, true)");
					asignarAtributo("COMBO","paisOidPais", "onshtab", "ejecutarAccionFocoModificacion(\"cccProceFrm\",2, false)");
				asignarAtributo("AREATEXTO", "codProc", "ontab","ejecutarAccionFocoModificacion(\"cccProceFrm\",3, true)");
				asignarAtributo("AREATEXTO", "codProc", "onshtab","ejecutarAccionFocoModificacion(\"cccProceFrm\",3, false)");
				asignarAtributo("AREATEXTO", "desProc", "ontab","ejecutarAccionFocoModificacion(\"cccProceFrm\",4, true)");
				asignarAtributo("AREATEXTO", "desProc", "onshtab","ejecutarAccionFocoModificacion(\"cccProceFrm\",4, false)");
				asignarAtributo("TEXT", "indCvDire", "ontab","ejecutarAccionFocoModificacion(\"cccProceFrm\",5, true)");
				asignarAtributo("TEXT", "indCvDire", "onshtab","ejecutarAccionFocoModificacion(\"cccProceFrm\",5, false)");
				asignarAtributo("AREATEXTO", "valObse", "ontab","ejecutarAccionFocoModificacion(\"cccProceFrm\",6, true)");
				asignarAtributo("AREATEXTO", "valObse", "onshtab","ejecutarAccionFocoModificacion(\"cccProceFrm\",6, false)");
	}
	...
	protected void removeLovElements() throws Exception{
		borrarNodo("TD","nombre","cccProceTdLovNullSelectionButton");
	}
	
	protected void removeQueryElements() throws Exception{
		//Borramos los botones para realizar la búsuqeda
		borrarNodo("TR","nombre","cccProceTrButtons");
	...
	*/
	protected void desactivateNonModifyFields() throws Exception{
			traza("MMG: en desactivateNonModifyFields ");		
				...
				traza("MMG: el atributo es desProc...");		
				
				traza("MMG: el atributo es indCvDire...");		
	...
	*/
	protected void establecerSizeCapas(String accion, String origen) throws Exception{
		...
		asignarAtributo("CAPA","cccProceFormLayer", "alto", String.valueOf(sizeForm));		
		
		//Estabelcemos la posicion y de la capa correspondiente a la lista y de su botonera
		asignarAtributo("CAPA","cccProceListLayer", "y", String.valueOf(sizeForm + 26));		
		asignarAtributo("CAPA","cccProceListButtonsLayer", "y", String.valueOf(sizeForm + 278));		
	}

	/**
	* Nos permite cargar las combos finales de la jerarquías de dependencia
	*/
	protected void loadLeafDependenceCombos(CccProceData cccProce) throws Exception{
		...
		if(cccProce != null && cccProce.getPaisOidPais() != null){
				checkCombo("paisOidPais" , cccProce.getPaisOidPais().getId().toString());
			}
	...
	* @param cccProce La el objetopar la cual se va a realizar la carga
	*/
	protected void loadNonLeafDependenceCombos(CccProceData cccProce) 
		throws Exception{
	...
	*/
	protected void loadNonLeafDependenceCombosForRequery() throws Exception{
		...
		CccProceData cccProce = 
			(CccProceData) conectorParametroSesion(SESSION_ATTRIBUTE_LAST_QUERY_OBJ_FROM);
		
		//Cargamos todos los objetos completos correspodnientes a los valores de los
		//Campos que sean raiz de una jerarquía de combos dependientes
		if(cccProce.getPaisOidPais() != null){
			cccProce.setPaisOidPais((es.indra.belcorp.mso.SegPaisViewData)
			getSingleObject(BUSINESSID_PAISOIDPAIS_QUERY, 
			cccProce.getPaisOidPais(),
			cccProce.getPaisOidPais().clone(),
			"segPaisView"));
		}
		
		//Una vez que tenemos completo los datos de los atributos raices de jerarquías de dependencias
		//cargamos las combos y seleccionamos elementos
		loadNonLeafDependenceCombos(cccProce);
		loadLeafDependenceCombos(cccProce);
	}
