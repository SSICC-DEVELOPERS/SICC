/*
    INDRA/CAR/mmg
    $Id: PreCatalOrdenLPStartUp.java,v 1.1 2009/12/03 18:41:04 pecbazalar Exp $
    DESC
*/
import java.util.*;
...
public class PreCatalOrdenLPStartUp extends MMGDruidaBase{
	...
	public static final String BUSINESSID_QUERY = "MMGPreCatalOrdenQueryFromToUserPage";
				...
				public static final String BUSINESSID_OCATOIDCATA_QUERY ="MMGPreCatalQueryFromToUser";
				public static final String OCATOIDCATA_COMBO_CONNECTOR = "PreCatalConectorCombo";
	...
	public void ejecucion() throws Exception {
		try{
			...
			asignarAtributo("PAGINA", "PreCatalOrdenPage", "cod", "PreCatalOrden." + accion + ".label");
			...
			limpiarBotoneraLista("preCatalOrden", accion);
	...
	protected void cmdLov() throws Exception{
		...
		//asignarAtributo("CAPA", "preCatalOrdenLovLayer", "visibilidad", "visible");
		asignarAtributo("PAGINA", "PreCatalOrdenPage", "onblur", "window.focus();");
	...
	*/
	protected void cmdView() throws Exception{
		...
		PreCatalOrdenData preCatalOrdenQuery= new PreCatalOrdenData();
		preCatalOrdenQuery.setId(new Long(conectorParametro("idSelection")));
		
		//Asiganmos el valor a la variable de acción anterior para poder volver hacia atras y hacer requery
		asignar("VAR", VAR_ACCION_ANTERIOR, conectorParametro(VAR_ACCION_ANTERIOR));
		
		//Sacamos de la base de datos la entidad a visualizar
		PreCatalOrdenData datosResult = (PreCatalOrdenData)getSingleObject(BUSINESSID_QUERY, preCatalOrdenQuery, 
			preCatalOrdenQuery.clone(), "preCatalOrden");
		...
		PreCatalOrdenData preCatalOrden = datosResult;
		es.indra.belcorp.mso.SegMarcaData marcOidMarc = preCatalOrden.getMarcOidMarc();
		if(marcOidMarc != null && marcOidMarc.getId() != null) checkCombo("marcOidMarc" , marcOidMarc.getId().toString());
		es.indra.belcorp.mso.SegCanalViewData canaOidCana = preCatalOrden.getCanaOidCana();
		if(canaOidCana != null && canaOidCana.getId() != null) checkCombo("canaOidCana" , canaOidCana.getId().toString());
		es.indra.belcorp.mso.PreCatalData ocatOidCata = preCatalOrden.getOcatOidCata();
		if(ocatOidCata != null && ocatOidCata.getId() != null) checkCombo("ocatOidCata" , ocatOidCata.getId().toString());
		asignar("TEXT", "numOrde", FormatUtils.formatObject(preCatalOrden.getNumOrde(), 
		...
		//Deshabilitamos todos los campos para que no se editen
					...
					asignarAtributo("COMBO", "ocatOidCata", "disabled", "S");asignarAtributo("TEXT", "numOrde", "disabled", "S");
	...
	protected void cmdCreate() throws Exception{
		...
		asignarAtributo("COMBO", "ocatOidCata", "req", "S");
		asignarAtributo("TEXT", "numOrde", "req", "S");
		...
		asignarAtributo("TEXT", "numOrde", "validacion", "mensaje = ValidaInt(cdos_trim(valor), 2, 0, 99);");
	...
	protected void cmdUpdateForm() throws Exception{
		...
		//asignarAtributo("FORMULARIO", "preCatalOrdenFrm", "oculto", "N");
		...
		//Construimos el mso con los elementos de la búsqueda
		PreCatalOrdenData preCatalOrdenQuery= new PreCatalOrdenData();
		preCatalOrdenQuery.setId(new Long(id));
		...
		//asignar("VAR","timestamp",Long.toString(preCatalOrden.jdoGetTimeStamp()));
	...
	*/
	protected void cambiarAcciones() throws Exception{
				asignarAtributo("AREATEXTO", "id", "ontab","ejecutarAccionFocoModificacion(\"preCatalOrdenFrm\",1, true)");
				asignarAtributo("AREATEXTO", "id", "onshtab","ejecutarAccionFocoModificacion(\"preCatalOrdenFrm\",1, false)");
					asignarAtributo("COMBO","marcOidMarc", "ontab", "ejecutarAccionFocoModificacion(\"preCatalOrdenFrm\",2, true)");
					asignarAtributo("COMBO","marcOidMarc", "onshtab", "ejecutarAccionFocoModificacion(\"preCatalOrdenFrm\",2, false)");
					asignarAtributo("COMBO","canaOidCana", "ontab", "ejecutarAccionFocoModificacion(\"preCatalOrdenFrm\",3, true)");
					asignarAtributo("COMBO","canaOidCana", "onshtab", "ejecutarAccionFocoModificacion(\"preCatalOrdenFrm\",3, false)");
					asignarAtributo("COMBO","ocatOidCata", "ontab", "ejecutarAccionFocoModificacion(\"preCatalOrdenFrm\",4, true)");
					asignarAtributo("COMBO","ocatOidCata", "onshtab", "ejecutarAccionFocoModificacion(\"preCatalOrdenFrm\",4, false)");
				asignarAtributo("AREATEXTO", "numOrde", "ontab","ejecutarAccionFocoModificacion(\"preCatalOrdenFrm\",5, true)");
				asignarAtributo("AREATEXTO", "numOrde", "onshtab","ejecutarAccionFocoModificacion(\"preCatalOrdenFrm\",5, false)");
	}
	...
	protected void removeLovElements() throws Exception{
		borrarNodo("TD","nombre","preCatalOrdenTdLovNullSelectionButton");
	}
	
	protected void removeQueryElements() throws Exception{
		//Borramos los botones para realizar la búsuqeda
		borrarNodo("TR","nombre","preCatalOrdenTrButtons");
			...
			traza("MMG: en desactivateNonModifyFields ");		
				...
				traza("MMG: el atributo es canaOidCana...");		
					...
					traza("MMG: en desactivateNonModifyFields encontrado atributo no editable...");
						...
						asignarAtributo("COMBO", "canaOidCana", "disabled", "S");
				traza("MMG: el atributo es ocatOidCata...");		
				
					traza("MMG: en desactivateNonModifyFields encontrado atributo no editable...");
				
						asignarAtributo("COMBO", "ocatOidCata", "disabled", "S");
				traza("MMG: el atributo es numOrde...");		
	...
	*/
	protected void establecerSizeCapas(String accion, String origen) throws Exception{
		...
		asignarAtributo("CAPA","preCatalOrdenFormLayer", "alto", String.valueOf(sizeForm));		
		
		//Estabelcemos la posicion y de la capa correspondiente a la lista y de su botonera
		asignarAtributo("CAPA","preCatalOrdenListLayer", "y", String.valueOf(sizeForm + 26));		
		asignarAtributo("CAPA","preCatalOrdenListButtonsLayer", "y", String.valueOf(sizeForm + 278));		
	}

	/**
	* Nos permite cargar las combos finales de la jerarquías de dependencia
	*/
	protected void loadLeafDependenceCombos(PreCatalOrdenData preCatalOrden) throws Exception{
		...
		if(preCatalOrden != null && preCatalOrden.getMarcOidMarc() != null){
				checkCombo("marcOidMarc" , preCatalOrden.getMarcOidMarc().getId().toString());
			}
		...
		if(preCatalOrden != null && preCatalOrden.getCanaOidCana() != null){
				checkCombo("canaOidCana" , preCatalOrden.getCanaOidCana().getId().toString());
			}
		...
		PreCatalData ocatOidCata = new PreCatalData();
		
						asignarComboResultados(BUSINESSID_OCATOIDCATA_QUERY,OCATOIDCATA_COMBO_CONNECTOR, 
			ocatOidCata, ocatOidCata.clone(), "preCatal", "ocatOidCata", "result_ROWSET" );
		if(preCatalOrden != null && preCatalOrden.getOcatOidCata() != null){
				checkCombo("ocatOidCata" , preCatalOrden.getOcatOidCata().getId().toString());
			}
	...
	* @param preCatalOrden La el objetopar la cual se va a realizar la carga
	*/
	protected void loadNonLeafDependenceCombos(PreCatalOrdenData preCatalOrden) 
		throws Exception{
	...
	*/
	protected void loadNonLeafDependenceCombosForRequery() throws Exception{
		...
		PreCatalOrdenData preCatalOrden = 
			(PreCatalOrdenData) conectorParametroSesion(SESSION_ATTRIBUTE_LAST_QUERY_OBJ_FROM);
		
		//Cargamos todos los objetos completos correspodnientes a los valores de los
		//Campos que sean raiz de una jerarquía de combos dependientes
		if(preCatalOrden.getMarcOidMarc() != null){
			preCatalOrden.setMarcOidMarc((es.indra.belcorp.mso.SegMarcaData)
			getSingleObject(BUSINESSID_MARCOIDMARC_QUERY, 
			preCatalOrden.getMarcOidMarc(),
			preCatalOrden.getMarcOidMarc().clone(),
			"segMarca"));
		}
		if(preCatalOrden.getCanaOidCana() != null){
			preCatalOrden.setCanaOidCana((es.indra.belcorp.mso.SegCanalViewData)
			getSingleObject(BUSINESSID_CANAOIDCANA_QUERY, 
			preCatalOrden.getCanaOidCana(),
			preCatalOrden.getCanaOidCana().clone(),
			"segCanalView"));
		}
		if(preCatalOrden.getOcatOidCata() != null){
			preCatalOrden.setOcatOidCata((es.indra.belcorp.mso.PreCatalData)
			getSingleObject(BUSINESSID_OCATOIDCATA_QUERY, 
			preCatalOrden.getOcatOidCata(),
			preCatalOrden.getOcatOidCata().clone(),
			"preCatal"));
		}
		
		//Una vez que tenemos completo los datos de los atributos raices de jerarquías de dependencias
		//cargamos las combos y seleccionamos elementos
		loadNonLeafDependenceCombos(preCatalOrden);
		loadLeafDependenceCombos(preCatalOrden);
	}
