/*
    INDRA/CAR/mmg
    $Id: CobValorArgumLPStartUp.java,v 1.1 2009/12/03 18:38:46 pecbazalar Exp $
    DESC
*/
import java.util.*;
...
public class CobValorArgumLPStartUp extends MMGDruidaBase{
	...
	public void ejecucion() throws Exception {
		try{
			...
			asignarAtributo("PAGINA", "CobValorArgumPage", "cod", "CobValorArgum." + accion + ".label");
			...
			limpiarBotoneraLista("cobValorArgum", accion);
	...
	protected void cmdLov() throws Exception{
		...
		asignarAtributo("TEXT", "valArgu", "validacion", "mensaje = ValidaLongitud(cdos_trim(valor), 1, 4);");
		asignarAtributo("TEXT", "valDescValoArgu", "validacion", "mensaje = ValidaLongitud(cdos_trim(valor), 1, 40);");
		
		
		//Primero determianmos el nombre del campo en la entidad de oriegen donde se van a cargar los datos
		asignar("VAR", "idAttributeLovOpener", conectorParametro(PARAMETRO_LOV_ATTRIBUTO));
		asignar("VAR", "idEntityLovOpener", conectorParametro(PARAMETRO_LOV_ENTIDAD));
		//asignarAtributo("CAPA", "cobValorArgumLovLayer", "visibilidad", "visible");
		asignarAtributo("PAGINA", "CobValorArgumPage", "onblur", "window.focus();");
	...
	*/
	protected void cmdView() throws Exception{
		...
		CobValorArgumData cobValorArgumQuery= new CobValorArgumData();
		cobValorArgumQuery.setId(new Long(conectorParametro("idSelection")));
		
		//Asiganmos el valor a la variable de acción anterior para poder volver hacia atras y hacer requery
		asignar("VAR", VAR_ACCION_ANTERIOR, conectorParametro(VAR_ACCION_ANTERIOR));
		
		//Sacamos de la base de datos la entidad a visualizar
		CobValorArgumData datosResult = (CobValorArgumData)getSingleObject(BUSINESSID_QUERY, cobValorArgumQuery, 
			cobValorArgumQuery.clone(), "cobValorArgum");
		...
		CobValorArgumData cobValorArgum = datosResult;
		asignar("TEXT", "valArgu", FormatUtils.formatObject(cobValorArgum.getValArgu(), 
			 MMGDruidaHelper.getUserDecimalFormatPattern(this), decimalFormatSymbols));
		asignar("TEXT", "valDescValoArgu", FormatUtils.formatObject(cobValorArgum.getValDescValoArgu(), 
			 MMGDruidaHelper.getUserDecimalFormatPattern(this), decimalFormatSymbols));
		asignar("TEXT", "valObse", FormatUtils.formatObject(cobValorArgum.getValObse(), 
			 MMGDruidaHelper.getUserDecimalFormatPattern(this), decimalFormatSymbols));
		es.indra.belcorp.mso.CobArgumData arguOidArgu = cobValorArgum.getArguOidArgu();
		...
		asignarAtributo("TEXT", "valArgu", "disabled", "S");
		asignarAtributo("TEXT", "valDescValoArgu", "disabled", "S");
	...
	protected void cmdCreate() throws Exception{
		...
		asignarAtributo("TEXT", "valArgu", "req", "S");
		asignarAtributo("TEXT", "valDescValoArgu", "req", "S");
	...
	protected void cmdUpdateForm() throws Exception{
		...
		//asignarAtributo("FORMULARIO", "cobValorArgumFrm", "oculto", "N");
		...
		//Construimos el mso con los elementos de la búsqueda
		CobValorArgumData cobValorArgumQuery= new CobValorArgumData();
		cobValorArgumQuery.setId(new Long(id));
		...
		//asignar("VAR","timestamp",Long.toString(cobValorArgum.jdoGetTimeStamp()));
	...
	*/
	protected void cambiarAcciones() throws Exception{
				asignarAtributo("AREATEXTO", "id", "ontab","ejecutarAccionFocoModificacion(\"cobValorArgumFrm\",1, true)");
				asignarAtributo("AREATEXTO", "id", "onshtab","ejecutarAccionFocoModificacion(\"cobValorArgumFrm\",1, false)");
				asignarAtributo("AREATEXTO", "valArgu", "ontab","ejecutarAccionFocoModificacion(\"cobValorArgumFrm\",2, true)");
				asignarAtributo("AREATEXTO", "valArgu", "onshtab","ejecutarAccionFocoModificacion(\"cobValorArgumFrm\",2, false)");
				asignarAtributo("AREATEXTO", "valDescValoArgu", "ontab","ejecutarAccionFocoModificacion(\"cobValorArgumFrm\",3, true)");
				asignarAtributo("AREATEXTO", "valDescValoArgu", "onshtab","ejecutarAccionFocoModificacion(\"cobValorArgumFrm\",3, false)");
				asignarAtributo("AREATEXTO", "valObse", "ontab","ejecutarAccionFocoModificacion(\"cobValorArgumFrm\",4, true)");
				asignarAtributo("AREATEXTO", "valObse", "onshtab","ejecutarAccionFocoModificacion(\"cobValorArgumFrm\",4, false)");
					asignarAtributo("COMBO","arguOidArgu", "ontab", "ejecutarAccionFocoModificacion(\"cobValorArgumFrm\",5, true)");
					asignarAtributo("COMBO","arguOidArgu", "onshtab", "ejecutarAccionFocoModificacion(\"cobValorArgumFrm\",5, false)");
	}
	...
	protected void removeLovElements() throws Exception{
		borrarNodo("TD","nombre","cobValorArgumTdLovNullSelectionButton");
	}
	
	protected void removeQueryElements() throws Exception{
		//Borramos los botones para realizar la búsuqeda
		borrarNodo("TR","nombre","cobValorArgumTrButtons");
	...
	*/
	protected void desactivateNonModifyFields() throws Exception{
			traza("MMG: en desactivateNonModifyFields ");		
				...
				traza("MMG: el atributo es valArgu...");		
				
				traza("MMG: el atributo es valDescValoArgu...");		
	...
	*/
	protected void establecerSizeCapas(String accion, String origen) throws Exception{
		...
		asignarAtributo("CAPA","cobValorArgumFormLayer", "alto", String.valueOf(sizeForm));		
		
		//Estabelcemos la posicion y de la capa correspondiente a la lista y de su botonera
		asignarAtributo("CAPA","cobValorArgumListLayer", "y", String.valueOf(sizeForm + 26));		
		asignarAtributo("CAPA","cobValorArgumListButtonsLayer", "y", String.valueOf(sizeForm + 278));		
	}

	/**
	* Nos permite cargar las combos finales de la jerarquías de dependencia
	*/
	protected void loadLeafDependenceCombos(CobValorArgumData cobValorArgum) throws Exception{
		...
		if(cobValorArgum != null && cobValorArgum.getArguOidArgu() != null){
				checkCombo("arguOidArgu" , cobValorArgum.getArguOidArgu().getId().toString());
			}
	...
	* @param cobValorArgum La el objetopar la cual se va a realizar la carga
	*/
	protected void loadNonLeafDependenceCombos(CobValorArgumData cobValorArgum) 
		throws Exception{
	...
	*/
	protected void loadNonLeafDependenceCombosForRequery() throws Exception{
		...
		CobValorArgumData cobValorArgum = 
			(CobValorArgumData) conectorParametroSesion(SESSION_ATTRIBUTE_LAST_QUERY_OBJ_FROM);
		
		//Cargamos todos los objetos completos correspodnientes a los valores de los
		//Campos que sean raiz de una jerarquía de combos dependientes
		if(cobValorArgum.getArguOidArgu() != null){
			cobValorArgum.setArguOidArgu((es.indra.belcorp.mso.CobArgumData)
			getSingleObject(BUSINESSID_ARGUOIDARGU_QUERY, 
			cobValorArgum.getArguOidArgu(),
			cobValorArgum.getArguOidArgu().clone(),
			"cobArgum"));
		}
		
		//Una vez que tenemos completo los datos de los atributos raices de jerarquías de dependencias
		//cargamos las combos y seleccionamos elementos
		loadNonLeafDependenceCombos(cobValorArgum);
		loadLeafDependenceCombos(cobValorArgum);
	}
