/*
    INDRA/CAR/mmg
    $Id: FacFormuViewLPStartUp.java,v 1.1 2009/12/03 18:38:03 pecbazalar Exp $
    DESC
*/
import java.util.*;
...
public class FacFormuViewLPStartUp extends MMGDruidaBase{
	...
	public void ejecucion() throws Exception {
		try{
			...
			asignarAtributo("PAGINA", "FacFormuViewPage", "cod", "FacFormuView." + accion + ".label");
			...
			limpiarBotoneraLista("facFormuView", accion);
	...
	protected void cmdLov() throws Exception{
		...
		//asignarAtributo("CAPA", "facFormuViewLovLayer", "visibilidad", "visible");
		asignarAtributo("PAGINA", "FacFormuViewPage", "onblur", "window.focus();");
	...
	*/
	protected void cmdView() throws Exception{
		...
		FacFormuViewData facFormuViewQuery= new FacFormuViewData();
		facFormuViewQuery.setId(new Long(conectorParametro("idSelection")));
		
		//Asiganmos el valor a la variable de acción anterior para poder volver hacia atras y hacer requery
		asignar("VAR", VAR_ACCION_ANTERIOR, conectorParametro(VAR_ACCION_ANTERIOR));
		
		//Sacamos de la base de datos la entidad a visualizar
		FacFormuViewData datosResult = (FacFormuViewData)getSingleObject(BUSINESSID_QUERY, facFormuViewQuery, 
			facFormuViewQuery.clone(), "facFormuView");
		...
		FacFormuViewData facFormuView = datosResult;
		asignar("TEXT", "codForm", FormatUtils.formatObject(facFormuView.getCodForm(), 
			 MMGDruidaHelper.getUserDecimalFormatPattern(this), decimalFormatSymbols));
		es.indra.belcorp.mso.FacDestiImpreData deimOidDestImpr = facFormuView.getDeimOidDestImpr();
	...
	protected void cmdUpdateForm() throws Exception{
		...
		//asignarAtributo("FORMULARIO", "facFormuViewFrm", "oculto", "N");
		...
		//Construimos el mso con los elementos de la búsqueda
		FacFormuViewData facFormuViewQuery= new FacFormuViewData();
		facFormuViewQuery.setId(new Long(id));
		...
		//asignar("VAR","timestamp",Long.toString(facFormuView.jdoGetTimeStamp()));
	...
	*/
	protected void cambiarAcciones() throws Exception{
				asignarAtributo("AREATEXTO", "id", "ontab","ejecutarAccionFocoModificacion(\"facFormuViewFrm\",1, true)");
				asignarAtributo("AREATEXTO", "id", "onshtab","ejecutarAccionFocoModificacion(\"facFormuViewFrm\",1, false)");
				asignarAtributo("AREATEXTO", "codForm", "ontab","ejecutarAccionFocoModificacion(\"facFormuViewFrm\",2, true)");
				asignarAtributo("AREATEXTO", "codForm", "onshtab","ejecutarAccionFocoModificacion(\"facFormuViewFrm\",2, false)");
					asignarAtributo("COMBO","deimOidDestImpr", "ontab", "ejecutarAccionFocoModificacion(\"facFormuViewFrm\",15, true)");
					asignarAtributo("COMBO","deimOidDestImpr", "onshtab", "ejecutarAccionFocoModificacion(\"facFormuViewFrm\",15, false)");
	}
	...
	protected void removeLovElements() throws Exception{
		borrarNodo("TD","nombre","facFormuViewTdLovNullSelectionButton");
	}
	
	protected void removeQueryElements() throws Exception{
		//Borramos los botones para realizar la búsuqeda
		borrarNodo("TR","nombre","facFormuViewTrButtons");
	...
	*/
	protected void establecerSizeCapas(String accion, String origen) throws Exception{
		...
		asignarAtributo("CAPA","facFormuViewFormLayer", "alto", String.valueOf(sizeForm));		
		
		//Estabelcemos la posicion y de la capa correspondiente a la lista y de su botonera
		asignarAtributo("CAPA","facFormuViewListLayer", "y", String.valueOf(sizeForm + 26));		
		asignarAtributo("CAPA","facFormuViewListButtonsLayer", "y", String.valueOf(sizeForm + 278));		
	}

	/**
	* Nos permite cargar las combos finales de la jerarquías de dependencia
	*/
	protected void loadLeafDependenceCombos(FacFormuViewData facFormuView) throws Exception{
		...
		if(facFormuView != null && facFormuView.getDeimOidDestImpr() != null){
				checkCombo("deimOidDestImpr" , facFormuView.getDeimOidDestImpr().getId().toString());
			}
	...
	* @param facFormuView La el objetopar la cual se va a realizar la carga
	*/
	protected void loadNonLeafDependenceCombos(FacFormuViewData facFormuView) 
		throws Exception{
	...
	*/
	protected void loadNonLeafDependenceCombosForRequery() throws Exception{
		...
		FacFormuViewData facFormuView = 
			(FacFormuViewData) conectorParametroSesion(SESSION_ATTRIBUTE_LAST_QUERY_OBJ_FROM);
		
		//Cargamos todos los objetos completos correspodnientes a los valores de los
		//Campos que sean raiz de una jerarquía de combos dependientes
		if(facFormuView.getDeimOidDestImpr() != null){
			facFormuView.setDeimOidDestImpr((es.indra.belcorp.mso.FacDestiImpreData)
			getSingleObject(BUSINESSID_DEIMOIDDESTIMPR_QUERY, 
			facFormuView.getDeimOidDestImpr(),
			facFormuView.getDeimOidDestImpr().clone(),
			"facDestiImpre"));
		}
		
		//Una vez que tenemos completo los datos de los atributos raices de jerarquías de dependencias
		//cargamos las combos y seleccionamos elementos
		loadNonLeafDependenceCombos(facFormuView);
		loadLeafDependenceCombos(facFormuView);
	}
