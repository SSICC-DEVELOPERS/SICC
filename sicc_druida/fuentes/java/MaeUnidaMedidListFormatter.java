...
/*
    INDRA/CAR/mmg
    $Id: MaeUnidaMedidListFormatter.java,v 1.1 2009/12/03 18:39:06 pecbazalar Exp $
    DESC
*/
 ...
 * @author Indra
 */
public class MaeUnidaMedidListFormatter extends MMGDruidaFormatoObjeto {
	
	public MaeUnidaMedidListFormatter() {
	}
		...
		for (int i = 0; i < maeUnidaMedidList.size(); i++) {
			...
			row.add((maeUnidaMedidData.getDesAbre() != null ? 
				FormatUtils.formatObject(maeUnidaMedidData.getDesAbre(), 
				 MMGDruidaHelper.getUserDecimalFormatPattern(this), 
				symbols) : ""));
			row.add((maeUnidaMedidData.getMagnOidMagn() != null ? 
				FormatUtils.formatObject(maeUnidaMedidData.getMagnOidMagn(), 
				 MMGDruidaHelper.getUserDecimalFormatPattern(this), 
				symbols) : ""));
