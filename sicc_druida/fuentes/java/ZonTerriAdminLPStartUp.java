/*
    INDRA/CAR/mmg
    $Id: ZonTerriAdminLPStartUp.java,v 1.1 2009/12/03 18:43:29 pecbazalar Exp $
    DESC
*/
import java.util.*;
...
public class ZonTerriAdminLPStartUp extends MMGDruidaBase{
	...
	public static final String BUSINESSID_ZSCCOIDSECC_QUERY ="MMGZonSecciQueryFromToUser";
	public static final String ZSCCOIDSECC_COMBO_CONNECTOR = "ZonSecciConectorCombo";
	public static final String BUSINESSID_TERROIDTERR_QUERY ="MMGZonTerriQueryFromToUser";
	public static final String TERROIDTERR_COMBO_CONNECTOR = "ZonTerriConectorCombo";
	...
	public void ejecucion() throws Exception {
		try{
			...
			asignarAtributo("PAGINA", "ZonTerriAdminPage", "cod", "ZonTerriAdmin." + accion + ".label");
			...
			limpiarBotoneraLista("zonTerriAdmin", accion);
	...
	protected void cmdLov() throws Exception{
		...
		//asignarAtributo("CAPA", "zonTerriAdminLovLayer", "visibilidad", "visible");
		asignarAtributo("PAGINA", "ZonTerriAdminPage", "onblur", "window.focus();");
	...
	protected void cmdView() throws Exception{
		...
		ZonTerriAdminData zonTerriAdminQuery= new ZonTerriAdminData();
		zonTerriAdminQuery.setId(new Integer(conectorParametro("idSelection")));
		
		//Asiganmos el valor a la variable de acción anterior para poder volver hacia atras y hacer requery
		asignar("VAR", VAR_ACCION_ANTERIOR, conectorParametro(VAR_ACCION_ANTERIOR));
		
		//Sacamos de la base de datos la entidad a visualizar
		ZonTerriAdminData datosResult = (ZonTerriAdminData)getSingleObject(BUSINESSID_QUERY, zonTerriAdminQuery, 
			zonTerriAdminQuery.clone(), "zonTerriAdmin");
		...
		ZonTerriAdminData zonTerriAdmin = datosResult;
		asignar("TEXT", "indBorr", FormatUtils.formatObject(zonTerriAdmin.getIndBorr(), 
			 MMGDruidaHelper.getUserDecimalFormatPattern(this), MMGDruidaHelper.getUserDecimalFormatSymbols(this)));
		es.indra.belcorp.mso.ZonSecciData zsccOidSecc = zonTerriAdmin.getZsccOidSecc();
		if(zsccOidSecc != null && zsccOidSecc.getId() != null) checkCombo("zsccOidSecc" , zsccOidSecc.getId().toString());
		es.indra.belcorp.mso.ZonTerriData terrOidTerr = zonTerriAdmin.getTerrOidTerr();
		if(terrOidTerr != null && terrOidTerr.getId() != null) checkCombo("terrOidTerr" , terrOidTerr.getId().toString());
		asignar("TEXT", "fecRtz", FormatUtils.formatObject(zonTerriAdmin.getFecRtz(), 
	...
	protected void cmdCreate() throws Exception{
		...
		asignarAtributo("COMBO", "zsccOidSecc", "req", "S");
		asignarAtributo("COMBO", "terrOidTerr", "req", "S");
	...
	protected void cmdUpdateForm() throws Exception{
		...
		//asignarAtributo("FORMULARIO", "zonTerriAdminFrm", "oculto", "N");
		...
		//Construimos el mso con los elementos de la búsqueda
		ZonTerriAdminData zonTerriAdminQuery= new ZonTerriAdminData();
		zonTerriAdminQuery.setId(new Integer(id));
		...
		//asignar("VAR","timestamp",Long.toString(zonTerriAdmin.jdoGetTimeStamp()));
	...
	protected void removeLovElements() throws Exception{
		borrarNodo("TD","nombre","zonTerriAdminTdLovNullSelectionButton");
	}
	
	protected void removeQueryElements() throws Exception{
		//Borramos los botones para realizar la búsuqeda
		borrarNodo("TR","nombre","zonTerriAdminTrButtons");
	...
	*/
	protected void establecerSizeCapas(String accion, String origen) throws Exception{
		...
		asignarAtributo("CAPA","zonTerriAdminFormLayer", "alto", String.valueOf(sizeForm));		
		
		//Estabelcemos la posicion y de la capa correspondiente a la lista y de su botonera
		asignarAtributo("CAPA","zonTerriAdminListLayer", "y", String.valueOf(sizeForm + 26));		
		asignarAtributo("CAPA","zonTerriAdminListButtonsLayer", "y", String.valueOf(sizeForm + 278));		
	}

	/**
	* Nos permite cargar las combos finales de la jerarquías de dependencia
	*/
	protected void loadLeafDependenceCombos(ZonTerriAdminData zonTerriAdmin) throws Exception{
		HashMap checkedCombos = new HashMap();
		ZonSecciData zsccOidSecc = new ZonSecciData();
		asignarComboResultados(BUSINESSID_ZSCCOIDSECC_QUERY,ZSCCOIDSECC_COMBO_CONNECTOR, 
			zsccOidSecc, zsccOidSecc.clone(), "zonSecci", "zsccOidSecc", "result_ROWSET" );
		if(zonTerriAdmin != null && zonTerriAdmin.getZsccOidSecc() != null){
				checkCombo("zsccOidSecc" , zonTerriAdmin.getZsccOidSecc().getId().toString());
			}
		String zsccOidSeccDefaultValue = MMGDruidaHelper.gerUserStructuralEntityValue(this, "ZonSecci");
		if(zsccOidSeccDefaultValue != null && !zsccOidSeccDefaultValue.trim().equals("")){
			checkCombo("zsccOidSecc", zsccOidSeccDefaultValue);
			checkedCombos.put("zsccOidSecc",zsccOidSeccDefaultValue);
			cascadeComboLoad(checkedCombos, "zsccOidSecc");
		}						 
		ZonTerriData terrOidTerr = new ZonTerriData();
		asignarComboResultados(BUSINESSID_TERROIDTERR_QUERY,TERROIDTERR_COMBO_CONNECTOR, 
			terrOidTerr, terrOidTerr.clone(), "zonTerri", "terrOidTerr", "result_ROWSET" );
		if(zonTerriAdmin != null && zonTerriAdmin.getTerrOidTerr() != null){
				checkCombo("terrOidTerr" , zonTerriAdmin.getTerrOidTerr().getId().toString());
			}
		String terrOidTerrDefaultValue = MMGDruidaHelper.gerUserStructuralEntityValue(this, "ZonTerri");
		if(terrOidTerrDefaultValue != null && !terrOidTerrDefaultValue.trim().equals("")){
			checkCombo("terrOidTerr", terrOidTerrDefaultValue);
			checkedCombos.put("terrOidTerr",terrOidTerrDefaultValue);
			cascadeComboLoad(checkedCombos, "terrOidTerr");
		}						 
	...
	* @param zonTerriAdmin La el objetopar la cual se va a realizar la carga
	*/
	protected void loadNonLeafDependenceCombos(ZonTerriAdminData zonTerriAdmin) 
		throws Exception{
	...
	*/
	protected void loadNonLeafDependenceCombosForRequery() throws Exception{
		...
		ZonTerriAdminData zonTerriAdmin = 
			(ZonTerriAdminData) conectorParametroSesion(SESSION_ATTRIBUTE_LAST_QUERY_OBJ_FROM);
		
		//Cargamos todos los objetos completos correspodnientes a los valores de los
		//Campos que sean raiz de una jerarquía de combos dependientes
		if(zonTerriAdmin.getZsccOidSecc() != null){
			zonTerriAdmin.setZsccOidSecc((es.indra.belcorp.mso.ZonSecciData)
			getSingleObject(BUSINESSID_ZSCCOIDSECC_QUERY, 
			zonTerriAdmin.getZsccOidSecc(),
			zonTerriAdmin.getZsccOidSecc().clone(),
			"zonSecci"));
		}
		if(zonTerriAdmin.getTerrOidTerr() != null){
			zonTerriAdmin.setTerrOidTerr((es.indra.belcorp.mso.ZonTerriData)
			getSingleObject(BUSINESSID_TERROIDTERR_QUERY, 
			zonTerriAdmin.getTerrOidTerr(),
			zonTerriAdmin.getTerrOidTerr().clone(),
			"zonTerri"));
		}
		
		//Una vez que tenemos completo los datos de los atributos raices de jerarquías de dependencias
		//cargamos las combos y seleccionamos elementos
		loadNonLeafDependenceCombos(zonTerriAdmin);
		loadLeafDependenceCombos(zonTerriAdmin);
	}
