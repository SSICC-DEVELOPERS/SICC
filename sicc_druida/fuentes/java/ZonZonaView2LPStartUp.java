/*
    INDRA/CAR/mmg
    $Id: ZonZonaView2LPStartUp.java,v 1.1 2009/12/03 18:38:42 pecbazalar Exp $
    DESC
*/
import java.util.*;
...
public class ZonZonaView2LPStartUp extends MMGDruidaBase{
	...
	public void ejecucion() throws Exception {
		try{
			...
			asignarAtributo("PAGINA", "ZonZonaView2Page", "cod", "ZonZonaView2." + accion + ".label");
			...
			limpiarBotoneraLista("zonZonaView2", accion);
	...
	protected void cmdLov() throws Exception{
		...
		//asignarAtributo("CAPA", "zonZonaView2LovLayer", "visibilidad", "visible");
		asignarAtributo("PAGINA", "ZonZonaView2Page", "onblur", "window.focus();");
	...
	*/
	protected void cmdView() throws Exception{
		...
		ZonZonaView2Data zonZonaView2Query= new ZonZonaView2Data();
		zonZonaView2Query.setId(new Long(conectorParametro("idSelection")));
		
		//Asiganmos el valor a la variable de acción anterior para poder volver hacia atras y hacer requery
		asignar("VAR", VAR_ACCION_ANTERIOR, conectorParametro(VAR_ACCION_ANTERIOR));
		
		//Sacamos de la base de datos la entidad a visualizar
		ZonZonaView2Data datosResult = (ZonZonaView2Data)getSingleObject(BUSINESSID_QUERY, zonZonaView2Query, 
			zonZonaView2Query.clone(), "zonZonaView2");
		...
		ZonZonaView2Data zonZonaView2 = datosResult;
		es.indra.belcorp.mso.SegMarcaData marcOidMarc = zonZonaView2.getMarcOidMarc();
		if(marcOidMarc != null && marcOidMarc.getId() != null) checkCombo("marcOidMarc" , marcOidMarc.getId().toString());
		es.indra.belcorp.mso.SegCanalViewData canaOidCana = zonZonaView2.getCanaOidCana();
		if(canaOidCana != null && canaOidCana.getId() != null) checkCombo("canaOidCana" , canaOidCana.getId().toString());
		asignar("TEXT", "codZona", FormatUtils.formatObject(zonZonaView2.getCodZona(), 
			 MMGDruidaHelper.getUserDecimalFormatPattern(this), decimalFormatSymbols));
		asignar("TEXT", "indActi", FormatUtils.formatObject(zonZonaView2.getIndActi(), 
			 MMGDruidaHelper.getUserDecimalFormatPattern(this), decimalFormatSymbols));
		asignar("TEXT", "indBorr", FormatUtils.formatObject(zonZonaView2.getIndBorr(), 
			 MMGDruidaHelper.getUserDecimalFormatPattern(this), decimalFormatSymbols));
		...
		//Deshabilitamos todos los campos para que no se editen
					...
					asignarAtributo("COMBO", "canaOidCana", "disabled", "S");asignarAtributo("TEXT", "codZona", "disabled", "S");
	...
	protected void cmdUpdateForm() throws Exception{
		...
		//asignarAtributo("FORMULARIO", "zonZonaView2Frm", "oculto", "N");
		...
		//Construimos el mso con los elementos de la búsqueda
		ZonZonaView2Data zonZonaView2Query= new ZonZonaView2Data();
		zonZonaView2Query.setId(new Long(id));
		...
		//asignar("VAR","timestamp",Long.toString(zonZonaView2.jdoGetTimeStamp()));
	...
	*/
	protected void cambiarAcciones() throws Exception{
				asignarAtributo("AREATEXTO", "id", "ontab","ejecutarAccionFocoModificacion(\"zonZonaView2Frm\",1, true)");
				asignarAtributo("AREATEXTO", "id", "onshtab","ejecutarAccionFocoModificacion(\"zonZonaView2Frm\",1, false)");
					asignarAtributo("COMBO","marcOidMarc", "ontab", "ejecutarAccionFocoModificacion(\"zonZonaView2Frm\",2, true)");
					asignarAtributo("COMBO","marcOidMarc", "onshtab", "ejecutarAccionFocoModificacion(\"zonZonaView2Frm\",2, false)");
					asignarAtributo("COMBO","canaOidCana", "ontab", "ejecutarAccionFocoModificacion(\"zonZonaView2Frm\",3, true)");
					asignarAtributo("COMBO","canaOidCana", "onshtab", "ejecutarAccionFocoModificacion(\"zonZonaView2Frm\",3, false)");
				asignarAtributo("AREATEXTO", "codZona", "ontab","ejecutarAccionFocoModificacion(\"zonZonaView2Frm\",4, true)");
				asignarAtributo("AREATEXTO", "codZona", "onshtab","ejecutarAccionFocoModificacion(\"zonZonaView2Frm\",4, false)");
					asignarAtributo("COMBO","paisOidPais", "ontab", "ejecutarAccionFocoModificacion(\"zonZonaView2Frm\",5, true)");
					asignarAtributo("COMBO","paisOidPais", "onshtab", "ejecutarAccionFocoModificacion(\"zonZonaView2Frm\",5, false)");
				asignarAtributo("AREATEXTO", "indActi", "ontab","ejecutarAccionFocoModificacion(\"zonZonaView2Frm\",6, true)");
				asignarAtributo("AREATEXTO", "indActi", "onshtab","ejecutarAccionFocoModificacion(\"zonZonaView2Frm\",6, false)");
				asignarAtributo("AREATEXTO", "indBorr", "ontab","ejecutarAccionFocoModificacion(\"zonZonaView2Frm\",7, true)");
				asignarAtributo("AREATEXTO", "indBorr", "onshtab","ejecutarAccionFocoModificacion(\"zonZonaView2Frm\",7, false)");
	}
	...
	protected void removeLovElements() throws Exception{
		borrarNodo("TD","nombre","zonZonaView2TdLovNullSelectionButton");
	}
	
	protected void removeQueryElements() throws Exception{
		//Borramos los botones para realizar la búsuqeda
		borrarNodo("TR","nombre","zonZonaView2TrButtons");
	...
	*/
	protected void establecerSizeCapas(String accion, String origen) throws Exception{
		...
		asignarAtributo("CAPA","zonZonaView2FormLayer", "alto", String.valueOf(sizeForm));		
		
		//Estabelcemos la posicion y de la capa correspondiente a la lista y de su botonera
		asignarAtributo("CAPA","zonZonaView2ListLayer", "y", String.valueOf(sizeForm + 26));		
		asignarAtributo("CAPA","zonZonaView2ListButtonsLayer", "y", String.valueOf(sizeForm + 278));		
	}

	/**
	* Nos permite cargar las combos finales de la jerarquías de dependencia
	*/
	protected void loadLeafDependenceCombos(ZonZonaView2Data zonZonaView2) throws Exception{
		...
		if(zonZonaView2 != null && zonZonaView2.getMarcOidMarc() != null){
				checkCombo("marcOidMarc" , zonZonaView2.getMarcOidMarc().getId().toString());
			}
		...
		if(zonZonaView2 != null && zonZonaView2.getCanaOidCana() != null){
				checkCombo("canaOidCana" , zonZonaView2.getCanaOidCana().getId().toString());
			}
		...
		if(zonZonaView2 != null && zonZonaView2.getPaisOidPais() != null){
				checkCombo("paisOidPais" , zonZonaView2.getPaisOidPais().getId().toString());
			}
	...
	* @param zonZonaView2 La el objetopar la cual se va a realizar la carga
	*/
	protected void loadNonLeafDependenceCombos(ZonZonaView2Data zonZonaView2) 
		throws Exception{
	...
	*/
	protected void loadNonLeafDependenceCombosForRequery() throws Exception{
		...
		ZonZonaView2Data zonZonaView2 = 
			(ZonZonaView2Data) conectorParametroSesion(SESSION_ATTRIBUTE_LAST_QUERY_OBJ_FROM);
		
		//Cargamos todos los objetos completos correspodnientes a los valores de los
		//Campos que sean raiz de una jerarquía de combos dependientes
		if(zonZonaView2.getMarcOidMarc() != null){
			zonZonaView2.setMarcOidMarc((es.indra.belcorp.mso.SegMarcaData)
			getSingleObject(BUSINESSID_MARCOIDMARC_QUERY, 
			zonZonaView2.getMarcOidMarc(),
			zonZonaView2.getMarcOidMarc().clone(),
			"segMarca"));
		}
		if(zonZonaView2.getCanaOidCana() != null){
			zonZonaView2.setCanaOidCana((es.indra.belcorp.mso.SegCanalViewData)
			getSingleObject(BUSINESSID_CANAOIDCANA_QUERY, 
			zonZonaView2.getCanaOidCana(),
			zonZonaView2.getCanaOidCana().clone(),
			"segCanalView"));
		}
		if(zonZonaView2.getPaisOidPais() != null){
			zonZonaView2.setPaisOidPais((es.indra.belcorp.mso.SegPaisViewData)
			getSingleObject(BUSINESSID_PAISOIDPAIS_QUERY, 
			zonZonaView2.getPaisOidPais(),
			zonZonaView2.getPaisOidPais().clone(),
			"segPaisView"));
		}
		
		//Una vez que tenemos completo los datos de los atributos raices de jerarquías de dependencias
		//cargamos las combos y seleccionamos elementos
		loadNonLeafDependenceCombos(zonZonaView2);
		loadLeafDependenceCombos(zonZonaView2);
	}
