/*
    INDRA/CAR/mmg
    $Id: SegPerioCorpoLPStartUp.java,v 1.1 2009/12/03 18:40:25 pecbazalar Exp $
    DESC
*/
import java.util.*;
...
public class SegPerioCorpoLPStartUp extends MMGDruidaBase{
	...
	public static final String BUSINESSID_QUERY = "MMGSegPerioCorpoQueryFromToUserPage";
				...
				public static final String BUSINESSID_TIPEOIDTIPOPERI_QUERY ="MMGSegTipoPerioQueryFromToUser";
				public static final String TIPEOIDTIPOPERI_COMBO_CONNECTOR = "SegTipoPerioConectorCombo";
	...
	public SegPerioCorpoLPStartUp(){
		super();
	}

	public void inicio() throws Exception {
		pagina("SegPerioCorpoPage");
	}
	...
	public void ejecucion() throws Exception {
		try{
			...
			asignarAtributo("PAGINA", "SegPerioCorpoPage", "cod", "SegPerioCorpo." + accion + ".label");
			...
			limpiarBotoneraLista("segPerioCorpo", accion);
	...
	protected void cmdLov() throws Exception{
		...
		asignarAtributo("TEXT", "codPeri", "validacion", "mensaje = ValidaLongitud(cdos_trim(valor), 1, 6);");
		...
		//asignarAtributo("CAPA", "segPerioCorpoLovLayer", "visibilidad", "visible");
		asignarAtributo("PAGINA", "SegPerioCorpoPage", "onblur", "window.focus();");
	...
	protected void cmdPreView() throws Exception{
		...
		conectorAction("SegPerioCorpoLPStartUp");
	...
	*/
	protected void cmdView() throws Exception{
		...
		SegPerioCorpoData segPerioCorpoQuery= new SegPerioCorpoData();
		segPerioCorpoQuery.setId(new Long(conectorParametro("idSelection")));
		
		//Asiganmos el valor a la variable de acción anterior para poder volver hacia atras y hacer requery
		asignar("VAR", VAR_ACCION_ANTERIOR, conectorParametro(VAR_ACCION_ANTERIOR));
		
		//Sacamos de la base de datos la entidad a visualizar
		SegPerioCorpoData datosResult = (SegPerioCorpoData)getSingleObject(BUSINESSID_QUERY, segPerioCorpoQuery, 
			segPerioCorpoQuery.clone(), "segPerioCorpo");
		...
		preservaHiddensI18n(new Integer(1), "SEG_PERIO_CORPO", "segPerioCorpoFrm", "Descripcion", true);
		...
		SegPerioCorpoData segPerioCorpo = datosResult;
		es.indra.belcorp.mso.SegTipoPerioData tipeOidTipoPeri = segPerioCorpo.getTipeOidTipoPeri();
		if(tipeOidTipoPeri != null && tipeOidTipoPeri.getId() != null) checkCombo("tipeOidTipoPeri" , tipeOidTipoPeri.getId().toString());
		asignar("TEXT", "codPeri", FormatUtils.formatObject(segPerioCorpo.getCodPeri(), 
			 MMGDruidaHelper.getUserDecimalFormatPattern(this), decimalFormatSymbols));
		asignar("TEXT", "Descripcion", FormatUtils.formatObject(segPerioCorpo.getDescripcion(), 
			 MMGDruidaHelper.getUserDecimalFormatPattern(this), decimalFormatSymbols));
		asignar("TEXT", "valAnio", FormatUtils.formatObject(segPerioCorpo.getValAnio(), 
			 MMGDruidaHelper.getUserDecimalFormatPattern(this), decimalFormatSymbols));
		...
		loadLocalizationVars(segPerioCorpo);
		
			
		//Deshabilitamos todos los campos para que no se editen
		
					asignarAtributo("COMBO", "tipeOidTipoPeri", "disabled", "S");asignarAtributo("TEXT", "codPeri", "disabled", "S");
		asignarAtributo("TEXT", "Descripcion", "disabled", "S");
		asignarAtributo("TEXT", "valAnio", "disabled", "S");
	...
	protected void cmdCreate() throws Exception{
		//Creamos los campos ocultos para la internacionalización
		preservaHiddensI18n(new Integer(1), "SEG_PERIO_CORPO", "segPerioCorpoFrm", "Descripcion", false);
		...
		asignarAtributo("COMBO", "tipeOidTipoPeri", "req", "S");
		asignarAtributo("TEXT", "codPeri", "req", "S");
		asignarAtributo("TEXT", "Descripcion", "req", "S");
		asignarAtributo("TEXT", "valAnio", "req", "S");
		
		
		//Asiganamos a los campos internacionalizables la función de onblur para actualizar el 
		//valor del idioma por defecto
		asignarAtributo("TEXT", "Descripcion", "onblur", "setDefaultLanguageValue('Descripcion', get('segPerioCorpoFrm.Descripcion'))");
		...
		asignarAtributo("TEXT", "codPeri", "validacion", "mensaje = ValidaLongitud(cdos_trim(valor), 1, 6);");
		asignarAtributo("TEXT", "Descripcion", "validacion", "mensaje = ValidaLongitud(cdos_trim(valor), 1, 40);");
		asignarAtributo("TEXT", "valAnio", "validacion", "mensaje = ValidaInt(cdos_trim(valor), 4, 0, 9999);");
	...
	protected void cmdUpdateForm() throws Exception{
		...
		//asignarAtributo("FORMULARIO", "segPerioCorpoFrm", "oculto", "N");
		...
		traza("MMG::Creando SegPerioCorpo");
		String id = conectorParametro("idSelection");
		traza("MMG:: id del atributo a modificar: " + id);
	
		//Construimos el mso con los elementos de la búsqueda
		SegPerioCorpoData segPerioCorpoQuery= new SegPerioCorpoData();
		segPerioCorpoQuery.setId(new Long(id));
		...
		//asignar("VAR","timestamp",Long.toString(segPerioCorpo.jdoGetTimeStamp()));
	...
	*/
	protected void cambiarAcciones() throws Exception{
				asignarAtributo("AREATEXTO", "id", "ontab","ejecutarAccionFocoModificacion(\"segPerioCorpoFrm\",1, true)");
				asignarAtributo("AREATEXTO", "id", "onshtab","ejecutarAccionFocoModificacion(\"segPerioCorpoFrm\",1, false)");
					asignarAtributo("COMBO","tipeOidTipoPeri", "ontab", "ejecutarAccionFocoModificacion(\"segPerioCorpoFrm\",2, true)");
					asignarAtributo("COMBO","tipeOidTipoPeri", "onshtab", "ejecutarAccionFocoModificacion(\"segPerioCorpoFrm\",2, false)");
				asignarAtributo("AREATEXTO", "codPeri", "ontab","ejecutarAccionFocoModificacion(\"segPerioCorpoFrm\",3, true)");
				asignarAtributo("AREATEXTO", "codPeri", "onshtab","ejecutarAccionFocoModificacion(\"segPerioCorpoFrm\",3, false)");
				asignarAtributo("AREATEXTO", "Descripcion", "ontab","ejecutarAccionFocoModificacion(\"segPerioCorpoFrm\",4, true)");
				asignarAtributo("AREATEXTO", "Descripcion", "onshtab","ejecutarAccionFocoModificacion(\"segPerioCorpoFrm\",4, false)");
				asignarAtributo("AREATEXTO", "valAnio", "ontab","ejecutarAccionFocoModificacion(\"segPerioCorpoFrm\",5, true)");
				asignarAtributo("AREATEXTO", "valAnio", "onshtab","ejecutarAccionFocoModificacion(\"segPerioCorpoFrm\",5, false)");
	}
	...
	protected void removeLovElements() throws Exception{
		borrarNodo("TD","nombre","segPerioCorpoTdLovNullSelectionButton");
	}
	
	protected void removeQueryElements() throws Exception{
		//Borramos los botones para realizar la búsuqeda
		borrarNodo("TR","nombre","segPerioCorpoTrButtons");
	...
	*/
	protected void desactivateNonModifyFields() throws Exception{
			traza("MMG: en desactivateNonModifyFields ");		
				...
				traza("MMG: el atributo es tipeOidTipoPeri...");		
				
					traza("MMG: en desactivateNonModifyFields encontrado atributo no editable...");
				
						asignarAtributo("COMBO", "tipeOidTipoPeri", "disabled", "S");
				traza("MMG: el atributo es codPeri...");		
				
					traza("MMG: en desactivateNonModifyFields encontrado atributo no editable...");
				
						// borrarCampo("codPeri");
						asignarAtributo ("TEXT", "codPeri", "disabled", "S");
				traza("MMG: el atributo es Descripcion...");		
				
				traza("MMG: el atributo es valAnio...");		
	...
	*/
	protected void establecerSizeCapas(String accion, String origen) throws Exception{
		...
		asignarAtributo("CAPA","segPerioCorpoFormLayer", "alto", String.valueOf(sizeForm));		
		
		//Estabelcemos la posicion y de la capa correspondiente a la lista y de su botonera
		asignarAtributo("CAPA","segPerioCorpoListLayer", "y", String.valueOf(sizeForm + 26));		
		asignarAtributo("CAPA","segPerioCorpoListButtonsLayer", "y", String.valueOf(sizeForm + 278));		
	}

	/**
	* Nos permite cargar las combos finales de la jerarquías de dependencia
	*/
	protected void loadLeafDependenceCombos(SegPerioCorpoData segPerioCorpo) throws Exception{
		HashMap checkedCombos = new HashMap();
		SegTipoPerioData tipeOidTipoPeri = new SegTipoPerioData();
		
						asignarComboResultados(BUSINESSID_TIPEOIDTIPOPERI_QUERY,TIPEOIDTIPOPERI_COMBO_CONNECTOR, 
			tipeOidTipoPeri, tipeOidTipoPeri.clone(), "segTipoPerio", "tipeOidTipoPeri", "result_ROWSET" );
		if(segPerioCorpo != null && segPerioCorpo.getTipeOidTipoPeri() != null){
				checkCombo("tipeOidTipoPeri" , segPerioCorpo.getTipeOidTipoPeri().getId().toString());
			}
	...
	* @param segPerioCorpo La el objetopar la cual se va a realizar la carga
	*/
	protected void loadNonLeafDependenceCombos(SegPerioCorpoData segPerioCorpo) 
		throws Exception{
	...
	*/
	protected void loadNonLeafDependenceCombosForRequery() throws Exception{
		...
		SegPerioCorpoData segPerioCorpo = 
			(SegPerioCorpoData) conectorParametroSesion(SESSION_ATTRIBUTE_LAST_QUERY_OBJ_FROM);
		
		//Cargamos todos los objetos completos correspodnientes a los valores de los
		//Campos que sean raiz de una jerarquía de combos dependientes
		if(segPerioCorpo.getTipeOidTipoPeri() != null){
			segPerioCorpo.setTipeOidTipoPeri((es.indra.belcorp.mso.SegTipoPerioData)
			getSingleObject(BUSINESSID_TIPEOIDTIPOPERI_QUERY, 
			segPerioCorpo.getTipeOidTipoPeri(),
			segPerioCorpo.getTipeOidTipoPeri().clone(),
			"segTipoPerio"));
		}
		
		//Una vez que tenemos completo los datos de los atributos raices de jerarquías de dependencias
		//cargamos las combos y seleccionamos elementos
		loadNonLeafDependenceCombos(segPerioCorpo);
		loadLeafDependenceCombos(segPerioCorpo);
	}
	...
	*/
	...
	protected void loadLocalizationVars(SegPerioCorpoData segPerioCorpo) throws Exception{
		GenDetaSiccData mmgLocalizationQuery = null;
		Vector mmgLocalizationDatosResult = null;
		mmgLocalizationQuery = new GenDetaSiccData();
		mmgLocalizationQuery.setEnti("SEG_PERIO_CORPO");
		mmgLocalizationQuery.setEntiPk(segPerioCorpo.getId());
